...
(function () {
  ...
  const PAGES = [
    { label: 'ADMIN', color: '#6b7a99',
      features: [
        { label: '10 abas no total (Dashboard, Eventos, Certificados, Repositório, Cadastrados, Administradores, Manual, Mapa, Testes, Pedidos); no mobile quebram em 2 linhas', p: ['admin'] },
        { label: 'Aba Dashboard — visão geral das avaliações: cards de participantes, avaliações recebidas (%), média geral, NPS e comentários; gráfico de barras com média por turma; distribuição das notas (0–10, 6 faixas); gauge de recomendação; destaques por seção (calculado das notas estruturadas, não de texto livre); temas mais solicitados; amostra de feedbacks em destaque', p: ['admin'] },
        ...
        { label: 'Aba Eventos — justificar por que um interessado não foi confirmado: "Sem vagas" (badge laranja) ou "Substituída" (badge lilás), turma aberta ou com interesse encerrado', p: ['admin'] },
        { label: 'Aba Eventos — remover participante (interessado ou inscrito), turma aberta ou com interesse encerrado', p: ['admin'] },
        { label: 'Aba Eventos — gerar certificados (aba Certificados — selecionar evento → turma; prévia antes de encerrar; após encerrar: badge de frequência por participante, downloads habilitados só para quem atingiu o percentual mínimo do evento)', p: ['admin'] },
        ...
        { label: 'Aba Eventos — consultar participantes (nome, e-mail, área, data de registro); contagem no cabeçalho do card mostra "X não confirmados · Y confirmados"', p: ['admin'] },
        { label: 'Aba Eventos — filtro por status na tabela de participantes (Todos / Confirmados / Não confirmados, com contagem em cada botão); aparece só quando a turma tem os dois grupos', p: ['admin'] },
        ...
        { label: 'Aba Pedidos — filtro por status (Pendentes / Respondidos / Excluídos / Todos, com contagem), combinável com o filtro por tipo; começa em "Pendentes" ao abrir a aba', p: ['admin'] },
    ...
    const ARCH = [
      ...
      },
      {
        ...
        items: [
          ...
          { name: 'forca-agil/router.js',        desc: 'Roteamento por hash — controla qual seção da página está visível. Login obrigatório: o body inteiro fica oculto (class "aguardando-auth") até o Firebase confirmar a sessão via evento fa-auth-ready; visitante não autenticado vê apenas o modal de login com fundo totalmente opaco (sem botão de fechar, sem site por baixo). Após login, a classe é removida e o site é revelado. Rotas protegidas: #conteudos e #treinamento exigem nível "enrolled"; demais rotas exigem apenas "member". Nível detectado via window.faAuth.getAccessLevel(). API: window.faRouter' },
          ...
          { name: 'forca-agil/dashboard.js',     desc: 'Aba Dashboard — agrega dados de avaliacoes/ (Firebase) em estatísticas: participantes (soma de inscritos confirmados em turmas-interesse), % participação, média geral e NPS (fórmula clássica: %promotores 9–10 menos %detratores 0–6), distribuição de notas em 6 faixas, média por turma (gráfico de barras em SVG inline), gauge semicircular de recomendação (SVG), temas mais solicitados (agregação de temasDesejados). "Destaques dos feedbacks" usa só as notas por seção (orgGeral, conteudoRelevancia, facilitadoresNota, dinamicasNota, aplicacaoPreparado) — nunca interpreta texto livre. API: window.faInitDashboard' },
          { name: 'forca-agil/pedidos.js',       desc: 'Formulário público "Faça um pedido" (seção Ajuda) e painel admin Aba Pedidos. Público: seleção de tipo (tema/curso/material/dúvida/outros) + textarea + envio autenticado para Firebase pedidos/. Admin: lê node pedidos/, filtro por status (Pendentes/Respondidos/Excluídos/Todos) combinado com filtro por tipo, ordem mais recente primeiro. Cada item tem link mailto: (Responder por e-mail — abre o cliente de e-mail do admin, não envia pelo site), fluxo "Marcar como respondido"/"✎ Editar" com seletor de qual admin respondeu e data/hora editável (fa-admins + super-admins via fa-users), e fluxo "🗑 Excluir" com justificativa obrigatória — soft-delete (excluido:true), reversível via "↺ Restaurar". Calcula e exibe prazo em dias úteis (função diasUteisEntre — sábado/domingo contam como segunda-feira 8h) tanto para pedidos em aberto quanto respondidos.' },
