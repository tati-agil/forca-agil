  ...
  const PAGES = [
    ...
    { label: 'TURMAS', color: '#f5c542',
      features: [
        ...
        { label: 'Inscrito vê apenas o card da própria turma confirmada (as demais ficam ocultas)',    p: ['inscrito','admin'] },
        { label: 'Card da turma confirmada mostra: nome da turma, mês/período CONFIRMADA, dias — sem botões de interesse', p: ['inscrito','admin'] },
        ...
        { label: 'Falha silenciosa na leitura inicial do Firebase (rede instável, regra negando, token ainda propagando) — sem callback de erro nem timeout: se a leitura falhar, o card/botão fica travado no estado estático padrão do HTML, sem nenhuma mensagem de erro. Para visitante, afeta só a checagem de "turma finalizada" (mesmo sem sessão)', p: ['visitante','logado','admin'] },
        ...
        { label: 'Check-in de presença acontece numa página separada (#checkin), não altera o card de Turmas — ver seção CHECK-IN', p: ['inscrito','admin'] },
        { label: 'Botão de interesse é religado só uma vez por elemento (data-wired) — corrigido bug em que revisitar a página Turmas (ou logar/deslogar nela) empilhava um novo listener de clique a cada vez; para logado/admin duplicava registros no log de interesse, para visitante reabria o modal de login repetidamente por clique', p: ['visitante','logado','admin'] },
        { label: 'Botão fica desabilitado durante a gravação no Firebase — corrigido bug em que um clique duplo rápido (antes da resposta chegar) disparava duas gravações e duplicava registros no log de interesse', p: ['logado','admin'] },
      ]
    },
    { label: 'CHECK-IN', color: '#42a5f5',
      features: [
        ...
        { label: 'Pessoa não inscrita nessa turma → "Você não está inscrita nesta turma"', p: ['inscrito'] },
        { label: 'Já fez check-in nesse dia → "Presença já registrada" (nome da turma + dia)', p: ['inscrito'] },
        { label: 'Sucesso → "Presença confirmada com sucesso!" (nome, turma e dia) — grava em turmas-checkin/<turma>/<data>/<emailKey>', p: ['inscrito'] },
      ]
    },
    ...
    { label: 'ADMIN', color: '#ff5252',
      features: [
        ...
        { label: 'Layout responsivo das ações de turma — desktop: todas em linha única (seletor de dia + ações primárias e secundárias inline); mobile/tablet (≤768px): header em coluna, ações primárias visíveis, botão "⋯" abre dropdown com ações secundárias (QR, + Participante, Reabrir, CSV, Certificados)', p: ['admin'] },
        ...
        { label: 'Desfazer check-in — etiquetas "✓ adm" e "✓ qr" na tabela de presença são botões clicáveis; hover mostra risco (line-through); clicar abre confirmação (adminConfirm) e remove o registro do Firebase com frequência atualizada', p: ['admin'] },
        { label: 'Adicionar participante após inscrições fechadas — botão "＋ Participante" abre modal visual com campos Nome, E-mail, Área (mesmas 20 áreas do cadastro); valida @previ.com.br; grava com status inscrito e addedByAdminName (nome do admin logado) sem reabrir turma', p: ['admin'] },
        ...
        { label: 'Ver todos os cadastrados com filtro de busca — colunas: Nome, E-mail, Área, Cadastro (sem XP)', p: ['admin'] },
