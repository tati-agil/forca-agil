  ...
  const PAGES = [
    { label: 'ADMIN', color: '#ff5252',
      features: [
        ...
        { label: 'Aba Turmas — turmas organizadas dentro de containers de evento; turmas sem evento ficam em "TURMAS SEM EVENTO"', p: ['admin'] },
        { label: 'Aba Turmas — criar evento (nome + carga horária + frequência mínima p/ certificado, padrão 75%)', p: ['admin'] },
        { label: 'Aba Turmas — editar evento (renomear, alterar carga horária, alterar percentual mínimo de frequência)', p: ['admin'] },
        { label: 'Aba Turmas — excluir evento (turmas dentro dele passam para "SEM EVENTO")', p: ['admin'] },
        { label: 'Aba Turmas — criar turma (nome + evento + datas dos encontros)', p: ['admin'] },
        { label: 'Aba Turmas — editar turma (renomear, incluir/excluir datas, trocar evento)', p: ['admin'] },
        ...
        { label: 'Aba Turmas — encerrar interesse da turma (card público passa a orientar pro CMFlex, quando turma ainda não iniciou)', p: ['admin'] },
        { label: 'Aba Turmas — encerrar turma manualmente (marca como "Realizada" antes do prazo automático, grava dataConclusao e habilita downloads de certificado)', p: ['admin'] },
        ...
        { label: 'Aba Turmas — adicionar participante por busca em cadastros existentes, escolhendo o status (Interessada ou Inscrita); pessoa precisa ter cadastro prévio no site', p: ['admin'] },
        { label: 'Aba Turmas — justificar por que um interessado não foi confirmado: "Sem vagas" (badge laranja) ou "Substituída" (badge lilás), turma aberta ou com interesse encerrado', p: ['admin'] },
        { label: 'Aba Turmas — remover participante (interessado ou inscrito), turma aberta ou com interesse encerrado', p: ['admin'] },
        { label: 'Aba Turmas — gerar certificados (aba Certificados — selecionar evento → turma; prévia antes de encerrar; após encerrar: badge de frequência por participante, downloads habilitados só para quem atingiu o percentual mínimo do evento)', p: ['admin'] },
    ...
    const ARCH = [
      ...
      },
      {
        ...
        items: [
          ...
          { name: 'Cache (firebase.json)', desc: 'Três regras de header: (1) **/*.js → no-cache; (2) **/*.css → no-cache; (3) /index.html → no-cache + Content-Security-Policy personalizada (restringe fontes permitidas de scripts, estilos, fontes e conexões). JS e CSS são no-cache para garantir que a versão publicada chegue imediatamente a todos os usuários sem cache de navegador. index.html nunca fica em cache pois é a entrada da SPA.' },
