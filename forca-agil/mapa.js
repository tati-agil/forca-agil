...
(function () {
  ...
  const HIERARCHY = [
    ...
    { key: 'logado',  label: 'Logado (sem turma confirmada)', color: '#1ab2ae',
      adds: ['Acessar Início, Turmas e Ajuda', 'Acessar o Repositório', 'Adicionar conteúdos e remover os próprios no Repositório', 'Manifestar interesse em quantas turmas existirem (sem limite fixo)', 'Remover interesse em turmas', 'Entrar na lista de espera de um evento específico para próximas turmas dele (um card por evento com turma na vitrine; dá para esperar por mais de um evento ao mesmo tempo; acesso de Logado mantido enquanto aguarda)', '⚠️ Manifestar interesse NÃO muda o nível de acesso: quem clicou em "Tenho interesse" (status "interessado") vê exatamente as mesmas páginas de quem nunca clicou. A única diferença é o botão do card virar "Remover interesse". Por isso "interessado" não é uma persona separada — quem libera as páginas exclusivas é o admin confirmar a inscrição, não a pessoa manifestar interesse.'] },
    { key: 'inscrito', label: 'Inscrito (turma confirmada pelo admin)', color: '#4caf7d',
      adds: ['Acessar Conteúdos', 'Acessar Treinamento Jedi (autodiagnóstico 0–60)', 'Acessar Avaliação da Oficina (quando o admin libera a avaliação daquela turma)', 'Revelar patente (entra no histórico; a própria pessoa pode refazer sem precisar do admin)', 'Exige status "inscrito" E o campo confirmedByAdmin preenchido — é isso que vira o nível "enrolled" no código e destrava as 3 páginas acima.'] },
    { key: 'admin',   label: 'Admin (marcação à parte, não é degrau)',          color: '#6b7a99',
      adds: ['NÃO é o degrau seguinte de Inscrito — é uma marcação paralela: quem é admin recebe o nível "enrolled" automaticamente, mesmo sem estar inscrito em turma nenhuma. Por isso enxerga Conteúdos e Avaliação sem precisar de confirmação de admin, e vê todos os treinamentos cadastrados (o Treinamento deixou de depender do nível de acesso: ele pertence a eventos).', 'Acessar o Painel Admin (15 abas: Dashboard, Eventos, Certificados, Treinamentos, Repositório, Cadastrados, Administradores, Tipos de atividade, Diretores, Facilitadores, Manual, Mapa, Testes, Pedidos, Sorteios)', 'Criar, editar e excluir turmas; cadastrar link do CMFlex por turma', 'Confirmar/desconfirmar inscrição e adicionar/remover participantes manualmente', 'Abrir/fechar check-in, gerar QR Code e emitir certificados de participação', 'Exportar CSV (estado atual, histórico e por turma)', 'Gerar lista de presença por turma pronta para impressão/PDF', 'Moderar Repositório (ocultar, restaurar, deletar conteúdos)', 'Ver e gerenciar cadastrados (filtrar, redefinir senha, resetar progresso, criar conta diretamente com senha 12345678, confirmar cadastro manualmente para quem não recebeu o link de verificação de e-mail)', 'Gerenciar lista de espera: ver quem entrou, mover pessoa diretamente para uma turma como Inscrita ou remover da lista; migrar participante de uma turma para a lista de espera (caixa dentro do modal de "Remover" — data original de interesse é preservada)', 'Gerenciar lista de administradores (restrito a tatianefdirene e danielfrazao)', 'Gerenciar lista de diretores — aba separada de Administradores, qualquer admin pode adicionar/remover; só decide quem enxerga um evento marcado "Visível só para diretores e administradores", não dá acesso de admin', 'Gerenciar lista de facilitadores — mesmo formato da aba Diretores, mas decide quem enxerga o link e a página Facilitador (#facilitador) no menu, não dá acesso de admin', 'Configurar formato dos encontros (presencial/remoto/híbrido), modalidade e participação de cada evento — chips do bloco "Como funciona" na página Turmas', 'Consultar Manual, Mapa e Testes (documentação viva do sistema)'] },
  ];
  ...
  const PAGES = [
    { label: 'ADMIN', color: '#6b7a99',
      features: [
        ...
        { label: 'Aba Eventos — editar o conteúdo público do evento na página Turmas: Missão (título e texto), Tópicos abordados, e um itinerário dia a dia (adicionar/remover dias, cada um com um título) — tudo opcional, e some da página pública quando vazio', p: ['admin'] },
        { label: 'Aba Eventos — excluir evento (turmas dentro dele passam para "SEM EVENTO")', p: ['admin'] },
        { label: 'Aba Eventos — criar turma (nome + evento + datas dos encontros + caixa "Público restrito")', p: ['admin'] },
        { label: 'Aba Eventos — editar turma (renomear, incluir/excluir datas, trocar evento, marcar/desmarcar "Público restrito")', p: ['admin'] },
        { label: 'Aba Eventos — turma de público restrito: a caixa "Público restrito" fecha a turma, e a lista de quem pode participar (busca no cadastro, uma pessoa por vez, remover por linha) se abre por dois caminhos equivalentes: o selo "👥 PÚBLICO RESTRITO DESTA TURMA · N" no cabeçalho, que é botão, e o menu "⋯" → "👥 Público restrito desta turma". O próprio modal "＋ Participante" também leva até ela, por um botão no aviso de público restrito. Dentro da lista, "＋ Adicionar à turma as N que ainda não estão" matricula de uma vez, como Inscritas, quem está na lista e fora da turma — quarta porta para o estado "inscrita", com as mesmas recusas das outras três (público restrito do evento e da turma, e uma inscrição por pessoa). Fora da lista ninguém vê a turma no site nem pode ser adicionado por você — as quatro portas (Confirmar, "＋ Participante", "Mover para turma" e "＋ Adicionar à turma as N que ainda não estão") recusam e dizem que é preciso incluir na lista primeiro. Turma aberta e turma restrita convivem no mesmo evento', p: ['admin'] },
        { label: 'Aba Eventos — selo "👥 PÚBLICO RESTRITO DESTA TURMA · N" no cabeçalho da turma fechada, clicável (abre a lista da turma sem recolher o card), e aviso com nomes quando alguém está na turma (ou no evento dela, se também for restrito, com seu próprio selo "PÚBLICO RESTRITO DO EVENTO · N" no cabeçalho do evento) sem estar na lista correspondente (marcar a caixa numa turma que já tem gente não remove ninguém) — o aviso vem com um botão "Incluir as N na lista" que regulariza todo mundo dele de uma vez, sem precisar buscar pessoa por pessoa no "👥 Público restrito" do nível certo. Os rótulos dizem "desta turma"/"do evento" de propósito: os dois níveis são listas separadas, e sem o nível escrito os dois pareciam a mesma restrição duplicada na tela', p: ['admin'] },
        ...
        { label: 'Aba Eventos — gerar lista de presença por turma pronta para impressão/PDF (só inscritos, com presença por dia e assinatura), para entregar fora do sistema (ex.: RH)', p: ['admin'] },
        ...
        { label: 'Aba Eventos — liberar/encerrar a dinâmica Construção da Aposta por turma (grava apostaHabilitada) — inscritos confirmados daquela turma passam a ver o convite da dinâmica dentro do Treinamento. Encerrar só tira da vista: o que os grupos escreveram continua guardado em apostas/<turma>', p: ['admin'] },
        ...
        { label: 'Aba Cadastrados — criar conta diretamente para colaboradora (nome + e-mail + área, lista fechada com as 26 áreas + senha do admin; senha padrão 12345678; sem verificação de e-mail — não precisa ser uma caixa real, adminApproved pula a checagem)', p: ['admin'] },
        ...
        { label: 'Aba Eventos — a fila é do evento (eventoKey, gravado na migração ou no join direto pelo card do site) e mora DENTRO do card daquele evento; guarda um registro por PESSOA e por ORIGEM (turma de onde saiu, ou "lista:<eventoKey>" para quem entrou pelo card do site); quem passou por duas turmas do MESMO evento aparece duas vezes, e sair da mesma turma de novo mantém um registro só, com a data mais antiga. Registro de antes desta separação, sem eventoKey, cai numa seção "sem evento" à parte', p: ['admin'] },
        { label: 'Aba Eventos — tirar alguém da fila (Remover da lista, Mover para turma, ou o botão do próprio site) vale só para as origens DAQUELE evento: se ela também espera por outro evento, essa espera não é tocada', p: ['admin'] },
        { label: 'Aba Eventos — seção recolhida "Saíram da fila" abaixo da lista: quem já esteve na fila e não está mais, com "🗑 excluir registro" por linha (apaga de vez, para teste e duplicado; só em quem já saiu, e não toca no cadastro nem nas turmas), origem, quando saiu, destino e motivo, e a autoria quando ela está gravada — nome do admin, ou "pela própria pessoa" quando a saída foi pelo botão do card do site. Sair da fila nunca apaga o registro, mas até então não havia como enxergá-lo', p: ['admin'] },
        { label: 'Aba Eventos — bloco de conferência acima da Lista de Espera de CADA evento: mostra por que a soma dos "Foram para a espera" das turmas daquele evento não bate com o tamanho da fila dele (uma entrada por PESSOA contra uma saída por TURMA) e separa o que é diferença esperada do que é registro engolido pela sobrescrita', p: ['admin'] },
        { label: 'Aba Eventos — Lista de Espera: duas colunas de data lado a lado — Data interesse (define a ordem da fila) e Data remoção (quando saiu da turma) —, ambas com data E hora no formato brasileiro (10/08/2026 12:26); a hora sempre esteve gravada, só não era exibida', p: ['admin'] },
        { label: 'Aba Eventos — Lista de Espera: coluna "Origem" mostra de qual turma a pessoa foi migrada e o motivo; quem entrou direto pelo card do site (por evento) aparece como "Entrou pela lista", com traço na data de remoção', p: ['admin'] },
        { label: 'Aba Eventos — Lista de Espera: remover da lista exige escolher o motivo (Desistiu / Já participou / Não respondeu / Evento encerrado / Duplicado / Outro); "Outro" abre campo de texto obrigatório, e "Já participou" pede QUAL turma ela já fez, como na saída da turma', p: ['admin'] },
        ...
        { label: 'Aba Pedidos — ver todos os pedidos enviados pelo site, com filtro por tipo (tema / curso / material / dúvida / iniciativas do Time Força Ágil / outros), contagem por tipo e reenquadramento (mudar o tipo de um pedido já enviado, com registro de quem mudou)', p: ['admin'] },
    ...
    { label: 'AJUDA', color: '#7ecbff',
      features: [
        ...
        { label: 'Formulário "Faça um pedido" — escolher tipo (Quero aprender sobre um tema / Quero sugerir um curso / Preciso de material / Tenho uma dúvida / Quero fazer parte de iniciativas do Time Força Ágil / Outros) e descrever com mais detalhes', p: ['logado','inscrito','admin'] },
    ...
    { label: 'CADASTRAR', color: '#9b7fff',
      features: [
        ...
        { label: 'Selecionar área/setor — dropdown com campo de busca por digitação (26 áreas — gerências, diretorias e presidência —, filtra em tempo real)', p: ['visitante'] },
    ...
    { label: 'ENTRAR', color: '#c084fc',
      features: [
        ...
        { label: 'Rede de segurança: se o servidor não responder em 10 segundos, a tela de login aparece assim mesmo com o aviso "Não conseguimos retomar sua sessão" — antes o site ficava totalmente preto, sem login e sem explicação', p: ['visitante','logado','inscrito','admin'] },
    ...
    { label: 'FACILITADOR', color: '#8bc34a',
      features: [
        { label: 'Página nova (#facilitador), protegida como #admin — exige isAdmin() OU isFacilitador() (router.js); sem acesso, redireciona pra #home, tanto pelo link do menu quanto por URL direta/F5. Link "Facilitador" no menu (entre Minha Área e Ajuda) só aparece pra quem tem acesso. Conteúdo ainda é placeholder ("em construção") — quem gerencia a lista de facilitadores é a aba Facilitadores do painel, ao lado de Diretores', p: ['admin'] },
      ]
    },
    ...
    { label: 'MINHA ÁREA', color: '#26a69a',
      features: [
        ...
        { label: 'Dentro do card, "Encontros: dia N" (ou "dias N, M e O" com mais de uma data) — mesma correção do card da página Turmas: o número sozinho depois de "Encontros:" lia como se fosse uma contagem, não uma data; vale também na lista de "Turmas abertas no momento" (quem nunca interagiu) e no card de "Inscrições em análise". O certificado usa a mesma data por um campo separado, sem esse rótulo — formato próprio do documento, não muda', p: ['logado','inscrito','admin'] },
    ...
    { label: 'AVALIAÇÃO', color: '#a78bfa',
      features: [
        ...
        { label: 'Treinamento: para quem está confirmada numa turma com a Construção da Aposta liberada, aparece no topo o convite da dinâmica, que abre em tela cheia. Turma sem a flag não mostra nada — o convite não ocupa espaço nenhum para quem não vai fazer a dinâmica', p: ['inscrito','admin'] },
    ...
    { label: 'TURMAS', color: '#f5c542',
      features: [
        { label: 'Grid com um card por turma cadastrada — as mesmas turmas aparecem pra todos os perfis autenticados (logado, inscrito ou admin), nenhuma é escondida por perfil, EXCETO um evento marcado "Visível só para diretores e administradores": esse some pra quem não está na aba Diretores nem é admin; cada card mostra nome, mês, dia(s) do mês (rotulado "dia N" ou "dias N, M e O" — nunca o número sozinho, que sem a palavra "dia" na frente não lê como data) e horário de início/término (por turma — ver "Início"/"Término" em ✎ Editar Turma; turma sem horário salvo usa 09:00–13:00); o estado do card muda automaticamente por data ou ação do admin', p: ['logado','inscrito','admin'] },
        ...
        { label: 'Card de turma com interesse encerrado (antes do primeiro dia) — "Inscrições encerradas: as vagas já foram preenchidas; a turma será realizada no dia [N]/nos dias [N, M e O]". Sem botão e sem link para o CMFlex: o interesse é encerrado porque as vagas do CMFlex acabaram', p: ['logado','inscrito','admin'] },
        { label: 'Card de turma Em andamento — a partir do primeiro dia da turma, o card troca automaticamente para "Turma em andamento · As aulas estão acontecendo. Acompanhe as próximas turmas." sem nenhum botão', p: ['logado','inscrito','admin'] },
        ...
        { label: 'Bloco "A Missão" + "Como funciona" + itinerário dia a dia por evento (título, texto, tópicos, métricas calculadas do itinerário/carga horária) — só quando o admin preencheu esse conteúdo naquele evento. Vem logo abaixo do título do evento (ANTES da grade de turmas), dentro de um acordeão ("Sobre [nome do evento]", com seta): fechado por padrão quando a vitrine tem 2+ eventos (o próprio resumo fechado já marca onde um evento termina e o outro começa, sem rolar o bloco inteiro pra notar a virada); com um evento só, abre sozinho, igual sempre foi. Fica antes da grade, não depois, para a seção terminar na grade de turmas (o que tem ação), não numa tira de resumo solta embaixo de vários cards', p: ['logado','inscrito','admin'] },
      ]
    },
  ];
  ...
  /* Converte a descrição em HTML estruturado. Antes era texto corrido com "•"
     ...
     (ex: as 21 estruturas do banco + 6 regras de segurança) viravam um paredão
  ...
  function render() {
    ...
    html += '<p class="mapa-sub" style="margin-top:10px">Visitante não logado não acessa NADA: com o login obrigatório, o site inteiro fica oculto e ele vê apenas o modal de autenticação (entrar, cadastrar, recuperar senha). "Turmas" mostra as mesmas turmas pra todos os perfis autenticados — nenhuma é escondida por perfil, com uma exceção: um evento marcado "Visível só para diretores e administradores" some pra quem não está na aba Diretores nem é admin, como se estivesse fora da página. Fora esse caso, o que muda de um card para outro é o estado da turma (interesse aberto, inscrições encerradas, em andamento ou realizada) e, dentro de uma turma aberta, o botão de interesse conforme o status da própria pessoa (Tenho interesse / Remover interesse / ✓ Inscrita travado). "Avaliação" marca inscrito como ✓ porque é o nível mínimo exigido, mas na prática só aparece pra quem está confirmado numa turma com avaliacaoHabilitada:true — admin vê sempre, independente do flag, pra poder revisar/testar.</p>';
    ...
    const ARCH = [
      ...
      },
      {
        ...
        items: [
          ...
          { name: 'Firebase Realtime Database', desc: '27 estruturas principais (equivalente a tabelas), agrupadas pelo que representam:\n\nQUEM\n• fa-users — perfil de cada cadastrado; nome e área são editáveis pelo painel (aba Cadastrados → "✎ Editar"), e o e-mail só quando o cadastro foi criado pelo próprio painel (createdByAdmin), porque é a única conta cuja senha o painel conhece\n• fa-users-log/<emailKey> — histórico das correções de cadastro: o que mudou, de quê para quê, por qual admin, quando e em quantos registros a correção foi propagada. Só admin escreve. Existe porque um nome que muda sem rastro vira mistério — ninguém sabe depois se foi correção ou engano\n• fa-admins — lista de quem é admin\n• fa-diretores — lista de quem enxerga, na página Turmas, um evento marcado "Visível só para diretores e administradores"; não dá acesso de admin, gerenciável por qualquer admin (não só os dois super-admins)\n• fa-facilitadores — mesmo formato de fa-diretores, decide quem enxerga o link e a página Facilitador (#facilitador) no menu\n\nO QUE SE OFERECE\n• eventos/<evento> — o container pai: nome, carga horária, frequência mínima para certificado, o conteúdo público da página Turmas (missão, tópicos e itinerário dia a dia), publicado (aparece na página Turmas), restritoADiretores (some da vitrine de quem não está em fa-diretores nem é admin, com publicado continuando true), publicoRestrito (evento fechado a um público predefinido: só quem está em eventos-publico daquele evento vê e participa, em qualquer turma dele), formato (presencial/remoto/hibrido, chip "dia(s) de encontro(s)"), modalidadeLabel/modalidadeDesc e publicoLabel/publicoDesc (os outros dois chips do bloco "Como funciona" — todos vazios por padrão, texto de sempre preservado). Turmas e treinamentos pendem daqui. Escrita: qualquer admin de verdade (os dois super-admins ou quem está em fa-admins) — chegou a ficar restrita só aos dois super-admins por um erro de cópia da regra de fa-admins (faltava o "ou está em fa-admins"), o que deixava qualquer admin abrir \"+ Novo evento\"/\"Editar evento\" no painel normalmente e ter a gravação recusada; corrigido nas regras do banco (ver skill editar-e-salvar)\n• eventos-publico/<evento>/<emailKey> — o público restrito de um EVENTO fechado: quem PODE vê-lo e participar dele. Mesmo formato e mesmas regras de turmas-publico (escrita só de admin), um nível acima: vale para TODAS as turmas do evento e para o card de lista de espera dele, e é o caminho para abrir um evento a um público predefinido sem repetir a mesma lista de e-mails em cada turma nova. As duas restrições são independentes e se somam — turma restrita dentro de evento restrito exige estar nas duas listas. Diferente de restritoADiretores, que vale a aba Diretores inteira: aqui vale só quem a admin listar\n• turmas/<turma> — nome, datas dos encontros, link do CMFlex, a QUAL evento pertence (eventoKey) e publicoRestrito (turma fechada: só quem está em turmas-publico daquela turma participa); criada/editada/excluída pelo admin (não são mais fixas em código)\n• treinamentos/<treinamento> — nome, conteudoKey e eventos: { <eventoKey>: true } — a QUAIS eventos aquele treinamento pertence. Quem está confirmada numa turma de um desses eventos tem acesso a ele. conteudoKey aponta para um conjunto do catálogo em game-data.js; VAZIO significa conteúdo próprio, escrito no painel e guardado em treinamentos-conteudo\n• treinamentos-conteudo/<treinamento> — o conteúdo de um treinamento criado no painel: blocos (cada um com título, ícone e a lista de afirmações), levels (os rótulos da escala de resposta — a nota de cada afirmação vai de 0 a levels-1) e ranks (as patentes, com faixa de pontuação, descrição, características, próximos passos e frase). Existe para que um treinamento novo não dependa de publicação do site: antes o conteúdo vivia só no código e, na prática, só existia um treinamento. Conteúdo pela metade não chega a quem responde — o mesmo critério (window.faTreinoConteudo em game-data.js) decide dos dois lados: o painel mostra o que falta, a página não oferece\n\nQUEM ↔ O QUE (é aqui que mora a regra de negócio)\n• turmas-interesse/<turma>/<emailKey> — a ligação pessoa ↔ turma: status vira "inscrito" quando o admin confirma alguém que já estava interessada, ou quando adiciona alguém direto como inscrita pelo "＋ Participante" (inclusive ao mover da lista de espera) — nunca automaticamente só por encerrar o interesse da turma. Guarda também a saída: removed, motivo, destino e autoria\n• fa-espera/<emailKey>/<origem> — a ligação pessoa ↔ evento na fila de espera. Uma entrada por pessoa E por origem (a turma de onde saiu, ou "lista:<eventoKey>" para quem entrou pelo card do site); cada uma com eventoKey, name, email, area, date (data original de interesse, preservada na migração) e removed (soft-delete)\n• turmas-publico/<turma>/<emailKey> — o público restrito de uma turma fechada: quem PODE estar nela. Vale nos dois sentidos — a turma não aparece no site de quem está fora, e o painel recusa Confirmar / ＋ Participante / Mover para turma para quem não está na lista. Guarda name, email, area, date e quem incluiu. Único nó de turma cuja ESCRITA é só de admin nas regras do banco; turmas-interesse aceita escrita de qualquer pessoa logada, então é aqui que a restrição tem dente de verdade. Estar na lista não põe ninguém na turma: a lista diz quem pode, turmas-interesse diz quem está\n• turmas-interesse-log/<turma>/<emailKey> — histórico de quem registrou/removeu interesse, e quando\n\nA TURMA ACONTECENDO\n• turmas-config/<turma> — configuração da turma (finalizada, encerrada, dia de check-in ativo, dataConclusao)\n• turmas-checkin/<turma>/<data>/<emailKey> — presença registrada em cada dia\n• turmas-sorteio/<turma>/<sorteioKey> — sorteios feitos na turma; cada registro guarda os ganhadores (nome e e-mail), quando foi sorteado, quem sorteou e se excluiu quem já havia sido sorteado\n• avaliacoes/<turmaKey>/<emailKey> — a avaliação da oficina respondida por cada pessoa, por turma\n• apostas/<turma> — a dinâmica Construção da Aposta daquela turma: atual aponta a execução em curso, e execucoes/<execId> guarda uma execução inteira (missão cadastrada pela facilitadora — em lacunas, e é ela que abre pré-preenchida na etapa 1 dos grupos —, revelado, quem abriu e quando) com grupos/<grupoId> dentro — nome, membros, etapa em que o grupo está e dados de cada uma das nove etapas (Ciclo 1 implícito); a partir do Ciclo 2 (Fase 4), esses mesmos dados congelam ali e passam a viver em ciclos/porId/<cicloId>/dados, com ciclos/atual apontando para o ciclo em edição — nenhum grupo de ciclo único ganha essa estrutura à toa. grupos-resumo/<grupoId> (Fase 5, mesma chave do grupo real) é um espelho não sensível — só nome e qtdMembros, nunca membros/etapa/dados/ciclos — que existe para a tela de escolha de grupo funcionar sem que quem participa precise ler o conteúdo dos grupos dos outros; nasce junto com o grupo (mesmo update() atômico) e qtdMembros só é aceito pelo servidor quando bate com a contagem real de membros. Reiniciar a dinâmica cria uma execução NOVA e não toca na anterior: o que um grupo escreveu numa oficina não pode sumir porque outra turma usou a mesma tela depois. Só aparece para quem está confirmada numa turma com apostaHabilitada\n\nCONTEÚDO E PROGRESSO\n• holocron — conteúdos enviados no Repositório (o nó chama-se holocron, sem prefixo; fa-holocron-hidden guarda quais estão ocultos pela moderação)\n• fa-progress/<emailKey> — progresso de cada pessoa (autodiagnóstico do treinamento, conteúdos lidos), sincronizado para funcionar em qualquer navegador\n• fa-progress-historico/<emailKey>/<treino> — uma linha por vez que a pessoa revelou a patente daquele treinamento (data, patente e pontuação), não apagada quando ela refaz o autodiagnóstico — refazer só reabre o quiz, o histórico é que registra cada rodada\n• pedidos — pedidos abertos pelas pessoas, com status e resposta\n\nO ROTEIRO DE FACILITAÇÃO\n• roteiros-evento/<evento> — o roteiro-base do evento: dias (nome, ordem, janela de horário) e atividades (título, tipo, duração, horário, objetivo, resultado esperado, observações, sub-etapas). Toda turma criada no evento herda este roteiro\n• turmas-roteiro/<turma> — a personalização daquela turma sobre o roteiro-base: o que foi alterado, acrescentado ou removido só ali. O roteiro efetivo da turma é a base mesclada com isto, nunca uma cópia — mudar a base reflete nas turmas que não personalizaram aquele ponto\n• turmas-equipe/<turma>/<emailKey> — a equipe de facilitação daquela turma, com papel responsavel (um por turma) ou facilitador. É diferente de fa-facilitadores: aquele nó só libera a rota #facilitador, este diz de quais turmas a pessoa faz parte de fato\n• roteiro-tipos-atividade — a lista editável de \"Tipo de atividade\" (Abertura, Dinâmica, Conceituação, Intervalo…) usada pelos roteiros, gerenciada na aba \"Tipos de atividade\" do painel. Semeada com a lista padrão de roteiro.js na primeira vez que a aba é aberta com o nó vazio\n\n\nAUXILIARES: fa-seeds-hidden e fa-seeds-deleted (moderação dos conteúdos-semente do Repositório) e fa-reset-signal (avisa o navegador da pessoa que o admin resetou o progresso dela).\n\nLEGADO, não usar: fa-ranking não é lido nem escrito por nenhum código hoje, e players só sobrevive numa consulta por e-mail no painel — os dois são resquício de versões anteriores e não fazem parte do modelo atual.\n\nRegras de segurança (7 regras — todas aplicadas no servidor Firebase, não no cliente):\n• Ninguém lê ou escreve na raiz do banco diretamente\n• Todos os nós exigem autenticação com e-mail @previ.com.br para leitura e escrita — não existe nenhum nó público; mesmo quem descobrir a URL do banco não lê nada sem autenticar\n• A verificação de domínio é feita no servidor (auth.token.email), não só na tela — impossível burlar pelo browser\n• Qualquer admin @previ.com.br pode fazer tudo no painel\n• Só tatianefdirene e danielfrazao podem adicionar/remover admins\n• fa-diretores e fa-facilitadores são mais permissivos: qualquer admin (não só os dois super-admins) pode adicionar/remover diretores e facilitadores\n• apostas/<turma> (Fase 5) é a exceção ao domínio-basta-para-escrever: nem todo @previ.com.br autenticado escreve em qualquer turma. Participante só grava no próprio grupo, dentro da execução ATUAL e nunca num ciclo já FINALIZADO; facilitador precisa das DUAS condições — flag global fa-facilitadores E vínculo real em turmas-equipe NAQUELA turma (a flag sozinha não abre mais turma nenhuma); execução com status encerrada é imutável no servidor, não só na tela. A LEITURA também é restrita, não só a escrita: participante confirmado nunca lê a execução inteira (isso cascatearia para os grupos de todos os outros) — só missao/revelado soltos, grupos-resumo/ (nome+qtdMembros de cada grupo, para escolher) e a PRÓPRIA chave em membros/ de qualquer grupo (para descobrir a qual pertence); o conteúdo de um grupo (membros, dados, ciclos) só é lido por quem é membro dele ou conduz a turma. Ver AUTH_FACILITADOR_TURMA em forca-agil/aposta.js e os testes em .github/scripts/teste-rules.js' },
      ...
      },
      {
        ...
        items: [
          { name: 'index.html',                  desc: 'Entrada única da aplicação. Contém todo o HTML, carrega os scripts e gerencia as seções por hash (#home, #turmas…). São 11 seções, entre elas #minha-area (área do participante) e #facilitador ("Minhas Facilitações", visível só a admin/facilitador), e o painel Admin com suas 15 abas — a última é Sorteios' },
          ...
          { name: 'forca-agil/turmas-util.js',   desc: 'Utilidade compartilhada de turmas — deriva mês e texto de datas ("Agosto", "11, 12, 18, 19 e 20") a partir do array de datas ISO de cada turma, usada por app.js, admin.js e checkin.js. listarPessoasVerComo(cb) junta turmas-interesse com turmas e devolve, por pessoa, nome/e-mail/situação mais relevante (confirmada > inscrita sem confirmação > interesse > removida) — a consulta por trás do seletor "Ver esta tela como", extraída daqui para a Minha Área (aluno.js) e a página Turmas (app.js) reusarem a mesma pergunta em vez de duplicá-la. API: window.faTurmasUtil' },
          { name: 'forca-agil/roteiro.js',       desc: 'Roteiro de Facilitação — dados e telas, sem página própria: usado por admin.js (edição) e facilitador.js (leitura, ou edição pela responsável). Roteiro-base por evento (roteiros-evento/<evento>/dias e /atividades, ordem reordenável com ▲▼); cada turma personaliza por cima sem tocar o base (turmas-roteiro/<turma>/customizacoes — só os campos alterados, o resto continua herdando; /exclusivas — atividades que existem só ali; /facilitacao — condução principal/apoio por atividade, sempre por turma). Visão da turma ordena por horário de início, não por uma posição fixa — evita ter que decidir onde uma atividade exclusiva entra no meio das atividades base. Equipe de facilitação por turma em turmas-equipe/<turma>, papel responsavel|facilitador; único responsável garantido por construção (o campo turmas/<turma>/responsavelFacilitadorKey e o papel na linha da equipe trocam juntos, na mesma escrita atômica — faRoteiro.definirResponsavel). roteiros-evento/$eventoKey é gravável por qualquer admin de verdade (os dois super-admins ou quem está em fa-admins), mesmo padrão de fa-diretores/fa-facilitadores/turmas-publico — chegou a ficar restrito só aos dois super-admins por engano (cópia da regra de fa-admins sem o "ou está em fa-admins"), o que deixava outro admin abrir e editar o formulário normalmente e ter a escrita recusada em silêncio; corrigido nas regras do banco. Toda gravação deste arquivo (criar/editar/excluir/mover/duplicar atividade e dia, customização e exclusividade por turma, condução) passa pelo helper interno ok(cb), que mostra um alerta de erro em vez de simplesmente recarregar como se tivesse dado certo — proteção que continua valendo para qualquer outra causa de falha (rede, por exemplo), não só aquela regra. As caixas de texto rico (Objetivo, Passo a passo etc.) decidem se um valor é HTML novo ou texto puro antigo olhando só se tem alguma tag reconhecida — esc() (usada nesse caminho) precisa ser IDEMPOTENTE por causa disso: um valor sem tag nenhuma mas com um "&" (um "&nbsp;" digitado à mão, ou um "&" de verdade no texto) é reavaliado TODA vez que a atividade é aberta, então escapar de novo um "&" que já introduz uma entidade de verdade acumularia mais uma camada a cada abrir-e-salvar, mesmo sem editar aquele campo — foi assim que um "&nbsp;" virou "&amp;nbsp;" literal na tela. API: window.faRoteiro (renderRoteiroBaseEditor, renderRoteiroTurma, carregarEquipeTurma/Global, definirResponsavel, adicionarFacilitadorApoio, removerDaEquipe)' },
          { name: 'forca-agil/router.js',        desc: 'Roteamento por hash — controla qual seção da página está visível. Rede de segurança contra tela preta: se a autenticação não resolver em 10 segundos, o login é exibido assim mesmo com aviso e um diagnóstico que testa os quatro caminhos (gstatic, identitytoolkit, banco por HTTPS e banco por WebSocket) e, quando nada está bloqueado, mede os tempos de carregamento. Login obrigatório: o body inteiro fica oculto (class "aguardando-auth") até o Firebase confirmar a sessão via evento fa-auth-ready; visitante não autenticado vê apenas o modal de login com fundo totalmente opaco (sem botão de fechar, sem site por baixo). Após login, a classe é removida e o site é revelado. Rotas protegidas: #conteudos e #treinamento exigem nível "enrolled"; demais rotas exigem apenas "member" — #minha-area é deliberadamente uma delas, para quem ainda não tem turma confirmada poder acompanhar a própria situação. Nível detectado via window.faAuth.getAccessLevel(). #admin e #facilitador não passam por nível de acesso: exigem isAdmin() (a segunda também aceita isFacilitador()) — sem isso, redireciona pra #home, tanto em navigate() (clique no menu) quanto em show() (link direto, F5, voltar/avançar do navegador). Só que "não tem acesso" e "ainda não sei" são coisas diferentes, e confundir os dois expulsava gente legítima num F5, reescrevendo a URL sem volta. Dois jeitos de confundir: (1) isAuthReady() também vira true no ramo "sem usuário" do onAuthStateChanged, então existe uma janela com sessão nula antes da sessão real chegar — tratar essa janela como "não logado" atingia QUALQUER admin, super-admin inclusive; (2) isAdmin()/isFacilitador() respondem false enquanto as leituras de fa-admins/fa-facilitadores não voltam, e elas correm em paralelo à sessão — isso atingia só quem não está na lista fixa de super-admins, justamente quem não testava. Por isso show() só redireciona quando há sessão resolvida E as listas confirmaram (faAuth.isAdminReady()/isFacilitadorReady()); sem sessão, quem cobre a tela é o modal de login forçado, e manter o endereço leva a pessoa ao lugar certo depois de entrar. Quando as listas chegam, fa-admin-ready/fa-facilitador-ready mandam decidir de novo. navigate() (clique no menu) continua exigindo sessão, porque só se chega nele por ação explícita e o link fica escondido pra quem não tem acesso. API: window.faRouter' },
          { name: 'forca-agil/auth.js',          desc: 'Autenticação — login, cadastro, logout, redefinição de senha. Senha exige apenas dígitos numéricos (mín. 8). Verifica adminship lendo só o próprio registro em fa-admins — leitura assíncrona que corre em paralelo à sessão, então isAdmin() responde false até ela chegar; isAdminReady() diz se esse false já é confiável, e o evento fa-admin-ready avisa quem decidiu antes. Esse par existe porque o mesmo engano — tratar "ainda não sei" como "não é admin" — apareceu em cinco lugares diferentes, todos com o mesmo sintoma de punir o próprio admin: o router expulsando de #admin e das rotas por nível de acesso num F5; enforceCurrentRouteAccess barrando Conteúdos/Treinamento com um aviso falso de nível (pior pra admin que não está inscrito em turma nenhuma, pra quem isAdmin() é a única fonte de "enrolled"); avaliacao.js caindo no fluxo de participante em vez do modo admin; admin.js desistindo de carregar o painel de vez, com as abas presas em "Carregando…"; e init.js mostrando "Acesso Restrito" pro próprio admin. São DUAS esperas, não uma: além da lista de admins, o getAccessLevel() só devolve "enrolled" pra admin se _session já existir — e a lista costuma chegar ANTES da sessão, então checar só a lista deixava passar uma janela em que o admin ainda parecia "member" e era expulso de Conteúdos/Treinamento/Avaliação. A regra pra quem escrever código novo: se a decisão for definitiva (redirecionar, esconder, desistir de carregar), exija sessão resolvida E isAdminReady() antes de decidir, e refaça no fa-admin-ready. Detecta perfil de acesso via window.faAuth.getAccessLevel() → "member" (logado sem turma confirmada) ou "enrolled" (inscrito com turma confirmada pelo admin). "Enrolled" exige status="inscrito" E confirmedByAdmin preenchido — manifestar interesse (status="interessado") não concede acesso. Perfil "guest" removido — login é obrigatório. Admin sempre recebe "enrolled". Dispara evento fa-auth-ready ao concluir verificação inicial. Verifica diretor em paralelo, lendo só o próprio registro em fa-diretores (window.faAuth.isDiretor) — não afeta getAccessLevel(), só decide visibilidade de eventos restritos em app.js; dispara fa-diretor-ready quando resolve, para quem já tiver renderizado a página Turmas antes disso conseguir re-render. Facilitador é o mesmo padrão, outro nó (fa-facilitadores, window.faAuth.isFacilitador) — decide acesso à rota #facilitador (router.js) e a visibilidade do link no menu (.nav-link-facilitador, atualizado dentro do próprio listener). autoPreviDominio(input): preenche "@previ.com.br" no foco de um campo de e-mail vazio, cursor antes do @; exposta em window.faAuth pra admin.js aplicar nos próprios campos de e-mail (criados dinamicamente, depois deste arquivo já ter carregado). API pública: window.faAuth' },
          { name: 'forca-agil/stars.js',         desc: 'Animação de estrelas do fundo (canvas)' },
          { name: 'forca-agil/app.js',           desc: 'Interações de UI — nav scroll, reveals, botões de interesse em turmas (perfil logado sem turma), controle de acesso por perfil via getAccessLevel(), crawl de abertura (exibido para qualquer sessão autenticada — não exige turma confirmada; pausar/retomar no clique, botão "Ler texto" para modo estático). Página Turmas: renderTurmasGrid agrupa os cards por evento (.turmas-evento-grupo; grupo sem título para turma sem evento). Ordem dos grupos: NÃO é mais só o campo "order" do admin — prioridadeEvento(ev, souDiretor) ordena por quão exclusivo o evento é PARA QUEM está vendo (0 = restritoADiretores e a pessoa é diretora; 1 = publicoRestrito — só chega aqui quem já passou por eventoVisivelPara, então já está na lista; 2 = sem nenhuma restrição), com "order" como desempate dentro do mesmo nível (Array.sort é estável). Mesma ideia um nível abaixo, entre as turmas de um mesmo evento: prioridadeTurma(t) põe turma de publicoRestrito (a pessoa já está na lista, mesmo raciocínio) antes da aberta. Existia pedido de ordenação nenhum antes disto: quem era ao mesmo tempo diretora e alvo de uma lista via as duas turmas na ordem que o admin tivesse digitado, sem refletir que uma delas é mais dirigida a ela. O cabeçalho de cada grupo (.turmas-evento-cab) usa o idioma de seção do site — eyebrow em mono acima (.turmas-evento-eyebrow, com traço via ::before) e o nome em Oswald abaixo (.turmas-evento-titulo), um degrau acima do "A Missão" que vem dentro do próprio bloco; entre um grupo e o outro, régua e respiro (margin-top + padding-top + border-top no seletor irmão). Já foi só uma linha de mono .78rem: com um evento passava, mas era ele que tinha de marcar onde um bloco termina e o outro começa. Níveis de título: h1 da página > h2 do evento > h3 da Missão > h4 de "Como funciona"/"A jornada dia a dia" — só a tag muda, .sec-title controla fonte, tamanho, espaçamento e margem. Quais eventos entram: eventoNaVitrine(ev) — foi divulgado (eventos/<evento>/publicado, ausente vale como publicado), E, se restritoADiretores estiver marcado, a sessão atual é admin OU está em fa-diretores (window.faAuth.isAdmin/isDiretor; sem sessão ou sem nenhum dos dois, o evento inteiro some, mesmo tendo publicado:true) E, se publicoRestrito estiver marcado, a sessão atual é admin OU está em eventos-publico daquele evento (eventoVisivelPara — mesma forma de turmaVisivelPara, um nível acima; lista vazia esconde o evento de todo mundo que não é admin) E tem turma OU aceita fila (esperaAtiva). publicado é a chave-mestra: desmarcada, o evento inteiro some da página das pessoas — turmas, card de espera e Missão. Não tira acesso nem esconde nada no painel: o nível de acesso vem de turmas-interesse (auth.js não consulta publicado) e a Minha Área lê o evento pela chave (aluno.js também não consulta); no painel o evento segue completo, com um selo "fora da página" no cabeçalho. restritoADiretores e publicoRestrito são filtros por PESSOA, diferentes de publicado (que é tudo ou nada pra todo mundo): usam _sess (setado em initTurmaInterest a cada render) e reagem a fa-auth-change, a fa-diretor-ready E a fa-admin-ready — os dois últimos cobrem o caso de isDiretor()/isAdmin() resolverem depois do primeiro render da página (sem o de admin, a própria admin que não está na lista ficava sem ver a turma ou o evento restrito até navegar de novo). turmas-publico e eventos-publico são lidos na MESMA corrente das turmas, antes de desenhar: chegar depois faria a turma ou o evento restrito piscar na tela de quem não pode vê-los, e um vislumbre já é o vazamento que a restrição existe para evitar; leitura que falha esvazia as duas listas e esconde tudo que é restrito, porque falhar escondendo é o lado seguro. As portas de escrita revalidam no clique contra o banco, não contra o que a página leu ao abrir: registerInterest (checarPublicoRestrito, evento antes da turma) e joinEspera (a fila é do evento, então a lista do evento vale nela igual). Mesma regra da fila: o admin decide se aparece, ninguém some por não estar aparecendo. A fila é do evento e não depende de turma, então evento sem turma nenhuma com fila ligada aparece só com o título e o card de espera; é assim que ela mede demanda antes de o admin decidir quantas turmas abrir. A mensagem "Nenhuma turma aberta no momento" só sai quando não há NADA a mostrar — antes ela era disparada por _turmasList vazia, o que escondia justamente esse caso dentro do container .turmas-eventos — turmaCardHtml ficou extraído à parte para servir os grupos. Card de lista de espera (renderEsperaCard) deixou de ser único e permanente: agora é um por evento que está na vitrine E cujo admin aceita fila (eventos/<evento>/esperaAtiva — ausente vale como ligada, para o evento que existia antes deste controle continuar mostrando o card sem migração; a caixa fica em openEventoFormModal e evento novo já nasce marcado), anexado ao final da .turmas-grid daquele grupo, escrevendo em fa-espera/<email>/lista:<eventoKey> (ORIGEM_DIRETA + eventoKey, ver ehOrigemDireta em turmas-util.js) — permite esperar por mais de um evento sem uma entrada sobrescrever a outra. checkEsperaState/joinEspera/removeEspera recebem eventoKey e só enxergam/mexem nas entradas daquele evento; entrar/sair não altera nível de acesso. renderMissaoEventos monta, para cada evento na vitrine (mesmo eventoNaVitrine da grade — sem isso, um evento que aparece só com o card de espera ficaria sem uma linha explicando o que ele é) E com conteúdo preenchido (missaoTexto, topicos ou itinerario), um bloco "A Missão"/"Como funciona"/"Plano de Voo" escrito DENTRO do grupo do próprio evento (.turmas-evento-missao, aberto por renderTurmasGrid) — cada evento é um bloco fechado na página: título, turmas, card de espera e missão dele. Já houve um container único .turmas-missao-eventos no fim da página, fora dos grupos: com um evento dava na mesma, com dois a página listava todas as grades e só depois todas as missões, e cada missão ficava órfã do evento dela — a pessoa via os cards do segundo evento seguidos da descrição do primeiro. Geometria conferida ao mudar: aninhar não desloca nada, porque .turmas-evento-grupo não tem padding próprio e o .section continua aplicando os mesmos 40px de recuo do texto em relação aos cards reaproveitando as classes CSS do bloco estático que existia antes (.section, .oficina-info, .agenda, .day day--static) — dias e horas por encontro na grade "Como funciona" são calculados (itinerario.length; cargaHoraria / dias), não digitados à parte. Elementos injetados dinamicamente usam "reveal in" (não só "reveal"): o coletor de scroll (collectReveals) já rodou no carregamento da página antes desses elementos existirem, então sem o "in" eles ficariam invisíveis até o próximo scroll/resize. Os quatro chips de "Como funciona" vêm de tabelas fixas no próprio app.js: FORMATO_TXT resolve o singular/plural certo do chip "dia(s) de encontro(s)" pra cada eventos/<evento>/formato (presencial/remoto/hibrido — hibrido é os dois formatos AO MESMO TEMPO no mesmo encontro, não dias alternados); PUBLICO_TXT resolve a frase completa pra cada eventos/<evento>/publicoDesc, guardado como só a categoria (Diretores/Executivos/Cedidos/Quadro próprio — "Quadro próprio" concorda no singular, "todo o" em vez de "todos os"), em DUAS variantes por categoria: "aberta a todos os... da Previ" (padrão) ou "restrita a uma lista de... da Previ" quando o evento também é eventos/<evento>/publicoRestrito — sem isso, um evento "Executivos" com público restrito dizia "aberta a todos os Executivos", falso duas vezes (nem aberta, nem a todos). Modalidade (ev.modalidadeLabel/modalidadeDesc) e o chip de Participação (ev.publicoLabel) têm padrão "Prática"/"dinâmicas, jogos e exercícios em grupo" e "Opcional" quando o campo está vazio — nenhum evento já existente muda sozinho. VER COMO (montarBarraVerComoTurmas, só admin): mesmo recurso que já existia na Minha Área (aluno.js), inserido antes de .turmas-eventos, com a mesma lista de pessoas (window.faTurmasUtil.listarPessoasVerComo). Troca só _sess — o que eventoVisivelPara/turmaVisivelPara/eventoNaVitrine e a ordenação de renderTurmasGrid usam para decidir o que aparece — nunca o `sess` que initInterestButtons/renderEsperaCard recebem, então "Tenho interesse" e "Entrar na lista de espera" continuam gravando em nome de quem está de fato logada; o preview nunca escreve nada em nome de outra pessoa. Persiste em _verComoPessoa (fora de initTurmaInterest) porque essa função roda de novo a cada fa-admin-ready/fa-diretor-ready — resetar a escolha do admin a cada uma dessas rodadas derrubaria o preview sem ele mexer em nada.' },
          ...
          { name: 'forca-agil/repo.js',          desc: 'Repositório (Holocron) — listagem, adição, remoção de conteúdos. render() só desenha os cards dentro de três listeners assíncronos do Firebase (fa-seeds-hidden, fa-holocron-hidden, holocron); #repoGrid nasce com "Carregando conteúdos…" (.loading-msg) antes da primeira resposta, porque sem isso a grade ficava vazia e indistinguível de "não tem nada aqui" ou "quebrou" — em rede lenta, gente clicava no filtro "Todos" (já ativo) tentando "religar" a tela, e funcionava só por coincidência com a resposta chegando por perto. render() troca esse aviso pelos cards de verdade no primeiro innerHTML=\'\' de cada chamada.' },
          { name: 'forca-agil/game-data.js',     desc: 'Conteúdo dos treinamentos — 4 blocos × 5 afirmações (BLOCOS), patentes (RANKS), níveis Likert (LEVELS). DIMS computado dinamicamente a partir de BLOCOS. MISSIONS existe no arquivo mas não é usada em v3. Expõe window.faGameConteudos, o CATÁLOGO de conteúdos indexado por chave (hoje só "jedi"): um treinamento cadastrado no banco aponta para um conjunto daqui pela conteudoKey — ou deixa conteudoKey VAZIO e tem conteúdo próprio, escrito no painel (nó treinamentos-conteudo). Expõe também window.faTreinoConteudo, o CONTRATO desse conteúdo — normalizar() (o banco devolve objeto com chaves numéricas onde deveria haver lista, e campo faltando não pode virar tela quebrada), problemas() (o que IMPEDE alguém de responder: sem afirmação, sem patente, faixa de pontuação com buraco, sobreposição ou invertida) e avisos() (não impede, mas não é o que se quis dizer: patente que ninguém alcança). É um critério só, usado pelo painel para dizer o que falta e pela página para decidir se oferece — se cada lado tivesse o seu, o painel diria "pronto" numa tela onde a pessoa vê um quiz sem pergunta. O conteúdo continua no código de propósito — dá para criar/ligar/desligar treinamento e trocar seus eventos sem deploy, mas conteúdo NOVO exige um conjunto novo aqui. window.faGameData segue apontando para o conteúdo "jedi" (compatibilidade: game.js e os testes já liam de lá)' },
          { name: 'forca-agil/game.js',          desc: 'Treinamento — autodiagnóstico, painel de patente e revelar patente. dentro de um cartão fechado por padrão (#treinoCard) — o mesmo peso visual do convite da Construção da Aposta (.convite-card, compartilhada entre os dois em pages.css) logo acima: antes o treinamento era um banner grande, sempre expandido, e o convite um card pequeno, empilhados como se fossem de telas diferentes. O cartão mostra o resumo sem abrir (nome do treinamento, status: não iniciado / X respondidas / pronto para revelar / patente revelada) e abre/fecha com "Abrir treinamento →" / "← Recolher"; troca de treinamento pelo seletor fecha o cartão de novo, para não deixar o corpo de um sobre o resumo de outro. REVELAR grava uma linha em fa-progress-historico (data, patente, pontuação) e a tela do resultado mostra esse histórico, mais recente primeiro. "🔁 Refazer autodiagnóstico" reabre o quiz zerado — pede confirmação num modal próprio (não window.confirm, para ficar no mesmo estilo do resto do site) e NÃO apaga o histórico, é um caminho diferente do reset do admin (que apaga tudo). O CONTEÚDO vem de dois lugares, resolvidos por conteudoDe(): do catálogo em código (conteudoKey) ou de treinamentos-conteudo/<treino>, escrito no painel; os dois passam pelo mesmo normalizador, então o resto da página não sabe de onde veio. A escada de patentes é DESENHADA a partir das patentes do treinamento ativo (desenharEscada) — era HTML fixo no index.html com os quatro nomes Jedi, o que já mostrava a escada errada em qualquer treinamento que não fosse ele. A pontuação máxima sai da escala do conteúdo (PONTO_MAX = levels-1), não dos 3 pontos fixos de antes. Treinamento com conteúdo incompleto NÃO é oferecido a quem responde, e o admin — que vê todos — recebe na própria página a lista do que falta (#treinoIncompleto). ACESSO POR EVENTO, não por nível: carregarDisponiveis() cruza o nó treinamentos (cada um com seus eventos) com os eventos das turmas em que a pessoa está confirmada (status inscrito, não removida); admin vê todos, como na Avaliação. Enquanto o nó treinamentos estiver VAZIO (ele só nasce quando um admin abre o painel), cai num modo de compatibilidade que mantém o comportamento anterior — inscrita ou admin veem o conteúdo padrão —, para não haver janela em que o Treinamento some do site para todo mundo, inclusive para o admin. O link do menu (.nav-link-trein) é controlado aqui, não por auth.js — quem não tem nenhum treinamento não vê o link e cai em "Nenhum treinamento disponível". Com mais de um, um seletor no topo troca entre eles (com um só ele não aparece). O quiz é remontado a cada troca (buildQuiz), porque dois treinamentos podem apontar para conteúdos diferentes. PROGRESSO POR TREINAMENTO: a chave fa-game-v3 guarda um mapa treinamento → {quiz, revealed} — antes era um progresso único, que o segundo treinamento apagaria; o formato antigo continua sendo lido e vale como o progresso do treinamento ativo, para ninguém perder o que já respondeu' },
          { name: 'forca-agil/admin.js',         desc: 'Painel Admin — turmas (criar/editar/excluir, com datas dinâmicas em turmas/ no Firebase — não são mais fixas em código), interessados, moderação de repositório, gestão de admins, Chave de e-mail: TODOS os módulos usam a mesma função — minúsculas, arroba e ponto viram sublinhado, o resto é descartado, corte em 64. A aba Certificados já teve uma variante própria (ponto virando vírgula) que nunca casava com a chave do check-in e mostrava 0% de frequência para todo mundo. exportação CSV UTF-8 BOM via URL.createObjectURL (window.faToXls — compartilhada por Manual e Testes; acentos corretos no Excel, arquivo sempre editável), barra expandir/recolher com botões por seção. Saída da turma por linha da tabela de participantes: um único botão "Remover" (modal adminConfirmComMotivo, lista MOTIVOS_REMOCAO_TURMA) grava removedMotivo/removedReason; uma caixa dentro desse modal migra a pessoa para fa-espera/ preservando a data original de interesse, gravando motivoEntrada/motivoEntradaDetalhe. O motivo "Substituída por outra pessoa" abre a escolha de quem ficou com a vaga (substituidaPor/substituidaPorNome). Remover da lista de espera também exige motivo (MOTIVOS_ESPERA_SAIDA), gravado em motivoSaida/motivoSaidaDetalhe. Ambas as listas incluem "Evento encerrado"; a opção "Outro" abre campo de texto obrigatório. Aba Cadastrados: coluna Situação com os dois selos juntos (Ativo/Bloqueado e Pendente/Verificado); "Confirmar cadastro" para quem ainda não clicou no link (seta adminApproved:true + registra quem aprovou); botão "✎ Editar" por linha (openCadastroEditModal) para corrigir nome, área/gerência e — só em cadastro criado pelo painel — o e-mail. A linha tem seis colunas e só duas portas: "✎ Editar" e o menu "⋯" (cad-mais-btn → openCadastroAcoesModal), que reúne Redefinir senha, Resetar progresso, Confirmar cadastro (só para pendente) e Bloquear/Desbloquear. Eram cinco colunas de botão, todas com a classe vermelha admin-del-btn, numa tabela de dez colunas que estourava a largura do painel (1169px num container de 1100): o que sobrava ficava atrás de uma rolagem lateral cuja barra só aparecia no fim de uma tabela de milhares de pixels. Como o menu é modal, as ações saíram de dentro do tbody e portanto da delegação de clique — por isso elas viraram funções próprias (acaoBloquear/acaoResetarProgresso/acaoConfirmarCadastro e handlePwdReset, que passou a receber e-mail e nome em vez do botão). No celular a tabela vira cartões por CSS (data-label em cada td alimenta o rótulo via ::before, em pages.css), em vez de esconder colunas. teste-acoes-cadastrado.js cobre as duas coisas no CI. O ponto delicado: nome e área NÃO moram só em fa-users, são COPIADOS para dentro de cada inscrição, da fila de espera, das listas de público restrito, da equipe de facilitação e de cada presença (CADASTRO_COPIAS + caminhosDaPessoa + lerCopiasDaPessoa listam onde). Corrigir só o cadastro deixaria o nome velho em todas as outras telas sem erro nenhum, então a edição grava tudo numa update() atômica; se a leitura das cópias falhar, a edição é recusada inteira em vez de gravar metade. Log e sorteios já realizados NÃO são reescritos de propósito — são história. O e-mail é a identidade (login no Auth + chave de tudo): corrigi-lo entra na conta com a senha padrão 12345678 (auth.js corrigirEmailPorAdmin, mesmo vai-e-vem de sessão do criarContaPorAdmin, com _criandoConta), troca o login e só depois move TODOS os dados da chave antiga para a nova (CADASTRO_NOS_POR_CHAVE + moverDadosDePessoa, incluindo pedidos por emailEnviou e holocron por authorEmail, que não são indexados por chave). Essa ordem é proposital: as duas etapas não são atômicas entre si, e trocar o login primeiro deixa a falha recuperável e visível. Se a pessoa já trocou a senha, o login falha e a recusa diz para criar a conta certa e bloquear esta. Toda correção é gravada em fa-users-log. Aba Eventos: dentro do card recolhível de CADA evento, abaixo das turmas dele, uma seção Lista de Espera própria (filtrarEsperaPorEvento por entry.eventoKey; renderEsperaTudo remove e reconstrói as .ev-espera-section a cada loadEspera(), chamado ao final do render de loadInterests() para os containers .ev-turmas-wrap já existirem no DOM) com ações "Mover para turma" (dropdown só com turmas do MESMO evento; adiciona como Inscrita) e "Remover da lista" — as duas escopadas: só mexem nas origens daquele evento. Registro sem eventoKey cai no elemento #adminEspera, uma seção "sem evento" à parte, abaixo de todos os cards — nunca some. A caixa "Aceitar lista de espera" (eventos/<evento>/esperaAtiva) NÃO tem efeito nenhum aqui: ela governa só o card público em app.js. No painel a seção do evento aparece sempre que houver alguém na fila, ligada ou desligada, com as mesmas ações — desligada, ganha só um aviso ACIMA da lista (renderEsperaTudo), nunca no lugar dela. Ninguém pode sumir da lista por marcar ou desmarcar a caixa: desligar impede entrada nova, não apaga registro. migrarEsperaEventoKey() conserta dado velho, não é regra corrente: hoje ninguém entra na fila sem evento, porque o card de espera é de um evento e a pessoa escolhe em qual entrar (renderEsperaCard em app.js) — o sistema não adivinha, quem entra diz. Sobrou só o que foi gravado antes disso: registro vindo de turma copia o eventoKey dela (turmas/<turmaKey>/eventoKey), e registro entrado pelo card antigo, que ainda era um só e não perguntava o evento, recebe o único evento que chegou a ter turma — nenhum outro teve fila para alguém entrar, por isso a conta é de eventos COM TURMA e um evento cadastrado e ainda vazio não muda nada. Roda a cada carga do painel, idempotente. Se um dia dois eventos tiverem turma e ainda houver registro sem dono, não escolhe por conta própria: deixa na seção "sem evento" para o admin resolver. Modal "Novo evento"/"Editar evento" (openEventoFormModal) também edita o conteúdo público da Missão de cada evento — missaoTitulo, missaoTexto, topicos e um editor de itinerario (array de strings, um título por dia; "+ Adicionar dia" e renumeração automática dos rótulos D1/D2... a cada adição/remoção, mesmo padrão do editor de datas da turma). seedMissaoJornadaImersao() é uma migração de ponte, uma vez só: como esse conteúdo era HTML fixo (só a Jornada de Imersão) antes de virar dado por evento, ela grava o texto que já existia na página no evento cujo nome bate exatamente com "FORÇA ÁGIL · JORNADA DE IMERSÃO" — só se ele ainda não tiver itinerário (não sobrescreve o que o admin já editou) — pra não sumir conteúdo do site público no deploy desta mudança. Botão "↓ QR de acesso ao site" baixa PNG estático com logotipo (forca-agil/assets/qrcode-acesso.png), para uso em materiais de divulgação. Botão "🖨 Lista de presença" (imprimirListaPresenca) abre uma nova janela (window.open + document.write, sem depender de CSP porque o documento é montado só em memória, não vem de resposta HTTP) com uma tabela HTML/CSS de impressão (@page A4 paisagem) — só participantes com status Inscrito, presença por dia igual ao CSV, e linhas de assinatura no rodapé; usuária aciona o "Imprimir / salvar como PDF" e usa o diálogo nativo do navegador. Geração de certificado NÃO está mais no menu ⋯ da turma — só existe na aba Certificados (window.faCertif, ver certif.js). Sorteio da turma (menu ⋯ → "🎲 Sorteio"): sorteia entre os participantes CONFIRMADOS (mesmo conjunto do filtro "Confirmados"), com quantidade configurável, opção de não repetir quem já saiu e modo Ensaio que não grava nada; embaralhamento Fisher-Yates com crypto.getRandomValues e descarte do resto para não enviesar; grava em turmas-sorteio/<turma>. O modal declara o modo vigente nos dois sentidos (faixa verde "vale" / âmbar "ensaio") e só confirma "Registrado" após o callback do banco. Aba Sorteios (15ª e última): consolida todos os sorteios de todas as turmas com filtros combináveis por evento e turma (os selects só oferecem o que tem sorteio) e exportação CSV com uma linha por pessoa. A aba Eventos preserva o estado da interface entre recargas (eventos e turmas expandidos, filtro de status por turma, acordeão de Removidos, filtro "Ver evento" e posição da rolagem) — sem isso, registrar presença recolhia tudo e voltava ao topo a cada pessoa. Datas: fmtDate exibe data + hora em pt-BR; uma data gravada SEM hora ("2026-08-10") seria lida como meia-noite em UTC e exibida como 21h do dia anterior no nosso fuso, então esse caso é montado na mão e sai sem hora. Nenhuma informação de XP é exibida no painel. Aba Diretores: lista + formulário de adicionar (nome + e-mail, com autoPreviDominio preenchendo "@previ.com.br" no foco) + botão Remover por linha (loadDiretores, nó fa-diretores); qualquer admin gerencia, ao contrário de Administradores. Aba Facilitadores (loadFacilitadores, nó fa-facilitadores): mesmo formulário de adicionar, mas a listagem ganhou Situação (Ativo/Inativo, clicável), "Próximas turmas" (conta turmas-equipe onde a pessoa está, fase ≠ concluída) e botão "Ver" (ficha com facilitações próximas/anteriores: evento, turma, período, papel) — ver openEquipeModal/openRoteiroEventoModal/openRoteiroTurmaModal e forca-agil/roteiro.js para a Equipe de facilitação e o Roteiro de Facilitação de cada turma/evento (botões no menu ⋯ de cada turma e no cabeçalho de cada evento). Modal "Novo evento"/"Editar evento" também tem: "Visível só para diretores e administradores" (restritoADiretores); "Visível só para um público predefinido (lista de e-mails)" (eventos/<evento>/publicoRestrito), que liga a lista de e-mails do evento — marcada, o cabeçalho do evento ganha o selo "público restrito do evento · N" e o botão "👥 Público restrito do evento", que abre o MESMO openPublicoModal da turma (agora parametrizado por { tipo, key, label }: muda o nó — eventos-publico ou turmas-publico —, o alcance e as palavras, nunca a mecânica) e escreve em eventos-publico/<evento>/<emailKey>; a coluna "Em alguma turma" do modal do evento junta os registros de turmas-interesse de todas as turmas dele, preferindo o registro ativo ao removido; barradoPeloPublico passa a checar o evento ANTES da turma (a restrição mais ampla primeiro, senão a recusa manda consertar na lista errada) e a busca do "＋ Participante" filtra pelas duas listas; marcar a caixa não remove ninguém, e o card de cada turma do evento passa a avisar, com os NOMES, quem está nela sem estar na lista; um select "Formato dos encontros" (presencial/remoto/hibrido — FORMATO_TXT em app.js resolve o texto certo pro chip "dia(s) de encontro(s)"); e os quatro campos do bloco "Como funciona" — Modalidade (chip curto: lista fechada Prática/Teórica/Mista, mais descrição livre) e Participação (chip curto: lista fechada Opcional/Obrigatória) com Quem participa (lista fechada Diretores/Executivos/Cedidos/Quadro próprio, resolvida em texto completo por PUBLICO_TXT em app.js — o <select> guarda só a categoria, o "aberta a todos os... da Previ" é montado no código). "+ Criar conta para colaboradora": e-mail com autoPreviDominio, Área/Setor virou <select> com AREAS_LIST (a mesma lista fechada do cadastro público). O "+ Participante" NÃO tem campo de área: ele busca um cadastro que já existe e herda a área dele; criarContaPorAdmin faz o Firebase logar como a conta nova e depois deslogar/logar de volta como admin — a flag _criandoConta (auth.js) faz os onAuthStateChanged ignorarem essas trocas internas, senão o painel piscava pro login no meio do processo. Nome do evento no cabeçalho do acordeão (evNome): flex:1;min-width:0;overflow-wrap:anywhere, sem white-space:nowrap nem text-overflow:ellipsis — um evento sem público restrito (poucos selos) cabia numa linha só e escondia o bug, mas um COM público restrito ganha dois elementos a mais na mesma linha (selo "público restrito do evento · N" + botão), e o nome perdia a disputa por espaço, virando "…". O nome é a única coisa ali que identifica qual evento é: quebra linha em vez de cortar, nunca o contrário — mas só nesse caso extremo: logo depois do nome vem um span vazio evQuebraLinha (flex-basis:100%) que empurra meta/selos/botões pra uma segunda linha do cabeçalho, deixando toggle+nome sozinhos na primeira. Sem ele, um nome de tamanho normal ("FORÇA ÁGIL - EXECUTIVOS") ainda quebrava em duas linhas por dividir espaço com os selos, ficando visualmente pior do que precisava — quebrar era a exceção seguindo pro caso raro de nome maior que o cabeçalho inteiro, não a regra.' },
          ...
          { name: 'forca-agil/dashboard.js',     desc: 'Aba Dashboard — agrega dados de avaliacoes/ (Firebase) em estatísticas: participantes (soma de inscritos confirmados em turmas-interesse), % participação, média geral e NPS (fórmula clássica: %promotores 9–10 menos %detratores 0–6), distribuição de notas (campo notaGeral) em 5 faixas de duas notas, com cortes em 7 e 9 alinhados ao NPS; a contagem exibida é a de quem respondeu essa pergunta, não o total de avaliações, média por turma agrupada por evento (barras horizontais em HTML/CSS — eram barras verticais em SVG com o nome da turma escrito embaixo, que se sobrepunham a partir da segunda turma e não diziam de qual evento era cada barra), gauge semicircular de recomendação (SVG), temas mais solicitados (agregação de temasDesejados). ESCOPO: a tela inteira é recalculada para os eventos/turmas escolhidos nos dois controles do topo (seleção múltipla; nenhum marcado = tudo). Os controles são botões de tamanho fixo que abrem uma lista em popover (busca acima de 8 opções, lista com rolagem própria): eram chips, um por evento e um por turma, sempre todos à vista — 3 eventos × 6 turmas davam 406px de filtro no desktop e 949px no celular, e cresciam a cada oficina. O clique na barra redesenha a barra, então o fechamento por clique-fora usa um aviso guardado antes do redesenho (veioDaBarra) e não contains(e.target), que responderia sobre um elemento já removido do DOM. montarCatalogo() junta turmas/ + eventos/ + turmas-interesse/ num catálogo com participantes POR TURMA — antes era um total único, que como divisor do % de participação de uma turma daria o número errado. Avaliação de turma que não existe mais em turmas/ entra no catálogo como turma órfã (rótulo gravado na própria avaliação), senão o escopo a descartaria em silêncio e o total mudaria sem explicação. A média do grupo de evento é calculada sobre TODAS as avaliações dele, não é média das médias das turmas. Turma sem avaliação tem media null (mostra \u201c—\u201d), nunca 0. Coberto por .github/scripts/teste-dashboard-escopo.js no CI. "Destaques dos feedbacks" usa só as notas por seção (orgGeral, conteudoRelevancia, facilitadoresNota, dinamicasNota, aplicacaoPreparado) — nunca interpreta texto livre. API: window.faInitDashboard' },
          { name: 'forca-agil/aluno.js',          desc: 'Minha Área — página do participante. A página nunca fica vazia: tem um estado nomeado para cada situação — confirmada em turma (cards agrupados em Em andamento / Programadas / Concluídas), interesse ainda não confirmado ("Inscrições em análise"), lista de espera, e boas-vindas com as turmas abertas para quem nunca interagiu. Os três primeiros podem coexistir; o de boas-vindas só aparece quando não há nenhum dos outros. A fase da turma é derivada dos dados existentes: concluída = admin encerrou; programada = o primeiro encontro ainda não chegou; em andamento = o resto. O selo de contagem ao lado de cada grupo ("Em andamento", "Programadas", "Concluídas") escreve "N turma"/"N turmas" por extenso — um número sozinho ali não diz o que está contando. A lista de espera mostra um cartão por ORIGEM ativa em fa-espera (uma por evento em que a pessoa entrou na fila — ver ORIGEM_DIRETA em turmas-util.js), cada um com o nome do evento; antes mostrava um cartão só, genérico e sem nome, com a mais antiga — quem esperava por mais de um evento não tinha como saber. Reúne o que antes só o admin via: turmas em que a pessoa está confirmada, frequência por encontro (dias presentes ÷ total, comparada ao mínimo do evento), certificado para download em PNG/PDF quando a turma é concluída e a frequência foi atingida (reaproveita window.faCertif, então o certificado é idêntico ao emitido pelo admin), situação da avaliação de cada turma e histórico dos próprios pedidos com status. NÃO expõe o QR Code de check-in — o QR é o mesmo todos os dias e daria para registrar presença sem estar no encontro, corrompendo a frequência que define o certificado. turmasAbertas() (o resumo de boas-vindas) respeita as mesmas regras de visibilidade de eventoNaVitrine em app.js — publicado===false nunca aparece, evento restrito a diretores só aparece pra quem é admin ou diretor — usando o e-mail de quem está vendo a tela: o real, ou o do "ver como" quando um admin simula a visão de outra pessoa (é exatamente o que "ver como" promete). A lista de pessoas do seletor vem de window.faTurmasUtil.listarPessoasVerComo — mesma consulta que a página Turmas usa para o seletor equivalente dela (app.js). API: window.faMinhaArea' },
          { name: 'forca-agil/aposta.js',         desc: 'Construção da Aposta — a dinâmica guiada de nove etapas (Missão, Sintoma, Problema, Mudanças mensuráveis, Hipótese, Ideia de solução, Experimento, Evidência, Decisão). Não tem rota própria: entra como um convite dentro do Treinamento (#apostaEntrada) e abre em TELA CHEIA, porque é feita para ser projetada numa sala. Quem vê o convite tem de estar CONFIRMADA numa turma com turmas/<turma>/apostaHabilitada — o mesmo par de exigências da Avaliação; admin e facilitadora veem sempre, inclusive turma ainda NÃO liberada, marcada \"· ensaio\" e com aviso de que só a facilitação está vendo — liberar é um ato público, e sem isso o ensaio da facilitadora estrearia na frente da turma. A trilha lateral mostra os NÚMEROS das nove etapas desde o começo, mas não os nomes das que ainda não chegaram — a revelação vai até onde o grupo chegou e é contínua, sem buracos — e elas não clicam: o valor da dinâmica está em percorrer o raciocínio, e tanto um formulário inteiro à mostra quanto o nome da etapa seguinte fazem o grupo pular direto para a resposta. Cada etapa é preenchida por LACUNAS dentro da própria frase — o texto fixo à vista, só as lacunas digitáveis, a frase se montando ao vivo embaixo com o que falta marcado no lugar e clicável; o que não entra na frase (responsável, custo, prazo) fica num bloco de complementos. O Problema é só [QUEM É AFETADO] + [SITUAÇÃO INDESEJADA] — sem campo à parte para "evidenciado por" nem para concordância de verbo (quem escreve digita o verbo dentro da própria situação indesejada), sem repetir o Sintoma (que fica só leitura no card acima) e com aviso didático se repetir. Refinamento — etapas opcionais: Sintoma e Ideia de solução (só essas duas) podem ser puladas por decisão explícita ("Pular esta etapa", discreto; grava `pulada: true` no lugar do conteúdo) — nunca bloqueiam CONTINUAR, contam como concluídas na trilha (selo "Pulada") e somem por completo do Mapa, da exportação em texto/CSV e dos prompts de IA (nunca "não informado"/"pulado"); Ideia é dado do ciclo, então cada ciclo novo decide de novo. `apostaBaseIA`/`faltamParaAnaliseIA` tratam as duas como "preencher OU pular", nunca como preenchimento obrigatório. A Hipótese virou duas frases — "Acreditamos que isso acontece porque…" e "Essa hipótese surgiu porque observamos que…", sem o antigo "pois" — porque o sinal observado motiva a hipótese, não a comprova. A frase da Evidência ("Esperávamos…") não repete mais o prazo da mudança mensurável (só a unidade/período). A Ideia de solução ganha um bloco só leitura, "Resultados que queremos produzir", entre a Hipótese e o título — todas as Mudanças mensuráveis já cadastradas, com seta de direção (↑ Aumentar, ↓ Reduzir, → Manter/Atingir). Prazo e duração são número + unidade (lista de segundos a anos, com a flexão feita pelo site), datas usam dd/mm/aaaa com máscara, custo sai com máscara de moeda, e em Mudanças mensuráveis a primeira já vem na tela em branco (sem contar como preenchida). Em Mudanças mensuráveis, "Direção da mudança" tem quatro opções — Aumentar, Reduzir, Manter (com Tipo de limite: Pelo menos/No máximo/Entre) e Atingir — cada uma com seu próprio molde de frase. Cada mudança tem um id interno: o Experimento escolhe (por esse id, em "O que vamos observar?") quais mudanças este teste observa, mostrando a lista à parte no "fica assim no mapa" (não emendada na frase do experimento), e a Evidência monta um card por escolha, com indicador/situação inicial/meta ou limite/unidade herdados e não editáveis ali. A etapa inteira (nunca card a card, já que o experimento é um só) alterna entre dois modos: Planejamento da evidência — Fonte prevista + Detalhe/como será medido, com "Resultado observado" ainda inexistente na tela — e Registro dos resultados — Resultado observado + Fonte utilizada (pré-selecionada com a prevista) + "O que aprendemos com esta evidência?". A troca de modo é um único clique no botão principal da etapa ("Salvar plano para execução" vira "Registrar resultados do experimento" assim que a Fonte prevista de todos os cards estiver definida, com confirmação "O experimento já foi executado?" antes de trocar de verdade), que pode acontecer dias ou semanas depois de planejar; até lá, a etapa mostra "✓ Plano de evidência salvo. Pronto para execução." com a opção de encerrar por agora (feedback antes de fechar), e quem retomar depois vê "PLANO DE EVIDÊNCIA SALVO — Aguardando execução do experimento.". No modo de registro, uma lista ao vivo mostra exatamente o que falta por resultado até virar "✓ Evidências completas.". Em toda a dinâmica, o botão principal bloqueado fica genuinely disabled (nunca só "amarelo sem funcionar"); na Decisão, "Reformular a hipótese" passou a exigir a Nova Hipótese completa para desbloquear CONTINUAR. O aprendizado é por resultado, não um veredito único — não existe mais classificação de "Nossa hipótese foi". Cada card calcula sozinho o avanço: "% do caminho até a meta" (a razão entre a mudança obtida e a pedida, que já inverte para "Reduzir") para Aumentar/Reduzir/Atingir, ou "Dentro"/"Fora do limite" para Manter; um card pode ser marcado "Não foi possível medir". Na Evidência, Continuar bloqueia de verdade (não só um aviso) até cada resultado ter observado+fonte+aprendizado ou não-medido+motivo — como também bloqueiam Hipótese, Ideia de solução, Experimento e Decisão enquanto faltar campo obrigatório, e Mudanças mensuráveis quando a direção contradiz os próprios números; sem observado ainda, a tela e o Mapa avisam "aguardando execução" em vez de fingir uma frase completa. A Decisão herda por id também a fonte e o aprendizado, além de situação inicial/meta/observado; ela também grava sozinha (sem o grupo digitar) a Data da Decisão, para auditoria, diferente da Data de reavaliação (quando o grupo PRETENDE voltar a olhar). Os quatro tempos da dinâmica têm rótulo próprio, para nunca se confundirem: "Prazo para atingir o resultado" (Mudanças mensuráveis), "Duração do experimento", "Prazo da próxima ação" (Decisão, antes só "Prazo") e "Data de reavaliação"; "Responsável" da Decisão virou "Responsável pela próxima ação". Na Ideia de solução, "para" é fixo no molde ("Poderíamos... para...") — se o segundo campo também começar com "para"/"para que", o site evita "para para" duplicado. A Decisão tem sete opções — Ampliar, Ajustar e testar novamente, Interromper esta ideia, Investigar mais, Reformular a hipótese, Rever a mudança mensurável e Rever o problema (as duas últimas, da Fase 4) —, cada uma com dica própria, e o bloco de Evidência no topo mostra só indicador/situação inicial/meta/observado, sem classificar automaticamente a hipótese ("Portanto, nossa hipótese foi X" saiu dessa tela). Fase 4 — Ciclos de Aprendizagem: "Ampliar" e "Interromper esta ideia" só encerram o ciclo atual; as outras cinco abrem um Ciclo novo — três com ponto de reinício fixo (Reformular a hipótese → Hipótese, Rever a mudança mensurável → Mudanças mensuráveis, Rever o problema → Problema) e duas ("Ajustar e testar novamente"/"Investigar mais") pedindo à dupla para escolher onde recomeçar, obrigatório para desbloquear CONTINUAR. Tudo antes do ponto de reinício é herdado do ciclo anterior; do ponto de reinício em diante, nasce vazio. Editar de propósito um campo herdado dispara cascata: tudo que vem depois dele NO MESMO ciclo (mesmo o que a dupla já preencheu nesse ciclo novo) volta a "ainda não preenchida", o ponto de reinício efetivo do ciclo passa a ser essa etapa, e um aviso conta o que foi limpo — nunca fica implicitamente válido em cima de um dado que acabou de mudar; o ciclo anterior (congelado) nunca é tocado por essa revisão. Depois da Decisão confirmada, o ciclo fica concluído: voltar a editar qualquer etapa dele (pelo Mapa ou pela trilha) pede confirmação para abrir um Ciclo novo a partir dali, nunca reabre e regrava por cima silenciosamente. O Mapa mostra uma seção por ciclo quando há mais de um — só o ciclo atual é editável, os anteriores são somente leitura, igual ao Histórico de Execuções; o CSV ganha as colunas Ciclo e Ponto de reinício. Criar um ciclo é protegido pelo mesmo lock com token da criação de execução (Fase 1) — nunca dois ciclos sucessores para a mesma decisão. A Nova Hipótese (mesmo molde da etapa Hipótese) não aparece do mesmo jeito para toda decisão: some para Ampliar/Investigar mais, fica recolhida (com o convite "Reformular hipótese também") para Ajustar e testar novamente/Interromper esta ideia, e abre sozinha, obrigatória, só para Reformular a hipótese — que também é a única a acrescentar "Nova hipótese: …" em "Fica assim no mapa". Valor antigo fora do formato não é reinterpretado: sai do campo e fica escrito ao lado. Do mapa, a aposta sai em PDF (impressão do navegador, com folha própria) e em texto markdown para colar onde for analisada. A missão que a facilitação cadastra no painel usa as mesmas lacunas da etapa 1 e abre pré-preenchida para os grupos, que podem ajustar. Cada etapa mostra a anterior num card de conexão, salva sozinha (com aviso visível quando a gravação falha) e valida sem bloquear na maioria dos casos — se o sintoma explicar a causa, se a hipótese vier como certeza, se a ideia focar em tecnologia antes da mudança, se a Evidência usar linguagem de prova, aparece um convite a reler e o segundo clique segue assim mesmo (os bloqueios de verdade — campo obrigatório faltando ou direção incoerente em Mudanças mensuráveis — são a exceção, listados acima); as regras são de texto, no navegador, porque o site não tem servidor e chave de modelo no cliente é chave publicada. Os termos Objective, Key Result e OKR NÃO existem em lugar nenhum da interface até a facilitadora clicar em "Revelar conexões" — a revelação final é o ponto alto da oficina e quem sabe que está escrevendo um Key Result escreve para agradar o conceito, não para descrever a realidade. Painel do facilitador: cadastrar uma missão-base opcional (com prévia "fica assim" compacta; botão Salvar/Atualizar missão-base, selo "✓ Missão-base salva" e "Remover missão-base" quando já existir uma) — vale só para grupos que ainda não escreveram a própria missão, nunca sobrescreve quem já começou —, criar grupos, acompanhar o progresso de cada um (X/9 etapas, com status "não iniciado"/"em andamento"/"concluído"), projetar um grupo, exportar CSV e reiniciar (execução nova, a anterior intacta). Grava em apostas/<turma>. API: window.faAposta. Escopo atual encerrado nas Fases 1-5 (execuções, Histórico, proteção contra duplo clique, Ciclos de Aprendizagem, Security Rules e isolamento entre grupos) — evoluções futuras previstas mas NÃO iniciadas: Fase 6 ("Meu Espaço"/"Minhas Apostas", um lugar para a pessoa ver as próprias apostas fora da sala) e Fase 7 (cópia dinâmica → pessoal, levar o conteúdo de uma execução de turma para um espaço individual). Qualquer trabalho nelas começa como escopo novo, a partir da main, preservando as Fases 1-5 intactas.' },
          { name: 'forca-agil/facilitador.js',    desc: '"Minhas Facilitações" — página #facilitador. Lê turmas-equipe inteiro e filtra pela chave da pessoa logada: mostra só as turmas em que ela está associada, nunca o painel administrativo. Agrupa por fase — Em andamento / Programadas / Concluídas — com o mesmo critério de aluno.js. Cada card mostra evento, turma, período e o papel ("⭐ Você é o responsável" ou "Você participa como facilitador"); botão "Abrir roteiro" chama window.faRoteiro.renderRoteiroTurma com editable só quando a pessoa é a responsável daquela turma — a mesma função e os mesmos dados que o admin usa, não uma tela paralela. API: window.faInitFacilitador' },
          { name: 'forca-agil/pedidos.js',       desc: 'Formulário público "Faça um pedido" (seção Ajuda) e painel admin Aba Pedidos. Público: seleção de tipo (TIPOS: tema/curso/material/dúvida/iniciativas/outros — a CHAVE é o que vai gravado em pedidos/<id>/tipo e nunca muda depois de existir, porque pedido antigo continua apontando para ela; o rótulo é só o que se lê e pode ser reescrito) + textarea + envio autenticado para Firebase pedidos/. O selo do tipo no painel quebra linha quando precisa: com nowrap, o primeiro rótulo longo passava do fim do card no celular e era cortado em silêncio. REENQUADRAR: "⇄ Mudar tipo" grava tipo + tipoAnterior (sempre o ORIGINAL, não o imediatamente anterior) + tipoAlteradoEm + tipoAlteradoPor (a sessão logada, não um select de admins como nos fluxos de responder/excluir — aqueles podem ser registrados em nome de outra pessoa depois do fato, este é correção na hora). Salvar o mesmo tipo não grava. O card passa a mostrar a linha .ped-admin-reenquadrado: mudar o tipo é mexer no que a pessoa escolheu e não pode ficar invisível. Admin: lê node pedidos/, filtro por status sobre ativos (Pendentes/Respondidos/Todos) + botão de Lixeira separado para os excluídos (dimensões distintas, evita "Todos" ambíguo), combinados com filtro por tipo cujas contagens seguem o recorte atual; ordem mais recente primeiro. Cada item tem link mailto: (Responder por e-mail — abre o cliente de e-mail do admin, não envia pelo site), fluxo "Marcar como respondido"/"✎ Editar" com seletor de qual admin respondeu e data/hora editável (fa-admins + super-admins via fa-users), e fluxo "🗑 Excluir" com justificativa obrigatória — soft-delete (excluido:true), reversível via "↺ Restaurar". Calcula e exibe prazo em dias úteis (função diasUteisEntre — sábado/domingo contam como segunda-feira 8h) tanto para pedidos em aberto quanto respondidos.' },
          ...
          { name: 'forca-agil/testes.js',        desc: 'Testes automatizados de regressão — técnicos e de comportamento, lista de regras manuais pendentes, botão exportar todos os testes em CSV (automáticos e manuais). Títulos, motivos, nomes de grupo e mensagens de erro passam por esc() antes de entrar na string de HTML: esses textos descrevem a tela usando marcadores como "<turma>", "<seu nome>" e "<select>", que sem escape o navegador lê como TAG. Com "<select>" o estrago era grave e silencioso — abria um elemento <select> de verdade e os grupos seguintes viravam filhos dele, sumindo da tela (um <select> só desenha <option>) sem nenhum erro no console: a lista parava na Avaliação enquanto o cabeçalho seguia contando os 174. O teste técnico "adm-testes-grupos-somam" guarda essa regra: confere que a soma dos grupos bate com o total do cabeçalho e que nenhum grupo ficou dentro de um <select>.' },
          ...
          { name: 'forca-agil/styles.css',       desc: 'Estilos globais — design tokens, layout, componentes, responsividade. Inclui os estilos da Minha Área (.aluno-*): cartões de turma por fase, barra de frequência, cartões informativos de inscrição em análise, lista de espera e boas-vindas. A regra de botão desabilitado é global e mora aqui: .btn:disabled sai apagado e o efeito de hover está preso em :not(:disabled) — sem isso um botão travado fica idêntico a um clicável, que foi a causa do "iPhone não envia pedido" de 11/09/2026 (ver pedidos.js). Qualquer botão novo herda a regra só por usar a classe .btn; estilo de :disabled próprio em pages.css (ex.: .cert-btn-dl) continua vencendo por ordem de cascata, porque pages.css carrega depois.' },
          ...
          { name: 'forca-agil/certif.js',        desc: 'Geração de certificados de participação — monta o layout com os 6 campos dinâmicos sobre o template PNG (cert-template-v3.png — a v3 é a v2 com o símbolo da Previ do cabeçalho trocado pelo mesmo do site, assets/previ-symbol.png; a arte é idêntica no restante e as posições dos campos não mudaram), calcula frequência e horas por participante elegível, aciona impressão. Cada campo tem posição, largura máxima e tamanho de fonte fixos em CFG; fitFont() encolhe a fonte até o texto caber na largura reservada, respeitando um mínimo por campo. O nome do evento é desenhado letra a letra (spacing), e o cálculo de "cabe ou não cabe" leva esse espaçamento em conta — medir só com measureText subestimava a largura real e deixava o texto estourar a área mesmo tendo "cabido". API: window.faCertif, incluindo CFG (o layout aprovado) e medirCampo(texto, cfg), que devolve o tamanho de fonte final e a largura ocupada — é o que os testes automáticos de regressão do certificado usam para verificar que cada campo cabe, sem depender de conferir a imagem no olho.' },
        ]
      },
      {
      ...
      },
      {
        ...
        items: [
          ...
          { name: 'Regra — testar logo após um deploy', desc: 'Mesmo com Cache-Control: no-cache correto, isso só faz o navegador revalidar o arquivo na PRÓXIMA vez que for buscá-lo — uma aba que já estava aberta antes do deploy terminar continua rodando o JS que já carregou em memória, sem revalidar nada sozinha. Ao conferir uma mudança logo depois do deploy (minutos, às vezes segundos), sempre dar um reload forçado (Ctrl+F5 / Ctrl+Shift+R) antes de concluir que o fix não funcionou — o comportamento antigo aparecendo aí é a aba desatualizada, não o deploy tendo falhado. Confirmar a hora do deploy nas execuções do workflow no GitHub Actions ajuda a diferenciar os dois casos.' },
          { name: 'Banco',       desc: 'Firebase Realtime Database — gratuito até 1 GB de dados e 10 GB/mês de transferência (Spark plan). Nós principais: fa-users, fa-admins, fa-diretores, fa-facilitadores, eventos (pai das turmas E dos treinamentos), turmas, treinamentos, turmas-interesse, turmas-interesse-log, fa-espera, turmas-config, turmas-checkin, turmas-sorteio, avaliacoes, holocron, fa-progress, pedidos. Ver a seção Estrutura de Dados para o que cada um guarda — fa-ranking e players aparecem no banco mas são legado, sem uso no código atual.' },
