...
(function () {
  ...
  const RULES = [
    ...
    { section: 'turmas', personas: ['inscrito'],
    ...
    { section: 'turmas', personas: ['logado', 'admin'],
      ...
      body: 'Disponível apenas para usuário logado sem turma confirmada. O botão reflete o status: "♡ Tenho interesse" — estilo primário (dourado sólido) → "♡ Remover interesse" — estilo secundário (fundo escuro, borda neutra). Pode manifestar interesse em até 3 turmas. Registros em turmas-interesse/<turma>/<emailKey> com campo status (interessado/inscrito). Log em turmas-interesse-log.' },
    { section: 'turmas', personas: ['inscrito', 'admin'],
      title: 'Check-in no evento (QR Code)',
      body: 'Admin abre o check-in escolhendo um dia da turma (select com todos os dias — passado ou futuro) e clicando "Abrir check-in"; isso define diaAtivo em turmas-config. Participante inscrito escaneia o QR Code com a conta logada → página #checkin?turma=t1 → sistema valida (turma finalizada, check-in aberto, inscrita, não fez check-in naquele dia) → registra em turmas-checkin/<turma>/<data>/<emailKey> com source:"qr" → exibe "Presença confirmada com sucesso!". Se não estiver logada, redireciona para login e faz check-in automaticamente após autenticação.' },
    { section: 'turmas', personas: ['visitante', 'logado'],
      title: 'Turma finalizada para não inscrito — "Inscrições encerradas"',
      body: 'Quando uma turma está finalizada e o usuário NÃO está confirmado nela: o botão de interesse desaparece e o card exibe "Inscrições encerradas" (estado bloqueado, sem ação disponível). Não abre modal de login ao clicar. Válido tanto para visitante quanto para usuário logado que não está inscrito nessa turma.' },
    { section: 'turmas', personas: ['visitante', 'logado', 'inscrito', 'admin'],
