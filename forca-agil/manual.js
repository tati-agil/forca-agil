...
(function () {
  ...
  const RULES = [
    ...
    { section: 'admin', personas: ['admin'],
      ...
      body: 'Disponível para qualquer pessoa cadastrada, não só colaboradores. Apaga fa-progress/<emailKey> e remove do ranking (players — busca por email, pois a chave de players é name__turma, não emailKey). Ação irreversível — pede confirmação antes de executar. Após o reset: (1) a tabela recarrega e XP exibe "—"; (2) se a pessoa estiver logada no momento, o listener em tempo real (fa-progress .on value) detecta a deleção, limpa o localStorage automaticamente e dispara fa-progress-change — todos os jogos ficam habilitados para refazer sem precisar relogar ou recarregar.' },
    { section: 'admin', personas: ['admin'],
