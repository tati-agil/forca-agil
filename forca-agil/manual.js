...
(function () {
  ...
  const RULES = [
    ...
    { section: 'quiz', personas: ['logado', 'colaborador', 'admin'],
      ...
      body: 'Mostra a pontuação do minigame e o XP ganho ("+X XP Kyber"). Não mostra patente — a patente é calculada com 5 componentes (auto + missões + kyber + conteúdos + repositório) e exibir um valor parcial nessa tela daria a impressão de um resultado oficial, quando na verdade é só uma das etapas.' },
    { section: 'quiz', personas: ['logado', 'colaborador', 'admin'],
    ...
    { section: 'quiz', personas: ['logado', 'colaborador', 'admin'],
      ...
      body: 'Ao confirmar "Sim, revelar minha patente": soma autodiagnóstico + missões + Kyber + conteúdos + repositório (XP máximo 100) e fixa o resultado — definitivo, não pode ser alterado. Esse resultado é só da pessoa: NÃO publica nada no ranking. Equivalente a "fa-patente-revealed = 1".' },
    { section: 'quiz', personas: ['logado', 'colaborador', 'admin'],
      title: 'Publicar no ranking — etapa separada e opcional',
      body: 'Só aparece depois de revelar. Botão "Publicar no ranking →" é a única ação que envia nome, área, turma e XP para o ranking público (players/ no Firebase) — equivalente a "fa-patente-publicada = 1". Quem revela mas não publica continua com a patente visível só para si, fora do ranking. Não tem como "despublicar" depois.' },
