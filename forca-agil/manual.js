...
(function () {
  ...
  const RULES = [
    ...
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Turmas — encerrar turma (marcar como realizada)',
      body: 'Botão "✓ Encerrar turma" (disponível no menu ⋯ quando o interesse já foi encerrado — ou seja, "finalizada: true" no banco e ainda não encerrada). Ao clicar, pede confirmação e marca a turma como encerrada no banco. O card público passa imediatamente para o estado "Turma realizada" para todos. Esse botão é opcional — se o admin não clicar, o sistema detecta automaticamente o fim da turma no dia seguinte ao último dia cadastrado. Use quando quiser antecipar o estado "Realizada" antes do prazo automático.' },
    { section: 'admin', personas: ['admin'],
    ...
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Turmas — adicionar participante (busca em cadastros)',
    ...
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Turmas — justificativa para interessados não confirmados',
    ...
    { section: 'admin', personas: ['admin'],
      ...
      body: 'Painel de geração de certificados individuais. Selecione a turma e o participante para pré-visualizar. O certificado usa template PNG fixo (cert-template-v2.png) como fundo imutável — o sistema sobrepõe apenas 6 campos dinâmicos: nome do participante, nome do evento, identificação da turma, período, carga horária e data de emissão com prefixo "Emitido em". Canvas interno fixo em 1448 × 1086 px; a prévia escala proporcionalmente (4:3) para caber na tela sem recalcular coordenadas. Textos longos reduzem fonte automaticamente por campo, de forma independente — os demais campos não são afetados. Botões por participante: ⬇ PNG (alta resolução 2×, sem margens) e ⬇ PDF (página customizada 4:3 = 864 × 648 pt, sem margens brancas, certificado ocupa 100% da página). Botões de lote: "Baixar todos (PNG)" e "Baixar todos (PDF)" com intervalo de 800ms entre downloads. Nomes de arquivo: Certificado_NOME.png / Certificado_NOME.pdf.' },
    { section: 'admin', personas: ['admin'],
    ...
    { section: 'admin', personas: ['admin'],
      ...
      body: 'Os 6 campos dinâmicos e suas configurações v1.0 (sistema 1448×1086): (1) nomeParticipante — x:724 y:385 maxW:1041 fonte:55/28px italic branco centralizado; (2) nomeEvento — x:724 y:535 maxW:920 fonte:29/13px bold caixa-alta dourado espaçamento:3 centralizado; (3) identificacaoTurma — x:724 y:581 maxW:680 fonte:19/12px normal dourado-escuro centralizado; (4) periodoTurma — x:598 y:671 maxW:400 fonte:24/13px normal branco-claro alinhado-esquerda; (5) cargaHoraria — x:613 y:757 maxW:160 fonte:48/24px bold dourado centralizado; (6) dataEmissao — x:1055 y:922 maxW:360 fonte:19/10px italic dourado alinhado-esquerda, prefixo fixo "Emitido em ". NÃO alterar estas coordenadas sem gerar nova versão e executar checklist completo.' },
    { section: 'admin', personas: ['admin'],
