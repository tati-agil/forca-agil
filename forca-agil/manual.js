...
(function () {
  ...
  const RULES = [
    ...
    { section: 'admin', personas: ['admin'],
      ...
      body: 'Terceiro botão de migração pontual, ao lado de "🔎 Migração Antes x Depois" (mesma regra de aparecer só no roteiro que já tem a atividade "EPISÓDIO FINAL — O COMPROMISSO DA LIDERANÇA"). Abre o MESMO tipo de janela — DRY RUN primeiro, "Aplicar" só depois de revisar tudo, backup automático de cada campo tocado, "↩ Desfazer última migração" restaura também essa leva — só muda a lista de itens: elimina a duplicidade do "Primeiro uso da IA" (Passo a passo e Preparação prévia trazem prompt/missões antigas enquanto o "Prompt para IA" já tem a versão nova e oficial), termina de corrigir "Saíram das iniciativas para o problema × Outcome × Output" (dessa vez no Passo a passo de "Agora dê nome ao que eles fizeram" — a Conexão com a mentalidade ágil já tinha sido corrigida numa leva anterior), aumenta a duração de "Decisão sem IA" de 5 para 8 minutos com recálculo de horário, simplifica "Faça a comparação" de 5 para 3 perguntas principais e revê os três conceitos do fechamento final (Experimentação, Empirismo, Feedback). Três mecanismos novos, todos com a mesma regra de nunca decidir sozinha na dúvida: (1) itens de "substituir campo inteiro" (Passo a passo/Preparação prévia do Primeiro uso da IA, Dicas de "Apresentação do desafio") não têm um "DE" pra localizar — nunca viram "OK PARA ALTERAR" sozinhos, sempre caem em revisão manual (com os botões de sempre: Manter atual/Substituir pelo PARA/Mesclar manualmente) mesmo quando o valor atual só precisa ser trocado, só "JÁ ATUALIZADO" quando o campo já bate com o texto novo; (2) o item de duração ("Decisão sem IA": 5→8 min) já mostra na própria linha da tabela, sem precisar clicar em nada, a duração atual, a nova, o horário final recalculado e a lista de atividades seguintes do mesmo dia (nível principal, com horário próprio) cujo horário de início desliza a mesma diferença — nunca mexe na duração de nenhuma outra atividade, só no início/fim delas; se a duração atual não bater nem com o "de" nem com o "para" esperado, vira revisão manual com um painel próprio (Manter atual / Aplicar os novos minutos mesmo assim); (3) os três itens do fechamento final (Experimentação/Empirismo/Feedback) não têm título de atividade fixo informado — o sistema procura o texto antigo em QUALQUER atividade do roteiro (dentro de Passo a passo, Dicas, Observações, Conexão com a mentalidade ágil e Preparação prévia; nunca no Prompt para IA), e só decide sozinha quando acha exatamente 1 ocorrência num único lugar ("OK PARA ALTERAR", mostrando em qual atividade achou); se não achar em lugar nenhum, mostra "TRECHO NÃO LOCALIZADO" (ou "JÁ ATUALIZADO" se o texto novo já aparece em algum lugar); se achar em mais de um lugar, mostra "TRECHO ENCONTRADO EM MAIS DE UM LUGAR" com a lista de onde apareceu — os dois últimos casos ficam só como pendência no relatório, nunca aplicados, porque escolher entre vários lugares possíveis exige alguém olhando de verdade.' },
    { section: 'admin', personas: ['admin'],
