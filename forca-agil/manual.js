...
(function () {
  ...
  const RULES = [
    ...
    { section: 'turmas', personas: ['visitante', 'logado', 'inscrito', 'admin'],
      ...
      body: 'A página Turmas mostra as mesmas turmas para todo mundo (visitante, logado, inscrito ou admin) — nenhuma é escondida por perfil, e não existe mais um card exclusivo pra quem já está inscrito. Vê um card por turma cadastrada (o número de turmas não é fixo — o admin cria/edita/exclui turmas na aba Admin › Turmas), cada um com nome, mês e datas. O que muda de um card para outro é o estado daquela turma — interesse aberto (botão "Tenho interesse", ver regras abaixo) ou interesse encerrado (orientação para o CMFlex, ver regra seguinte) — e, dentro de uma turma com interesse aberto, o botão de interesse muda conforme o status da própria pessoa (Tenho interesse / Remover interesse / ✓ Inscrita travado, ver regras abaixo). Abaixo dos cards (igual para todos os perfis): bloco "Como funciona a oficina" + Agenda D1–D5.' },
    { section: 'turmas', personas: ['visitante', 'logado', 'inscrito', 'admin'],
    ...
    { section: 'admin', personas: ['admin'],
      ...
      body: 'Botão "＋ Participante" (disponível com a turma aberta ou com interesse encerrado): abre um modal com campos Nome, E-mail, Área (mesma lista de 20 áreas do cadastro) e Status — o admin escolhe entre "Interessada" ou "Inscrita" na hora de adicionar. Valida e-mail @previ.com.br e exige os 4 campos preenchidos antes de salvar; registra o nome do admin que adicionou (independente do status escolhido). Escolher "Inscrita" adiciona a pessoa já com esse status direto (sem precisar de "Confirmar" depois) — uso típico: alguém que já se inscreveu no CMFlex; se ela já for Inscrita em outra turma (só interesse em outra turma não conta), o modal avisa e, ao continuar, remove automaticamente essa outra inscrição (mesma checagem de exclusividade do botão "Confirmar"). Escolher "Interessada" registra só a manifestação de interesse, sem essa checagem (interesse não tem limite de turmas) — uso típico: alguém que avisou o interesse fora do site (e-mail, WhatsApp) e ainda não é sabido se de fato se inscreveu no CMFlex; pode ser confirmada depois pelo botão "Confirmar", igual a quem se interessou pelo próprio site. Se o e-mail já estiver na turma, exibe aviso no modal. A tabela atualiza automaticamente.' },
    { section: 'admin', personas: ['admin'],
