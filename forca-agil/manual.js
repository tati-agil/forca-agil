...
(function () {
  ...
  const RULES = [
    ...
    { section: 'admin', personas: ['admin'],
      ...
      body: 'Cards por turma com badge ABERTA/FINALIZADA, contagem de inscritos/interessados e badge pulsante "CHECK-IN ABERTO · DD/MM" quando o check-in do dia está ativo. Botões globais "↓ Estado atual" e "↓ Histórico" exportam CSV com acentos corretos, sempre abrindo editável no Excel. Cada card tem botões de ação e tabela de participantes, além de um botão "↓ CSV" próprio para exportação individual. Tabelas com muitas colunas ficam roláveis horizontalmente quando não cabem na tela.' },
    { section: 'admin', personas: ['admin'],
    ...
    { section: 'admin', personas: ['admin'],
      ...
      body: 'Select com os dias da turma (pré-seleciona hoje se for um dia válido) + botão "Abrir check-in": define qual dia está liberado para registrar presença. Admin pode abrir para qualquer dia — passado ou futuro — sem precisar estar no dia exato. Participantes podem escanear o QR Code e registrar presença apenas enquanto um dia estiver aberto. Quando aberto, exibe "Check-in aberto: DD/MM" + botão "Fechar check-in". Apenas 1 check-in por pessoa por dia é permitido.' },
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Turmas — QR Code',
      body: 'Botão "⌘ QR Code" (disponível após finalizar): abre um popup com o QR Code da turma. Um único QR por turma, reutilizado em todos os dias — o controle de quando o check-in vale é feito pelo botão "Abrir/Fechar check-in", não pelo QR em si. A URL completa é exibida abaixo do QR.' },
    { section: 'admin', personas: ['admin'],
