...
(function () {
  ...
  const RULES = [
    ...
    { section: 'admin', personas: ['admin'],
      ...
      body: 'Botão "＋ Participante" (disponível com a turma aberta ou com interesse encerrado): abre um modal com campos Nome, E-mail, Área (mesma lista de 20 áreas do cadastro) e Status — o admin escolhe entre "Interessada" ou "Inscrita" na hora de adicionar. Valida e-mail @previ.com.br e exige os 4 campos preenchidos antes de salvar; registra o nome do admin que adicionou (independente do status escolhido). Escolher "Inscrita" adiciona a pessoa já com esse status direto (sem precisar de "Confirmar" depois) — uso típico: alguém que já se inscreveu no CMFlex. Escolher "Interessada" registra só a manifestação de interesse, sem confirmar — uso típico: alguém que avisou o interesse fora do site (e-mail, WhatsApp) e ainda não é sabido se de fato se inscreveu no CMFlex; pode ser confirmada depois pelo botão "Confirmar", igual a quem se interessou pelo próprio site. Se o e-mail já estiver na turma, exibe aviso no modal. A tabela atualiza automaticamente.' },
    { section: 'admin', personas: ['admin'],
