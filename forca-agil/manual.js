...
(function () {
  ...
  const RULES = [
    ...
    { section: 'admin', personas: ['admin'],
      ...
      body: 'Dois botões que aparecem junto de "📥 Importar resultados esperados" (mesma regra: só no roteiro que já tem a atividade "EPISÓDIO FINAL — O COMPROMISSO DA LIDERANÇA"). "🔎 Migração Antes x Depois" abre uma janela com um DRY RUN — só leitura, nada é gravado ainda — comparando o valor ATUAL de 22 campos (Passo a passo, Dicas, Observações, Conexão com a mentalidade ágil, Prompt para IA e dois Tipos) com o texto revisado de uma planilha externa. Cada linha da tabela mostra um status: "OK PARA ALTERAR" (o valor atual bate exatamente com o texto antigo esperado — clicar em "Aplicar" na mesma janela grava só essas linhas), "JÁ ATUALIZADO" (o valor já é o novo texto — nada a fazer), "VALOR ATUAL DIVERGENTE" (o campo tem outra coisa, diferente do antigo e do novo esperado — não mexe, precisa de revisão manual), "ATIVIDADE NÃO ENCONTRADA" (nenhuma atividade com esse título neste roteiro) ou "REGISTRO AMBÍGUO" (mais de uma atividade com esse título — não dá pra saber qual). Nunca decide sozinha em cima de dúvida. As linhas "VALOR ATUAL DIVERGENTE" ganham um botão "REVISAR": clicar abre, na própria tela (sem precisar copiar nada do banco na mão), o valor ATUAL no sistema, o "DE" e o "PARA" do Excel lado a lado, com três opções — "Manter atual" (não mexe nesse campo), "Substituir pelo PARA" (usa o texto novo) ou "Mesclar manualmente" (abre uma caixa de texto já preenchida com o "PARA", editável à vontade, com os dois textos originais visíveis acima pra comparar — só grava depois de clicar "Confirmar mesclagem"). Cada decisão é lembrada só durante aquela sessão da janela (nada é gravado ainda) e o painel avança sozinho pra próxima divergência ainda sem decisão, mostrando "Revisadas X de Y divergências". Um resumo ao vivo mostra a contagem de cada situação (OK PARA ALTERAR, JÁ ATUALIZADO, MANTER ATUAL, USAR PARA, MESCLA APROVADA, PENDÊNCIAS — essas últimas juntando "não encontrada"/"ambígua" com qualquer divergência ainda sem decisão). O botão "Aplicar alterações confirmadas" só aparece depois que TODA divergência tiver uma decisão (pendências de não-encontrada/ambígua não travam isso — elas simplesmente nunca entram na aplicação). Ao aplicar, o valor de cada campo tocado é salvo antes de sobrescrever, num histórico interno do roteiro (nunca aparece na tela normal); "↩ Desfazer última migração" lê esse histórico e restaura os valores de antes da última aplicação, um botão só, sem precisar lembrar manualmente o que mudou. Duas frases sem campo real associado ("Substituir" completo do texto todo) e duas frases que são só um trecho dentro de um campo maior (ex: dentro de "Conexão com a mentalidade ágil") são tratadas de formas diferentes: a primeira troca o campo inteiro, a segunda só substitui o trecho que bater exatamente — se o trecho não for encontrado (ou aparecer mais de uma vez), fica como divergente em vez de adivinhar onde trocar.' },
    { section: 'admin', personas: ['admin'],
