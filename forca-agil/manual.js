...
(function () {
  ...
  const RULES = [
    ...
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Mapa',
      body: 'Mostra cinco seções: (1) Hierarquia de Personas — o que cada perfil consegue fazer; (2) Acesso por Tipo de Pessoa — tabela com o que cada perfil vê/acessa em cada página; (3) Mapa do Site — 12 seções com suas funcionalidades e o perfil mínimo de cada uma. Atenção ao número: são as 8 páginas do menu (Início, Turmas, Conteúdos, Repositório, Treinamento Jedi, Avaliação, Ajuda e Admin) MAIS 4 seções que não são páginas do menu — Check-in (aberta pelo QR Code), Entrar e Cadastrar (abas do modal de login) e Menu / Sessão (a barra de navegação em si). Por isso o total não bate com a contagem de links do menu; (4) Arquitetura Técnica e Regras Operacionais — acordeão com tecnologias, módulos, padrões de código, padrões de UX, deploy e regras de governança; (5) Diagrama da Arquitetura — visão visual gerada automaticamente a partir dos dados da Arquitetura Técnica.' },
    { section: 'admin', personas: ['admin'],
