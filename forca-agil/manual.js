...
(function () {
  ...
  const RULES = [
    ...
    { section: 'admin', personas: ['admin'],
      ...
      body: 'As duas exportações (não só o cabeçalho/rodapé do navegador) agora abrem com uma barra escura no topo reproduzindo a logo do site — o mesmo ícone do menu superior, "FORÇA ÁGIL" e "Previ · Oficina de Agilidade Organizacional" — e usam as mesmas fontes do site (Anton no ícone/título de marca, Oswald nos títulos, Barlow no corpo do texto), em vez de aparecer como um documento genérico do navegador sem nada do site. Como a janela de impressão é um documento novo, sem acesso ao CSS nem ao sprite de ícones do site, o ícone vai embutido como SVG dentro do próprio HTML gerado (não depende de nenhum arquivo carregado à parte) — funciona mesmo se a pessoa salvar o PDF sem internet, só as fontes do Google Fonts precisam de conexão (caem pra uma fonte padrão do sistema se não carregarem, sem quebrar nada). Pra essa barra escura sair na exportação, é preciso a opção "Gráficos de fundo" (ou "Imprimir cores e imagens de fundo") estar marcada no diálogo de impressão do navegador — a mesma dica que já pedia pra desmarcar "Cabeçalhos e rodapés" agora também lembra disso.' },
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Manual',
