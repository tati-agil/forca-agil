...
(function () {
  ...
  const RULES = [
    ...
    { section: 'turmas', personas: ['logado', 'colaborador', 'admin'],
      ...
      body: 'O botão reflete o status da pessoa na turma: "♡ Tenho interesse" (status: interessado) → "♡ Remover interesse" → após turma finalizada pelo admin vira "✓ Inscrita" (desabilitado) → após check-in no evento vira "✓ Presente" (desabilitado). Se a turma estiver finalizada e a pessoa não estiver inscrita, exibe "Inscrições encerradas". Registros em turmas-interesse/<turma>/<emailKey> com campo status (interessado/inscrito). Log em turmas-interesse-log.' },
    { section: 'turmas', personas: ['logado', 'colaborador', 'admin'],
      title: 'Check-in no evento (QR Code)',
      body: 'No dia do evento, admin abre o check-in do dia (define diaAtivo em turmas-config). Participante escaneia o QR Code com a conta logada → página #checkin?turma=t1 → sistema valida (turma finalizada, check-in aberto, inscrita, não fez check-in hoje) → registra em turmas-checkin/<turma>/<data>/<emailKey> com source:"qr" → exibe "Presença confirmada com sucesso!". Se não estiver logada, redireciona para login e faz check-in automaticamente após autenticação.' },
