...
(function () {
  ...
  const RULES = [
    ...
    { section: 'inicio', personas: ['visitante', 'logado', 'inscrito', 'admin'],
      ...
      body: 'Três cards decorativos (sem hover) logo abaixo do hero, cada um navega direto para a página correspondente. Visível e funcional para todos.' },
    { section: 'inicio', personas: ['visitante', 'logado', 'inscrito', 'admin'],
      title: 'Seção "O que está acontecendo" (Destaques)',
      body: 'Removida. Os mini-blocos de Turmas, Conteúdos e Ranking mini não existem mais na página Início.' },
    { section: 'inicio', personas: ['visitante', 'logado', 'inscrito', 'admin'],
    ...
    { section: 'turmas', personas: ['visitante'],
      ...
      body: 'Ao concluir o login pelo modal aberto a partir do botão "Tenho interesse", o interesse NÃO é registrado sozinho — o botão volta ao estado normal ("Tenho interesse") e a pessoa precisa clicar de novo. Diferente da página de check-in, que retoma a ação automaticamente após o login.' },
    { section: 'turmas', personas: ['logado', 'admin'],
      title: 'Botão de interesse / status da turma (logado sem turma)',
      body: 'Disponível apenas para usuário logado sem turma confirmada. O botão reflete o status: "♡ Tenho interesse" — estilo primário (dourado sólido) → "♡ Remover interesse" — estilo secundário (fundo escuro, borda neutra). Ao registrar interesse, exibe a mensagem "Sua intenção foi registrada! Usaremos para dimensionar as turmas." embaixo do botão. Ao remover, exibe "Interesse removido." e o botão volta a "Tenho interesse". Pode manifestar interesse em até 3 turmas. Registros em turmas-interesse/<turma>/<emailKey> com campo status (interessado/inscrito). Log em turmas-interesse-log.' },
    { section: 'turmas', personas: ['logado', 'admin'],
    ...
    { section: 'turmas', personas: ['visitante', 'logado', 'admin'],
      title: 'Falha silenciosa na leitura inicial do Firebase',
      body: 'As leituras de turmas-interesse e turmas-config (verificação de status e de finalizada) não têm callback de erro nem timeout. Se alguma leitura falhar (rede instável, regra negando, token ainda propagando após cadastro recente), o card/botão fica travado no estado estático padrão do HTML — sem nenhuma mensagem de erro visível para a pessoa. Para visitante (sem sessão), afeta só a checagem de turma finalizada.' },
    { section: 'turmas', personas: ['visitante', 'logado', 'admin'],
    ...
    { section: 'turmas', personas: ['inscrito', 'admin'],
      title: 'Check-in de presença fica na seção "Check-in"',
      body: 'O check-in de presença (QR Code, mensagens de sucesso/erro) não acontece nesta página nem altera o card de Turmas — é uma página separada (#checkin). Veja os cenários completos na seção "Check-in" deste manual.' },
    { section: 'turmas', personas: ['visitante', 'logado', 'admin'],
      title: 'Correção: listener de clique duplicado ao revisitar a página',
      body: 'Antes, cada vez que a pessoa saía e voltava para a página Turmas (ou fazia login/logout nela), um novo listener de clique era adicionado ao mesmo botão sem remover o anterior. Para logado/admin, um único clique acabava disparando o registro de interesse várias vezes, duplicando entradas em turmas-interesse-log; para visitante, reabria o modal de login repetidamente por clique. Corrigido: o botão agora é religado apenas uma vez por elemento (marcado com data-wired).' },
    { section: 'turmas', personas: ['logado', 'admin'],
      title: 'Correção: clique duplo antes da resposta do Firebase',
      body: 'Antes, o botão não era desabilitado enquanto a gravação em turmas-interesse estava em andamento — um clique duplo rápido (antes da resposta do Firebase chegar) disparava registerInterest/removeInterest duas vezes, duplicando entradas em turmas-interesse-log. Corrigido: o botão fica desabilitado assim que o clique é processado e só volta a ficar habilitado quando a resposta (sucesso ou erro) chega.' },
    ...
    { section: 'checkin', personas: ['inscrito'],
      ...
      body: 'Se o admin não abriu o check-in do dia (sem diaAtivo em turmas-config), exibe "O check-in não está aberto no momento. Aguarde a organização abrir o check-in do dia."' },
    { section: 'checkin', personas: ['inscrito'],
    ...
    { section: 'checkin', personas: ['inscrito'],
      ...
      body: 'Se já existe registro em turmas-checkin/<turma>/<diaAtivo>/<emailKey>, exibe "Presença já registrada" com nome da turma e a data do dia aberto.' },
    { section: 'checkin', personas: ['inscrito'],
      title: 'Sucesso',
      body: 'Quando turma finalizada, check-in do dia aberto, pessoa inscrita e sem check-in prévio nesse dia: grava em turmas-checkin/<turma>/<data>/<emailKey> com source:"qr" e exibe "Presença confirmada com sucesso!" com nome, turma e dia.' },

    /* ── CONTEÚDOS ── */
    { section: 'conteudos', personas: ['inscrito', 'admin'],
      title: 'Acesso',
      body: 'Página Conteúdos acessível apenas para usuário inscrito (turma confirmada) e admin. Visitante e logado sem turma não acessam esta página. As 7 seções numeradas (01 Mapa da Galáxia, 02 Os 4 Valores, 03 Os 12 Princípios, 04 A Força do Ágil, 05 Personagens, 06 Lado Sombrio, 07 A Trilogia) — cada uma ocupa 100vh, uma por vez na tela.' },
    { section: 'conteudos', personas: ['inscrito', 'admin'],
    ...
    { section: 'quiz', personas: ['inscrito', 'admin'],
      ...
      body: 'Acesso ao quiz (autodiagnóstico), painel de patente e revelar patente. Não há Missões nem Kyber Game na v3.' },
    { section: 'quiz', personas: ['inscrito', 'admin'],
    ...
    { section: 'quiz', personas: ['inscrito', 'admin'],
      ...
      body: 'Visível em tempo real no Treinamento Jedi. Enquanto o quiz não estiver concluído: exibe patente provisória. Após revelar: mostra a patente definitiva. 4 patentes possíveis (Youngling → Padawan → Cavaleiro Jedi → Mestre Jedi) com base na pontuação do quiz.' },
    { section: 'quiz', personas: ['inscrito', 'admin'],
      title: 'Revelar patente — pré-requisito',
      body: 'Exige completar o quiz. O botão "Revelar minha Patente →" só aparece ao concluir as 20 afirmações.' },
    { section: 'quiz', personas: ['inscrito', 'admin'],
    ...
    { section: 'quiz', personas: ['inscrito', 'admin'],
      ...
      body: 'Ao clicar em "Revelar minha Patente Final" (com o quiz concluído), abre pop-up com: ilustração, nome do rank alcançado e pontuação total. Grava fa-patente-revealed=1 (faSyncProgress). O resultado é definitivo e não pode ser alterado.' },
    { section: 'quiz', personas: ['admin'],
      title: 'Reset de progresso (Admin)',
      body: 'Admin pode resetar o progresso de qualquer cadastrado. Apaga: quiz, patente. Ação irreversível. Após o reset, a pessoa pode refazer o quiz e revelar a patente novamente — essa é a única forma de refazer.' },
    ...
    { section: 'admin', personas: ['admin'],
      ...
      body: 'Cards por turma com badge ABERTA/FINALIZADA, contagem de inscritos/interessados e badge pulsante "CHECK-IN ABERTO · DD/MM" quando o check-in do dia está ativo. Botões globais "↓ Estado atual" e "↓ Histórico" exportam CSV (encoding UTF-8 BOM — acentos corretos no Excel, arquivo sempre abre em modo editável, sem modo protegido). Cada card tem botões de ação e tabela de participantes. Cada turma tem botão "↓ CSV" para exportação individual. Tabelas com muitas colunas ficam roláveis horizontalmente quando a largura excede o viewport (classe .table-scroll-wrap).' },
    { section: 'admin', personas: ['admin'],
      title: 'Exportar CSV — Estado Atual',
      body: 'Botão global "↓ Estado atual": exporta todas as turmas em um único arquivo CSV. Inclui apenas usuários ativos (removed !== true) — removidos não aparecem. Coluna "Adicionado por" preenchida com nome do admin quando o participante foi adicionado manualmente (addedByAdmin:true); vazia quando o participante se inscreveu por conta própria.' },
    { section: 'admin', personas: ['admin'],
      title: 'Exportar CSV — Histórico',
    ...
    { section: 'admin', personas: ['admin'],
      title: 'Exportar CSV — individual por turma',
      body: 'Botão "↓ CSV" em cada card de turma: exporta apenas os participantes ativos daquela turma (removed !== true). Para turmas finalizadas inclui colunas de presença por dia, frequência e critério atingido. Inclui coluna "Adicionado por" quando aplicável.' },
    { section: 'admin', personas: ['admin'],
    ...
    { section: 'admin', personas: ['admin'],
      ...
      body: 'Desktop: todas as ações ficam em linha única — seletor de dia + botão "Abrir check-in" (ou "Fechar check-in") sempre visíveis; QR Code, + Participante, Reabrir, CSV e Certificados também inline. Mobile/tablet (≤768px): header do card vira coluna; apenas ações primárias ficam visíveis (seletor de dia + Abrir/Fechar check-in para turma finalizada, ou "Finalizar inscrição" para turma aberta); botão "⋯" abre dropdown com ações secundárias (QR Code, + Participante, ↺ Reabrir, CSV, Certificados). Para turma aberta no mobile, CSV também vai para o dropdown.' },
    { section: 'admin', personas: ['admin'],
    ...
    { section: 'admin', personas: ['admin'],
      ...
      body: 'As etiquetas "✓ adm" e "✓ qr" nas células da tabela de presença são botões clicáveis. Hover mostra risco (text-decoration: line-through) sinalizando remoção. Clicar abre modal de confirmação (adminConfirm). Ao confirmar, remove o registro em turmas-checkin/<turma>/<data>/<emailKey> e recarrega a tabela com a frequência atualizada. Funciona tanto para presenças registradas pelo admin (source:"admin") quanto por QR Code (source:"qr").' },
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Turmas — adicionar participante após finalizar',
      body: 'Botão "＋ Participante" (disponível apenas com turma finalizada): abre modal visual com campos Nome, E-mail e Área (mesma lista de 20 áreas do cadastro). Valida e-mail @previ.com.br antes de salvar. Grava em turmas-interesse/<turma>/<emailKey> com status:"inscrito", addedByAdmin:true e addedByAdminName com o nome do admin logado — sem precisar reabrir a turma. Se o e-mail já estiver na turma, exibe aviso no modal. A tabela de presença atualiza automaticamente.' },
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Turmas — remover inscrito da turma finalizada',
      body: 'Botão "Remover" (coluna extra na tabela de presença): abre modal de confirmação visual (adminConfirm). Ao confirmar, grava removed:true, removedDate (data+hora), removedByAdmin (e-mail do admin) e removedByAdminName (nome do admin) no registro em turmas-interesse. A pessoa some da tela imediatamente. Diferente de "faltou a um dia" — é uma remoção da turma, não uma ausência. Os check-ins registrados não são apagados, mas a pessoa deixa de constar como inscrita.' },
    { section: 'admin', personas: ['admin'],
      title: 'Pop-ups do admin — modais visuais (não nativos)',
      body: 'Todos os confirm() e alert() nativos foram substituídos por modais visuais (adminConfirm e adminAlert). Mesmo design visual dos modais de cadastro/login. Nenhuma ação do painel admin usa confirm() ou alert() do browser — todos os diálogos são modais customizados.' },
    { section: 'admin', personas: ['admin'],
    ...
    { section: 'admin', personas: ['admin'],
      ...
      body: 'Lista TODAS as pessoas que já se cadastraram (fa-users). Mostra nome, e-mail, área e data de cadastro. A coluna XP foi removida da tabela — nenhuma informação de XP ou pontuação é exibida no painel Admin.' },
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Cadastrados — resetar progresso',
      body: 'Disponível para qualquer pessoa cadastrada. Apaga fa-progress/<emailKey> e escreve fa-reset-signal/<emailKey> = {at: serverTimestamp}. Ação irreversível — pede confirmação antes de executar. Após o reset: se a pessoa estiver logada, o listener contínuo em fa-reset-signal/<emailKey> detecta a mudança, limpa o localStorage e recarrega a página automaticamente — quiz fica disponível para refazer sem nenhuma ação da pessoa.' },
    { section: 'admin', personas: ['admin'],
    ...
    { section: 'admin', personas: ['admin'],
      title: 'Sem XP no painel Admin',
      body: 'O painel Admin não exibe nenhuma informação relacionada a XP, pontuação ou experiência. Isso inclui: colunas de tabela, badges, cabeçalhos, textos de apoio, botões e mensagens. A coluna XP foi removida da aba Cadastrados. Qualquer referência visual ou textual a XP deve ser removida se aparecer no Admin.' },
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Testes',
    ...
    { section: 'auth', personas: ['visitante', 'logado', 'inscrito', 'admin'],
      ...
      body: 'Em telas com largura ≤ 600px: logo exibido em versão compacta, botões do menu reduzidos, ícone hamburguer sempre visível e sem sobreposição com outros elementos do header. Corrigido via media query específica para ≤ 600px.' },
  ];
