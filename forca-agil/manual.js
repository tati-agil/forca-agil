...
(function () {
  ...
  const SECTIONS = [
    ...
    { key: 'facilitador', label: 'Facilitador',         color: '#f5a742' },
  ...
  const RULES = [
    ...
    { section: 'menu', personas: ['admin'],
      title: 'Link Facilitador no menu',
      body: 'O link "Facilitador" no menu (entre Minha Área e Ajuda) aparece para administradores e para quem está cadastrado na aba Facilitadores. Acessar #facilitador direto pela URL sem ter acesso redireciona para o início, mesma proteção da rota #admin.' },
    ...
    { section: 'menu', personas: ['inscrito', 'admin'],
      title: 'O acesso do Inscrito não pode cair enquanto o site ainda está carregando',
      body: 'Saber se alguém é Inscrito exige ler, no banco, a inscrição dela em CADA turma cadastrada — e cada uma dessas leituras chega no seu tempo. Enquanto elas não chegaram todas, o site NÃO sabe o nível de acesso da pessoa, e "não sei ainda" nunca pode ser tratado como "não tem acesso": enquanto isso, ninguém é redirecionado nem tem link escondido do menu; a decisão fica suspensa até a resposta chegar. Isso vale também para a lista de admins, que é lida em paralelo. No wi-fi do computador essas leituras voltam praticamente juntas e nada disso é perceptível; no celular em rede lenta a janela dura segundos — que é exatamente quando a pessoa clica. Em 08/09/2026, na turma de setembro, essa distinção não existia e o efeito foi este: gente confirmada em turma sendo jogada de volta ao Início com "Esta área não está disponível para o seu nível de acesso" ao abrir o Treinamento, links sumindo do menu, e o falso conserto de promover a pessoa a admin (que só mascara o problema, porque admin é verificado por outro caminho). Segundo efeito da mesma origem: se qualquer uma dessas leituras não voltasse, o site nunca era revelado e a pessoa ficava numa tela preta depois de logada — hoje existe um limite de 8 segundos que mostra o site assim mesmo e corrige o nível sozinho quando os dados finalmente chegam.' },
    { section: 'menu', personas: ['logado', 'inscrito', 'admin'],
    ...
    { section: 'entrar', personas: ['visitante', 'logado', 'inscrito', 'admin'],
      ...
      body: 'Enquanto o site verifica quem você é, ele esconde tudo e mostra só a tela de login. Se o servidor não responder — rede lenta, proxy da empresa, internet ruim —, essa espera não pode durar para sempre: antes, a pessoa ficava diante de uma tela totalmente preta, sem login e sem explicação. Agora, passados 10 segundos sem resposta, a tela de login aparece assim mesmo, com o aviso "Não conseguimos retomar sua sessão", mais os botões "Testar conexão" e "Limpar sessão e entrar de novo". Dá para tentar entrar normalmente; se o servidor responder depois, o site segue o fluxo normal e o aviso some. MAS 10 SEGUNDOS OLHANDO PARA O NADA JÁ É UM SITE QUEBRADO: até 09/09/2026, durante toda essa espera não havia absolutamente nada na tela — foi medido, zero elemento visível aos 2 segundos, no computador e no celular, em quatro situações diferentes (rede lenta, leitura travada, autenticação sem resposta e a biblioteca do Firebase fora do ar). Era esse o "site preto" relatado na oficina e de novo na noite seguinte, e ele nunca esteve quebrado: estava esperando, calado. Agora, desde o primeiro instante, aparece um anel girando com a palavra "Carregando…"; se a espera passar de 4 segundos, entra também a frase "A conexão está lenta. Se esta tela não sair, verifique a rede e recarregue a página". Quem entra numa rede boa não chega a ver nada disso, porque o site aparece antes. Esse indicador é feito só de CSS, de propósito: ele precisa funcionar exatamente quando o JavaScript é o que não está carregando.' },
    { section: 'entrar', personas: ['visitante', 'logado', 'inscrito', 'admin'],
    ...
    { section: 'cadastrar', personas: ['visitante'],
      ...
      body: 'Dropdown customizado com campo de busca por digitação no topo. Ao abrir, o foco vai direto para o campo de busca — a pessoa digita parte do nome da área e a lista filtra em tempo real. As 26 opções em ordem alfabética — gerências, diretorias e a presidência: ASJUR, AUDIT, CONIN, DIPAR, DIPLA, DIRAD, DIRIN, DISEG, GABIN, GEBEN, GECAP, GECAT, GECON, GEINT, GEPAR, GEPRO, GERAI, GERAT, GEROP, GESOP, GETHO, INFOR, OUVIR, PNSEG, PRESI, SECEX.' },
    { section: 'cadastrar', personas: ['visitante'],
    ...
    { section: 'cadastrar', personas: ['admin'],
      title: 'Cadastrados — corrigir nome, gerência e e-mail',
      body: 'Cada linha da aba Cadastrados tem "✎ Editar", que abre o cadastro da pessoa para corrigir Nome e Área/Gerência — qualquer pessoa pode ter digitado errado ao se cadastrar. A correção NÃO fica só no cadastro: o nome e a área são copiados para dentro de cada inscrição em turma, da fila de espera, das listas de público restrito, da equipe de facilitação e de cada presença já registrada, e a edição atualiza tudo de uma vez, numa gravação só. Sem isso o painel continuaria mostrando o nome antigo nas tabelas das turmas e pareceria que o site ignorou a correção. O modal avisa, antes de salvar, em quantos outros registros a correção vai chegar; se não conseguir ler esses registros, recusa a edição inteira em vez de gravar metade. O que NÃO é reescrito, de propósito: o histórico de interesse e os sorteios já realizados — são registros do que aconteceu na época, e reescrevê-los seria falsificar. O E-MAIL só é editável quando o cadastro foi criado pelo próprio painel (botão "+ Criar conta para colaboradora"), que é justamente o caso de quem digitou errado na hora de criar e deixou a pessoa sem conseguir entrar; o painel pede a sua senha de admin, troca o login da pessoa e move todos os dados dela para a chave nova (inclusive pedidos e conteúdos do Repositório, que são achados pelo e-mail). Isso só funciona enquanto a senha ainda for a padrão 12345678: se a pessoa já entrou e trocou a senha, a conta é dela e o painel recusa, mandando criar a conta certa e bloquear a errada. Em cadastro feito pela própria pessoa o e-mail fica travado, com a explicação na tela. Toda correção é gravada em fa-users-log com o que mudou, de quê para quê, por qual admin e quando.' },
    { section: 'cadastrar', personas: ['admin'],
      title: 'Cadastrados — a linha da pessoa e o menu "⋯"',
      body: 'Cada linha mostra o que identifica a pessoa (nome, e-mail, área, data do cadastro e a Situação, com os selos de Ativo/Bloqueado e Verificado/Pendente juntos) e duas portas: "✎ Editar", que é o do dia a dia, e "⋯", que abre o resto — Redefinir senha, Resetar progresso do jogo, Confirmar cadastro (só para quem está Pendente) e Bloquear/Desbloquear. Antes cada uma dessas ações era uma coluna própria de botão: dez colunas, cinco só de botão, todas com a cara vermelha de "excluir" — e não cabia na tela nem no computador. O que sobrava ficava atrás de uma rolagem lateral cuja barra só aparecia no fim de uma tabela de milhares de pixels de altura: ninguém rola três mil pixels para descobrir que existe um "Bloquear". Ação que não se alcança é ação que não existe, e isso não dá erro nenhum — a tela parece inteira, só está cortada. O menu abre como um modal com o NOME e o e-mail da pessoa no topo, e cada ação vem com uma linha dizendo o que faz; numa lista de centenas, errar de linha é fácil demais para depender de mirar no botão certo. Só o que é destrutivo continua vermelho. No celular a tabela deixa de ser tabela: cada pessoa vira um cartão com os campos um embaixo do outro — antes três colunas ficavam escondidas no celular e o resto rolava para o lado.' },
    { section: 'cadastrar', personas: ['admin'],
      title: 'Cadastrados — bloquear/desbloquear acesso',
      body: 'O "Bloquear" fica no menu "⋯" da linha da pessoa. Ao bloquear: a pessoa é deslogada imediatamente na próxima verificação de sessão e vê a mensagem "Acesso desativado". O botão vira "Desbloquear" para reverter. O bloqueio não apaga a conta — apenas impede o acesso.' },
    { section: 'cadastrar', personas: ['admin'],
    ...
    { section: 'turmas', personas: ['logado', 'inscrito', 'admin'],
      ...
      body: 'A página Turmas é organizada por evento, na mesma ordem do painel admin: abaixo do título da página, cada evento é um bloco fechado — o nome dele, a grade das turmas dele com o card de Lista de Espera no fim, e logo abaixo a Missão / Como funciona / Plano de Voo daquele evento. Só então começa o próximo. Tudo que é de um evento fica junto, e nada de um evento aparece embaixo dos cards de outro. Cada evento que aparece na vitrine e tem a fila ligada ganha, como último card DAQUELE grupo, um card "Lista de Espera" — aparecer na vitrine é ter turma OU ter a fila ligada (a fila não exige turma, ver adiante), e a fila é do evento, não um card único global como antes. Logada pode clicar "Entrar na lista de espera" no card de um evento específico; dá para estar na fila de mais de um evento ao mesmo tempo, um clique por card. Dado salvo em fa-espera/ no Firebase, com o eventoKey daquele card. Quem está na lista mantém acesso de Logada (sem turma), não de Inscrita. A fila NÃO depende de turma: o evento pode não ter nenhuma e mesmo assim ter fila — é assim que ela serve para medir demanda e dizer ao admin quantas turmas abrir. Quando o evento é criado, a fila já nasce junto; o que o admin escolhe é se ela aparece (caixa "Aceitar lista de espera neste evento", em "+ Novo evento" / "✎ Editar evento"), porque nem todo evento quer lista de espera. Marcada, o evento aparece na página Turmas mesmo sem turma nenhuma: só o título dele e o card de espera (mais o bloco "A Missão", se o conteúdo estiver preenchido). Desmarcada e sem turma, o evento não aparece; desmarcada e com turma, aparecem as turmas e nenhum card de fila. Em qualquer caso, desmarcar não apaga ninguém: quem já estava na fila continua nela, visível no painel.' },
    { section: 'turmas', personas: ['logado', 'inscrito', 'admin'],
      title: 'Lista de espera — sair',
      body: 'Quem está na lista pode sair clicando "Na lista — Sair" no card daquele evento. O registro é marcado como removed:true (não apagado, mantém histórico) — só para o evento daquele card; se a pessoa também espera por outro evento, essa espera continua intacta no card dele.' },
    { section: 'turmas', personas: ['admin'],
      title: 'Lista de espera — quem saiu da fila',
      body: 'Abaixo da lista há a seção recolhida "Saíram da fila", com quem já esteve na fila e não está mais: nome, área, data do interesse, origem, quando saiu, destino (foi para uma turma ou saiu de vez) e o motivo. Abaixo do motivo vem quem tirou a pessoa, e isso é dito só quando está gravado: o nome do admin (saída pelo painel) ou "pela própria pessoa" (quando ela mesma clicou no botão do card do site, que diz "✓ Na lista — Sair"). Sem nenhum dos dois — registro anterior a essa marcação — nada é afirmado sobre quem tirou. Sair da fila NUNCA apaga o registro — ele fica no banco marcado como removido —, mas até então não havia como enxergá-lo, e portanto não havia como responder "e a fulana, que estava aqui?" nem conferir se alguém saiu por engano. É a mesma ideia do filtro "Removidos" que as turmas já têm. Cada linha traz também "🗑 excluir registro", que é OUTRA coisa: apaga o registro daquela origem de vez, sem deixar histórico — some da seção e do banco. Serve para o que nunca foi uma espera de verdade, teste ou duplicado, que a saída comum não resolve porque o registro continua listado para sempre. Não pode ser desfeito, e por isso só aparece em quem já saiu da fila: excluir de vez exige duas decisões separadas. O cadastro da pessoa e os registros dela nas turmas não são tocados. Quem entrou pelo card do site aparece com a origem "Entrou pela lista", e o bloco de conferência conta essas entradas incluindo as que já saíram — porque a pergunta "alguém entrou direto pela lista?" não se responde olhando só quem continua esperando.' },
    { section: 'turmas', personas: ['admin'],
      title: 'Lista de espera — conferência da fila',
      body: 'Acima da Lista de Espera de CADA evento há um bloco (contando só as turmas e a fila DAQUELE evento) que explica por que a soma dos filtros "Foram para a espera" das turmas do evento NÃO bate com o tamanho da fila dele. São contagens diferentes por construção: a fila guarda uma entrada por PESSOA, e cada turma registra uma saída por TURMA — quem foi mandada para a fila a partir de duas turmas conta duas vezes lá e uma aqui. O bloco mostra quatro números lado a lado — pessoas na fila, registros na fila (um por turma de origem), saídas para a espera registradas nas turmas e pessoas distintas nessas saídas — e reparte essas saídas: com registro próprio na fila, já foram da fila para uma turma, foram removidas da fila, vindas do card do site, e sem rastro na fila. Só "sem rastro" aparece em vermelho, e só quando houver: é o único caso que seria coisa nova e inexplicada. As saídas anteriores à separação por turma — quando uma segunda migração escrevia por cima da primeira e levava junto a data do primeiro interesse, que é o que ordena a fila — aparecem como nota de rodapé, não como número em vermelho: é um total fechado, que a estrutura atual não produz mais, e tratá-lo como pendência seria alarme permanente sobre história já registrada. O detalhe embaixo nomeia essas pessoas e de qual turma saíram.' },
    { section: 'turmas', personas: ['admin'],
    ...
    { section: 'turmas', personas: ['admin'],
      ...
      body: 'Na aba Eventos do painel admin, dentro do card de CADA evento, abaixo das turmas dele, uma seção "Lista de Espera" mostra quem está aguardando por aquele evento especificamente — a fila é do evento (eventoKey), não uma lista única global. O admin escolhe a turma num select — que só lista turmas DESTE MESMO evento — e clica "Mover para turma": a pessoa é adicionada à turma já como Inscrita (confirmada) e sai da lista de espera deste evento automaticamente; se ela também esperava por outro evento, essa espera continua intacta, no card dele. Ela ganha o mesmo acesso de quem foi confirmado manualmente. Registro de antes desta separação por evento, sem eventoKey, aparece à parte, numa seção "Lista de Espera — sem evento" abaixo de todos os cards de evento. Se a fila do evento estiver desligada (caixa "Aceitar lista de espera" desmarcada) e ainda houver gente nela, a seção aparece do mesmo jeito, com um aviso em cima explicando que o card não está no site e ninguém novo entra — desligar impede entrada nova, não apaga quem já entrou.' },
    { section: 'turmas', personas: ['admin'],
      title: 'Lista de espera — remover sem mover',
      body: 'Na seção Lista de Espera de CADA evento no painel admin, o botão "Remover da lista" tira a pessoa da fila DAQUELE evento, sem movê-la para turma nenhuma — e sem tocar numa eventual espera dela por outro evento. Ao clicar, é obrigatório escolher o motivo: Desistiu / não tem mais interesse, Já participou de uma turma (aqui você escolhe QUAL turma ela já fez, igual à saída da turma — e se o sistema conhecer uma só, ela já vem escolhida), Não respondeu aos contatos, Evento encerrado, Registro duplicado ou Outro. Escolhendo "Outro", abre um campo de texto que também precisa ser preenchido — assim nunca fica registro sem justificativa. O motivo é gravado junto com a data e o nome de quem removeu.' },
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Eventos — turma de público restrito',
      body: 'A caixa "Público restrito" (em "Nova turma" / "✎ Editar turma") fecha a turma: a partir dela, quem manda é a LISTA daquela turma, montada no menu ⋯ → "👥 Público restrito". Marcada, o cabeçalho da turma ganha o selo "👥 PÚBLICO RESTRITO · N". A lista vale nos dois sentidos, e é isso que a torna confiável: quem está fora dela não vê a turma no site (nem o card, nem o botão de interesse) E não pode ser colocada na turma por você — as três portas recusam, dizendo o conserto: Confirmar, "＋ Participante" e "Mover para turma" da lista de espera. Querer incluir alguém de fora é incluir na lista primeiro; não existe atalho, senão a lista viraria enfeite ao lado de uma turma que aceita qualquer um por outro caminho. No "＋ Participante" a busca já sai restrita: numa turma fechada ela só encontra quem está na lista, porque recusar depois de escolher é pior do que não oferecer. Só entra na lista quem já tem cadastro no site — sem cadastro a pessoa não consegue entrar para ver a turma, então convidar um e-mail solto criaria uma linha que nunca vira participante. A lista mostra, em cada linha, se a pessoa já está na turma (Inscrita / Interessada / —), quando foi incluída e por quem. Remover da lista NÃO remove da turma: são duas decisões, a lista diz quem PODE e a turma diz quem ESTÁ. Marcar a caixa numa turma que já tem gente também não expulsa ninguém — quem já estava continua valendo, e o card passa a mostrar um aviso com os NOMES de quem está na turma sem estar na lista, para a decisão ser sua, vendo. Turma aberta e turma de público restrito convivem no mesmo evento. Desmarcar a caixa devolve a turma ao comportamento aberto e não apaga a lista: voltar a marcar reencontra as mesmas pessoas. A lista é o único dado de turma que só admin pode escrever nas regras do banco (ver Mapa → Firebase Realtime Database).' },
    { section: 'admin', personas: ['admin'],
    ...
    { section: 'admin', personas: ['admin'],
      ...
      body: 'Dentro do modal de remoção existe a caixa "Colocar na lista de espera", que já vem MARCADA. A leitura é essa: quem sai da turma quase sempre continua querendo fazer, então o caminho normal é a fila — e quem não quer, desmarca. Antes a caixa vinha marcada só em alguns motivos, o que fazia a decisão depender de lembrar quais eram. Continua não sendo automático: a caixa está à vista e a palavra final é sua, uma pessoa por vez. As exceções são os dois motivos que pedem para escolher uma turma — "Vai fazer em outra turma" e "Já participou de uma turma": neles a caixa some e não vale. No primeiro, a turma de destino JÁ é o destino, e pôr na fila ao mesmo tempo se contradiz. Nos dois, a turma escolhida não é gravada junto com a entrada na fila, então marcar a caixa perderia essa escolha em silêncio. IMPORTANTE: a lista de espera recebe sempre a DATA E HORA DO INTERESSE ORIGINAL, nunca a data da remoção — é isso que preserva a ordem de chegada na fila.' },
    { section: 'admin', personas: ['admin'],
    ...
    { section: 'turmas', personas: ['logado', 'inscrito', 'admin'],
      ...
      body: 'A página Turmas mostra as mesmas turmas para todo mundo que está autenticado (logado, inscrito ou admin) — nenhuma é escondida por perfil, com uma exceção: um evento marcado "Visível só para diretores e administradores" (✎ Editar evento) some da vitrine de quem não está na aba Diretores nem é admin, do mesmo jeito que sumiria se estivesse "fora da página" — só que condicionado à pessoa, não a todo mundo (ver aba Diretores, ao lado de Administradores, no painel) — mesma regra vale para o resumo "turmas abertas no momento" que a Minha Área mostra a quem nunca interagiu com turma nenhuma. Fora esse caso, cada card mostra nome, mês, datas e horário (9h – 13h). O estado do card muda automaticamente conforme as datas e ações do admin — 4 estados possíveis: (1) interesse aberto: botão "Tenho interesse"; (2) interesse encerrado mas turma ainda não iniciou: "Inscrições encerradas — as vagas desta turma já foram preenchidas. Ela será realizada em [datas]. Acompanhe as próximas turmas para participar.", sem botão; (3) em andamento: a partir do primeiro dia da turma, o card troca automaticamente para "Turma em andamento · As aulas estão acontecendo", sem nenhum botão — independente de o interesse ter sido encerrado ou não; (4) realizada: após o último dia OU quando o admin clica "Encerrar turma", o card mostra "Turma realizada · Esta turma já foi concluída. Fique de olho nas próximas", sem botão. Os cards são agrupados por evento, com o nome dele como título acima da grade (mesma ordem do painel admin). Abaixo de cada grupo de evento (igual para todos os perfis, e só quando o admin preencheu esse conteúdo): bloco "A Missão" + "Como funciona" + itinerário dia a dia daquele evento específico.' },
    { section: 'turmas', personas: ['logado', 'inscrito', 'admin'],
      title: 'Turma de público restrito — só aparece para quem está na lista',
      body: 'Turma marcada como "Público restrito" pelo admin não aparece na página Turmas de quem está fora da lista dela: não é um card sem botão, é card nenhum — oferecer o que a pessoa não pode fazer é pior do que não oferecer. Quem está na lista vê o card igual a qualquer outro, com "♡ Tenho interesse" e os mesmos quatro estados por data. Admin vê todas as turmas, restritas ou não, como já acontece com evento "Visível só para diretores e administradores". Se a leitura da lista falhar (rede instável), a turma restrita é escondida — falhar escondendo é o lado seguro. Um clique que escape pela corrida (a turma virou restrita, ou a pessoa saiu da lista, com a página já aberta) é recusado na hora de gravar, com a mensagem "Esta turma é de público restrito e você não está na lista dela." — o mesmo vale para o resumo "Turmas abertas no momento" da Minha Área: turma restrita não entra nele para quem está fora da lista.' },
    { section: 'turmas', personas: ['logado', 'inscrito', 'admin'],
    ...
    { section: 'turmas', personas: ['logado', 'inscrito', 'admin'],
      ...
      body: 'A partir da data do primeiro dia da turma, o card troca automaticamente — sem nenhuma ação do admin — para o estado "Em andamento": mostra "Turma em andamento" e a mensagem "As aulas estão acontecendo. Acompanhe as próximas turmas." Não há botão de interesse nem de CMFlex. Esse estado é detectado por data (hoje ≥ primeiro dia da turma), comparando a data local de quem está vendo a página — antes comparava com a data em UTC, o que fazia o card virar "Em andamento" cerca de 3h mais cedo do que deveria (a partir de ~21h do dia anterior, horário de Brasília). Acontece mesmo que o interesse ainda não tenha sido encerrado.' },
    { section: 'turmas', personas: ['logado', 'inscrito', 'admin'],
    ...
    { section: 'turmas', personas: ['logado', 'inscrito', 'admin'],
      title: 'Bloco "A Missão" e "Como funciona" — por evento, opcional',
      body: 'Cada evento que aparece na vitrine E tem esse conteúdo preenchido pelo admin ("✎ Editar evento" na aba Eventos do painel) ganha, logo abaixo da grade de cards dele, um bloco "A Missão" (título curto + texto; sem o título curto, o nome do evento faz as vezes) e, se o evento tiver itinerário cadastrado ou tópicos preenchidos, "Como funciona": até 4 chips — dias de encontro e horas por encontro, calculados automaticamente a partir do itinerário e da carga horária do evento (sem itinerário não existem esses dois; sem carga horária não existe o de horas), mais Modalidade e Participação, que são texto do próprio evento e usam "Prática" e "Opcional" apenas como padrão de quem não preencheu — e uma descrição dos tópicos abordados. Aparecer na vitrine é a mesma condição da grade: ter turma OU ter a fila ligada. Evento que aparece só com o card de espera também ganha a Missão — sem isso, a pessoa entraria na fila de algo que a página não descreve em lugar nenhum. Evento sem nenhum desse conteúdo preenchido não ganha bloco nenhum, nem vazio.' },
    { section: 'turmas', personas: ['logado', 'inscrito', 'admin'],
      title: 'Itinerário dia a dia — por evento, itens estáticos',
      body: 'Cada evento com pelo menos um dia cadastrado no itinerário ("Editar evento" no painel) ganha sua própria agenda "Plano de Voo", com um item por dia — nenhum perfil pode expandir, mostram só o título do dia (ex: "D1 · O Despertar da Força"), sem conteúdo interno. O número de dias e os títulos são os que o admin cadastrou para aquele evento especificamente; evento sem itinerário não ganha essa seção.' },
    { section: 'turmas', personas: ['logado', 'inscrito', 'admin'],
    ...
    { section: 'ajuda', personas: ['logado', 'inscrito', 'admin'],
      ...
      body: 'A página abre com "Faça um pedido" — logo abaixo do título, ANTES das perguntas frequentes. O formulário já ficou no fim da página, embaixo das sete perguntas, e quem não rolasse até o rodapé não descobria que dava para pedir alguma coisa: no celular os botões de tipo só apareciam depois de mais de duas telas de rolagem. Um canal que ninguém vê não existe, então a posição virou requisito, com teste automático no CI. Por isso também o cabeçalho da página Ajuda é mais curto que o das outras (o padrão sozinho gastava a tela inteira do celular) e o texto de apoio é enxuto — os próprios botões já dizem o que cada tipo de pedido é. Depois do formulário vêm as perguntas frequentes, sob o título "Perguntas frequentes", em formato de acordeão: clicar no título expande ou recolhe a resposta, e cada pergunta abre de forma independente das outras. O texto acima do título é "Central de Ajuda" e o título principal é "Como podemos ajudar?". A seção "Faça um pedido" — formulário para escolher o tipo e descrever o pedido. São seis tipos: aprender sobre um tema, sugerir um curso, precisar de material, tirar uma dúvida, fazer parte de iniciativas do Time Força Ágil, e Outros — que fica sempre por último, porque é a saída para o que não coube nas outras e uma saída no meio da lista faz parar de ler o resto. Escolher o tipo é obrigatório, a descrição é opcional. O botão "Enviar pedido" está SEMPRE ativo: se ninguém escolheu o tipo, ele responde com "Escolha primeiro o tipo do pedido, ali em cima." e leva o foco para o primeiro chip. Ele já nasceu desabilitado até o tipo ser escolhido, e como não havia estilo de botão desabilitado ficava idêntico a um botão vivo: tocar nele não dava mensagem, não dava erro, não fazia nada — no iPhone, onde o :hover gruda depois do toque e um chip só encostado já parecia escolhido, isso virou "o site não envia pedido". Regra que ficou: recusa que não fala é falha. O envio também não fica preso em "Enviando…": se a gravação não responder em 12 segundos, o botão volta e avisa para tentar de novo. Depois de enviar, a confirmação verde substitui o formulário (confirmação tem que ser inconfundível), mas traz um botão "+ Fazer outro pedido" que devolve o formulário limpo na hora, sem recarregar a página — antes o caminho feliz terminava num beco sem saída e só o F5 trazia o formulário de volta. A confirmação também lembra que o pedido fica registrado em Minha Área → Meus pedidos. A lição virou regra de toda a interface, não só desta página: qualquer botão desabilitado do site agora aparece visivelmente apagado, e o efeito de passar o mouse só acontece em botão que está realmente clicável — antes um botão travado continuava com cara de vivo, e no celular isso é ainda pior, porque o toque não tem o "passar por cima" que entregaria que ele não responde.' },
    ...
    /* ── FACILITADOR ── */
    { section: 'facilitador', personas: ['logado', 'inscrito', 'admin'],
      title: '"Minhas Facilitações" — só as turmas da própria pessoa',
      body: 'A página #facilitador não é um painel administrativo: mostra só as turmas em que a pessoa logada está na equipe de facilitação (ver aba Eventos > Equipe de facilitação, no painel Admin), agrupadas em Em andamento, Programadas e Concluídas — mesmo critério de fase da Minha Área do aluno. Um admin que não esteja em nenhuma equipe vê a página vazia, do mesmo jeito que um facilitador sem turma nenhuma ainda.' },
    { section: 'facilitador', personas: ['logado', 'inscrito', 'admin'],
      title: '"Minhas Facilitações" — cada card mostra o papel',
      body: 'Cada card traz o evento, a turma, o período e o papel da pessoa nela: "⭐ Você é o responsável por esta turma" (destacado) ou "Você participa como facilitador". A mesma pessoa pode ver as duas frases ao mesmo tempo, em cards de turmas diferentes.' },
    { section: 'facilitador', personas: ['logado', 'inscrito', 'admin'],
      title: '"Abrir roteiro" — leitura para todos, edição só para a responsável',
      body: 'O botão "Abrir roteiro" de cada card mostra o roteiro completo daquela turma (base do evento + o que foi personalizado só nela). Quem é RESPONSÁVEL pela turma pode editar — personalizar atividades, adicionar exclusivas, restaurar o padrão, definir condução; quem é facilitador de apoio só visualiza, sem nenhum botão de edição na tela. A mesma tela e os mesmos dados são usados aqui e no painel Admin (função compartilhada em roteiro.js) — não são duas implementações.' },
    ...
    { section: 'admin', personas: ['admin'],
      ...
      body: '15 abas no painel: Dashboard, Eventos, Certificados, Treinamentos, Repositório, Cadastrados, Administradores, Tipos de atividade, Diretores, Facilitadores, Manual, Mapa, Testes, Pedidos e Sorteios. No desktop ficam em linha única. No mobile podem quebrar em 2 linhas com fonte reduzida para todas ficarem visíveis sem scroll horizontal.' },
    { section: 'admin', personas: ['admin'],
    ...
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Dashboard',
      body: 'Primeira aba do painel — resumo das avaliações que as pessoas enviaram. No topo de tudo ficam os filtros de ESCOPO: dois controles, \u201cEvento\u201d e \u201cTurma\u201d, que abrem uma lista quando você clica (a de turmas traz só as turmas dos eventos escolhidos, e ganha um campo de busca quando passa de oito). Eram fileiras de botões, um por evento e um por turma, todos sempre à vista: com três eventos e seis turmas cada, isso virava quase mil pixels de filtro no celular antes do primeiro número da tela, e crescia a cada oficina nova. Agora o filtro tem tamanho fixo — dez eventos ocupam o mesmo espaço que um. Eles decidem de quais eventos e turmas são TODOS os números da tela — cards, gráficos, destaques, temas, feedbacks e as respostas individuais. Dá para marcar mais de um evento e mais de uma turma ao mesmo tempo, porque comparar duas turmas do mesmo evento (ou dois eventos inteiros) é o uso normal a partir da segunda oficina; sem nada marcado, a tela soma tudo, como sempre somou. Marcar uma turma e depois trocar de evento não deixa um filtro invisível para trás: a turma que não pertence ao evento escolhido deixa de valer sozinha. Os filtros só aparecem quando há o que escolher — com um evento só, a linha de eventos nem é desenhada. Depois vêm os cinco números: Participantes (gente confirmada nas turmas do escopo), Avaliações recebidas (com o percentual de quem respondeu, dividido pelos inscritos desse mesmo escopo), Média geral das notas, NPS de recomendação e quantidade de comentários escritos. Abaixo vêm os gráficos: a média de cada turma em barras deitadas, agrupadas pelo evento de cada uma — com o nome do evento e a média dele no cabeçalho do grupo; turma que ainda não tem avaliação aparece com um travessão (—) e \u201csem avaliação\u201d, nunca com nota zero, que é a pior nota possível e não é o que \u201cninguém respondeu\u201d significa; a distribuição das notas em cinco faixas de duas notas cada (0-2, 3-4, 5-6, 7-8 e 9-10 — a primeira carrega três porque a escala tem 11 pontos e não dá divisão exata; os cortes em 7 e em 9 são os mesmos do NPS ao lado) e um medidor com a média de recomendação. Sobre o NPS: ele não é uma média simples — é o percentual de quem deu nota 9 ou 10 menos o percentual de quem deu de 0 a 6, que é como esse indicador é calculado no mercado. Enquanto ninguém tiver respondido a avaliação, todos os cards e gráficos continuam aparecendo, zerados, com um aviso no topo explicando que ainda não chegou nenhuma resposta — assim fica claro que a tela funciona e só falta dado.' },
    { section: 'admin', personas: ['admin'],
    ...
    { section: 'admin', personas: ['admin'],
      ...
      body: 'Botão "+ Novo evento" (topo da aba): cria um evento com nome, carga horária (horas), frequência mínima para certificado (%, padrão 75) e duas caixas, as duas já marcadas. "Divulgar este evento na página Turmas" decide se o evento aparece para as pessoas: desmarcada, o evento inteiro some da página — turmas, lista de espera e a Missão dele. Serve para preparar um evento antes de divulgar, e para tirar do ar um que já acabou. Desmarcar NÃO remove ninguém: quem está inscrita continua inscrita, com o mesmo acesso e a mesma Minha Área, e no painel o evento segue completo, com um selo "fora da página" no cabeçalho dele. "Aceitar lista de espera neste evento" é a mesma ideia, só que da fila. A fila é do evento e não depende de turma: com a caixa marcada, o evento já aparece na página Turmas mesmo sem turma nenhuma, com o card de espera — serve justamente para medir demanda antes de decidir quantas turmas abrir. Desmarcada, o card não aparece e ninguém novo entra na fila desse evento — quem já está na fila continua no painel, nada é apagado, e voltar a marcar reabre a entrada. Eventos são os containers pai das turmas — um evento pode ter múltiplas turmas. Os dados do evento (nomeEvento, cargaHoraria, percentualMinimo) são herdados automaticamente pelo certificado de cada turma vinculada. O mesmo modal ("+ Novo evento" ou "✎ Editar evento") também tem o conteúdo público da página Turmas, opcional: Missão (título curto e texto), Tópicos abordados, e um editor de Itinerário — dia a dia, mesmo padrão de "+ Adicionar data" das turmas, com "+ Adicionar dia" e um campo de texto por dia (numerados D1, D2... automaticamente pela posição na lista). Deixar tudo em branco não gera bloco nenhum na página pública para aquele evento. Terceira caixa, "Visível só para diretores e administradores": marcada, o evento some da página Turmas (e do resumo "turmas abertas" da Minha Área) para quem não está na aba Diretores nem é admin — mesmo efeito de despublicar, mas só para elas (ver aba Diretores). Quarta caixa, "Visível só para um público predefinido (lista de e-mails)": mesmo efeito, só que em vez de valer a aba Diretores inteira vale só quem você listar, nome por nome. Marcada, o cabeçalho do evento no painel ganha o selo "público restrito · N" (N é o tamanho da lista) e o botão "👥 Público restrito", onde a lista se monta — a mesma tela do público restrito da turma, um nível acima: só aparecem pessoas já cadastradas no site, porque sem cadastro ninguém consegue entrar para ver o evento. A lista vale para o evento INTEIRO: todas as turmas dele e o card de lista de espera. Quem está fora não vê o evento na página Turmas nem no resumo da Minha Área, não consegue manifestar interesse nem entrar na fila, e o painel recusa Confirmar, "＋ Participante" e "Mover para turma" — a busca do "＋ Participante" já sai filtrada pela lista. Enquanto a lista estiver vazia, o evento fica invisível para todo mundo que não é admin; admin sempre vê. Marcar a caixa NÃO remove ninguém: quem já está inscrita continua inscrita, com o mesmo acesso, e o card de cada turma passa a avisar, com os nomes, quem ficou dentro da turma sem estar na lista — para a decisão ser sua. É independente do público restrito da turma: se os dois estiverem marcados, a pessoa precisa estar nas duas listas. Logo abaixo do Itinerário, um seletor "Formato dos encontros" (Presencial / Remoto pelo Teams / Híbrido — os dois ao mesmo tempo no mesmo encontro) ajusta sozinho o texto do chip "dia(s) de encontro(s)" do bloco "Como funciona" da página Turmas (ex: "dia de encontro pelo Teams"). Os dois últimos chips desse mesmo bloco também são configuráveis ali: "Modalidade" (chip curto: Prática/Teórica/Mista, mais uma descrição livre) e "Participação" (chip curto: Opcional/Obrigatória) com "Quem participa" (uma lista fechada: Diretores/Executivos/Cedidos/Quadro próprio) — cada um com uma opção "— usar padrão —" que mantém o texto de sempre ("Prática"/"dinâmicas, jogos e exercícios em grupo" e "Opcional"/"aberta a todos os empregados da Previ"), então nenhum evento já existente muda sozinho. Editar ou excluir evento: ações disponíveis via botões no cabeçalho de cada container de evento. Excluir evento não exclui as turmas dentro dele — elas passam para a seção "TURMAS SEM EVENTO".' },
    { section: 'admin', personas: ['admin'],
    ...
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Eventos — editar turma (nome, datas, link do CMFlex e resultado esperado)',
      body: 'Botão "✎ Editar turma" no menu "⋯" de cada card: abre o mesmo modal de criação, já preenchido com o nome, o link do CMFlex, o resultado esperado e as datas atuais. Permite renomear a turma, incluir ou excluir datas individuais dos encontros, e cadastrar/trocar o link do CMFlex a qualquer momento — inclusive depois do interesse já encerrado (o campo continua sendo gravado, mas hoje NENHUMA tela o exibe: o botão de ir para o CMFlex foi retirado do card público de propósito — ver a regra "turma com interesse encerrado" na seção Turmas —, e o CMFlex passou a ser indicado só na mensagem que aparece ao registrar interesse com a turma aberta. O link fica guardado como registro; se voltar a ser exibido algum dia, é ele que será usado). Alterar as datas não apaga check-ins já registrados nos dias que continuarem na lista; remover uma data cuja presença já foi registrada não apaga o histórico de presença, apenas tira aquele dia das tabelas/certificados a partir da próxima geração. "Resultado esperado da turma" é um campo de texto livre opcional, um por TURMA (não por atividade do roteiro, que já tem seu próprio "Resultado esperado" independente deste) — pensado pra registrar o que se espera alcançar com aquele grupo específico (ex: um objetivo combinado com a área que solicitou a turma). Só é editável aqui, pelo admin. Aparece também no PDF exportado "🖨 Agenda resumida" do Roteiro DESTA turma (nunca no Roteiro-base, que é do evento inteiro e não tem turma) — um bloco logo no topo, antes do resumo do dia, com o mesmo rótulo "Resultado esperado da turma"; continua não aparecendo em nenhuma outra tela (nem pro facilitador, nem pros inscritos).' },
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Eventos — excluir turma',
      body: 'Botão "🗑 Excluir turma" no menu "⋯" de cada card: pede confirmação informando quantas pessoas ativas e removidas serão afetadas. Ao confirmar, apaga a turma e todos os dados ligados a ela: interessados/inscritos, configuração (aberta/finalizada), check-ins, histórico de log, equipe de facilitação, a personalização do roteiro daquela turma e os sorteios feitos nela. Ação permanente, sem desfazer. Os sorteios ficavam de fora dessa limpeza até 10/09/2026: a turma sumia e as linhas dela continuavam na aba Sorteios, rotuladas com a chave crua sob "(sem evento)", sem jeito de remover — o "Limpar histórico" mora no card da turma, que já não existia.' },
    { section: 'admin', personas: ['admin'],
    ...
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Eventos — lista de presença para impressão/PDF',
      body: 'Botão "🖨 Lista de presença" em cada card de turma: abre em nova aba um documento pronto para imprimir ou salvar como PDF (Ctrl+P), formatado em papel A4 paisagem — para entregar a um terceiro (ex.: RH) fora do sistema. Diferente do CSV, essa lista traz só quem está com status Inscrito (só quem confirmou pode ter check-in — ver seção Check-in), com nome, e-mail, área, uma coluna por dia da turma (✓ quando há check-in registrado, por QR ou lançado pelo admin, em branco quando não), e — se a turma já foi finalizada — as colunas Frequência e "Atingiu critério". No rodapé há duas linhas para assinatura (Responsável pela turma / RH). Se não houver ninguém Inscrito na turma, o botão avisa que não há o que gerar em vez de abrir um documento vazio.' },
    { section: 'admin', personas: ['admin'],
    ...
    { section: 'admin', personas: ['admin'],
      title: 'Aplicar migração no Roteiro-base não tem desfazer',
      body: 'Ao aplicar as mudanças da revisão "Antes x Depois" no Roteiro-base, o aviso de confirmação diz que NÃO há como desfazer automaticamente — confira o Antes x Depois antes de aplicar. Isso já foi diferente: existiu um botão "↩ Desfazer última migração", retirado do Roteiro-base por decisão de quem usa. O que ficou para trás foi pior que a ausência do botão: dois avisos continuavam prometendo "dá pra desfazer depois pelo botão…", as funções que restauravam viraram código que ninguém chamava, e o valor anterior de cada campo tocado continuava sendo gravado no banco a cada aplicação — sem leitor e sem limpeza, acumulando para sempre. Uma auditoria em 10/09/2026 encontrou isso e terminou a remoção: os textos passaram a dizer a verdade, o código morto saiu e a gravação parou.' },
    { section: 'admin', personas: ['admin'],
      title: 'Modais que carregam dados não piscam mais de tamanho',
      body: 'Os modais que precisam ler o banco antes de ter conteúdo — Roteiro de Facilitação do evento, Roteiro da turma e Equipe de facilitação — abriam com a caixa quase vazia e PULAVAM para o tamanho final quando os dados chegavam. Como o modal é centralizado na tela, ele crescia para os dois lados de uma vez, e a impressão era de que um modal menor tinha aparecido e sumido por baixo do certo. Medido no roteiro do evento: a caixa abria com 170px no computador e 274px no celular, contra 606px e 654px depois de carregar — menos de um terço do tamanho final. O pior caso era o Roteiro do evento, que abria sem nem avisar que estava carregando; o da turma já dizia \'Carregando…\', mas pulava igual. Agora os três reservam a altura enquanto carregam e mostram \'Carregando…\' centralizado, então a caixa abre praticamente no tamanho em que vai ficar.' },
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Eventos — o selo "Confirmação incompleta" e por que ele existe',
      body: 'Uma inscrição só vale quando tem DUAS coisas gravadas: o status "inscrito" E quem confirmou (confirmedByAdmin). O painel checava só o status; o resto do sistema — acesso a Conteúdos/Treinamento e a Minha Área da própria pessoa — sempre exigiu os dois. Quando um registro fica com o status mas sem quem confirmou, essa diferença vira um problema invisível: o painel diz "Inscrito" e a pessoa, do lado dela, não tem acesso a nada e vê a Minha Área como se ninguém a tivesse confirmado. Aconteceu de verdade na turma de setembro (08-09/09/2026), com duas pessoas. Havia DUAS origens possíveis, as duas já corrigidas. A primeira era o próprio botão Confirmar: se a sessão do admin não estivesse ativa naquele instante (ela some sozinha quando a leitura do perfil falha na rede), o clique gravava o status e apagava o campo de quem confirmou, sem avisar nada. A segunda, e a mais provável nesses dois casos, era o "＋ Participante" com o status "Inscrita": ele gravava só o status e o addedByAdmin, e nunca o confirmedByAdmin — ou seja, TODA pessoa adicionada direto como Inscrita nascia nesse estado quebrado, mesmo com tudo funcionando perfeitamente. E ISSO JÁ FUNCIONOU: o "＋ Participante" existe desde 17/06/2026 e gravava só o status porque, na época, o acesso também olhava só o status. O que mudou foi a regra de leitura, em 14/08/2026: para impedir que apenas manifestar interesse desse acesso a Conteúdos e Treinamento, o acesso passou a exigir também o confirmedByAdmin. A mudança estava certa, mas elegeu como prova de confirmação um campo que uma das três portas de escrita nunca soube gravar — o "＋ Participante" gravava addedByAdmin, outro nome. Nada quebrou na hora e nenhum erro apareceu; só passou a existir um jeito de criar gente inscrita-mas-sem-acesso, e ficou assim 27 dias, com a oficina de setembro caindo dentro dessa janela. A lição que fica: quando uma regra de leitura ganha uma exigência nova, é preciso percorrer TODAS as portas de escrita que produzem aquele estado. Não importa por qual caminho a pessoa chegou (manifestou interesse e foi confirmada, veio da lista de espera, ou foi adicionada direto): em todos eles ela é Inscrita, e em todos eles o registro passa a ter os dois campos. Hoje, sem sessão ativa, tanto o Confirmar quanto o "＋ Participante como Inscrita" recusam e avisam para recarregar a página, sem gravar nada. E o painel passou a usar a mesma regra do resto do sistema: quem está nesse estado aparece como pendente, com o selo vermelho "Confirmação incompleta" e o botão "Confirmar" na linha, que regrava o registro certo num clique. Essa regra é uma só para todo o painel — a contagem do cabeçalho ("N confirmados"), o filtro "Confirmados", quem entra no sorteio e a linha da tabela respondem todos pela mesma função. Elas já divergiram: o cabeçalho contava só pelo status e dizia "2 confirmados" numa turma onde uma única linha aparecia como Inscrito, e o sorteio chegava a incluir quem a tabela mostrava como pendente.' },
    { section: 'admin', personas: ['admin'],
    ...
    { section: 'admin', personas: ['admin'],
      ...
      body: 'Uma lista com os dias da turma + botão "Abrir check-in". A lista já vem com o PRIMEIRO DIA SEM PRESENÇA REGISTRADA marcado — não com "hoje": se o dia 1 já teve check-in e o dia 2 não, ela abre no dia 2, mesmo que hoje não seja nenhum dos dois (só cai em "hoje" se todos os dias já tiverem presença). E há uma regra que muda a rotina do dia e não estava escrita: ENQUANTO UM DIA ESTIVER ABERTO, A LISTA DE DIAS NÃO APARECE. No lugar dela fica o dia aberto como texto e o botão "Fechar check-in" — para liberar outro dia é preciso fechar o atual primeiro, e só então a lista reaparece, já marcada no primeiro dia sem presença: escolhe qual dia fica liberado para registrar presença. Ao clicar em "Abrir check-in", aparece uma confirmação mostrando o dia exato que será aberto — se o dia selecionado for diferente de hoje, a confirmação inclui um aviso ⚠️. O admin pode abrir qualquer dia — passado ou futuro — sem precisar estar no dia exato. Participantes só conseguem escanear o QR Code e registrar presença enquanto um dia estiver aberto. Quando aberto, o card mostra o selo "CHECK-IN ABERTO · DD/MM" e, nas ações, o dia aberto em texto + botão "Fechar check-in". Cada pessoa só pode registrar presença uma vez por dia.' },
    { section: 'admin', personas: ['admin'],
    ...
    { section: 'admin', personas: ['admin'],
      ...
      body: 'A lista de participantes (Nome, E-mail, Área, Status) é a mesma com a turma aberta ou com interesse encerrado — só que com o interesse encerrado ganha colunas extras por dia (ex: 11/08, 12/08…) e uma coluna Freq., já que é aí que o check-in passa a existir de verdade. Linhas "Interessado" mostram traço em todas as colunas de dia e frequência. Atenção ao que o traço quer dizer: é "não conta", não "não existe". A presença é gravada em turmas-checkin, separada da inscrição, e o check-in por QR só exige o status — então um registro com o selo "Confirmação incompleta" pode ter presença gravada e ainda assim aparecer com traço em todos os dias, porque a tabela só exibe presença de quem tem a inscrição completa. Foi o que aconteceu com 12 pessoas da turma de setembro em 09/09/2026: tinham presença desde o primeiro dia e a tela mostrava traço; ao completar a confirmação, os ✓ e a frequência apareceram todos de uma vez, sem nada ser relançado. Linhas "Inscrito" mostram, em cada dia, um selo verde "✓ qr" (registrado pelo próprio QR Code — ver seção Check-in), um selo ciano "✓ adm" (admin registrou manualmente pelo botão "—") ou o botão "—" clicável para lançar presença retroativa a qualquer hora — as duas formas contam igual para a frequência, exibida na última coluna como X/5 em verde (atingiu o critério) ou vermelho (não atingiu). Critério padrão: 75% de presença (4 de 5 dias). Com a turma ainda aberta, a lista mostra só Nome/E-mail/Área/Status/Data de registro — sem colunas de dia, já que check-in ainda não pode acontecer.' },
    { section: 'admin', personas: ['admin'],
    ...
    { section: 'admin', personas: ['admin'],
      ...
      body: 'Botão "＋ Participante" (disponível com a turma aberta ou com interesse encerrado): abre um modal com campo de busca por nome ou e-mail. O admin digita e o sistema filtra em tempo real os cadastros existentes (fa-users) — ao clicar numa pessoa da lista, nome, e-mail e área são preenchidos automaticamente. O admin só precisa escolher o status: "Interessada" ou "Inscrita". A pessoa precisa ter cadastro prévio no site para aparecer na busca — sem cadastro ela não consegue acessar o sistema, então o admin deve orientá-la a se cadastrar primeiro. Se a pessoa não aparecer na busca, o modal exibe esse aviso. Escolher "Inscrita" adiciona direto com esse status (sem precisar de "Confirmar" depois) e vale como uma confirmação para todos os efeitos: grava confirmedByAdmin/confirmedByAdminName/confirmedDate igual ao botão "Confirmar", que é o que de fato libera Conteúdos, Treinamento e a Minha Área da pessoa. Por isso, se a sessão do admin não estiver ativa naquele instante, adicionar como "Inscrita" é recusado com aviso para recarregar a página (adicionar como "Interessada" continua funcionando, porque não é uma confirmação). Se ela já for Inscrita em outra turma, o modal avisa e ao continuar remove automaticamente essa outra inscrição. Escolher "Interessada" registra só o interesse, sem essa checagem. Se o e-mail já estiver na turma, exibe aviso. A tabela atualiza automaticamente. Nomes são sempre normalizados para maiúsculas ao salvar.' },
    { section: 'admin', personas: ['admin'],
    ...
    { section: 'admin', personas: ['admin'],
      ...
      body: 'Quase todas as confirmações e avisos do painel usam janelas com o mesmo visual dos modais de cadastro/login (adminConfirm/adminAlert). DUAS AINDA USAM A CAIXA PADRÃO DO NAVEGADOR, e vale saber quais: (1) a confirmação de "Abrir check-in para o dia ..." — justamente a que aparece todo dia de oficina, e é ela que traz o aviso ⚠️ quando o dia escolhido é diferente de hoje; (2) o aviso "Selecione uma turma primeiro." do download em lote da aba Certificados. O texto aqui dizia "nenhuma ação" até 10/09/2026 — era falso nesses dois pontos. Converter as duas para o modal do site é uma mudança pequena, mas mexe no fluxo do check-in (a caixa nativa é síncrona, o modal do site responde por callback), então não deve ser feita em dia de oficina.' },
    { section: 'admin', personas: ['admin'],
    ...
    { section: 'admin', personas: ['admin'],
      ...
      body: 'Lista todas as pessoas que já se cadastraram. Mostra nome, e-mail, área, data de cadastro e a Situação (Ativo/Bloqueado e Verificado/Pendente). As ações ficam na última coluna: "✎ Editar" e o menu "⋯".' },
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Cadastrados — criar conta pelo admin',
      body: 'Botão "+ Criar conta para colaboradora": nome completo, e-mail @previ.com.br (o campo já vem com "@previ.com.br" preenchido ao focar vazio, cursor antes do @ — só falta digitar o nome de usuário; mesmo comportamento nos campos de e-mail de Administradores, Diretores e Facilitadores), Área/Setor (lista fechada com as 26 áreas, a mesma do cadastro público) e a senha do próprio admin, para confirmar a ação. A conta nasce com senha fixa 12345678 e a flag adminApproved — isso pula por completo a verificação de e-mail: a pessoa consegue logar direto, mesmo que o endereço não corresponda a uma caixa de e-mail real (não é enviado nenhum e-mail para essa conta). Ao criar, o Firebase automaticamente loga como a conta nova por um instante antes do código deslogar e logar de volta como o admin — três trocas de sessão internas que não aparecem na tela: o painel não pisca nem redireciona durante essa operação.' },
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Cadastrados — resetar progresso',
      body: 'No menu "⋯" da linha, disponível para qualquer pessoa cadastrada. Ação irreversível — pede confirmação antes de executar. Após o reset: se a pessoa estiver logada, a página dela recarrega automaticamente — o autodiagnóstico fica disponível para refazer sem nenhuma ação da pessoa.' },
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Cadastrados — redefinir senha',
      body: '"Redefinir senha", no menu "⋯" da linha: dispara o e-mail de redefinição de senha do Firebase para qualquer pessoa cadastrada. Só funciona se a pessoa já tiver feito o primeiro cadastro/login.' },
    { section: 'admin', personas: ['admin'],
    ...
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Diretores',
      body: 'Lista separada de Administradores, gerenciável por qualquer admin (não só os dois super-admins): nome, e-mail e desde quando está na lista, com formulário para adicionar (nome completo + e-mail @previ.com.br) e botão "Remover" em cada linha. Estar aqui não dá nenhum acesso de admin nem muda nível de acesso (member/enrolled) — serve só para decidir se a pessoa enxerga, na página Turmas, um evento marcado "Visível só para diretores e administradores" (✎ Editar evento, aba Eventos). Sem estar nesta lista e sem ser admin, esse evento some da vitrine da pessoa como se estivesse fora da página.' },
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Tipos de atividade',
      body: 'Gerencia a lista de opções do campo "Tipo de atividade" do formulário de atividade do Roteiro (Aba Eventos → Roteiro), gerenciável por qualquer admin (não só os dois super-admins). Lista sempre exibida em ordem alfabética, com formulário para adicionar um tipo novo (nome não pode repetir um já existente, comparação sem diferenciar maiúsculas/minúsculas) e botões "Editar" (renomear, inline na própria linha) e "Remover" (com confirmação) em cada tipo. Renomear ou remover um tipo NÃO altera nenhuma atividade já cadastrada com o valor antigo — ela continua guardando o texto de antes; só deixa de aparecer pré-selecionada no formulário até alguém escolher um tipo novo e salvar (mesma regra que já valia pra qualquer tipo "fora da lista" antes desta funcionalidade existir). "Intervalo" tem efeito especial no roteiro — conta como pausa no cálculo do dia e ganha destaque visual (badge laranja) na lista de atividades — independente desta lista de opções; removê-lo ou renomeá-lo aqui só tira a possibilidade de escolher esse tipo em atividades NOVAS pelo formulário, não desliga o efeito especial de atividades que já usam o texto exato "Intervalo". Na primeira vez que esta aba é aberta (banco novo, sem nenhum tipo cadastrado ainda), a lista é semeada automaticamente com os mesmos tipos que já existiam fixos no código antes desta funcionalidade — ninguém perde nenhuma opção com a mudança.' },
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Facilitadores — cadastro global',
      body: 'Cadastro de "pessoas autorizadas a atuar como facilitadoras" (nome, e-mail, situação Ativo/Inativo, desde quando, quantas próximas turmas), gerenciável por qualquer admin. Estar aqui NÃO significa participar de nenhuma turma: dá só duas coisas — acesso à página #facilitador (link no menu, entre Minha Área e Ajuda) e a possibilidade de ser colocada na equipe de uma turma específica. A participação de fato (e o papel — responsável ou facilitador de apoio) é decidida turma por turma, na aba Eventos ("Equipe de facilitação" dentro de cada turma), nunca aqui. Só facilitadores marcados Ativo aparecem como opção para entrar na equipe de uma turma; inativar alguém não a remove das equipes onde já está. Botão "Ver" abre a ficha da pessoa com suas facilitações — Próximas (inclui as em andamento) e Anteriores —, cada linha com evento, turma, período e papel.' },
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Eventos — Equipe de facilitação de uma turma',
      body: 'Botão "👥 Equipe de facilitação" no menu "⋯" de cada turma. Mostra a RESPONSÁVEL pela turma (⭐, no máximo uma) e os OUTROS FACILITADORES, com botão "+ Adicionar facilitador" que lista só quem está cadastrado (e Ativo) na aba Facilitadores e ainda não está nesta turma. Ao adicionar/tornar alguém responsável quando já existe uma, o sistema pergunta "Esta turma já possui [NOME] como facilitador responsável. Deseja substituir por [NOVO NOME]?" — só troca após confirmar, e a pessoa anterior vira facilitadora de apoio automaticamente (nunca fica sem responsável nenhuma turma que já teve uma definida). Remover alguém da equipe tira só a associação com aquela turma — o cadastro dela na aba Facilitadores continua intacto. A mesma pessoa pode ser responsável numa turma e apoio em outra, ou não participar de uma terceira, tudo ao mesmo tempo.' },
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Eventos — Roteiro de Facilitação do evento (roteiro-base)',
      body: 'Botão "📋 Roteiro" no cabeçalho de cada evento. Roteiro-base dividido por dia (abas "DIA 1", "DIA 2"...), cada dia com suas atividades em ordem (setas ▲▼ para reordenar, sem arrastar). Cada atividade tem: título, tipo, horário de início/fim/duração (preencher dois calcula o terceiro sozinho), objetivo, resultado esperado, passo a passo, dicas para o facilitador, prompt para IA, conexão com a agilidade, materiais necessários, preparação prévia e observações. Todos esses campos de texto (menos título) têm uma barra de formatação básica acima da caixa — negrito, itálico, sublinhado, lista com marcadores, lista numerada, centralizar, justificar, "← Recuo"/"→ Recuo" (diminui/aumenta o recuo do parágrafo selecionado, funciona dentro ou fora de uma lista), um seletor "Tamanho" (Pequena/Normal/Grande/Enorme, aplicado só ao trecho selecionado), "↵ Espaço" (insere uma linha em branco), cor do texto ("A") e cor de fundo/destaque (ícone de caneta) — que também aparece na tela de leitura e na impressão completa. Quem digita a numeração à mão (ex: "1. texto") não precisa digitar "2. " na linha seguinte: apertar Enter no fim de uma linha assim já continua a numeração ou o marcador sozinho (e some se a linha estiver vazia, pra dar pra "sair" da lista). Dentro da caixa, Tab não pula para o próximo campo do formulário: dentro de uma lista (marcadores ou numerada, criada pelos botões) ele indenta o item como sub-item, e fora de uma lista insere um recuo no texto (Shift+Tab desfaz o recuo de um item de lista). Colar (Ctrl+V) sempre traz só o texto, nunca a formatação de origem: colar uma tabela do Excel/Google Sheets/Word, por exemplo, não traz uma tabela de verdade, mas preserva uma linha por linha da origem, com as colunas separadas por um espaço largo — evita o texto todo emaranhado que aparecia colando o HTML de origem sem tratamento. Toda turma do evento herda este roteiro-base; personalizações ficam na turma (ver regra seguinte) e nunca alteram o roteiro-base nem vazam para outras turmas. Excluir um dia apaga junto suas atividades e qualquer personalização que alguma turma tivesse feito nelas.' },
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Eventos — Roteiro da turma (personalização)',
      body: 'Botão "📋 Roteiro da turma" no menu "⋯" de cada turma. Mostra o roteiro-base do evento com as datas reais da turma (dia 1 = primeira data cadastrada, e assim por diante) e ordena as atividades pelo HORÁRIO, não por uma posição fixa — mudar o horário de uma atividade nesta turma já reordena a visão dela sozinha. Cada atividade tem um selo: sem selo é padrão do evento; "Alterado nesta turma" (amarelo) quando algum campo foi personalizado só aqui; "Exclusivo desta turma" (azul) quando foi criada só para esta turma via "+ Adicionar atividade somente nesta turma" — nunca aparece em outra turma nem no roteiro-base. "Editar apenas nesta turma" grava só os campos que você de fato mudou: se o roteiro-base for atualizado depois, os campos que esta turma nunca tocou continuam acompanhando a atualização — só os campos alterados ficam travados na versão desta turma. "Restaurar padrão do evento" (com confirmação) descarta a personalização inteira da atividade e ela volta a seguir o roteiro-base. "Remover desta turma" tira uma atividade da visão desta turma sem afetar o roteiro-base nem outras turmas; fica listada em "Atividades removidas desta turma", de onde pode ser restaurada. Cada atividade também tem Condução principal e Apoio, escolhidos só entre quem está na equipe de facilitação desta turma — a mesma pessoa não pode ser principal e apoio na mesma atividade.' },
    { section: 'admin', personas: ['admin'],
      title: 'Roteiro — Tipo de atividade (lista administrável)',
      body: 'O campo "Tipo de atividade" é uma lista fechada, em ordem alfabética, administrável na aba "Tipos de atividade" do painel Admin (ver regra própria) — não é mais um array fixo no código. "Intervalo" é o único com efeito prático — conta em Pausas no resumo do dia e ganha destaque laranja na lista; os demais são só um rótulo mostrado ao lado do título da atividade. Uma atividade antiga com um tipo que não existe mais nessa lista (removido/renomeado na aba Tipos de atividade, ou nunca existiu) continua guardando o texto original no banco — o campo só aparece sem seleção (nem em branco nem marcando outro por engano) até alguém escolher um tipo novo e salvar.' },
    { section: 'admin', personas: ['admin'],
      title: 'Roteiro — campo de texto não força mais maiúsculas ao digitar',
      body: 'Os campos de texto do formulário de atividade (Título, e os mesmos campos em qualquer outro formulário que usa o mesmo estilo de rótulo do site) mostravam o texto digitado inteiro em maiúsculas dentro da caixa, mesmo a pessoa digitando em minúsculas — um efeito puramente visual (o valor salvo sempre foi exatamente o que foi digitado), mas que dava a impressão de "conversão automática" e podia levar a digitar tudo em maiúscula sem necessidade. O rótulo do campo (ex: "TÍTULO", "TIPO DE ATIVIDADE") continua em maiúsculas — isso é estilo intencional do site — só o conteúdo que a pessoa digita ou seleciona passou a aparecer exatamente como foi escrito.' },
    { section: 'admin', personas: ['admin'],
      title: 'Roteiro — resumo do dia (janela, programado, facilitação, pausas, lacunas reais)',
      body: 'Acima e abaixo da lista de atividades de cada dia (roteiro-base e roteiro da turma), uma barra mostra números calculados a partir dos dados reais — nunca digitados à mão: Janela (do início da primeira à conclusão da última atividade com horário; quando o dia tem mais de uma sessão, o rótulo vira "Janela da sessão" pra deixar claro que aquele número é só daquela sessão, nunca do dia inteiro); Tempo programado (soma das durações de tudo, inclusive pausas); Tempo de facilitação (o programado descontando pausas/intervalos); Pausas / intervalos (soma das atividades do tipo "Intervalo", incluindo uma que esteja como sub-etapa dentro de uma seção — o que conta é o tipo de quem realmente tem a duração, não o tipo da seção que a contém); Lacunas reais (soma dos buracos sem nada programado entre atividades vizinhas da MESMA sessão/janela — nunca o intervalo entre uma sessão e a seguinte, ver regra de Sessões abaixo; o rótulo "Lacunas reais" é o mesmo tanto na barra de cada sessão quanto no resumo agregado, pra não parecer coisa diferente); Atividades principais (quantidade de atividades de nível principal — sub-etapas de uma seção continuam aparecendo normalmente na lista, só não entram nessa contagem, pra não sugerir que o dia tem mais blocos de facilitação do que realmente tem); Sobreposições (só aparece quando existe algum conflito de horário — minutos de sobreposição e quantidade de conflitos, além do banner vermelho detalhado). Duas contas fecham sozinhas, e vale saber quando. Programado = Facilitação + Pausas vale SEMPRE: a facilitação é literalmente o programado menos as pausas, calculada assim no código. Já Janela = Programado + Lacunas reais só vale QUANDO NÃO HÁ SOBREPOSIÇÃO — e é justamente o caso que o sistema destaca em vermelho. O motivo: a janela mede do primeiro início ao último fim, enquanto o programado soma as durações; se duas atividades ocupam o mesmo minuto, esse minuto é contado duas vezes no programado e uma só na janela. As lacunas não compensam isso, porque só somam vãos positivos (sobreposição vai para outra lista, a que alimenta o banner). Com conflito, a conta que fecha é Janela = Programado + Lacunas reais − Sobreposições. Todos calculados por sessão, quando o dia usa sessões.' },
    { section: 'admin', personas: ['admin'],
      title: 'Roteiro — diferença entre Intervalo (pausa) e Lacuna (buraco)',
      body: 'Intervalo é uma atividade cadastrada de propósito (tipo "Intervalo") — conta como tempo programado e como pausa, mas não como facilitação líquida. Lacuna é tempo entre duas atividades vizinhas da mesma sessão SEM nada cadastrado no meio — aparece como uma faixa tracejada laranja "⚠ Lacuna no planejamento" entre as duas, não é uma atividade, não conta como programado nem como pausa, só entra na janela. As duas coisas não podem ser confundidas: uma é planejada, a outra é ausência de planejamento.' },
    { section: 'admin', personas: ['admin'],
      title: 'Roteiro — Sessões/janelas (ex: Manhã e Tarde) num mesmo dia',
      body: 'Cada atividade tem um campo opcional "Sessão / janela" (texto livre, ex: "Manhã", "Tarde"). Atividades com o mesmo texto nesse campo formam uma sessão: cada sessão calcula sua PRÓPRIA barra de resumo (janela, programado, facilitação, pausas, lacunas) só com as atividades dela — o intervalo entre o fim de uma sessão e o início da próxima (ex: o almoço entre Manhã e Tarde) nunca vira "lacuna no planejamento", porque uma sessão simplesmente não enxerga as atividades da outra. Quando pelo menos duas sessões diferentes existem no dia, cada uma aparece com um cabeçalho próprio (nome da sessão + quantidade de atividades) antes da sua lista; se ninguém usa o campo, o dia continua se comportando como uma janela só, exatamente como antes dessa opção existir. Quando NINGUÉM preenche "Sessão / janela" naquele dia, o sistema tenta detectar sozinho onde as sessões começam e terminam: um vão de 90 minutos ou mais entre duas atividades vizinhas vira um corte automático de sessão (nomeada "Manhã", "Tarde" ou "Noite" pelo horário de início de cada bloco, com um número extra se repetir — ex: "Manhã 2") em vez de aparecer como buraco de planejamento; um vão menor que 90 minutos continua sendo lacuna de verdade. Uma dica discreta abaixo do resumo avisa quando as sessões vieram dessa detecção automática, sugerindo preencher "Sessão / janela" manualmente pra nomear ou ajustar o corte do jeito que a pessoa quiser — preencher o campo sempre tem prioridade sobre a detecção automática.' },
    { section: 'admin', personas: ['admin'],
      title: 'Roteiro — resumo agregado "Sessões" no topo do dia',
      body: 'Quando o dia tem mais de uma sessão (manual ou detectada automaticamente), uma barra extra aparece no topo, antes das barras "Janela da sessão" de cada uma: quantas sessões existem, o nome/horário/duração de cada uma lado a lado, e depois os números agregados do dia inteiro — Tempo disponível (soma do tempo de CADA sessão, nunca o período entre a primeira e a última atividade do dia todo — Manhã 1h30 + Tarde 1h23 dá 2h53, não um falso "7h23" contínuo), Tempo programado, Tempo de facilitação, Pausas e Lacunas reais (essas quatro somadas de todas as sessões) e Atividades principais (total do dia). Uma linha pequena embaixo mostra, só como referência secundária, o período entre a primeira e a última atividade do dia inteiro — deixando claro que esse número NÃO é tempo disponível para facilitação. Essa barra não substitui as barras de cada sessão (que continuam existindo do jeito que já funcionavam), só evita que a pessoa precise somar os números das sessões na cabeça.' },
    { section: 'admin', personas: ['admin'],
      title: 'Roteiro — aviso de sobreposição de horário',
      body: 'Se uma atividade começa antes da anterior (da mesma sessão) terminar, aparece um banner vermelho "⚠ [atividade A] se sobrepõe a [atividade B]" com o tanto de sobreposição em minutos — tanto no roteiro-base quanto no roteiro da turma, e também na impressão. Diferente da lacuna (que é um buraco), a sobreposição é um conflito real de horário: as duas atividades continuam aparecendo cada uma na sua linha normal, o aviso só chama atenção pra o problema.' },
    { section: 'admin', personas: ['admin'],
      title: 'Roteiro — seções com sub-etapas (etapas filhas)',
      body: 'Uma etapa (sub-etapa) é uma atividade completa igual a qualquer outra — mesmo formulário, todos os campos (objetivo, passo a passo, materiais etc.) — só que "filha" de uma atividade-mãe. O botão "+ Etapa" (dentro de uma atividade de nível principal) abre esse formulário completo já vinculado a ela; não existe uma versão reduzida só de título e duração — e não deve ser confundido com "+ Adicionar atividade", que cria uma atividade de nível principal nova, fora de qualquer seção. Uma atividade vira "seção" automaticamente assim que passa a ter etapas: sua duração exibida em toda tela (lista, impressão, resumo do dia) passa a ser a SOMA das etapas, calculada sozinha — o campo Duração dela própria, no formulário de edição, fica travado mostrando essa soma (não dá pra digitar um valor diferente enquanto houver alguma etapa; Início/Fim continuam editáveis normalmente e se completam sozinhos com base nessa duração fixa) e aparece um botão "▲ Recolher"/"▼ Ver etapas", além de uma linha de resumo "Tipo · Duração · N etapas" abaixo do título da seção. No roteiro-base, se a duração de uma etapa muda e a seção-mãe tem horário de início definido, o horário de FIM da seção-mãe é recalculado sozinho pra acompanhar a nova soma (e, se houver atividades seguintes com horário na mesma sessão, o sistema oferece deslocar os horários delas também, do mesmo jeito que já faz quando a duração de uma atividade de nível principal muda) — a duração da seção-mãe em si nunca é editável na mão, sempre deriva das etapas. Na lista, a seção aparece numerada (1, 2, 3...) e as etapas recuadas logo abaixo com um visual mais discreto (indentação maior, linha vertical fina, fundo levemente diferente, botões menores), numeradas 4.1, 4.2... — essa numeração hierárquica nunca é um valor fixo salvo, ela é sempre recalculada a partir da posição atual: se a atividade 4 vira 5, as etapas dela viram 5.1, 5.2 sozinhas. Uma etapa sem horário próprio simplesmente não mostra coluna de horário (nunca um "—" solto). Etapas não têm posição própria no dia (não entram na ordenação cronológica, no aviso de ordem divergente nem na cadeia de recálculo de horários seguintes — só a atividade-mãe conta pra tudo isso, e reordenar etapas com ▲▼ nunca move uma etapa pra fora da seção dela). Excluir a atividade-mãe apaga junto todas as suas etapas; duplicar a atividade-mãe duplica as etapas junto.' },
    { section: 'admin', personas: ['admin'],
      title: 'Roteiro-base — "+ Etapa" atrás do menu "⋯" em atividade simples',
      body: 'No roteiro-base, uma atividade de nível principal que AINDA não tem etapas mostra um botão "⋯" no lugar de "+ Etapa" solto na linha — clicar abre um menuzinho só com "+ Etapa" (mesmo padrão visual do menu "⋯" dos cards de turma). Assim que a primeira etapa é criada, a atividade vira seção e o botão "+ Etapa" passa a aparecer direto na linha, sem precisar do menu — a ação é sempre a mesma, só muda onde ela mora, pra manter a linha de uma atividade simples mais enxuta.' },
    { section: 'admin', personas: ['admin'],
      title: 'Roteiro-base — aviso de atividades fora da ordem cronológica',
      body: 'Se a ordem de exibição (a mesma que ▲▼ reordena) não bate com a ordem dos horários cadastrados, aparece um aviso "⚠ Existem atividades fora da ordem cronológica" com um botão "Ordenar por horário" que reorganiza a lista pelos horários de uma vez — sem fazer isso sozinho sem avisar.' },
    { section: 'admin', personas: ['admin'],
      title: 'Roteiro-base — recalcular horários seguintes ao mudar a duração',
    ...
    { section: 'admin', personas: ['admin'],
      title: 'Roteiro — exportar (Agenda resumida / Agenda + Objetivos / Roteiro completo / Passo a passo)',
      body: 'Botão "🖨 Agenda resumida" (roteiro-base e roteiro da turma; antes chamado só de "Imprimir") abre uma janela com o resumo do dia (uma barra por sessão/janela, se o dia usar sessões) e a lista de atividades formatada para impressão — mesmo padrão da Lista de presença: aciona "Imprimir / salvar como PDF" e usa o diálogo nativo do navegador, sem depender de nenhuma biblioteca de PDF. Ao lado, "🖨 Agenda + Objetivos" abre a MESMA tabela (#, Horário, Atividade, Tipo, Duração), mas acrescenta, logo abaixo de cada linha que tiver Objetivo e/ou Resultado esperado preenchido, uma linha extra só com esses dois campos (rótulo em dourado maiúsculo, igual ao "Roteiro completo") — pensada pra quem quer entender o que cada etapa entrega sem abrir todos os campos internos do "Roteiro completo" nem ficar só na tabela crua; atividade sem os dois campos preenchidos não ganha linha extra nenhuma. "🖨 Roteiro completo" (antes "Imprimir completo") abre a mesma janela, mas com um bloco por atividade e sub-etapa trazendo todo o conteúdo de facilitação preenchido, nesta ordem: objetivo, resultado esperado, passo a passo, dicas para o facilitador, prompt para IA, conexão com a agilidade, materiais, preparação prévia, observações — só os campos que têm valor aparecem, campo em branco não é exibido e nunca aparece nenhuma mensagem tipo "Nenhum campo preenchido"; uma atividade só com título e duração simplesmente imprime só título e duração, sem mais nada embaixo. "🖨 Passo a passo" abre o mesmo formato de bloco por atividade/etapa, mas mostra só o campo "Passo a passo" — sem Objetivo, Dicas para o facilitador, Conexão com a agilidade, Materiais, Preparação prévia ou Observações — pensado pra uma versão mais enxuta pra levar impressa ou compartilhar com um co-facilitador, sem as notas internas de preparo/reflexão. Cada bloco de etapa filha (no "Roteiro completo" e no "Passo a passo") traz uma linha discreta "[título da seção-mãe] — continuação" acima do próprio título, pra nunca perder o contexto de qual seção aquela etapa pertence caso a impressão corte a página bem ali. Nenhum bloco de atividade trava o corte de página (evita PDF truncado quando uma atividade tem texto longo — o Chrome corta silenciosamente conteúdo que não cabe numa única página quando isso é forçado); título e cabeçalho de cada campo ficam sempre junto do início do conteúdo deles, pra não sobrar um título sozinho no fim de uma página. A janela de impressão mostra uma dica visível na tela (nunca impressa) sugerindo desmarcar "Cabeçalhos e rodapés" e marcar "Gráficos de fundo" nas opções de impressão do navegador para uma exportação mais fiel — o documento já traz sua própria numeração de página e marca no rodapé (ver regra "Roteiro — rodapé próprio de página no PDF exportado"), então o cabeçalho/rodapé nativo do navegador (data, título da aba, URL, "1/2") só duplicaria informação; ainda assim, desligar esse cabeçalho/rodapé nativo continua sendo uma preferência do navegador da pessoa, o site não consegue fazer isso sozinho. Na tela, o conteúdo da janela fica travado numa largura de página (não estica até a borda da janela do navegador) — numa tela larga, sem esse limite a tabela da "Agenda resumida" ficava com colunas desproporcionais (um vão enorme entre "Tipo" e "Duração", por exemplo) e o documento parecia torto comparado a uma folha de verdade; a tabela em si também tem largura fixa por coluna (#, Horário, Tipo e Duração com um tamanho previsível, o resto pra "Atividade"), então as colunas não saem do lugar não importa o tamanho da janela. Na impressão física, esse limite de largura é ignorado (quem manda no tamanho da página aí é a impressora/o PDF, via as configurações de página do navegador).' },
    { section: 'admin', personas: ['admin'],
      title: 'Roteiro — hierarquia visual atividade/sub-etapa nas exportações em tabela ("Agenda resumida" e "Agenda + Objetivos")',
      body: 'Nas duas exportações em formato de tabela, cada atividade de nível principal (numerada "1", "2", "3"...) ganha uma linha divisória dourada no topo, separando visualmente onde ela começa do fim da atividade anterior — inclusive de todas as sub-etapas dela, que não têm essa linha entre si nem em relação à atividade-mãe, deixando claro que pertencem ao mesmo bloco. O título de cada sub-etapa (numerada "1.1", "1.2"...) ganha um prefixo "↳" antes do nome, além do recuo e da cor mais apagada que já existiam, reforçando visualmente que ela é uma etapa DENTRO da atividade acima, não uma atividade nova. A atividade principal e todas as suas sub-etapas também compartilham a mesma leve variação de tom de fundo (alternando a cada bloco de atividade), então mesmo rolando a página é fácil perceber onde um bloco termina e o próximo começa sem precisar ler os números.' },
    { section: 'admin', personas: ['admin'],
      title: 'Roteiro — campos "Resultado esperado" e "Prompt para IA"',
      body: '"Resultado esperado" (bloco "Propósito", logo abaixo de Objetivo) é um campo de texto opcional pra deixar claro o que deve existir ao término daquela atividade — ex: "8 iniciativas priorizadas pelo Conselho.", "Plano final após recebimento dos sinais da realidade.". "Prompt para IA" mora num bloco próprio chamado "IA", entre "Como conduzir" e "Recursos" no formulário — pensado principalmente para atividades de experimentação com inteligência artificial, e propositalmente separado de Passo a passo, Dicas para o facilitador, Observações e Preparação prévia, pra não misturar o prompt em si com instruções de condução. Os dois são campos de texto rico iguais aos demais (mesma barra de formatação, mesmo sanitizador ao salvar), opcionais, e seguem a mesma regra de todo campo opcional do roteiro: se estiverem vazios, não aparecem em lugar nenhum — nem título de campo, nem placeholder — na tela de leitura nem no "🖨 Roteiro completo". Nenhum dos dois aparece na "🖨 Agenda resumida" (só tabela, sem campos de facilitação) nem no "🖨 Passo a passo" (só mostra o campo Passo a passo). Este é puramente um roteiro de facilitação: os cartões, escolhas, discussões, decisões e qualquer uso de IA acontecem fisicamente durante a oficina — o sistema não executa nem registra nada disso (sem votação eletrônica, formulário para participantes, chatbot ou execução de IA dentro do site).' },
    { section: 'admin', personas: ['admin'],
      title: 'Roteiro — identidade visual no PDF exportado (Agenda resumida e Roteiro completo)',
      body: 'As duas exportações assumem o tema espacial escuro do site inteiro, não só uma faixa de cabeçalho: fundo escuro (mesmo tom de --space-0), os blocos de resumo/atividade em cartões com o mesmo cinza-azulado dos painéis do site, títulos em dourado (Oswald), horários em ciano, rótulos de campo em dourado maiúsculo, tudo com a mesma paleta e as mesmas fontes do site (Anton no ícone/título de marca, Oswald nos títulos, Barlow no corpo do texto) — em vez de aparecer como um documento genérico do navegador sem nada do site. No topo, uma barra reproduz a logo do site: o mesmo ícone do menu superior, "FORÇA ÁGIL" e "PREVI · OFICINA DE AGILIDADE ORGANIZACIONAL" (as duas em caixa alta no HTML gerado, não por estilo — a janela de impressão não tem o CSS do site). Como a janela de impressão é um documento novo, sem acesso ao CSS nem ao sprite de ícones do site, o ícone vai embutido como SVG dentro do próprio HTML gerado (não depende de nenhum arquivo carregado à parte) — funciona mesmo se a pessoa salvar o PDF sem internet, só as fontes do Google Fonts precisam de conexão (caem pra uma fonte padrão do sistema se não carregarem, sem quebrar nada). Por padrão o documento inteiro depende de fundo escuro pra fazer sentido (texto claro sobre fundo escuro) — é preciso marcar "Gráficos de fundo" (ou "Imprimir cores e imagens de fundo") no diálogo de impressão do navegador pra sair fiel à tela; um cartão de aviso destacado no topo da janela (nunca impresso) explica isso, além de insistir pra desmarcar "Cabeçalhos e rodapés". Um checkbox "Modo econômico (fundo claro, menos tinta)", ao lado do botão de imprimir (nunca sai na exportação), troca o documento inteiro pra fundo claro e texto escuro — pensado pra quem vai imprimir em papel de verdade e não quer gastar tinta com tanto fundo escuro — mantendo a mesma paleta de acento (dourado/ciano) e as mesmas fontes, só com tons mais escuros pra continuar legível no branco. A faixa da logo no topo é a única exceção: continua sempre com as cores originais do site (fundo azul-marinho escuro, ícone e "ÁGIL" em dourado vivo, "Previ" em ciano vivo) mesmo com o modo econômico marcado — é a logo de verdade, não um elemento de conteúdo, e gasta pouquíssima tinta por ser só uma faixa.' },
    { section: 'admin', personas: ['admin'],
      title: 'Roteiro — paginação sem grandes áreas vazias no PDF exportado ("Roteiro completo" e "Passo a passo")',
      body: 'Nenhum bloco (card de atividade, sub-etapa ou campo) trava a página inteira consigo: um card pode continuar normalmente na página seguinte quando não cabe todo na atual, aproveitando o espaço disponível em vez de pular a página inteira. Só duas coisas continuam sempre juntas, nunca separadas por uma quebra de página: o título da atividade com o horário logo abaixo dele (nunca um sozinho no fim de uma página com o outro começando a próxima), e o rótulo de um campo com pelo menos a primeira linha do conteúdo dele (nunca um rótulo tipo "OBJETIVO" sozinho no rodapé de uma página). A linha discreta "[título da seção] — continuação", que aparece acima do título de uma sub-etapa pra manter o contexto de qual seção ela pertence, é a ÚNICA coisa que pode ficar separada do título dela num corte de página raro — corrigido de propósito depois de um bug real: essa linha estava "colada" ao título e ao horário da sub-etapa como um bloco só, e quando os três juntos não cabiam no espaço restante da página, o card inteiro pulava pra próxima, desperdiçando uma área grande da página atual mesmo sobrando bastante espaço; a linha de contexto sozinha no rodapé de uma página é um efeito colateral bem mais barato que isso.' },
    { section: 'admin', personas: ['admin'],
      title: 'Roteiro — Markdown de campos de texto renderizado no PDF exportado ("Agenda + Objetivos", "Roteiro completo" e "Passo a passo")',
      body: 'Campo de texto (Objetivo, Passo a passo, Dicas para o facilitador, Conexão com a agilidade, Materiais necessários, Preparação prévia, Observações, Prompt para IA) que guarda sintaxe de Markdown como texto puro — "1. item"/"1\\. item", "- item"/"\\* item", "**negrito**"/"\\*\\*negrito\\*\\*" — sai no PDF já formatado: lista numerada de verdade (mantendo o número inicial de cada item, mesmo que comece em 9 ou outro valor), bullet, negrito sem asterisco e sem barra invertida nenhuma à mostra. Uma sequência de linhas com marcador diferente logo após um item numerado (ex: perguntas com "-"/"*" logo depois do item "9.") vira sublista DENTRO daquele item, não novos itens 10/11/12 da lista principal — funciona tanto quando essas linhas vêm recuadas quanto quando vêm coladas na mesma coluna (bem comum em texto colado sem indentação). Quebra de linha simples vira quebra de linha; uma linha em branco separa parágrafos. A análise roda sempre, mesmo em campo MISTO — parte já editada pelo editor de texto rico (com `<b>`/`<div>` de verdade) e parte ainda em Markdown puro por baixo, comum quando só um trecho do campo foi tocado pelo editor depois de uma migração em massa: a parte já formatada sai como está (negrito/lista de verdade preservados), e a parte em Markdown puro é interpretada do mesmo jeito que um campo 100% puro — nenhuma das duas "contamina" a outra. Só quando o campo já tem uma LISTA de verdade (`<ul>`/`<ol>`, produzida pelos botões de lista do editor) é que o agrupamento em lista não é refeito por cima dela — evita reprocessar uma lista que já está correta. Isto é só uma correção de EXIBIÇÃO do PDF: o valor gravado no banco nunca muda, e a tela normal do roteiro (editor e "ver detalhes") continua mostrando o campo exatamente como sempre mostrou, símbolos de Markdown incluídos — só as exportações "🖨 Agenda + Objetivos", "🖨 Roteiro completo" e "🖨 Passo a passo" interpretam essa sintaxe (a "🖨 Agenda resumida" não tem campos de texto livre, só a tabela).' },
    { section: 'admin', personas: ['admin'],
      title: 'Roteiro — rodapé próprio de página no PDF exportado',
      body: 'As quatro exportações (Agenda resumida, Agenda + Objetivos, Roteiro completo e Passo a passo) trazem, no rodapé de CADA página, "FORÇA ÁGIL · PREVI | Roteiro de Facilitação" à esquerda e "Página X de Y" à direita — via CSS de página impressa (@page), não via JavaScript contando página por página, então a numeração acompanha exatamente onde o navegador realmente corta cada folha. Esse rodapé é próprio do documento: aparece independente de a pessoa ter marcado ou desmarcado "Cabeçalhos e rodapés" nas opções de impressão do navegador (esse checkbox só controla o cabeçalho/rodapé NATIVO do navegador — data/hora, título da aba, URL e uma numeração tipo "1/2" — que o site nunca teve como desligar sozinho, só a numeração e a marca próprias é que o site controla). A caixa desse rodapé sempre usa fundo branco e texto num cinza-azulado escuro fixo, independente do tema escuro do documento ou do "Modo econômico" estarem ligados, pra continuar legível nos dois casos. Como o navegador não deixa nenhum site desligar seu próprio cabeçalho/rodapé por código — só a pessoa desmarcando "Cabeçalhos e rodapés" no diálogo de impressão —, a janela de impressão abre com um cartão de aviso destacado (cor de alerta, no topo, antes até do botão "Imprimir/salvar como PDF") repetindo isso de forma bem visível, em vez de um texto discreto ao lado do botão; esse cartão nunca aparece no PDF final, some junto com o resto de ".rp-actions" em @media print.' },
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Manual',
    ...
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Pedidos',
      body: 'Mostra os pedidos que as pessoas enviaram pelo formulário "Faça um pedido" (página Ajuda), do mais recente para o mais antigo. Cada pedido exibe o tipo, o texto escrito, o nome e o e-mail de quem enviou. A aba abre já filtrada em "Pendentes". Há dois filtros que funcionam juntos: em cima, por situação (Pendentes, Respondidos, Todos) e, separado deles, um botão de Lixeira que mostra só os excluídos. "Todos" mostra todos os pedidos ativos e nunca inclui os excluídos — quem quiser vê-los precisa entrar na Lixeira. Embaixo, o filtro por tipo (tema, curso, material, dúvida, iniciativas do Time Força Ágil, outros), cujos números acompanham o filtro de cima: se você está vendo só os Pendentes, cada tipo mostra quantos pendentes existem daquele tipo, não o total geral.' },
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Pedidos — reenquadrar (mudar o tipo de um pedido)',
      body: 'O botão "⇄ Mudar tipo" em cada pedido troca o tipo dele. Existe porque a pessoa escolhe o tipo na hora em que escreve, e o tipo certo pode nem existir ainda: quem pediu para participar das iniciativas do Time antes de essa opção existir só tinha "Outros" para marcar. Escolha o tipo novo e salve; o pedido passa a contar no filtro do tipo novo. Mudar o tipo é mexer no que a PESSOA escolheu, então isso nunca fica invisível: o pedido passa a mostrar uma linha "⇄ Reenquadrado de X por Fulana em tal dia", para quem olhar a lista depois saber que aquele selo foi posto pelo painel. Quem aparece como autor da mudança é quem está logada — diferente de "marcar como respondido" e de "excluir", que perguntam o nome porque podem ser registrados em nome de outra pessoa, depois do fato; reenquadrar é uma correção feita na hora, por quem está olhando a tela. O tipo guardado como "anterior" é sempre o ORIGINAL, o que a pessoa escolheu: se o pedido for reenquadrado duas vezes, o que interessa continua sendo de onde ele veio. Salvar o mesmo tipo não grava nada, para não carimbar um reenquadramento que não aconteceu.' },
    { section: 'admin', personas: ['admin'],
  ...
  function render() {
    ...
    if (filtered.length === 0) {
      ...
      SECTIONS.forEach(function (s) {
        ...
        if (s.key === 'admin') {
          /* A categoria Admin junta itens das 15 abas do painel — agrupa por aba para não virar uma lista única confusa */
          const subgroups = [];
          const byTab = {};
