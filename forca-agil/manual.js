...
(function () {
  ...
  const RULES = [
    ...
    { section: 'turmas', personas: ['logado', 'inscrito', 'admin'],
      ...
      body: 'A página Turmas agrupa os cards por evento, na mesma ordem do painel admin — um título com o nome do evento acima da grade das turmas dele. Cada evento que já tem pelo menos uma turma na vitrine ganha, como último card DAQUELE grupo, um card "Lista de Espera" — a fila é do evento, não um card único global como antes. Logada pode clicar "Entrar na lista de espera" no card de um evento específico; dá para estar na fila de mais de um evento ao mesmo tempo, um clique por card. Dado salvo em fa-espera/ no Firebase, com o eventoKey daquele card. Quem está na lista mantém acesso de Logada (sem turma), não de Inscrita. Duas coisas precisam ser verdade para o card aparecer. Primeira: o evento já tem turma na vitrine — evento recém-criado ainda não tem, as turmas entram depois e o card aparece junto com elas. Segunda: o admin aceita fila nesse evento (caixa "Aceitar lista de espera neste evento", em "+ Novo evento" / "✎ Editar evento"), porque nem todo evento quer lista de espera. Desmarcada, o card some da página Turmas e ninguém novo entra na fila — quem já estava continua na fila, visível no painel, sem ser apagado.' },
    { section: 'turmas', personas: ['logado', 'inscrito', 'admin'],
