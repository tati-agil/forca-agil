...
(function () {
  ...
  const RULES = [
    ...
    { section: 'inicio', personas: ['visitante', 'logado', 'inscrito', 'admin'],
      ...
      body: 'Texto introdutório em estilo Star Wars sobe lentamente ao entrar na Home. Funciona igual para todos os perfis.' },
    { section: 'inicio', personas: ['visitante', 'logado', 'inscrito', 'admin'],
    ...
    { section: 'turmas', personas: ['visitante', 'logado', 'inscrito', 'admin'],
      ...
      body: 'A página Turmas mostra o mesmo grid de cards para todo mundo (visitante, logado, inscrito ou admin) — não existe mais um card exclusivo pra quem já está inscrito, nem uma versão da página que esconde as demais turmas. Vê um card por turma cadastrada (o número de turmas não é fixo — o admin cria/edita/exclui turmas na aba Admin › Turmas), cada um com nome, mês e datas. O que muda de um card para outro é só o estado daquela turma — interesse aberto (botão "Tenho interesse", ver regras abaixo) ou interesse encerrado (orientação para o CMFlex, ver regra seguinte) — nunca o perfil de quem está olhando. Abaixo dos cards (igual para todos os perfis): bloco "Como funciona a oficina" + Agenda D1–D5.' },
    { section: 'turmas', personas: ['visitante', 'logado', 'inscrito', 'admin'],
      title: 'Turma com interesse encerrado — card do CMFlex',
      body: 'Quando o admin encerra o interesse de uma turma (botão "Encerrar interesse" no Admin), o card daquela turma troca imediatamente para todo mundo: no lugar do botão "Tenho interesse", mostra o texto "Faça sua inscrição no CMFlex — As inscrições desta turma agora devem ser feitas diretamente no sistema oficial." e um botão "Ir para o CMFlex →" que abre, em nova aba, o link cadastrado pelo admin para aquela turma específica. Se a turma ainda não tem link cadastrado, mostra "Link do CMFlex ainda não disponível — consulte a organização." no lugar do botão. Ninguém vira inscrita automaticamente nesse momento — quem de fato se inscreveu no CMFlex só passa a ter os privilégios de inscrita depois que o admin confirma essa pessoa manualmente (ver Aba: Turmas — confirmar inscrição no Admin).' },
    { section: 'turmas', personas: ['visitante', 'logado', 'inscrito', 'admin'],
    ...
    { section: 'turmas', personas: ['logado', 'admin'],
      ...
      body: 'Disponível para qualquer usuário logado, em qualquer turma com interesse aberto — inclusive quem já está inscrita (confirmada) em outra turma. Ao clicar em "♡ Tenho interesse" (estilo primário, dourado sólido), o botão vira "♡ Remover interesse" (estilo secundário, fundo escuro) e exibe a mensagem "Sua intenção foi registrada! Usaremos para dimensionar as turmas." embaixo do botão. É possível manifestar interesse em quantas turmas existirem ao mesmo tempo — não há limite fixo, o número de turmas depende do que o admin cadastrou; só a confirmação como inscrita é exclusiva de uma turma (ver regra "confirmar inscrição" no Admin).' },
    { section: 'turmas', personas: ['logado', 'admin'],
    ...
    { section: 'admin', personas: ['admin'],
      ...
      body: 'Cada turma aparece como um card, com um selo ABERTA ou INTERESSE ENCERRADO e a contagem "X interessados · Y confirmados" (sempre os dois números, independente do estado da turma). Dentro do card: botões de ação e a lista de participantes ativos, com nome, e-mail, área e uma coluna Status ("Interessado" ou "Inscrito" — inscrito é quem o admin já confirmou como matriculada no CMFlex, ver regra "confirmar inscrição"). A lista é a mesma tabela em qualquer estado da turma; só as colunas de presença por dia aparecem exclusivamente quando o interesse já foi encerrado.' },
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Turmas — criar turma',
      body: 'Botão "+ Nova turma" (topo da aba): abre um modal com campo de nome, uma lista de datas — cada data é um seletor de calendário, com botão "+ Adicionar data" para incluir mais encontros e "✕" para remover um — e um campo opcional "Link do CMFlex" (URL da inscrição oficial daquela turma específica no sistema externo). Ao salvar, a turma aparece imediatamente como um novo card, com um botão de interesse próprio na página pública de Turmas. Turmas não têm mais um número fixo (as antigas T1/T2/T3 eram cravadas no código) — o admin decide quantas existem.' },
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Turmas — editar turma (nome, datas e link do CMFlex)',
    ...
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Turmas — excluir turma',
    ...
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Turmas — encerrar interesse',
    ...
    { section: 'admin', personas: ['admin'],
      ...
      body: 'Desktop: todas as ações ficam em linha única — seletor de dia + botão "Abrir check-in" (ou "Fechar check-in") sempre visíveis; QR Code, + Participante, Reabrir, CSV e Certificados também na mesma linha. Celular/tablet: o cabeçalho do card vira coluna; só as ações principais ficam visíveis (seletor de dia + Abrir/Fechar check-in para turma com interesse encerrado, ou "Encerrar interesse" para turma aberta); o botão "⋯" abre um menu com as ações secundárias (QR Code, + Participante, ↺ Reabrir, CSV, Certificados).' },
    { section: 'admin', personas: ['admin'],
    ...
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Turmas — adicionar participante manualmente (escolhe o status)',
      body: 'Botão "＋ Participante" (disponível com a turma aberta ou com interesse encerrado): abre um modal com campos Nome, E-mail, Área (mesma lista de 20 áreas do cadastro) e Status — o admin escolhe entre "Interessada" ou "Inscrita" na hora de adicionar. Valida e-mail @previ.com.br e exige os 4 campos preenchidos antes de salvar; registra o nome do admin que adicionou (independente do status escolhido). Escolher "Inscrita" adiciona a pessoa já com esse status direto (sem precisar de "Confirmar" depois) — uso típico: alguém que já se inscreveu no CMFlex; se ela também tiver registro ativo em outra turma, o modal avisa e, ao continuar, remove automaticamente esse outro registro (mesma checagem de exclusividade do botão "Confirmar"). Escolher "Interessada" registra só a manifestação de interesse, sem essa checagem (interesse não tem limite de turmas) — uso típico: alguém que avisou o interesse fora do site (e-mail, WhatsApp) e ainda não é sabido se de fato se inscreveu no CMFlex; pode ser confirmada depois pelo botão "Confirmar", igual a quem se interessou pelo próprio site. Se o e-mail já estiver na turma, exibe aviso no modal. A tabela atualiza automaticamente.' },
    { section: 'admin', personas: ['admin'],
    ...
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Turmas — pessoa removida pode ser readicionada',
    ...
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Turmas — seção "Removidos"',
