...
(function () {
  ...
  const RULES = [
    ...
    { section: 'quiz', personas: ['logado', 'colaborador', 'admin'],
      ...
      body: 'Ao confirmar "Sim, revelar minha patente": soma autodiagnóstico + missões + Kyber + conteúdos + repositório (XP máximo 100) e fixa o resultado — definitivo, não pode ser alterado. Esse resultado é só da pessoa: NÃO publica nada no ranking. Imediatamente grava fa-patente-revealed=1 em fa-progress/ no Firebase (faSyncProgress) para que outro dispositivo não consiga refazer o jogo.' },
    { section: 'quiz', personas: ['logado', 'colaborador', 'admin'],
      title: 'Publicar no ranking — etapa separada e opcional',
      body: 'Só aparece depois de revelar. Botão "Publicar no ranking →" é a única ação que envia nome, área, turma e XP para o ranking público (players/ no Firebase). Também grava fa-patente-publicada=1 em fa-progress/ no Firebase (faSyncProgress) para preservar o estado publicado em outros dispositivos. Quem revela mas não publica continua com a patente visível só para si, fora do ranking. Não tem como "despublicar" depois.' },
