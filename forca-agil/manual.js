...
(function () {
  ...
  const RULES = [
    ...
    { section: 'admin', personas: ['admin'],
      ...
      body: 'A coluna Data mostra quando a pessoa entrou na fila, com data E hora no formato brasileiro (por exemplo, 10/08/2026 12:26). A hora sempre esteve guardada; antes a tela mostrava só a data, e ainda no formato do banco. Para quem veio migrada de uma turma, a coluna Origem também informa a hora da migração, além da turma e do motivo. Detalhe técnico que vale saber ao mexer em datas neste site: uma data guardada sem hora seria interpretada como meia-noite no fuso de Greenwich e apareceria como 21h do DIA ANTERIOR aqui — por isso esse caso é tratado à parte e exibido sem hora, em vez de exibir uma hora inventada e um dia errado.' },
    ...
    { section: 'turmas', personas: ['admin'],
      ...
      body: 'Na seção Lista de Espera do painel admin, o botão "Remover da lista" tira a pessoa sem movê-la para turma nenhuma. Ao clicar, é obrigatório escolher o motivo: Desistiu / não tem mais interesse, Já participou de uma turma, Não respondeu aos contatos, Evento encerrado, Registro duplicado ou Outro. Escolhendo "Outro", abre um campo de texto que também precisa ser preenchido — assim nunca fica registro sem justificativa. O motivo é gravado junto com a data e o nome de quem removeu.' },
    { section: 'admin', personas: ['admin'],
      title: 'Sair da turma — um caminho só, sempre com motivo',
      body: 'Existe um único botão para tirar alguém da turma: "Remover". Ele abre um modal que pede o MOTIVO e pergunta se a pessoa vai para a lista de espera. Antes eram dois botões para a mesma decisão — "→ Espera" e "Remover" — com listas de motivo que se sobrepunham; na prática, ir para a espera sempre foi remover por um motivo. Os motivos são: a pedido da própria pessoa; vai fazer em outra turma (aqui você escolhe QUAL, senão o registro não diz para onde ela foi); turma sem vagas; a data não serviu; já participou; não respondeu aos contatos; registro duplicado; evento encerrado; outro (com descrição obrigatória). O histórico de presença nunca é apagado.' },
    { section: 'admin', personas: ['admin'],
      title: 'Colocar na lista de espera é decisão sua',
      body: 'Dentro do modal de remoção existe a caixa "Colocar na lista de espera". Nada é automático: quem decide é você, uma pessoa por vez. A caixa já vem marcada nos motivos em que a pessoa normalmente ainda quer fazer — turma sem vagas e a data não serviu —, e você pode desmarcar. Quem pediu para sair, ou quem já vai fazer em outra turma, normalmente não vai para a fila. IMPORTANTE: a lista de espera recebe sempre a DATA E HORA DO INTERESSE ORIGINAL, nunca a data da remoção — é isso que preserva a ordem de chegada na fila.' },
    { section: 'admin', personas: ['admin'],
      title: 'Substituída — por quem',
      body: 'A justificativa "Substituída", na linha da pessoa, pede que você diga QUEM entrou no lugar dela: aparece a lista das pessoas da própria turma, e há a opção de digitar um nome para quem ainda não está cadastrada ali. Sem isso, o histórico não responde "quem entrou no lugar de quem", que é a pergunta que aparece depois. O selo passa a mostrar o nome de quem entrou, e acompanha a pessoa mesmo depois de ela sair da turma — inclusive na lista de quem saiu. Substituição NÃO é um destino: a pessoa continua como não confirmada na turma, com o selo, e só depois se decide se vai para a espera, se sai, ou se fica. Tirar a justificativa apaga também o nome de quem substituiu, para não sobrar registro solto.' },
    { section: 'admin', personas: ['admin'],
      title: 'Filtros da turma — cada pessoa em um grupo só',
      body: 'Os filtros acima da lista separam TODAS as pessoas que manifestaram interesse pela turma, e cada uma cai em exatamente um grupo: TODOS OS INTERESSADOS (o total, incluindo quem já saiu — é o número que responde quantas pessoas quiseram participar); CONFIRMADOS; AGUARDANDO DECISÃO (não confirmada e ainda SEM destino: é o grupo que exige ação sua); NÃO CONFIRMADOS (já tem um motivo registrado); FORAM PARA A ESPERA; FORAM PARA OUTRA TURMA (encaminhadas, com o nome da turma de destino); e REMOVIDOS (saíram de vez). Na lista de quem saiu existe uma coluna DESTINO, com um selo dizendo para onde a pessoa foi — antes essa informação ficava espremida dentro do texto do motivo, e não dava para responder de bate-pronto quem foi para outra turma e qual. A soma dos grupos sempre fecha com o total. Antes, quem tinha saído ficava fora da conta, escondido num acordeão, e "não confirmado" misturava quem ainda não foi analisada com quem já tinha destino decidido. A barra de filtros só aparece quando há mais de um grupo com gente.' },
    { section: 'turmas', personas: ['admin'],
      title: 'Lista de espera — migrar da turma',
      body: 'Na tabela de participantes de cada turma, cada linha (interessada ou inscrita) tem o botão "→ Espera". Ao clicar, o admin confirma e a pessoa é removida da turma e adicionada à lista de espera. Ao clicar, é obrigatório escolher o motivo da migração: Turma sem vagas, Turma já encerrada, Remanejada para próxima turma, A pedido da pessoa, Evento encerrado ou Outro (que abre campo de texto, também obrigatório). A data preservada na lista de espera é a data original em que ela manifestou interesse na turma — não o momento da migração. Na lista de espera, a coluna "Origem" mostra de qual turma a pessoa veio e em que dia foi migrada (ex: "↩ Turma 1 — Agosto · em 18/08/2026") e, abaixo, o motivo escolhido na migração; quem entrou direto pelo card do site aparece como "Entrou pela lista". Útil quando a turma está cheia e o admin precisa realocar pessoas para a próxima.' },
