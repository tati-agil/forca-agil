...
(function () {
  ...
  const RULES = [
    ...
    { section: 'admin', personas: ['admin'],
      ...
      body: 'Acima e abaixo da lista de atividades de cada dia (roteiro-base e roteiro da turma), uma barra mostra 6 números calculados a partir dos dados reais — nunca digitados à mão: Janela do dia (do início da primeira à conclusão da última atividade com horário); Tempo programado (soma das durações de tudo, inclusive pausas); Tempo de facilitação (o programado descontando pausas/intervalos); Pausas (soma só das atividades do tipo "Intervalo"); Lacunas (soma dos buracos sem nada programado entre atividades vizinhas); Atividades (quantidade de atividades principais — sub-etapas de uma seção não contam à parte). Por construção sempre vale: Janela = Programado + Lacunas, e Programado = Facilitação + Pausas.' },
    { section: 'admin', personas: ['admin'],
      title: 'Roteiro — diferença entre Intervalo (pausa) e Lacuna (buraco)',
      body: 'Intervalo é uma atividade cadastrada de propósito (tipo "Intervalo") — conta como tempo programado e como pausa, mas não como facilitação líquida. Lacuna é tempo entre duas atividades vizinhas SEM nada cadastrado no meio — aparece como uma faixa tracejada laranja "⚠ Lacuna no planejamento" entre as duas, não é uma atividade, não conta como programado nem como pausa, só entra na janela do dia. As duas coisas não podem ser confundidas: uma é planejada, a outra é ausência de planejamento.' },
    { section: 'admin', personas: ['admin'],
      title: 'Roteiro — seções com sub-etapas',
      body: 'Uma sub-etapa é uma atividade completa igual a qualquer outra — mesmo formulário, todos os campos (descrição, objetivo, passo a passo, materiais etc.) — só que "filha" de uma atividade-mãe. O botão "+ Sub-etapa" (dentro de uma atividade de nível principal) abre esse formulário completo já vinculado a ela; não existe uma versão reduzida só de título e duração. Uma atividade vira "seção" automaticamente assim que passa a ter sub-etapas: sua duração exibida na lista passa a ser a SOMA das sub-etapas (o campo Duração dela própria fica guardado mas ignorado enquanto houver alguma filha) e aparece um botão "▲ Recolher"/"▼ Ver etapas". Na lista, a seção aparece numerada (1, 2, 3...) e as sub-etapas recuadas logo abaixo, numeradas 4.1, 4.2... Sub-etapas não têm posição própria no dia (não entram na ordenação cronológica nem no aviso de ordem divergente — só a atividade-mãe conta pra isso) e, ao editá-las, o sistema não oferece recalcular horários seguintes (quem tem horário e posição é sempre o pai). Excluir a atividade-mãe apaga junto todas as suas sub-etapas; duplicar a atividade-mãe duplica as sub-etapas junto.' },
    { section: 'admin', personas: ['admin'],
