...
(function () {
  ...
  const RULES = [
    ...
    { section: 'admin', personas: ['admin'],
      ...
      body: '14 abas no painel: Dashboard, Eventos, Certificados, Treinamentos, Repositório, Cadastrados, Administradores, Diretores, Facilitadores, Manual, Mapa, Testes, Pedidos e Sorteios. No desktop ficam em linha única. No mobile podem quebrar em 2 linhas com fonte reduzida para todas ficarem visíveis sem scroll horizontal.' },
    { section: 'admin', personas: ['admin'],
    ...
    { section: 'admin', personas: ['admin'],
      title: 'Roteiro-base — botão "📥 Importar resultados esperados" (ação pontual, só na DIRETORES)',
      body: 'Botão que aparece ao lado de "🗑 Excluir dia" no roteiro-base, mas só nos roteiros que já têm a atividade "EPISÓDIO FINAL — O COMPROMISSO DA LIDERANÇA" cadastrada — ou seja, hoje só aparece no roteiro-base "FORÇA ÁGIL - DIRETORES", não polui o editor de nenhum outro evento. Foi criado para preencher de uma vez o campo "Resultado esperado" (ver regra própria) das 31 atividades desse roteiro (Manhã + Tarde, incluindo as 9 sub-etapas) com um texto já revisado fora do sistema, em vez de digitar cada uma manualmente. Pede confirmação antes de rodar; casa cada atividade pelo TÍTULO exato — nunca sobrescreve nenhum outro campo (horário, duração, tipo etc.), nunca adivinha quando um título não bate com nenhuma atividade do roteiro ou bate com mais de uma (title duplicado): nesses dois casos a atividade em questão simplesmente não é tocada, e o relatório final lista exatamente quais títulos ficaram de fora, para resolver manualmente. Pode ser clicado mais de uma vez sem problema — roda de novo em cima do que já foi preenchido (sobrescrevendo com o mesmo texto).' },
    { section: 'admin', personas: ['admin'],
      title: 'Roteiro-base — botões "🔎 Migração Antes x Depois" e "↩ Desfazer última migração" (ação pontual, só na DIRETORES)',
      body: 'Dois botões que aparecem junto de "📥 Importar resultados esperados" (mesma regra: só no roteiro que já tem a atividade "EPISÓDIO FINAL — O COMPROMISSO DA LIDERANÇA"). "🔎 Migração Antes x Depois" abre uma janela com um DRY RUN — só leitura, nada é gravado ainda — comparando o valor ATUAL de 22 itens (Passo a passo, Dicas, Observações, Conexão com a mentalidade ágil, Prompt para IA e dois Tipos) com o texto revisado de uma planilha externa. Para toda ação de "substituir" (inclusive a de trocar só um trecho dentro de um campo maior), o "DE" da planilha é tratado sempre como um TRECHO a localizar dentro do campo — nunca como o campo inteiro: comparar o campo inteiro geraria falsa divergência sempre que o campo tivesse mais conteúdo em volta daquele trecho (o caso comum aqui, já que a planilha às vezes lista só um parágrafo de um "Passo a passo" com vários). A busca do trecho tolera diferença INVISÍVEL de formatação — aspas tipográficas “ ” vs retas " ", &nbsp; vs espaço normal, \r\n vs \n, espaços duplicados, linha em branco a mais/a menos, uma tag HTML no meio de duas palavras — sem tolerar diferença de palavra, número ou pontuação de verdade; a comparação nunca muda o texto salvo, só ajuda a achar o trecho onde ele já está. A troca, quando acontece, substitui só aquele trecho dentro do valor ORIGINAL (nunca uma versão "achatada"), preservando literalmente tudo o mais que já estava no campo — formatação, espaços, o resto do texto. Cada linha da tabela mostra um status: "OK PARA ALTERAR" (o trecho "DE" aparece exatamente 1 vez no valor atual — clicar em "Aplicar" na mesma janela grava só essas linhas), "JÁ ATUALIZADO" (o trecho "DE" não aparece mais, mas o "PARA" já está lá — nada a fazer), "VALOR ATUAL DIVERGENTE" (nem o "DE" nem o "PARA" aparecem no campo — precisa de revisão manual), "TRECHO AMBÍGUO" (o "DE" aparece mais de uma vez — trocar automaticamente poderia acertar o trecho errado ou trocar todas as ocorrências sem querer), "ATIVIDADE NÃO ENCONTRADA" (nenhuma atividade com esse título neste roteiro) ou "REGISTRO AMBÍGUO" (mais de uma atividade com esse título — não dá pra saber qual). Nunca decide sozinha em cima de dúvida — só os casos "VALOR ATUAL DIVERGENTE" e "TRECHO AMBÍGUO" (os dois com um valor atual de verdade pra mostrar) ganham um botão "REVISAR": clicar abre, na própria tela (sem precisar copiar nada do banco na mão), o valor ATUAL no sistema e o "DE"/"PARA" do Excel. Para "TRECHO AMBÍGUO" (e para divergência de Tipo, que não tem o que mesclar), as opções são "Manter atual", "Substituir pelo PARA" (no caso de trecho repetido, troca TODAS as ocorrências de uma vez) e "Mesclar manualmente" (abre uma caixa de texto pré-preenchida com o "PARA", editável à vontade — só grava depois de "Confirmar mesclagem"). Para "VALOR ATUAL DIVERGENTE" num campo de texto, a tela já mostra uma quarta seção, "Proposta de mesclagem (gerada automaticamente)": um merge de 3 vias (base = "DE" da planilha, comparado linha a linha com o valor ATUAL de um lado e o "PARA" do outro) que preserva o que a pessoa já tinha acrescentado no campo, incorpora a mudança pretendida pela planilha e nunca duplica nem inventa conteúdo — sem precisar montar a mescla do zero. As opções viram "Aprovar mesclagem" (grava a proposta gerada direto), "Editar mescla" (abre a mesma proposta numa caixa de texto editável, pra ajustar antes de confirmar) e "Manter atual". Quando o algoritmo detecta que o valor atual e a planilha acrescentaram coisas DIFERENTES no mesmo ponto do texto (um conflito de verdade, não uma diferença de formatação), a proposta marca isso visivelmente com "⚠️ CONFLITO" e desativa "Aprovar mesclagem" até a pessoa resolver isso manualmente em "Editar mescla" — o algoritmo nunca escolhe um lado sozinho nesse caso. Cada decisão é lembrada só durante aquela sessão da janela (nada é gravado ainda) e o painel avança sozinho pra próxima pendência ainda sem decisão, mostrando "Revisadas X de Y divergências". Um resumo ao vivo mostra a contagem de cada situação (OK PARA ALTERAR, JÁ ATUALIZADO, MANTER ATUAL, USAR PARA, MESCLA APROVADA, PENDÊNCIAS — essas últimas juntando "não encontrada"/"ambígua" de título com qualquer divergência/trecho ambíguo ainda sem decisão). O botão "Aplicar alterações confirmadas" só aparece depois que toda pendência revisável tiver uma decisão (pendências de não-encontrada/ambígua de título não travam isso — elas simplesmente nunca entram na aplicação). Ao aplicar, o valor de cada campo tocado é salvo antes de sobrescrever, num histórico interno do roteiro (nunca aparece na tela normal); "↩ Desfazer última migração" lê esse histórico e restaura os valores de antes da última aplicação, um botão só, sem precisar lembrar manualmente o que mudou. Uma ação de "acrescentar" (Dicas/Observações que ganham um bloco novo) nunca substitui o campo — sempre soma ao que já existe, checando antes se aquele bloco já não foi acrescentado (pra não duplicar clicando duas vezes).' },
    { section: 'admin', personas: ['admin'],
      title: 'Roteiro-base — botão "🧹 Limpeza final de consistência" (3ª leva de ajustes, só na DIRETORES)',
      body: 'Terceiro botão de migração pontual, ao lado de "🔎 Migração Antes x Depois" (mesma regra de aparecer só no roteiro que já tem a atividade "EPISÓDIO FINAL — O COMPROMISSO DA LIDERANÇA"). Abre o MESMO tipo de janela — DRY RUN primeiro, "Aplicar" só depois de revisar tudo, backup automático de cada campo tocado, "↩ Desfazer última migração" restaura também essa leva — só muda a lista de itens, que já foi ajustada mais de uma vez conforme o dry run real revelava o texto exato ainda cadastrado: elimina a duplicidade do "Primeiro uso da IA" (Passo a passo e Preparação prévia trazem prompt/missões antigas enquanto o "Prompt para IA" já tem a versão nova e oficial; Observações tinha um bloco de prompt antigo inteiro, que é REMOVIDO — pode ficar vazio), termina de corrigir "Saíram das iniciativas para o problema × Outcome × Output" no Passo a passo de "Agora dê nome ao que eles fizeram", troca a dica de bastidor de "Apresentação do desafio", aumenta a duração de "Decisão sem IA" de 5 para 8 minutos, substitui o Passo a passo de "Faça a comparação" inteiro pela versão com 3 perguntas principais e revê quatro formulações nas Observações de "EPISÓDIO FINAL — O COMPROMISSO DA LIDERANÇA" (Feedback, Empirismo, Experimentação e Outcome × Output), preservando os demais itens da lista (Priorização, Foco, Incerteza, Adaptação, Fail small, IA, Pensamento crítico, Liderança ágil), e troca o rótulo "Exemplos:" por "Os 24 cartões são:" no Passo a passo de "DESAFIO 1: ESCOLHAM", preservando a lista dos 24 cartões que vem logo depois. Dois mecanismos, os dois sem nunca decidir sozinha na dúvida: (1) itens de "substituir campo inteiro" (Passo a passo/Preparação prévia/Observações do Primeiro uso da IA, Dicas de "Apresentação do desafio", Passo a passo de "Faça a comparação") não têm um "DE" pra localizar — nunca viram "OK PARA ALTERAR" sozinhos, sempre caem em revisão manual (com os botões de sempre: Manter atual/Substituir pelo PARA/Mesclar manualmente, ou "(campo deve ficar vazio)" no lugar do "PARA" quando o pedido é remover o conteúdo por completo) mesmo quando o valor atual só precisa ser trocado, só "JÁ ATUALIZADO" quando o campo já bate com o texto novo (inclusive já vazio, no caso da remoção); (2) o item de duração ("Decisão sem IA": 5→8 min) NUNCA aplica sozinho, mesmo quando a duração atual bate exatamente com o "de" esperado — clicar "REVISAR" abre um painel com dois cenários lado a lado, cada um com uma tabela "antes/depois" de toda atividade afetada e o novo horário final + nova duração da sessão do dia e do dia inteiro (a mesma conta que a tela normal do roteiro usa pra "Programado/Facilitação", não um cálculo à parte): CENÁRIO A — aceitar terminar mais tarde (desloca o início de todas as atividades seguintes do mesmo dia pela mesma diferença, nunca mexendo na duração delas) — um clique aplica; CENÁRIO B — retirar a diferença de outra atividade escolhida num menu (só oferece atividades com duração suficiente pra não zerar) — essa atividade escolhida entra deslocada mas encolhe na mesma medida, saindo no MESMO horário de fim de antes, e tudo o que vem depois dela no dia fica exatamente como estava (a pessoa vê a prévia do impacto ao trocar a escolha no menu, antes de confirmar); se a duração atual não bater nem com o "de" nem com o "para" esperado, os dois cenários continuam disponíveis, só com um aviso a mais. Quando mais de um item da lista mexe no MESMO campo da MESMA atividade (como as quatro trocas nas Observações do "EPISÓDIO FINAL"), "Aplicar" faz as trocas em CADEIA sobre o valor real do campo — nunca escritas paralelas independentes, que fariam a última vencer e apagar as outras (bug corrigido antes de qualquer uso real: cada uma delas mostrava "OK PARA ALTERAR" e "Aplicado: X de X", mas só a última troca de fato sobrevivia no campo). O backup guarda o valor de ANTES de qualquer troca do lote — "↩ Desfazer última migração" restaura o campo inteiro para esse valor original, não um passo intermediário.' },
    { section: 'admin', personas: ['admin'],
