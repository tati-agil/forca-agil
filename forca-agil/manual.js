...
(function () {
  ...
  const RULES = [
    ...
    { section: 'admin', personas: ['admin'],
      ...
      body: 'Terceiro botão de migração pontual, ao lado de "🔎 Migração Antes x Depois" (mesma regra de aparecer só no roteiro que já tem a atividade "EPISÓDIO FINAL — O COMPROMISSO DA LIDERANÇA"). Abre o MESMO tipo de janela — DRY RUN primeiro, "Aplicar" só depois de revisar tudo, backup automático de cada campo tocado, "↩ Desfazer última migração" restaura também essa leva — só muda a lista de itens: elimina a duplicidade do "Primeiro uso da IA" (Passo a passo e Preparação prévia trazem prompt/missões antigas enquanto o "Prompt para IA" já tem a versão nova e oficial), termina de corrigir "Saíram das iniciativas para o problema × Outcome × Output" (dessa vez no Passo a passo de "Agora dê nome ao que eles fizeram" — a Conexão com a mentalidade ágil já tinha sido corrigida numa leva anterior), aumenta a duração de "Decisão sem IA" de 5 para 8 minutos com recálculo de horário, simplifica "Faça a comparação" de 5 para 3 perguntas principais e revê os três conceitos do fechamento final (Experimentação, Empirismo, Feedback). Três mecanismos, todos com a mesma regra de nunca decidir sozinha na dúvida: (1) itens de "substituir campo inteiro" (Passo a passo/Preparação prévia do Primeiro uso da IA, Dicas de "Apresentação do desafio") não têm um "DE" pra localizar — nunca viram "OK PARA ALTERAR" sozinhos, sempre caem em revisão manual (com os botões de sempre: Manter atual/Substituir pelo PARA/Mesclar manualmente) mesmo quando o valor atual só precisa ser trocado, só "JÁ ATUALIZADO" quando o campo já bate com o texto novo; (2) o item de duração ("Decisão sem IA": 5→8 min) NUNCA aplica sozinho, mesmo quando a duração atual bate exatamente com o "de" esperado — clicar "REVISAR" abre um painel com dois cenários lado a lado, cada um com uma tabela "antes/depois" de toda atividade afetada e o novo horário final + nova duração da sessão do dia e do dia inteiro (a mesma conta que a tela normal do roteiro usa pra "Programado/Facilitação", não um cálculo à parte): CENÁRIO A — aceitar terminar mais tarde (desloca o início de todas as atividades seguintes do mesmo dia pela mesma diferença, nunca mexendo na duração delas) — um clique aplica; CENÁRIO B — retirar a diferença de outra atividade escolhida num menu (só oferece atividades com duração suficiente pra não zerar) — essa atividade escolhida entra deslocada mas encolhe na mesma medida, saindo no MESMO horário de fim de antes, e tudo o que vem depois dela no dia fica exatamente como estava (a pessoa vê a prévia do impacto ao trocar a escolha no menu, antes de confirmar); se a duração atual não bater nem com o "de" nem com o "para" esperado, os dois cenários continuam disponíveis, só com um aviso a mais; (3) os três itens do fechamento final (Experimentação/Empirismo/Feedback) não têm título de atividade fixo informado — o sistema procura o texto antigo em QUALQUER atividade do roteiro (dentro de Passo a passo, Dicas, Observações, Conexão com a mentalidade ágil e Preparação prévia; nunca no Prompt para IA), e só decide sozinha quando acha exatamente 1 ocorrência num único lugar ("OK PARA ALTERAR", mostrando em qual atividade achou); se achar em mais de um lugar ("TRECHO ENCONTRADO EM MAIS DE UM LUGAR") ou não achar em lugar nenhum ("TRECHO NÃO LOCALIZADO", ou "JÁ ATUALIZADO" se o texto novo já aparece em algum lugar), o item ganha um "REVISAR" próprio mostrando de qual pedido original ele veio, o texto DE/PARA esperado e o motivo técnico de não ter resolvido sozinho; no caso de não ter achado em lugar nenhum, aparece ainda uma busca aproximada por PALAVRAS-CHAVE (nunca decide nada sozinha, só sugere onde o texto pode ter sido reescrito demais pra bater exatamente) — só um botão "Marquei como visto" fecha a pendência (nada é gravado, é só uma confirmação de que alguém olhou), e "Aplicar" só libera depois que TODOS os itens desse tipo tiverem essa confirmação.' },
    { section: 'admin', personas: ['admin'],
