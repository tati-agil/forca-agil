...
(function () {
  ...
  const RULES = [
    ...
    { section: 'admin', personas: ['admin'],
      ...
      body: 'Disponível para qualquer pessoa cadastrada, não só colaboradores. Apaga fa-progress/<emailKey> e remove do ranking (players — busca por email, pois a chave de players é name__turma, não emailKey). Ação irreversível — pede confirmação antes de executar. Após o reset: (1) a tabela admin recarrega e XP exibe "—"; (2) se a pessoa estiver logada no momento, o listener em tempo real (fa-progress .on value) detecta a deleção, limpa o localStorage e recarrega a página automaticamente — todos os jogos (autodiagnóstico, missões, Kyber, revelar patente) ficam disponíveis para refazer sem nenhuma ação da pessoa.' },
    { section: 'admin', personas: ['admin'],
