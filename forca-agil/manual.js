...
(function () {
  ...
  const RULES = [
    ...
    { section: 'admin', personas: ['admin'],
      ...
      body: '9 abas no painel: Eventos, Certificados, Repositório, Cadastrados, Administradores, Manual, Mapa, Testes e Pedidos. No desktop ficam em linha única. No mobile podem quebrar em 2 linhas com fonte reduzida para todas ficarem visíveis sem scroll horizontal.' },
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Eventos — visão geral',
      body: 'As turmas são organizadas por evento: cada evento é um container com título próprio e seus cards de turma dentro. Os containers de evento começam recolhidos (apenas o cabeçalho visível). As turmas dentro de cada evento também começam recolhidas (apenas o cabeçalho do card visível). Clicar no cabeçalho do evento expande/recolhe suas turmas; clicar no cabeçalho do card de turma expande/recolhe os participantes e ações. Turmas sem evento associado aparecem numa seção "TURMAS SEM EVENTO" ao final. Cada card de turma exibe: seta de toggle, selo ABERTA ou INTERESSE ENCERRADO, contagem "X interessados · Y confirmados" (sempre os dois números), botões de ação e a lista de participantes ativos. A lista é a mesma em qualquer estado; colunas de presença por dia aparecem exclusivamente quando o interesse já foi encerrado.' },
    { section: 'admin', personas: ['admin'],
    ...
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Pedidos',
      body: 'Lista todos os pedidos enviados pelo formulário público "Faça um pedido" (página Ajuda), mais recente primeiro. Chips de filtro por tipo — tema, curso, material, dúvida, outros — cada um com a contagem de pedidos daquele tipo; "Todos" mostra tudo. Cada item exibe tipo, descrição, nome e e-mail de quem enviou.' },
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Pedidos — responder um pedido',
      body: 'Cada item tem dois botões. "✉ Responder por e-mail" abre o programa de e-mail padrão do admin (Gmail, Outlook etc.) com o destinatário, assunto e uma citação do pedido já preenchidos — a resposta em si é escrita e enviada pelo e-mail pessoal do admin, o site não manda nada sozinho (não há envio automático de e-mail configurado — exigiria plano pago do Firebase). "✓ Marcar como respondido" é manual e reversível (clique de novo desmarca): grava respondido:true e a data em respondidoEm no Firebase; o item ganha um badge "✓ Respondido" e fica com opacidade reduzida na lista, só para controle visual de quem já foi tratado — nada é enviado automaticamente ao marcar.' },
    { section: 'menu', personas: ['visitante', 'logado', 'inscrito', 'admin'],
