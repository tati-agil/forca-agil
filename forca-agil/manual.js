...
(function () {
  ...
  const RULES = [
    ...
    { section: 'admin', personas: ['admin'],
      ...
      body: 'Aparece ao lado de "🗑 Excluir dia" no roteiro-base, mas só nos roteiros que já têm a atividade "EPISÓDIO FINAL — O COMPROMISSO DA LIDERANÇA" cadastrada — ou seja, hoje só aparece no roteiro-base "FORÇA ÁGIL - DIRETORES". Os botões que DISPARAVAM migrações de conteúdo ("📥 Importar resultados esperados", "🔎 Migração Antes x Depois" e "🧹 Limpeza final de consistência") eram ações pontuais, já aplicadas nesse roteiro, e foram removidos da tela — não fazia mais sentido mantê-los como botão permanente. "↩ Desfazer última migração" continua existindo: lê o histórico de backups que essas ações já gravaram (o valor de cada campo tocado, salvo automaticamente antes de qualquer sobrescrita) e restaura os valores de antes da última aplicação, com confirmação antes de agir. Se não houver nenhuma migração aplicada nesse roteiro para desfazer, avisa em vez de fazer qualquer coisa.' },
    { section: 'admin', personas: ['admin'],
