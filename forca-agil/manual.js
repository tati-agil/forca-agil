...
(function () {
  ...
  const RULES = [
    ...
    { section: 'admin', personas: ['admin'],
      ...
      body: 'As turmas são organizadas por evento: cada evento é um container com título próprio e seus cards de turma dentro. Turmas sem evento associado aparecem numa seção "TURMAS SEM EVENTO" ao final. Cada card de turma exibe: selo ABERTA ou INTERESSE ENCERRADO, contagem "X interessados · Y confirmados" (sempre os dois números), botões de ação e a lista de participantes ativos com nome, e-mail, área e status ("Interessado" ou "Inscrito"). A lista é a mesma em qualquer estado; colunas de presença por dia aparecem exclusivamente quando o interesse já foi encerrado.' },
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Turmas — Eventos (entidade pai das turmas)',
      body: 'Botão "+ Novo evento" (topo da aba): cria um evento com nome e carga horária (em horas). Eventos são os containers pai das turmas — um evento pode ter múltiplas turmas. Os dados do evento (nomeEvento, cargaHoraria) são herdados automaticamente pelo certificado de cada turma vinculada. Editar ou excluir evento: ações disponíveis via botões no cabeçalho de cada container de evento. Excluir evento não exclui as turmas dentro dele — elas passam para a seção "TURMAS SEM EVENTO".' },
    { section: 'admin', personas: ['admin'],
    ...
    { section: 'admin', personas: ['admin'],
      ...
      body: 'Painel de geração de certificados. Selecione a turma no seletor — nome do evento e carga horária são carregados automaticamente do Firebase (entidade eventos/). Os 6 campos dinâmicos são preenchidos sem entrada manual: nome do participante, nome do evento, identificação da turma, período, carga horária e data de emissão ("Emitido em"). Canvas fixo 1448 × 1086 px; prévia escala 4:3. Textos longos reduzem fonte por campo de forma independente. Nomes de arquivo: Certificado_NOME.png / Certificado_NOME.pdf.' },
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Certificados — estados: Prévia Administrativa vs. Emissão',
      body: 'TURMA NÃO ENCERRADA (estado Prévia): banner laranja "PRÉVIA DO CERTIFICADO — TURMA AINDA NÃO CONCLUÍDA" aparece acima do canvas; botões ⬇ PNG e ⬇ PDF por participante ficam desabilitados com tooltip "Disponível após a conclusão da turma."; botões de lote (Baixar todos PNG/PDF) também desabilitados; dataEmissao não é gerada. Qualquer participante pode ser selecionado para visualizar a prévia do layout. TURMA ENCERRADA (estado Emissão): banner oculto; todos os downloads habilitados; dataConclusao (definida automaticamente ao encerrar, ver regra "encerrar turma") é usada como dataEmissao no certificado. Não há como baixar certificado definitivo antes da turma ser encerrada.' },
    { section: 'admin', personas: ['admin'],
