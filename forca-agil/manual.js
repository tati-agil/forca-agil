...
(function () {
  ...
  const RULES = [
    ...
    { section: 'admin', personas: ['admin'],
      ...
      body: 'Uma inscrição só vale quando tem DUAS coisas gravadas: o status "inscrito" E quem confirmou (confirmedByAdmin). O painel checava só o status; o resto do sistema — acesso a Conteúdos/Treinamento e a Minha Área da própria pessoa — sempre exigiu os dois. Quando um registro fica com o status mas sem quem confirmou, essa diferença vira um problema invisível: o painel diz "Inscrito" e a pessoa, do lado dela, não tem acesso a nada e vê a Minha Área como se ninguém a tivesse confirmado. Aconteceu de verdade na turma de setembro (08-09/09/2026), com duas pessoas. A origem era o próprio botão Confirmar: se a sessão do admin não estivesse ativa naquele instante (ela some sozinha quando a leitura do perfil falha na rede), o clique gravava o status e apagava o campo de quem confirmou, sem avisar nada. Hoje isso não acontece mais — sem sessão ativa, o Confirmar recusa e avisa para recarregar a página, sem gravar nada. E o painel passou a usar a mesma regra do resto do sistema: quem está nesse estado aparece como pendente, com o selo vermelho "Confirmação incompleta" e o botão "Confirmar" na linha, que regrava o registro certo num clique.' },
    { section: 'admin', personas: ['admin'],
