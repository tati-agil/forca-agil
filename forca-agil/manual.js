...
(function () {
  ...
  const RULES = [
    ...
    { section: 'admin', personas: ['admin'],
      ...
      body: 'Campo de texto (Objetivo, Passo a passo, Dicas para o facilitador, Conexão com a agilidade, Materiais necessários, Preparação prévia, Observações, Prompt para IA) que guarda sintaxe de Markdown como texto puro — "1. item"/"1\\. item", "- item"/"\\* item", "**negrito**"/"\\*\\*negrito\\*\\*" — sai no PDF já formatado: lista numerada de verdade (mantendo o número inicial de cada item, mesmo que comece em 9 ou outro valor), bullet, negrito sem asterisco e sem barra invertida nenhuma à mostra. Uma sequência de linhas com marcador diferente logo após um item numerado (ex: perguntas com "-"/"*" logo depois do item "9.") vira sublista DENTRO daquele item, não novos itens 10/11/12 da lista principal — funciona tanto quando essas linhas vêm recuadas quanto quando vêm coladas na mesma coluna (bem comum em texto colado sem indentação). Quebra de linha simples vira quebra de linha; uma linha em branco separa parágrafos. Campo que já foi editado pelo editor de texto rico (barra de formatação do formulário) não passa por essa análise — já é HTML de verdade, sai exatamente como no editor. Isto é só uma correção de EXIBIÇÃO do PDF: o valor gravado no banco nunca muda, e a tela normal do roteiro (editor e "ver detalhes") continua mostrando o campo exatamente como sempre mostrou, símbolos de Markdown incluídos — só as exportações "🖨 Roteiro completo" e "🖨 Passo a passo" interpretam essa sintaxe (a "🖨 Agenda resumida" não tem campos de texto livre, só a tabela).' },
    { section: 'admin', personas: ['admin'],
