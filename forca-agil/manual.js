...
(function () {
  ...
  const RULES = [
    ...
    { section: 'admin', personas: ['admin'],
      ...
      body: 'Acima e abaixo da lista de atividades de cada dia (roteiro-base e roteiro da turma), uma barra mostra 6 números calculados a partir dos dados reais — nunca digitados à mão: Janela (do início da primeira à conclusão da última atividade com horário); Tempo programado (soma das durações de tudo, inclusive pausas); Tempo de facilitação (o programado descontando pausas/intervalos); Pausas (soma das atividades do tipo "Intervalo", incluindo uma que esteja como sub-etapa dentro de uma seção — o que conta é o tipo de quem realmente tem a duração, não o tipo da seção que a contém); Lacunas (soma dos buracos sem nada programado entre atividades vizinhas da MESMA sessão/janela — nunca o intervalo entre uma sessão e a seguinte, ver regra de Sessões abaixo); Atividades (quantidade de atividades principais — sub-etapas de uma seção não contam à parte). Por construção sempre vale: Janela = Programado + Lacunas, e Programado = Facilitação + Pausas — cada um calculado por sessão, quando o dia usa sessões.' },
    { section: 'admin', personas: ['admin'],
    ...
    { section: 'admin', personas: ['admin'],
      title: 'Roteiro — seções com sub-etapas',
      body: 'Uma sub-etapa é uma atividade completa igual a qualquer outra — mesmo formulário, todos os campos (objetivo, passo a passo, materiais etc.) — só que "filha" de uma atividade-mãe. O botão "+ Sub-etapa" (dentro de uma atividade de nível principal) abre esse formulário completo já vinculado a ela; não existe uma versão reduzida só de título e duração. Uma atividade vira "seção" automaticamente assim que passa a ter sub-etapas: sua duração exibida em toda tela (lista, impressão, resumo do dia) passa a ser a SOMA das sub-etapas, calculada sozinha — o campo Duração dela própria, no formulário de edição, fica travado mostrando essa soma (não dá pra digitar um valor diferente enquanto houver alguma sub-etapa; Início/Fim continuam editáveis normalmente e se completam sozinhos com base nessa duração fixa) e aparece um botão "▲ Recolher"/"▼ Ver etapas". Na lista, a seção aparece numerada (1, 2, 3...) e as sub-etapas recuadas logo abaixo, numeradas 4.1, 4.2... Sub-etapas não têm posição própria no dia (não entram na ordenação cronológica, no aviso de ordem divergente nem na cadeia de recálculo de horários seguintes — só a atividade-mãe conta pra tudo isso). Excluir a atividade-mãe apaga junto todas as suas sub-etapas; duplicar a atividade-mãe duplica as sub-etapas junto.' },
    { section: 'admin', personas: ['admin'],
