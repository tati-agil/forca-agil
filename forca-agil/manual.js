...
(function () {
  ...
  const RULES = [
    ...
    { section: 'menu', personas: ['visitante'],
      ...
      body: 'Desde que o login virou obrigatório, o visitante (não logado) não vê o site nem o menu: o body inteiro fica oculto (class "aguardando-auth") e só o modal de autenticação aparece, com fundo opaco e sem botão de fechar. Dentro do modal ele acessa as abas "Entrar" e "Cadastrar", além de "Esqueci minha senha". Não existem botões ENTRAR/CADASTRAR no menu do topo para ele — o menu só é revelado depois da autenticação.' },
    ...
    { section: 'inicio', personas: ['logado', 'inscrito', 'admin'],
      ...
      body: 'Navega para a página Turmas. Na prática o rótulo é sempre "Ver turmas →": o código ainda troca para "Juntar-se à Força →" quando não há sessão, mas essa ramificação virou inalcançável depois que o login passou a ser obrigatório — quem não está autenticado não chega a ver o hero, só o modal de login.' },
    ...
    { section: 'turmas', personas: ['logado', 'inscrito', 'admin'],
      title: 'Botão de interesse sem sessão — proteção residual',
      body: 'O código do botão "Tenho interesse" ainda verifica se existe sessão e, se não houver, mostra "Faça login para registrar seu interesse." e abre o modal. Com o login obrigatório essa ramificação virou inalcançável pelo fluxo normal (ninguém sem sessão chega à página Turmas) — permanece como proteção residual, para o caso de a sessão expirar com a página já aberta.' },
    ...
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Pedidos',
      body: 'Lista os pedidos enviados pelo formulário público "Faça um pedido" (página Ajuda), mais recente primeiro. Os filtros seguem duas dimensões separadas de propósito. (1) Status, que opera SÓ sobre os pedidos ativos: Pendentes / Respondidos / Todos — aqui "Todos" significa, sem ambiguidade, todos os ativos; excluídos nunca aparecem nesses três. (2) Lixeira, um botão à parte (visual tracejado, separado dos chips de status) que mostra só os excluídos. Essa separação existe porque "excluído" não é um status irmão de pendente/respondido — um pedido excluído era pendente ou respondido antes de ser excluído; misturar as duas coisas na mesma fileira tornava "Todos" ambíguo. Abaixo, o filtro por tipo (Todos os tipos / tema / curso / material / dúvida / outros) se combina com o recorte atual, e suas contagens refletem esse recorte — estando em "Pendentes", cada tipo mostra quantos pendentes existem daquele tipo, não o total geral. A aba abre em "Pendentes". Cada item exibe tipo, descrição, nome e e-mail de quem enviou.' },
    { section: 'admin', personas: ['admin'],
