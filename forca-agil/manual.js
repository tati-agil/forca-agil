...
(function () {
  ...
  const RULES = [
    ...
    { section: 'turmas', personas: ['visitante', 'logado', 'admin'],
      ...
      body: 'Cards: vê um card por turma cadastrada (o número de turmas não é fixo — o admin cria/edita/exclui turmas na aba Admin › Turmas), cada um com nome, mês, datas e botão de interesse — visitante (sem turma) e logado (sem turma confirmada). Abaixo dos cards (igual para todos os perfis): bloco "Como funciona a oficina" + Agenda D1–D5.' },
    ...
    { section: 'admin', personas: ['admin'],
      ...
      body: 'Cada turma aparece como um card, com um selo ABERTA ou FINALIZADA e a contagem de inscritos/interessados. Dentro do card: botões de ação e a lista de participantes ativos, com nome, e-mail, área, data de registro e uma coluna Status ("Interessado" enquanto a turma está aberta, "Inscrito" após finalizada).' },
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Turmas — criar turma',
      body: 'Botão "+ Nova turma" (topo da aba): abre um modal com campo de nome e uma lista de datas — cada data é um seletor de calendário, com botão "+ Adicionar data" para incluir mais encontros e "✕" para remover um. Ao salvar, a turma aparece imediatamente como um novo card, com um botão de interesse próprio na página pública de Turmas. Turmas não têm mais um número fixo (as antigas T1/T2/T3 eram cravadas no código) — o admin decide quantas existem.' },
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Turmas — editar turma (nome e datas)',
      body: 'Botão "✎ Editar turma" no menu "⋯" de cada card: abre o mesmo modal de criação, já preenchido com o nome e as datas atuais. Permite renomear a turma e incluir ou excluir datas individuais dos encontros — inclusive turmas já finalizadas. Alterar as datas não apaga check-ins já registrados nos dias que continuarem na lista; remover uma data cuja presença já foi registrada não apaga o histórico de presença, apenas tira aquele dia das tabelas/certificados a partir da próxima geração.' },
    { section: 'admin', personas: ['admin'],
    ...
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Turmas — aviso de interesse em mais de uma turma ao finalizar',
      body: 'Antes de finalizar, o sistema verifica se algum dos futuros inscritos também está interessado em outra turma (alguém pode manifestar interesse em quantas turmas existirem ao mesmo tempo, não há limite fixo). Se houver, o próprio modal de confirmação lista essas pessoas e avisa que, ao continuar, o interesse delas nas outras turmas será removido automaticamente — ninguém pode ficar inscrita em mais de uma turma. Essas remoções aparecem na seção "Removidos" da outra turma, com o motivo registrado.' },
    { section: 'admin', personas: ['admin'],
    ...
    { section: 'admin', personas: ['admin'],
      ...
      body: 'Turma finalizada exibe tabela com colunas por dia (ex: 11/08, 12/08…) e uma coluna Status (Inscrito). Cada célula de dia mostra um selo verde "✓ qr" (registrado pelo próprio QR Code — ver seção Check-in), um selo ciano "✓ adm" (admin registrou manualmente pelo botão "—", a seguir) ou o botão "—" clicável. Clicar em "—" registra presença retroativa a qualquer hora, sem restrição de data — as duas formas contam igual para a frequência. Última coluna mostra frequência X/5 em verde (atingiu o critério) ou vermelho (não atingiu). Critério padrão: 75% de presença (4 de 5 dias).' },
    { section: 'admin', personas: ['admin'],
    ...
    { section: 'admin', personas: ['admin'],
      ...
      body: 'Botão "Remover" (coluna extra na tabela de presença): abre um modal de confirmação. Ao confirmar, registra quem e quando removeu, e a pessoa some da tabela de inscritos imediatamente. Diferente de "faltou a um dia" — é uma remoção da turma, não uma ausência. Os check-ins registrados não são apagados: a pessoa passa a aparecer na seção "Removidos" da própria turma, com o histórico de presença preservado (ver regra abaixo).' },
    { section: 'admin', personas: ['admin'],
      title: 'Aba: Turmas — seção "Removidos"',
      body: 'Aparece em qualquer card de turma (aberta ou finalizada) só quando há pelo menos uma pessoa removida. Mostra nome, e-mail, área, data de remoção e o motivo (manual pelo admin, ou automático por ter sido inscrita em outra turma); em turma finalizada mostra também o histórico de presença (mesmas colunas por dia, com os mesmos selos "✓ qr" / "✓ adm") — tudo só leitura, sem botões de ação.' },
    { section: 'admin', personas: ['admin'],
