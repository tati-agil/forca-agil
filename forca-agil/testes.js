...
(function () {
  ...
  const COMPORTAMENTO_AUTO = [
    ...
    },
    {
      ...
      tests: [
        ...
        { id: 'c-repo-cancelar-limpa', label: 'Formulário "Guardar no Holocron": "Cancelar" esconde o formulário e limpa os campos', run: function () {
  ...
  const COMPORTAMENTO_MANUAL = [
    ...
    { section: 'Entrar',
      title: 'A espera tem que mostrar alguma coisa desde o primeiro instante',
      motivo: 'AUTOMATIZADO NO CI, não aqui: .github/scripts/teste-tela-preta.js roda em toda PR, com o Firebase substituído por um falso, e reprova se em QUALQUER cenário a tela ficar sem nada visível aos 2 segundos — são 8 cenários (rede normal, rede lenta, leitura travada, leitura com erro, perfil com erro, lista de admins travada, autenticação sem resposta, biblioteca do Firebase fora do ar) em 2 formatos de tela (computador e celular), 16 combinações. Esse teste existe porque a suíte "▶ Automáticos" desta página NÃO consegue pegar esse defeito: ela só roda depois do login resolver, ou seja, nunca enxerga a espera — e foi por isso que a tela preta passou batida por ela. Se quiser conferir com os olhos: abrir o site com a rede estrangulada (DevTools → Network → Slow 3G) e ver o anel girando com "Carregando…" imediatamente, e a frase sobre conexão lenta entrando depois de 4 segundos. O indicador é só CSS — nenhum JavaScript — justamente para continuar funcionando quando o JavaScript é o que falhou.' },
    { section: 'Entrar',
...
{ section: 'Turmas',
      title: 'Ordem da vitrine: o mais exclusivo PARA QUEM VÊ vem primeiro',
      motivo: 'AUTOMATIZADO NO CI: .github/scripts/teste-ordem-vitrine.js roda em toda PR, desktop e iPhone. Não existia critério nenhum pra ordem de exibição quando a pessoa é destinatária de mais de uma oferta ao mesmo tempo — tudo saía só na ordem manual do admin (o campo "order"), mesmo uma diretora que também está numa lista de público restrito via as duas turmas na ordem que o admin tivesse digitado, sem refletir que uma delas é mais dirigida a ela. Corrigido: o evento/turma mais exclusivo PARA QUEM ESTÁ VENDO vem primeiro — pra uma diretora, nesta ordem: só-diretores, depois lista restrita (da qual ela também faz parte), depois aberto; pra quem está numa lista mas não é diretora: lista restrita, depois aberto (só-diretores nem aparece); pra quem não é nenhum dos dois: só o aberto. "order" continua valendo como desempate DENTRO do mesmo nível de exclusividade. Mesma lógica um nível abaixo, entre as turmas de um mesmo evento. Pra conferir com os olhos: com uma conta que é diretora E está na lista de público restrito de um evento aberto a todo mundo, mudar o "order" desses três eventos pra colocar o aberto PRIMEIRO no painel e abrir a página Turmas — o aberto tem que continuar aparecendo POR ÚLTIMO, provando que quem manda agora é a exclusividade, não o campo manual.' },
    { section: 'Turmas',
    ...
    { section: 'Admin',
      title: 'Confirmar inscrição — nunca pode gravar sem quem confirmou ("Confirmação incompleta")',
      motivo: 'Uma inscrição válida precisa de status "inscrito" E confirmedByAdmin; sem os dois, a pessoa fica sem acesso a Conteúdos/Treinamento e vê a Minha Área dela como não confirmada, mesmo o painel dizendo "Inscrito" (foi o que pegou duas pessoas da turma de setembro em 08-09/09/2026). Testar: (1) confirmar alguém normalmente e conferir no Firebase que os DOIS campos foram gravados — status inscrito e confirmedByAdmin com o e-mail de quem clicou; (2) simular o registro quebrado apagando só o confirmedByAdmin de alguém no Firebase e recarregar o painel: a linha deve mostrar "Interessado" com o selo vermelho "Confirmação incompleta" e o botão "Confirmar" (NÃO "Desconfirmar"); clicar em Confirmar deve regravar os dois campos e a pessoa volta ao normal, com acesso e Minha Área certos; (3) o painel nunca pode mostrar "Inscrito" para um registro sem confirmedByAdmin — se mostrar, a divergência que causou o problema voltou. O caminho de "mover para turma" (a partir da lista de espera) obedece à mesma regra.' },
    { section: 'Entrar',
      title: 'Celular / rede lenta — inscrita NÃO pode ser rebaixada nem ver tela preta',
      motivo: 'Os dois piores problemas do dia 08/09/2026 (turma de setembro) eram corridas de rede que só aparecem quando as leituras demoram — ou seja, celular em 4G, nunca no desktop em wi-fi. Testar de propósito com a rede estrangulada (DevTools > Network > Slow 4G, ou celular real com sinal ruim): (1) entrar com uma conta CONFIRMADA em turma e ir direto para Treinamento — o menu não pode piscar escondendo Conteúdos/Treinamento, e a pessoa NÃO pode ser jogada para o Início com "Esta área não está disponível para o seu nível de acesso"; a causa era o observador de inscrição responder com os dados de apenas parte das turmas (inscrita em setembro, mas o listener de agosto chegava primeiro e o "está inscrita em alguma?" respondia não), rebaixando para member por alguns segundos; quanto mais turmas cadastradas, maior a janela. (2) O site precisa aparecer mesmo se alguma leitura do banco não voltar — antes, uma única leitura de turmas-interesse sem resposta impedia o fa-auth-ready para sempre e a pessoa ficava numa TELA PRETA depois de logada, até o socorro de 10s derrubar a sessão dela; hoje há limite de 8s que revela o site assim mesmo e o nível se corrige sozinho quando os dados chegam. (3) Sintoma de que a regressão voltou: precisar colocar alguém como admin só para ela conseguir entrar em Treinamento — admin passa por um caminho diferente (isAdmin) e mascara exatamente esse bug.' },
    { section: 'Turmas',
...
{ section: 'Admin',
    ...
    { section: 'Admin',
      ...
      motivo: 'Com pelo menos dois eventos, cada um com gente esperando: abrir a aba Eventos e conferir que a seção "Lista de Espera" de cada evento aparece DENTRO do card recolhível dele — junto das turmas, some quando o card do evento é recolhido e volta quando expande — nunca flutuando abaixo de todos os eventos. Evento sem ninguém esperando (nem ativo, nem já saído) não ganha seção nenhuma, nem vazia. Registro sem eventoKey aparece numa seção separada "Lista de Espera — sem evento", abaixo de todos os cards de evento, nunca some — mas enquanto só um evento tiver turma essa seção não chega a existir, porque a migração adota essas entradas (ver "Migração: fila antiga, sem evento, ganha o dono certo").' },
    { section: 'Turmas',
      title: 'Divulgar ou tirar do ar um evento inteiro',
      motivo: 'O admin decide se o evento aparece para as pessoas — e ninguém some do sistema por ele não estar aparecendo. Em "✎ Editar evento", a caixa "Divulgar este evento na página Turmas". Verificar os dois lados. Some da página: (1) evento que existia ANTES deste controle abre com a caixa marcada e continua na página — nada some sozinho no deploy; (2) desmarcando e salvando, o evento INTEIRO some da página Turmas: o título, as turmas dele, o card de espera e o bloco "A Missão" dele; (3) outro evento publicado continua na página normalmente — o controle é por evento; (4) sem nenhum evento publicado e sem turma solta, a página mostra "Nenhuma turma aberta no momento". Ninguém some do sistema: (5) quem está inscrita numa turma desse evento continua com o mesmo acesso (Conteúdos e Treinamento seguem abertos para ela) e a Minha Área dela continua mostrando a turma, o nome do evento, a carga horária e a frequência — este é o ponto, despublicar é divulgação, não é encerrar nem apagar; (6) no painel o evento continua completo, com turmas, participantes, fila, presença e certificado funcionando igual, e ganha um selo "fora da página" no cabeçalho; (7) voltando a marcar, tudo reaparece na página como estava, sem ter perdido nada.' },
    { section: 'Turmas',
      title: 'Evento restrito a diretores some da vitrine de quem não é diretor nem admin',
      motivo: 'Em "✎ Editar evento", a caixa "Visível só para diretores e administradores" (aba Eventos), combinada com a aba Diretores (ao lado de Administradores). Precisa de um evento publicado, com turma ou fila, e de uma conta de teste que NÃO seja admin nem esteja na aba Diretores. Verificar: (1) marcando a caixa e salvando, o evento INTEIRO some da página Turmas dessa conta — título, turmas, card de espera e bloco "A Missão", igual ao efeito de despublicar, mas só pra ela; (2) logada como admin, o mesmo evento continua aparecendo normalmente — admin sempre vê, mesmo sem estar na aba Diretores; (3) adicionando o e-mail da conta de teste na aba Diretores (sem mexer no evento), o evento reaparece pra ela ao recarregar a página Turmas, ou ao navegar pra ela de novo se já estava logada — não precisa deslogar; (4) removendo da aba Diretores, o evento some de novo; (5) no painel (aba Eventos), o cabeçalho do evento restrito ganha o selo "só diretores/admin", visível mesmo pra quem não é diretor, porque o painel inteiro já é área de admin; (6) desmarcando a caixa, o evento volta a aparecer pra todo mundo, inclusive quem nunca foi diretor; (7) estar na aba Diretores não muda mais nada: a conta continua sem acesso ao painel admin, e Conteúdos/Treinamento continuam dependendo só de estar inscrita numa turma, não de ser diretor.' },
    { section: 'Turmas',
      title: 'Cada evento é um bloco fechado na página',
      motivo: 'Precisa de DOIS eventos na vitrine, cada um com turma e/ou fila, e com Missão, tópicos e itinerário preenchidos nos dois. Na página Turmas, conferir que cada evento é um bloco fechado, na ordem: nome do evento, turmas dele + card de espera dele, e logo abaixo a Missão / Como funciona / Plano de Voo DELE — só então começa o próximo evento. O erro que isto previne: a página já listou todas as grades primeiro e todas as missões depois, e aí os cards do segundo evento apareciam seguidos da descrição do primeiro, sem nada dizendo de quem era. Conferir também: (1) o número de dias em "Como funciona" e no "Plano de Voo" é o do evento daquele bloco, não do outro; (2) com UM evento só a página fica igual ao que sempre foi — este formato não muda nada nesse caso; (3) evento sem Missão/tópicos/itinerário preenchidos não ganha bloco vazio, aparece só com título e cards; (4) o recuo do texto da Missão em relação aos cards continua o mesmo de antes, e nada se desloca; (5) o cabeçalho de cada evento se lê como cabeçalho de seção — a palavra "Evento" em cima e o nome em letra grande abaixo, com régua embaixo —, e entre um evento e o outro há uma linha separando com respiro, dando para ver onde um termina e o outro começa; conferir também no celular, onde o nome quebra em duas linhas.' },
    { section: 'Turmas',
      title: 'Fila de espera sem turma nenhuma',
      motivo: 'A fila é do EVENTO e não depende de turma — este é o ponto. Criar um evento NOVO, sem turma nenhuma, com "Aceitar lista de espera" marcada. Verificar na página Turmas: (1) o evento aparece mesmo sem turma, com o título dele e o card "Lista de Espera" — antes desta mudança não aparecia nada, e a fila só servia depois de já existir turma, quando ela justamente serve ANTES, para medir demanda e dizer quantas turmas abrir; (2) dá para entrar na fila normalmente, e a pessoa aparece no painel, na seção Lista de Espera daquele evento; (3) se o evento tiver Missão/tópicos/itinerário preenchidos, o bloco "A Missão" também aparece — sem ele a pessoa veria "entre na fila" de algo que a página não explica; (4) desmarcando a caixa nesse evento sem turma, ele some inteiro da página Turmas — não há turma nem fila para mostrar; (5) com NENHUM evento aceitando fila e NENHUMA turma cadastrada, a página mostra "Nenhuma turma aberta no momento"; (6) criando uma turma nesse evento depois, ela aparece na grade e o card de espera continua no fim dela.' },
    { section: 'Admin',
      title: 'Lista de espera pode ser desligada por evento',
      motivo: 'Nem todo evento quer fila, e isso é decisão do admin. Em "✎ Editar evento", a caixa "Aceitar lista de espera neste evento". Verificar: (1) evento que existia ANTES deste controle abre com a caixa marcada e o card continua na página Turmas — nada some sozinho no deploy; (2) evento novo já nasce com a caixa marcada; (3) desmarcando e salvando, o card "Lista de Espera" some do grupo daquele evento na página Turmas, e o card do OUTRO evento (se houver, com a caixa marcada) continua lá — o controle é por evento, não global; (4) A REGRA QUE MANDA: a caixa governa só o que as pessoas veem no site. Para o admin, a fila aparece e se gerencia SEMPRE. Com a caixa desmarcada E gente na fila, a seção Lista de Espera daquele evento continua no painel, com as MESMAS pessoas, na mesma ordem, com "Mover para turma" e "Remover da lista" funcionando igual — muda só um aviso ACIMA da lista, nunca no lugar dela. Ninguém pode sumir da lista por marcar ou desmarcar a caixa; (5) voltando a marcar, o card reaparece no site e a fila volta a aceitar gente, com quem já estava ainda lá, na mesma ordem; (6) evento com a caixa marcada e SEM turma nenhuma aparece na página com o card de espera: a fila não depende de turma (ver "Fila de espera sem turma nenhuma").' },
    { section: 'Admin',
      title: 'Migração: fila antiga, sem evento, ganha o dono certo',
      motivo: 'Conserto de dado velho, não regra permanente: hoje ninguém entra na fila sem evento, porque o card de espera é de UM evento e a pessoa escolhe em qual entrar. O que se testa aqui é o registro gravado ANTES disso, sem eventoKey. Requer um registro assim na fila. Ao carregar o painel como admin ele ganha dono sozinho (migrarEsperaEventoKey, roda a cada carga): se veio de uma turma, recebe o eventoKey DA TURMA; se entrou direto pelo card antigo, recebe o evento que teve turma — o único que chegou a ter fila. Conferir: (1) depois de abrir o painel, a seção "Lista de Espera — sem evento" fica vazia e essas pessoas aparecem dentro do card do evento, cada uma com a data de interesse original, sem perder lugar na fila; (2) cadastrar um evento NOVO, ainda sem turma, e recarregar: nada muda, o evento vazio não desfaz a migração nem reabre a seção "sem evento"; (3) rodar de novo não duplica nem sobrescreve nada — idempotente, e nunca troca um eventoKey já gravado.' },
    { section: 'Admin',
    ...
    { section: 'Admin',
      title: 'Treinamento não some enquanto nenhum estiver cadastrado',
      motivo: 'Regressão real, vista em produção: o registro do treinamento só nasce quando um admin abre o painel, e até lá o nó "treinamentos" está vazio — o link Treinamento sumia do menu para TODO MUNDO, inclusive para o admin, que é justamente quem precisa entrar no painel para resolver. Verificar (dá para simular apagando o nó "treinamentos" no Firebase): (1) com o nó vazio, admin e quem está inscrita em qualquer turma continuam vendo o link e o autodiagnóstico padrão, como antes da mudança; (2) quem está apenas logada, sem turma, não vê; (3) assim que um admin abre o painel, o treinamento é criado e passam a valer as regras por evento. Ou seja: nenhuma janela em que o Treinamento desaparece do site.' },
    { section: 'Admin',
    ...
    { section: 'Turmas',
      title: 'Interesse encerrado — card diz que as vagas acabaram, igual pra qualquer pessoa',
      motivo: 'Com uma turma com interesse encerrado pelo admin E que ainda não chegou ao primeiro dia: (1) como usuário logado sem turma confirmada, verificar que o card mostra "Inscrições encerradas" em negrito, seguido de "As vagas desta turma já foram preenchidas. Ela será realizada em <datas>. Acompanhe as próximas turmas para participar." — SEM botão nenhum: nem de interesse, nem de CMFlex; (2) como inscrito em outra turma, mesmo resultado. ESTE TESTE DESCREVIA UMA TELA QUE NÃO EXISTE MAIS até 10/09/2026: falava de um título "Faça sua inscrição no CMFlex" e de um botão "Ir para o CMFlex →" que foram removidos de propósito — o interesse é encerrado justamente porque as vagas no CMFlex acabaram, então mandar a pessoa para lá era mandar bater numa porta fechada (ver a regra correspondente no Manual). Nenhum dos três textos citados existia mais no código; a auditoria de rótulos pegou.' },
    { section: 'Turmas',
      title: 'Card "Em andamento" — aparece automaticamente no primeiro dia da turma',
      motivo: 'Verificar (se existir turma com data iniciada): o card mostra "Turma em andamento" e o texto "As aulas estão acontecendo. Acompanhe as próximas turmas." — sem nenhum botão de interesse ou CMFlex. Esse estado é automático por data (hoje ≥ primeiro dia), comparando a data local de quem vê a página — não UTC (bug corrigido: antes o card virava "Em andamento" cerca de 3h antes da meia-noite local, já que a data "hoje" vinha em UTC e o Brasil está 3h atrás). Testar perto da virada do dia (por volta das 21h) do dia anterior ao primeiro dia da turma e conferir que o card só muda para "Em andamento" na virada da meia-noite local, não antes. Testar como logado, inscrito e admin — o card deve ser idêntico para todos.' },
    { section: 'Turmas',
    ...
    { section: 'Treinamento Jedi',
      ...
      motivo: 'Ação real, com dado real — o registro no histórico não deve ser gerado em cima da conta de produção. O automático equivalente é .github/scripts/teste-autodiagnostico-refazer.js, que roda no Firebase falso: revela, confirma que a linha entra no histórico, refaz (modal próprio, não window.confirm) e revela de novo, confirmando que as DUAS linhas continuam lá, a mais recente primeiro.' },
    { section: 'Treinamento Jedi',
      title: 'Refazer o autodiagnóstico não apaga o histórico',
      motivo: 'Cobre o mesmo automático acima. Na mão: revelar a patente, clicar em "🔁 Refazer autodiagnóstico", confirmar no modal e responder tudo de novo com respostas diferentes. Revelar de novo: o histórico embaixo do resultado precisa mostrar as DUAS linhas (a de agora e a de antes), cada uma com data, patente e pontuação, a mais recente no topo — e nenhuma tela em nenhum momento deve mostrar a mensagem antiga ("solicite ao admin o reset do seu progresso"), que não existe mais.' },
    { section: 'Treinamento Jedi',
      title: 'O cartão do Treinamento fecha, mostra resumo, e abre sem perder progresso',
      motivo: 'O automático equivalente é .github/scripts/teste-treinamento-cartao.js, que roda no Firebase falso: confirma que o cartão do Treinamento tem o MESMO raio e cor de borda do convite da Aposta (por CSS computado), nasce fechado com um resumo de status ("Ainda não iniciado" / "N/20 afirmações respondidas" / "Pronto para revelar" / "Patente revelada: …"), abre/fecha com o mesmo botão, e que recolher NÃO apaga as respostas em andamento. Na mão: com uma turma que tenha a Construção da Aposta liberada e um treinamento associado ao evento, abrir o Treinamento — os dois cartões (convite da Aposta e Treinamento) precisam ter o MESMO peso visual, nenhum dos dois já aberto. Responder algumas afirmações sem revelar, recolher e reabrir: as respostas continuam marcadas.' },
    { section: 'Admin',
    ...
    { section: 'Admin',
title: 'Cadastrados — corrigir nome/gerência chega em TODO lugar, não só no cadastro',
      motivo: 'O nome e a área da pessoa são COPIADOS para dentro de cada inscrição, da fila de espera, das listas de público restrito, da equipe de facilitação e de cada presença. Corrigir só o cadastro deixaria o nome antigo em todas as outras telas SEM ERRO NENHUM — é a falha silenciosa que a skill criterio-de-estado existe para impedir. Precisa de uma pessoa que esteja numa turma com presença registrada. (1) Aba Cadastrados, "✎ Editar" na linha dela: o modal tem que avisar, antes de salvar, em quantos outros registros a correção vai chegar. (2) Corrigir o nome e salvar; voltar na aba Eventos, abrir a turma e conferir que a tabela de participantes mostra o NOME NOVO — e não o antigo. (3) Conferir também na coluna de presença do dia já registrado, na Lista de Espera (se estiver nela), no "👥 Público restrito" da turma e na Equipe de facilitação. (4) Trocar a Área/Gerência e repetir a conferência. (5) Salvar com o nome em branco: tem que recusar com mensagem visível e NÃO gravar nada. (6) O histórico de interesse e os sorteios já realizados NÃO mudam de nome de propósito — são registro do que aconteceu na época. O automático equivalente é .github/scripts/teste-editar-cadastro.js, que roda desktop e iPhone no CI e confere caminho por caminho o que foi gravado.' },
    { section: 'Admin',
title: 'Cadastrados — corrigir e-mail só em cadastro criado pelo painel',
    ...
    { section: 'Admin',
      title: 'Turmas — público restrito: fechar a turma e montar a lista',
      motivo: 'Requer duas contas de teste além da sua (A e B, as duas cadastradas no site) e uma turma com interesse aberto. (1) Em "✎ Editar turma", marcar "Público restrito" e salvar — o cabeçalho da turma ganha o selo "👥 PÚBLICO RESTRITO" com a contagem zerada e surge no menu ⋯ a opção "👥 Público restrito"; conferir que nome, datas, link do CMFlex e resultado esperado continuam iguais. (2) Abrir "👥 Público restrito": a lista está vazia e o texto avisa que, vazia, a turma não aparece para ninguém. Buscar a pessoa A pelo nome e incluir — a linha aparece com área, "Na turma: —", data e o seu nome como quem incluiu. (3) Fechar o modal: o selo passa a "· 1". (4) Logar como A: a turma aparece na página Turmas, com "♡ Tenho interesse" funcionando normalmente. (5) Logar como B: a turma NÃO aparece — nem card, nem botão; conferir também que ela não aparece no resumo "Turmas abertas no momento" da Minha Área de B (que só aparece para quem nunca interagiu com turma nenhuma). (6) Como admin, conferir que VOCÊ continua vendo a turma na página Turmas. (7) Desmarcar "Público restrito" em "✎ Editar turma": a turma volta a aparecer para B, e reabrir a caixa reencontra a mesma lista — desmarcar não apaga ninguém.' },
    { section: 'Admin',
      title: 'Construção da Aposta — a dinâmica não entrega o final antes da hora',
      motivo: 'Requer uma conta de teste confirmada numa turma. (1) Com a turma SEM a flag, abrir o Treinamento com essa conta: nenhum convite da dinâmica aparece. (2) No menu ⋯ da turma, "🎯 Liberar Construção da Aposta"; recarregar o Treinamento da conta de teste: o convite aparece, com o nome da turma. (3) Abrir a dinâmica e percorrer as nove etapas. Em NENHUMA delas — nem em rótulo, nem em ajuda, nem no "Ver exemplo", nem no Mapa da Aposta no fim — pode aparecer OKR, Objective ou Key Result. Se aparecer, a oficina perdeu o ponto alto e é bug. (4) A trilha lateral mostra os nove números desde o começo, mas NÃO o nome das etapas que ainda não chegaram (elas aparecem como "· · ·" e não clicam); o nome surge quando chega a vez dela ou depois de preenchida. (4b) Em qualquer etapa, o texto fixo da frase aparece na tela e só as lacunas são digitáveis; embaixo, "fica assim no mapa" mostra a frase se montando, lista o que ainda falta e clicar numa lacuna leva ao campo dela. No Problema, o card do Sintoma acima é só leitura; preencher "Quem é afetado" e "Situação indesejada" monta a frase sem "evidenciado por" nenhum, e repetir o texto do Sintoma na Situação indesejada dispara um aviso didático (não bloqueia). Na Hipótese, o Problema aparece só leitura no card acima, e a frase virou duas — "Acreditamos que isso acontece porque…" e "Essa hipótese surgiu porque observamos que…" —, sem o antigo "pois"; os dois campos ("Hipótese causal", "Sinal que motivou a hipótese") têm tooltip próprio, e nenhuma linguagem de prova ("isso prova", "logo", "portanto") aparece na frase automática. Diferente de Missão/Sintoma/Problema (que sempre mostram a frase com lacunas por dentro, tipo "[a causa]"), Hipótese, Ideia de solução, Experimento e Decisão NUNCA mostram a frase incompleta: enquanto faltar qualquer campo obrigatório, "Fica assim no mapa" mostra só uma orientação do que falta ("Preencha a causa percebida e o que foi observado para completar a hipótese." / "Preencha o que poderíamos fazer e para quê, para visualizar a ideia de solução." / no Experimento, lista dinamicamente só o que falta de verdade, ex.: "Complete quanto tempo, quantas pessoas... para visualizar o experimento." / na Decisão, "Escolha o que faremos com base no que aprendemos." enquanto a decisão não foi escolhida, ou "Defina a próxima ação para completar a decisão." quando só falta a próxima ação) — nunca algo como "com quantas pessoas..." ou "vamos a decisão" como se fosse texto de verdade. Nessas quatro etapas, CONTINUAR (e, na Decisão, também "Ver o mapa da aposta →", mesmo botão na última etapa) também fica bloqueado de verdade (sem escape por segundo clique) enquanto faltar qualquer campo obrigatório — no Experimento isso inclui ter pelo menos um resultado marcado em "O que vamos observar?"; na Decisão, a decisão precisa estar escolhida E a Próxima ação preenchida (Responsável, Prazo e Data de reavaliação continuam opcionais, como complementos). Nessas quatro etapas (e em Mudanças mensuráveis/Evidência, que também bloqueiam de verdade), clicar CONTINUAR incompleto mostra um aviso amarelo dizendo o que falta — mas ele nunca fica sozinho na tela depois de o campo ser corrigido: completar o Experimento (por exemplo) sem clicar em CONTINUAR de novo já apaga sozinho o aviso "Complete o que será feito...", ao mesmo tempo em que "Fica assim no mapa" passa a dizer "A frase desta etapa está completa." — os dois nunca aparecem juntos, um dizendo que falta e outro dizendo que está completo. Nessas mesmas etapas (Mudanças mensuráveis, Hipótese, Ideia de solução, Experimento, Evidência, Decisão), o botão CONTINUAR/"Ver o mapa da aposta →" em si fica genuinely disabled (propriedade DOM `disabled`, sem glow amarelo, cursor de bloqueado) enquanto faltar algo — nunca só visualmente ativo sem funcionar; testável lendo `(document.getElementById("apostaSeguir")||{}).disabled` antes e depois de completar o campo que faltava. Na Evidência, a frase "Esperávamos…" não repete mais o prazo da mudança mensurável ("em 90 dias" não aparece ali, só a unidade/período — o prazo já está em Mudanças mensuráveis). No Sintoma, conferir o texto de apoio ("fato, sinal ou comportamento… ainda não tente explicar a causa") e a dica do "?" (traz um bom e um mau exemplo) — e que não existe botão para um segundo sintoma. Refinamento — etapas opcionais (Sintoma e Ideia de solução, só essas duas), 2 rodadas de bugfix pós-uso real. (1ª rodada) A primeira versão era um link discreto no fim do formulário (nunca mais chamativo que CONTINUAR), e o clique mudava de tela no mesmo instante, sem confirmação nenhuma — só ficava claro que tinha sido um pulo (e não um CONTINUAR clicado por engano) voltando depois e vendo o selo "Pulada"; a confirmação parecia "sutil na ida, evidente na volta". (2ª rodada) O toast sozinho não resolvia a descoberta ANTES do clique, e no celular, na Ideia de solução, o link chegava a ficar abaixo da dobra (atrás do card de Hipótese e do quadro "Resultados que queremos produzir"), invisível sem rolar. Corrigido com uma faixa discreta logo após a pergunta, ANTES do formulário — "Esta etapa é opcional. Você pode preenchê-la ou seguir sem ela." com um botão secundário PULAR ESTA ETAPA (classe `btn--ghost`, nunca `btn--primary` — testável conferindo que `#apostaPularEtapa` nunca tem a classe `btn--primary`, enquanto `#apostaSeguir` sempre tem) — testável conferindo `document.querySelector(".aposta-opcional-faixa")` na primeira visita, e no celular (375×812) que `getBoundingClientRect().top` de `#apostaPularEtapa` fica dentro de `window.innerHeight` sem rolar. Clicar em PULAR ESTA ETAPA mostra por um instante o toast "✓ Sintoma pulado."/"✓ Ideia de solução pulada." (mesmo padrão já usado na Decisão, "✓ Decisão registrada.") antes de avançar — testável conferindo `document.querySelector(".aposta-toast")` logo após o clique, antes da tela mudar. Reabrir uma etapa já pulada continua mostrando o estado forte "Esta etapa foi pulada" (a faixa discreta some — ela é só para a primeira visita, antes de decidir).Testar em Sintoma: (1) vazio, clicar "Pular esta etapa" — mostra o toast, nunca pede confirmação, avança direto para Problema, CONTINUAR nunca esteve bloqueado. (2) A trilha lateral marca Sintoma com o selo discreto "Pulada" (nunca vermelho/erro) e continua clicável. (3) Reabrir Sintoma pela trilha: a tela mostra "Esta etapa foi pulada" com "Preencher agora" (some o estado, mostra o formulário vazio, sem pedir confirmação) e "Manter como pulada" (só confirma, não navega — CONTINUAR, sempre visível, já segue). (4) Digitar algo e SÓ DEPOIS clicar "Pular esta etapa": pede confirmação ("Você já começou a preencher esta etapa. Deseja descartá-la e pular?", CANCELAR/PULAR E DESCARTAR) — cancelar preserva o texto digitado, confirmar descarta e avança; nunca some sozinho. (5) No Mapa da Aposta e na exportação em texto/CSV, Sintoma pulado simplesmente não aparece — nenhum card, nenhuma linha "não informado"/"pulado"/"—", e as setas do Mapa se reencaixam sozinhas (Missão liga direto em Problema). Mesmo roteiro para Ideia de solução, com duas diferenças: pular avança de Hipótese direto para Experimento (em vez de Sintoma→Problema), e CONTINUAR em Ideia continua bloqueado de verdade quando ela NÃO está pulada e falta preencher (a regra de frase estrita não mudou — só ganhou uma saída quando a decisão é pular); como Ideia é dado do CICLO, um Ciclo novo que recomeça antes dela (ex.: "Reformular a hipótese") nasce com a decisão em aberto de novo, nunca herdando "pulada" (nem o conteúdo) do ciclo anterior — só herda quando o ponto de reinício vem DEPOIS dela, e editá-la/pulá-la nesse caso já aciona a mesma cascata de "revisandoHerdado" de qualquer outro campo herdado. Com as duas puladas ao mesmo tempo, a aposta continua válida: "Analisar com IA" funciona normalmente (o núcleo obrigatório passou a ser Missão/Problema/Mudanças mensuráveis/Hipótese/Experimento/Evidência planejada — Sintoma e Ideia só precisam estar DECIDIDAS, preenchidas ou puladas, nunca preenchidas à força) e nenhum dos três prompts (Defende/Desafia/Investiga) menciona Sintoma, Ideia, "pulado" ou "não informado" em lugar nenhum — a seção inteira só não existe ali. O texto comum dos prompts e a lente Defende (que antes citava a cadeia "Missão → Sintoma → … → Ideia → …" por extenso) passaram a falar dos "elementos efetivamente presentes na aposta", para nunca presumir uma etapa que pode ter sido pulada. (4c) No Prazo da Missão, o rótulo colado à lacuna diz "Prazo" (não mais "em" solto) — a palavra "em" continua só na frase montada; no Experimento, o mesmo vale para a quantidade — o rótulo colado diz "Quantas pessoas", não a palavra "com" solta, que continua só na frase montada ("…com 50 pessoas…") — e para a duração, cujo rótulo diz "Duração do experimento" (com tooltip "Por quanto tempo este teste será executado?"), não só "Durante" solto. No Prazo (Missão, Experimento e Decisão), digitar "90 dias" deixa só o 90 no campo e a unidade vem da lista ao lado; com 1, a frase diz "1 mês", não "1 meses" — o prazo da Decisão é duração, não data. Bugfix pós-testes de produção (achado com o prazo visivelmente preenchido em 90/dias e a tela ainda pedindo "o prazo"): o Prazo da MISSÃO é OPCIONAL — testar deixando-o vazio e preenchendo só verbo/o quê/contexto: CONTINUAR habilita, "Ainda falta" nunca lista "o prazo" e a frase termina normalmente, sem "em o prazo", "em —" nem "em." sobrando no fim; digitar um valor (a unidade "dias" já vem pré-selecionada, o <select> nunca fica em branco) tem de manter CONTINUAR habilitado e a frase certa ("...em 90 dias."). Só bloqueia de verdade se o prazo for preenchido PELA METADE — valor sem trocar a unidade real (ela sempre normaliza para uma opção válida na tela, então esse caso é mais visível revisando um dado antigo/legado do que digitando) ou o inverso —, com uma mensagem específica de cada lado, nunca a genérica de "falta". A data de reavaliação (essa sim, um dia marcado) aceita dd/mm/aaaa (máscara ao digitar) e o custo estimado sai como R$ 2.500,00. (4d) Em Mudanças mensuráveis, a primeira já aparece em branco sem clicar em nada, e enquanto estiver vazia a etapa NÃO conta como preenchida na trilha. (4e) Reabrindo um grupo que já percorreu tudo, a trilha não pode mostrar nome no fim e número no meio: ou está revelada até onde o grupo chegou, ou não está. (4f) Nenhum pedaço de texto fixo pode ficar numa linha sozinha, longe do campo que ele apresenta. (4g) No painel do facilitador, a seção chama "Missão-base da dinâmica", com dois textos explicando que ela vale só para grupos que ainda não preencheram a Etapa 1 e que alterá-la não mexe em quem já começou; digitar nos campos atualiza, ao vivo, uma prévia compacta "MISSÃO-BASE" com a frase completa. Salvar sem nenhuma missão-base existente mostra o botão "Salvar missão-base"; depois de salvar, aparece "✓ Missão-base salva", o botão vira "Atualizar missão-base" e surge a ação discreta "Remover missão-base" (pede confirmação; remove só a missão-base, nunca as missões já salvas pelos grupos). Cadastrar essa missão-base e abrir a dinâmica com outra conta: a etapa 1 abre com essa missão já escrita, avisando que veio da facilitação e que dá para ajustar. Um grupo que já tinha salvo a própria missão (ou já está em etapa depois da 1) tem de continuar vendo a SUA ao voltar na etapa 1, mesmo depois de a facilitadora salvar, trocar ou remover a missão-base. Criar um grupo novo mostra "0/9 etapas · não iniciado" (e "X/9 etapas · em andamento"/"9/9 etapas · concluído" conforme o grupo avança); logo abaixo, a lista de quem está no grupo mostra "sem participantes ainda" enquanto ninguém entrou — nunca "ninguém ainda" (que ficava colado ao status de etapas e lia como se fosse sobre progresso, não sobre participantes). (4h) No mapa, "⧉ Copiar em texto" traz as nove etapas com a pergunta de cada uma (e nenhum termo do final), e "🖨 Salvar em PDF" abre a impressão com uma folha branca só com os cards. (5) No Sintoma, escrever algo com "porque": aparece o aviso pedindo só o que é observado, e o segundo clique em Continuar segue assim mesmo — o aviso convida, não bloqueia. (6) Em Mudanças mensuráveis, preencher indicador, situação atual, meta e prazo: a frase consolidada se monta sozinha embaixo. Escolher a Forma de medição muda as sugestões (chips) de Unidade — Quantidade sugere "contatos", "processos" etc., Tempo sugere "minutos" a "meses"; Percentual e Moeda preenchem a Unidade sozinhos ("%" e "R$"), sem pedir escolha. "Período de medição" é campo separado de Unidade (ex.: Unidade "contatos", Período de medição "por mês" — não é a mesma coisa, e o site nunca deduz um a partir do outro nem do Prazo); o rótulo tem tooltip explicando "esses valores são medidos em qual período?". Com Forma de medição "Quantidade" ou "Taxa / Razão" e Período de medição vazio, aparece o aviso didático pedindo o período (ou "não se aplica") — não bloqueia, é convite a reler como os demais desta dinâmica. A frase de Aumentar/Reduzir/direção livre repete a unidade e o período de medição TANTO na situação atual quanto na meta ("de 1000 contatos por mês para 700 contatos por mês em 90 dias"), não só depois da meta — para não parecer que a unidade/período valem só para o número final; percentual e "não se aplica" continuam sem período nenhum grudado. Com Direção "Aumentar" e uma Meta menor que a Situação atual (ou o inverso com "Reduzir"), aparece um alerta perto do campo com um botão para trocar a direção num clique — o alerta some ao corrigir; ESSE alerta não bloqueia, mas clicar CONTINUAR nesse estado bloqueia de verdade e sem escape por segundo clique, apontando para o card destacado (é a única validação desta dinâmica que impede seguir, porque a direção contradiz os próprios números, não é um convite a reler). Enquanto a inconsistência não é corrigida, a frase "de 1000 para 1100" NUNCA aparece como se fosse válida: depois de uma pausa curta sem digitar (a validação não reage agressivamente a cada tecla, para não brigar com quem ainda está no meio de escrever um número), o texto "Fica assim..." dá lugar a "Corrija a inconsistência acima para visualizar a mudança mensurável.", e volta a aparecer na hora, sem demora, assim que a direção e os números voltam a bater. Numa mudança com Percentual, a frase gruda o "%" nos dois números ("de 30% para 60%"), sem repetir a unidade. O rótulo colado ao Prazo diz "Prazo para atingir o resultado" (não mais só "Prazo"), com tooltip "Até quando queremos alcançar esta mudança?" — os quatro tempos da dinâmica (esse, a Duração do experimento, o Prazo da próxima ação e a Data de reavaliação, ambos na Decisão) nunca podem ter o mesmo rótulo genérico. (6a) "Direção da mudança" (antes "Queremos") tem quatro opções: Aumentar, Reduzir, Manter e Atingir. Escolher "Manter" troca a Meta desejada por "Tipo de limite" (Pelo menos/No máximo/Entre); escolher "Entre" troca a Meta por dois campos, Limite mínimo e Limite máximo — voltar para Aumentar/Reduzir/Atingir traz a Meta desejada de volta. A frase muda com a direção: "Manter X em pelo menos/no máximo Y durante Z.", "Manter X entre Y e W durante Z." e "Atingir Y de X em Z." "Manter" não usa "situação atual == meta" — usa o LIMITE escolhido: "Pelo menos X" exige situação atual ≥ X, "No máximo X" exige situação atual ≤ X, "Entre X e Y" exige X ≤ situação atual ≤ Y (as pontas contam); fora disso, esconde a frase (mesma mensagem "Corrija a inconsistência…") e mostra um alerta com DUAS opções de correção — "Aumentar"/"Atingir" quando está abaixo, "Reduzir"/"Atingir" quando está acima —, nenhuma delas escolhida sozinha; Limite mínimo maior que o máximo em "Entre" é inconsistente por si só, sem sugerir direção nenhuma. Continuar bloqueia nos dois casos, igual ao de Aumentar/Reduzir. Bugfix pós-PR#210 (achado em teste manual em produção): "Atingir" sozinho (fora de "Manter") também tem validação própria agora — antes aceitava qualquer meta, inclusive abaixo da situação atual (ex.: atual 67, meta 50, "Atingir 50% de DAD..." como se fosse coerente, quando é uma redução disfarçada de meta). Testar: situação atual 0/meta 50 e situação atual 40/meta 50 são válidos (meta maior, "patamar ainda não alcançado"); situação atual 50/meta 50 é inválido, com a mensagem "A meta informada já corresponde à situação atual...", sem sugerir direção nenhuma; situação atual 67/meta 50 é inválido, com "A meta informada está abaixo da situação atual. Para passar de 67 para 50, a direção da mudança é uma redução. Você quis selecionar Reduzir?" e um botão só, "Usar Reduzir" (nunca "Usar Aumentar"); clicar no botão troca só a direção, nunca os números; corrigido, CONTINUAR volta a funcionar. Testar com Forma de medição "Índice" e Unidade "NPS" num indicador que já começa com "NPS" (ex.: "NPS da experiência do participante"): a frase de Atingir fica "Atingir NPS 60 em 90 dias.", sem repetir "NPS" duas vezes. Ainda com Índice + NPS, o campo Período trava sozinho em "não se aplica" e fica desabilitado (não dá para escolher "por dia"/"por mês" etc. — NPS é um índice, não uma contagem ao longo do tempo); trocar a Forma de medição ou a Unidade para outra coisa destrava o campo de novo. Com Direção "Aumentar" ou "Reduzir" e Índice + NPS, a frase não repete a unidade depois dos números nem anexa período nenhum: "Aumentar NPS de 5 para 7 em 90 dias.", nunca "de 5 NPS por dia para 7 NPS por dia". (6b) No Experimento, a seção "O que vamos observar?" lista as mudanças mensuráveis cadastradas — com uma só, ela já vem marcada e travada (não dá para desmarcar); cadastrando uma segunda mudança mensurável e voltando ao Experimento, as duas aparecem marcadas por padrão, e desmarcar uma tira o card dela da Evidência. A frase "Fica assim no mapa" do Experimento descreve só o desenho do teste (duração, quantidade, com quem, o que será feito); os resultados escolhidos aparecem LOGO ABAIXO, numa lista própria "Resultados que vamos observar" (um item por linha, indicador + situação→meta), não emendados na frase principal. Na Evidência, cada resultado escolhido vira um card com indicador, situação inicial e meta (ou limite, para "Manter") já preenchidos e SEM poder editar (os campos aparecem desabilitados) — nunca a mesma unidade/período misturados, sempre os herdados da mudança mensurável. A etapa inteira (nunca card a card) começa no modo "Planejamento da evidência" (banner no topo, pergunta "Como saberemos o que aconteceu?"). Cada card tem "Fonte prevista da evidência" e "Detalhe/como será medido" (Outro exige detalhamento) editáveis desde o início, e no lugar de "Resultado observado" mostra só o texto "Resultado observado: aguardando execução" — o campo em si nem existe no DOM ainda. O botão principal da etapa diz "Salvar plano para execução" e fica genuinely disabled (testável via a propriedade DOM `disabled`, não só a aparência) se algum card ainda não tem Fonte prevista escolhida — clicar nele não faz nada, e a orientação já está no texto "Resultado observado: aguardando execução" de cada card, sempre visível, sem precisar do clique para aparecer. Assim que a Fonte prevista de todos os cards estiver definida, o botão muda sozinho para "Registrar resultados do experimento" e fica habilitado, e aparece um aviso "✓ Plano de evidência salvo. Pronto para execução." com um botão "Encerrar por agora": clicar nele fecha a dinâmica na hora e mostra, já na página de Treinamento por trás dela, um aviso fixo ("✓ Planejamento salvo. O experimento está pronto para execução...") que não é destruído pela navegação — continua na tela até ser fechado no ✕ ou por alguns segundos. Reabrindo essa mesma aposta depois (plano já salvo, sem ter clicado em Registrar), o bloco mostra só "PLANO DE EVIDÊNCIA SALVO — Aguardando execução do experimento.", sem repetir o botão de encerrar. Clicar em "Registrar resultados do experimento" abre um modal próprio do site (não o confirm() nativo do navegador) perguntando "O experimento já foi executado?", com os botões "AINDA NÃO" e "SIM, REGISTRAR RESULTADOS"; só com "SIM, REGISTRAR RESULTADOS" a etapa inteira (todos os cards de uma vez, nunca um por um) muda para o modo "Registro dos resultados" (banner e pergunta "O que aconteceu de fato?" voltam a aparecer) — cada card passa a recapitular só leitura "Fonte planejada"/"Como seria medido" e abre "Resultado observado"/"Fonte utilizada", esta já pré-selecionada com a fonte planejada (sem precisar de um clique de confirmação à parte, mas continua editável); cancelando, a etapa continua em planejamento. Nesse modo, uma mensagem sempre visível (não só ao clicar) lista exatamente o que falta por resultado — ex.: "Resultado observado do Resultado 2" — e vira "✓ Evidências completas." quando não falta nada; "Continuar para Decisão" fica genuinely disabled enquanto a lista não estiver vazia. Preencher "Resultado observado" e, se necessário, trocar a "Fonte utilizada" (pode ficar igual à prevista ou não) monta sozinha a frase "Esperávamos... Após o experimento, observamos..." (nunca "o experimento reduziu/aumentou X" — só o que foi observado, sem afirmar causalidade). Para Aumentar/Reduzir/Atingir, a mensagem logo abaixo é "N% da mudança esperada foi alcançada." (não mais "N% do caminho até a meta") — com direção "Reduzir", um valor observado menor que a situação inicial mostra um percentual maior (a conta inverte sozinha para reduzir, não é comparação simples); observado exatamente igual à meta mostra "A mudança esperada foi alcançada." sem percentual nenhum; superando a meta mostra "A mudança esperada foi superada em N [unidade/período]." — nunca um percentual acima de 100%; indo na direção contrária (piora) mostra "O resultado observado não avançou na direção esperada. Foram observados N, acima/abaixo da situação inicial de M." — nunca um percentual negativo. Toda unidade que acompanha um número nessas frases (e nas da Decisão que herdam da Evidência) concorda em número com o próprio valor: "1 contato", "1 erro", "1 processo", "1 ponto percentual" no singular — nunca "1 contatos" —, e "2 contatos" etc. no plural normalmente; cada número da frase concorda com o SEU próprio valor (situação atual, meta, observado e a diferença calculada podem discordar entre si, ex.: "de 1 contato por dia para 2 contatos por dia"). Para "Manter", em vez de percentual aparece uma de quatro mensagens conforme o tipo de limite e o lado da inconsistência: "dentro da condição que queríamos manter" (ok), "abaixo"/"acima"/"fora da condição que queríamos manter" (conforme Pelo menos/No máximo/Entre). Marcar "Não foi possível medir neste experimento" desliga Resultado observado e Fonte na hora, e passa a exigir Motivo (o rótulo perde o "(opcional)"); a frase passa a dizer isso em vez de exigir um número. Depois dos cards, o resumo "Resultados do experimento" traz cada indicador com situação → observado e a meta (ou limite) — os dois sempre distintos, nunca a meta substituída pelo observado. No modo de registro, ainda sem resultado observado, o card não inventa frase completa: mostra só a parte conhecida e o aviso "Ainda falta: o resultado observado" — e o Mapa da Aposta/resumos da etapa, pelo mesmo motivo, mostram "[indicador]: aguardando execução." por resultado já escolhido no Experimento, em vez de ficarem em branco (dá para ver a aposta planejada mesmo antes do teste rodar). Com Índice + NPS, nem "O que vamos observar?" nem os campos Situação inicial/Meta da Evidência mostram sufixo nenhum — "5 → 7", nunca "5 → 7 NPS por mês" — porque o indicador já aparece como título ao lado. Cada card tem também "O que aprendemos com esta evidência?" (por resultado, não uma classificação única da etapa — não existe "Nossa hipótese foi: Sustentada/Parcialmente sustentada/Não sustentada" em lugar nenhum da Evidência, porque um mesmo experimento pode produzir evidências em direções diferentes e a leitura do conjunto é da Decisão), com tooltip pedindo para não tratar aquela evidência isolada como prova definitiva; linguagem de prova ("comprovou", "confirmou", "sucesso"...) nesse campo recebe o mesmo tipo de aviso didático das outras etapas, sem bloquear — mas o aprendizado em si é OBRIGATÓRIO quando o resultado foi medido: sem ele, "Ainda falta: o aprendizado com esta evidência". No modo de registro, "Continuar para Decisão" bloqueia de verdade (botão genuinely disabled, sem escape por segundo clique) enquanto qualquer resultado medido não tiver Resultado observado + Fonte utilizada + Aprendizado (ou, se marcado "Não foi possível medir", o Motivo — sem exigir fonte nem aprendizado nesse caso). Depois de todos os resultados completos, ainda falta uma última coisa, UMA ÚNICA VEZ para o conjunto (nunca repetida por resultado): "Nossa hipótese foi:" (Sustentada/Parcialmente sustentada/Não sustentada), com o texto de apoio "O que as evidências nos dizem sobre a hipótese que testamos?" — deixando claro que a pergunta é sobre a hipótese ORIGINAL (a mesma da etapa H — Hipótese, nunca reescrita por essa escolha). Essa classificação só aparece no modo de registro (não faz sentido perguntar antes de haver o que interpretar); a pendência lista "Nossa hipótese foi" junto com o que falta por resultado, e escolher uma opção habilita CONTINUAR na hora, reativo, sem precisar clicar em Continuar para descobrir que faltava isso. É diferente da NOVA hipótese que existe lá na Decisão (ver adiante) — uma é a leitura do que já foi testado, a outra é uma hipótese nova para o próximo ciclo, e nenhuma sobrescreve a outra. (6c) Na Ideia de solução, entre o card da Hipótese e o título da etapa, o bloco só leitura "Resultados que queremos produzir" lista as Mudanças mensuráveis já cadastradas, com seta (↑ Aumentar, ↓ Reduzir, → Manter/Atingir); não dá para editar nada ali. Passar o mouse (ou tocar e segurar, no celular) em "Poderíamos" e "para" mostra a dica de cada campo. Digitar no campo "Mudança que pretendemos provocar" um texto que já começa com "para" ou "para que" (ex.: "para que o participante consiga acompanhar sozinho") não pode duplicar a palavra fixa da frase — "Poderíamos... para que o participante..." certo, nunca "...para para que...". (6d) Na Decisão, as oito opções são Ampliar, Ajustar e testar novamente, Interromper esta ideia, Investigar mais, Reformular a hipótese, Rever a mudança mensurável, Rever o problema (as três últimas, da Fase 4 — ver item 12) e Concluir a aposta (refinamento pós-teste manual, item 13 — "já aprendemos o suficiente e não precisamos de outra rodada agora"; nunca é Ampliar nem Interromper, e nunca é confundida com "Encerrar por agora", o botão da Evidência que só pausa e volta depois; como Ampliar/Interromper, só finaliza o ciclo atual, nunca cria um Ciclo N+1 sozinha — testável abrindo o Mapa e o CSV depois: a decisão aparece normal, sem sucessor nenhum nascendo; bugfix pós-PR#210 achado em teste manual em produção: diferente das outras sete decisões, "Concluir a aposta" NÃO exige Próxima ação — o rótulo muda para "Próxima ação (opcional):" ao vivo ao escolher a opção, "Reformular hipótese também" fica oculto de verdade (hidden, testável em `(document.getElementById("apostaGrupoNovaHipotese")||{}).hidden` e também na tela — não só no atributo), e os Complementos (Responsável/Prazo/Reavaliação) ficam ocultos enquanto nada for escrito, reaparecendo ao vivo se a dupla digitar um encaminhamento voluntário; "Ver o mapa da aposta →" já fica habilitado (`disabled === false`) assim que a decisão é escolhida, sem precisar preencher nada mais; sem Próxima ação nenhuma, o Mapa e o CSV mostram só "...vamos concluir a aposta." — nunca "Próxima ação:" sobrando sozinho, nem "Próxima ação: —"; decisões "Concluir a aposta" gravadas ANTES deste bugfix, com Próxima ação já preenchida, continuam lidas e mostradas normalmente, sem migração nenhuma), cada botão com sua dica (title, no hover/toque longo) e, assim que uma é escolhida, uma microexplicação fixa aparece embaixo dos botões, com texto próprio por opção (ex.: "As evidências são suficientes para aumentar a escala da aposta." para Ampliar) — nunca some sozinha, e troca na hora quando a escolha muda; o bloco de Evidência no topo mostra indicador/situação inicial/meta/observado por resultado, sem nenhuma frase do tipo "Portanto, nossa hipótese foi X" gerada sozinha — mas mostra, sim, o que a Evidência já registrou: por resultado, situação inicial/meta/observado e, quando preenchidos, Fonte e Aprendizado, herdados por id; e, uma única vez (não por resultado), a linha "Nossa hipótese foi: [o que o grupo escolheu lá]" — a mesma classificação (Sustentada/Parcialmente sustentada/Não sustentada) que a Evidência exige escolher, sempre sobre a hipótese ORIGINAL da etapa H, nunca reescrita por essa leitura. Escolher "Reformular a hipótese" e preencher só a Próxima ação (sem tocar a Nova Hipótese): CONTINUAR/"Ver o mapa da aposta →" continua genuinely disabled, mesmo com a Próxima ação completa — só desbloqueia depois de "Acreditamos que isso acontece porque…" e "Essa hipótese surgiu porque observamos que…" preenchidos; enquanto isso, "Fica assim no mapa" mostra "Defina a nova hipótese para completar a decisão." (só falta ela), "Defina a nova hipótese e a próxima ação para completar a decisão." (faltam as duas) ou só a mensagem de próxima ação (só ela falta) — nunca a frase como se estivesse completa. Tudo isso é reativo a cada tecla nos dois campos, sem precisar clicar em CONTINUAR; trocar de decisão também reage na hora do clique (não só ao digitar) — sair de uma decisão que já habilitava CONTINUAR e escolher "Reformular a hipótese" bloqueia CONTINUAR imediatamente se a Nova Hipótese ainda estiver vazia, e o inverso também: sair de "Reformular a hipótese" para outra decisão com a Próxima ação já preenchida habilita CONTINUAR na hora, sem digitar de novo. O texto já digitado na Nova Hipótese nunca se perde ao trocar de decisão e voltar depois; nas outras quatro decisões a Nova Hipótese continua opcional e não bloqueia nada. O placeholder de "Próxima ação" muda de exemplo conforme a decisão escolhida (um texto por opção — nunca preenche o campo sozinho, só troca a dica cinza). "Responsável" virou "Responsável pela próxima ação", e "Prazo" virou "Prazo da próxima ação" (com tooltip "Em quanto tempo executaremos o próximo passo decidido?") — nunca confundir com o Prazo para atingir o resultado (Mudanças mensuráveis) nem com a Duração do experimento. Avançar a Decisão com tudo completo mostra o toast "✓ Decisão registrada." antes de seguir para o Mapa da Aposta, e grava sozinha (sem o grupo digitar) a Data da Decisão, só para auditoria — diferente da Data de reavaliação, que é quando o grupo PRETENDE voltar a olhar a aposta (a tooltip dela diz isso). Escolher "Ampliar" quando pelo menos um resultado esperado não bateu a meta (incluindo "Não foi possível medir") mostra um alerta não-bloqueante logo abaixo da escolha, com dois botões: "MANTER AMPLIAR" (só dispensa o aviso, sem mudar nada) e "REVER DECISÃO" (limpa a decisão escolhida, para o grupo escolher de novo — a frase volta a pedir "Escolha o que faremos..."); esse alerta nunca bloqueia CONTINUAR. Preenchendo só o Prazo (sem Data de reavaliação), aparece uma sugestão de data calculada a partir do prazo, com um botão "Usar [data]" que preenche a Data de reavaliação com um clique (nunca sozinho); preenchendo os dois e eles não baterem, aparece um alerta "não bate com o prazo" com "Usar [data sugerida]" e "Manter [data que já estava]" — de novo, nenhum dos dois muda o campo sem o clique. O bloco "Nova hipótese" não aparece do mesmo jeito conforme a decisão clicada: some para Ampliar e Investigar mais; aparece recolhido, atrás do resumo "Reformular hipótese também", para Ajustar e testar novamente e Interromper esta ideia (clicar no resumo abre); e abre sozinho para Reformular a hipótese, com um aviso dizendo que é obrigatório. Preenchendo a Nova hipótese com "Reformular a hipótese" escolhida, "Fica assim no mapa" acrescenta "Nova hipótese: …" ao final. (7) Só depois do clique da facilitadora em "Revelar conexões" aparecem Missão→Objective e Mudanças mensuráveis→Key Results, mais o ciclo PHEED. (8) Fechar a dinâmica e abrir de novo: tudo o que foi escrito continua lá. (9) No painel do facilitador, o botão que reinicia a dinâmica agora chama "Iniciar nova execução" (não mais "Reiniciar dinâmica"), com o aviso explicando que a execução atual será encerrada e continua disponível no histórico. Clicar nele abre um modal próprio do site (não o confirm() nativo) perguntando se quer mesmo encerrar a atual e iniciar uma nova; "CANCELAR" fecha sem mudar nada. Confirmando: a execução nova nasce vazia com status "ativa" e um número sequencial (1, 2, 3…), "atual" passa a apontar para ela, e a execução anterior é encerrada de verdade (status "encerrada", com quando e quem encerrou registrados) sem perder NADA do que já tinha — grupos, membros e etapas preenchidas continuam intactos, só deixam de ser "a execução em andamento". Execuções antigas, de antes dessa mudança, continuam funcionando normalmente mesmo sem número ou status (compatibilidade sem migração). Mesmo que a facilitadora clique duas vezes bem rápido (ou em duas abas) partindo da mesma execução atual, a disputa é por um LOCK separado (não pelo próprio "atual"): só quem vence esse lock cria a execução nova; o outro é abortado sem criar execução nenhuma e mostra um aviso próprio, sem travar nem quebrar a tela. "atual" só muda dentro do MESMO update() atômico que já cria a execução nova inteira e encerra a antiga — nunca antes disso — então uma falha no meio do caminho (rede caindo entre um passo e outro) nunca deixa "atual" apontando para uma execução que não existe: na pior hipótese, sobra um número de execução pulado ou o lock preso por alguns segundos, nunca um ponteiro quebrado; o lock expira sozinho se isso acontecer, sem exigir conserto manual no banco. O lock guarda um token próprio de cada tentativa: uma tentativa antiga que só descobre que falhou depois de o lock já ter expirado e sido assumido por outra NUNCA remove o lock dessa tentativa mais nova — só quem tem o token certo consegue liberar o lock. Em qualquer desses casos, nunca existem duas execuções com status "ativa" ao mesmo tempo, nem uma execução ativa que não seja a apontada por "atual". (10) Fase 2 — Histórico de Execuções: no painel do facilitador, o botão "Histórico de execuções" abre uma tela própria, separada, com "Execução atual" e "Execuções anteriores". Cada execução anterior mostra número (real, quando a execução já tem esse campo; senão, um número estimado pela ordem de criação, marcado como estimativa — nunca uma migração de dados antigos), status, início, encerramento, quem iniciou/encerrou e quantos grupos teve — inclusive execuções bem antigas, de antes desta fase, sem nenhum desses campos gravados (a execução apontada por "atual" sempre aparece como "Ativa"; todas as outras, como "Encerrada", mesmo as que nunca tiveram `status`). Clicar em "Ver esta execução" mostra os grupos daquela execução; clicar em "Ver mapa" de um grupo mostra o mapa dela, com o título "SOMENTE LEITURA" bem visível — os cards são apenas informativos (não têm o clique de editar que o mapa da execução atual tem), e nenhum botão de escrita (Salvar, Continuar, Revelar, Criar grupo, Iniciar nova execução) existe em tela nenhuma do histórico. "Exportar esta execução (CSV)" gera o CSV só daquela execução, nunca misturado com a atual. Em nenhum momento — abrir a lista, abrir uma execução, abrir um mapa histórico, exportar, ou fechar tudo — o ponteiro "atual" muda, e fechar o histórico não deixa nenhuma referência à execução vista para trás: reabrir a dinâmica ou entrar num grupo depois mostra a execução atual normalmente, nunca dados do histórico. (11) Fase 3 — proteção contra clique duplo: no painel do facilitador, "Criar grupo" desabilita o próprio botão na hora do clique, antes mesmo de a gravação terminar — um clique duplo rápido, ou vários cliques seguidos enquanto a criação ainda está pendente, criam só UM grupo (testável lendo `(document.getElementById("apostaCriarGrupo")||{}).disabled` logo após o clique). Se a gravação falhar, o botão reabilita sozinho (nunca fica preso) e mostra o aviso de erro, permitindo tentar de novo — a nova tentativa cria o grupo normalmente. As demais ações estruturais da dinâmica (entrar em grupo, avançar etapa, revelar conexões, salvar/remover missão-base, "Continuar"/"Voltar" em qualquer etapa) já eram protegidas antes desta fase, por gravarem num caminho fixo (idempotentes a um segundo clique) ou por já desabilitarem o botão antes de gravar — só "Criar grupo" precisou de correção. (12) Fase 4 — Ciclos de Aprendizagem: uma mesma aposta (um grupo) pode passar por vários CICLOS — cada volta de Problema→Mudanças→Hipótese→Ideia→Experimento→Evidência→Decisão — sem que isso tenha relação nenhuma com Execução (Fase 1): uma execução nova nunca é um ciclo novo, e um ciclo novo nunca cria execução. Enquanto um grupo nunca precisou de um segundo ciclo, tudo continua funcionando exatamente como antes — nenhum campo novo aparece, nenhuma migração acontece; a estrutura de ciclos só nasce de verdade no instante em que o primeiro Ciclo 2 é criado, e nesse momento o que já existia é congelado como "Ciclo 1", sem perder nada. Depois da Decisão confirmada: "Ampliar" e "Interromper esta ideia" só encerram o ciclo atual, sem criar nada; as outras cinco decisões abrem um Ciclo novo — "Reformular a hipótese" recomeça na Hipótese (com a Nova Hipótese que a Decisão coletou virando a hipótese do novo ciclo — a original nunca é reescrita), "Rever a mudança mensurável" recomeça em Mudanças mensuráveis, "Rever o problema" recomeça no Problema, e "Ajustar e testar novamente"/"Investigar mais" abrem uma pergunta própria ("O que precisa ser revisto antes do próximo teste?"/"O que precisamos investigar ou rever?") com opções para a dupla escolher onde recomeçar — sem escolher uma opção, CONTINUAR fica bloqueado de verdade, igual à Nova Hipótese. Tudo que vem ANTES do ponto de reinício escolhido é herdado do ciclo anterior (a dupla pode ajustar se quiser, mas não é obrigada) — e se ela de fato editar um campo herdado, tudo que vem DEPOIS dele no mesmo ciclo (inclusive o que a própria dupla já tinha preenchido neste ciclo novo) volta a "ainda não preenchida", o ponto de reinício efetivo passa a ser essa etapa editada, e um aviso conta o que foi limpo — nunca fica implicitamente válido em cima de um dado que acabou de mudar; o ciclo anterior nunca é tocado por essa revisão. tudo do ponto de reinício em diante nasce vazio, pedindo para ser preenchido de novo — quanto mais cedo o ponto de reinício, mais etapas seguintes ficam em branco outra vez. Missão e Sintoma nunca são ponto de reinício e não se repetem no Mapa a cada ciclo. Depois que a Decisão de um ciclo é confirmada, esse ciclo fica concluído: tentar voltar (pelo Mapa ou pela trilha) para editar qualquer etapa dele mostra a pergunta "Este ciclo já foi concluído. Deseja iniciar um novo ciclo a partir desta etapa?", com CANCELAR (não muda nada) e INICIAR NOVO CICLO (abre um ciclo novo a partir exatamente daquela etapa) — nunca reabre e regrava por cima do que já foi decidido. O Mapa da Aposta passa a mostrar uma seção por ciclo (numerado, com o ponto de reinício quando houver um) sempre que existir mais de um; só os cards do ciclo ATUAL são clicáveis — os de ciclos anteriores são só leitura (mesmo tratamento do Histórico de Execuções da Fase 2: nunca um <button>, nunca editável), e consultá-los não altera qual é o ciclo atual nem grava nada. O Histórico de Execuções (Fase 2) também passa a mostrar todos os ciclos de um grupo, sempre em somente leitura, mesmo em execuções antigas de antes desta fase (que continuam valendo como "Ciclo 1" sem precisar de nenhum dado novo). O CSV exportado ganha as colunas Ciclo e Ponto de reinício, para nunca misturar numa linha só o que são, na verdade, rodadas diferentes de aprendizado. Criar um Ciclo novo é uma ação estrutural: clique duplo, várias pessoas do mesmo grupo clicando quase ao mesmo tempo em abas diferentes, ou uma tentativa que falha no meio do caminho nunca podem resultar em dois ciclos sucessores para a mesma Decisão — a mesma proteção real de concorrência (lock com token próprio de cada tentativa) que a Fase 1 já usa para criar execuções. (13) Refinamentos pós-teste manual — validações e clareza de texto, sem mudar estrutura nenhuma: em Mudanças mensuráveis, além do alerta de direção invertida (Aumentar com meta menor, Reduzir com meta maior), agora bloqueia igual, sem escape por segundo clique, sempre que a Meta desejada for EXATAMENTE IGUAL à Situação atual em Aumentar ou Reduzir — a mensagem explica que a meta precisa ser maior (Aumentar) ou menor (Reduzir) que o valor atual, e sugere rever a Direção se a intenção for permanecer nesse nível; nem a direção nem a meta trocam sozinhas, é sempre a dupla quem corrige o número ou troca para Manter. "Manter" continua com a lógica própria de limite (Pelo menos/No máximo/Entre) — não virou meta igual a situação atual em lugar nenhum. Em "Revelar conexões", o bloco mudou de nome e de tom: o título agora é "VOCÊS CONECTARAM A APOSTA A UMA LÓGICA DE OKR" (nunca mais "é um OKR" nem "2 em 1"), com os cards "MISSÃO DA DINÂMICA → Objective" e "MUDANÇAS MENSURÁVEIS → Key Results" trazendo a missão e as mudanças mensuráveis de verdade do grupo; havendo mais de um ciclo, aparece explicitamente de qual ciclo vieram as Key Results (ex.: "FORMULAÇÃO ATUAL (CICLO 2)") — nunca deixa a dupla adivinhando; com um ciclo só, esse aviso de ciclo não aparece. No Mapa, o separador entre ciclos ("CICLO 1", "CICLO 2"...) ganhou hierarquia visual — número maior, ponto de reinício e status ("Concluído"/"Em andamento") em destaques próprios, mais fácil de notar batendo o olho — sem mudar estrutura nem dado nenhum que já existia. Na Evidência, enquanto o experimento ainda não foi executado, o resumo do resultado mostra "Situação inicial: X · Meta: Y · Resultado: aguardando execução" — nunca mais uma seta terminando num travessão solto ("→ —"), que parecia dado perdido; depois de um resultado observado, volta a mostrar a seta normal ("X → Y | Meta: Z"). O botão "Encerrar por agora" da Evidência ganhou um texto de apoio logo abaixo explicando que o plano já está salvo e que dá para voltar depois para registrar os resultados — a lógica do botão não mudou (nunca conclui a aposta, nunca cancela o experimento, nunca perde dado), só ficou mais claro na hora H que ninguém precisa perguntar o que ele faz. No Histórico de Execuções, o rótulo de execuções antigas sem número real deixou de dizer "Execução nº N (estimado pela ordem)", que parecia numeração de verdade regredindo, e passou a dizer "Execução legada — posição estimada N"; execuções com número real continuam como "Execução nº N", sem mistura visual entre as duas séries.' },
    { section: 'Admin',
      title: 'Construção da Aposta — Analisar com IA (Conselho Jedi: uma aposta, três lentes)',
      motivo: 'Nova funcionalidade: no Mapa da Aposta, o botão "🤖 Analisar com IA" (ao lado de "⧉ Copiar em texto"/"🖨 Salvar em PDF") gera três prompts prontos para copiar — 🛡️ A Voz que Defende, ⚠️ A Voz que Desafia, 🔎 A Voz que Investiga —, todos sobre a MESMA aposta do ciclo atual. É pensada para o Conselho Jedi trabalhando em duplas físicas na mesma sala: NÃO cria grupo, aposta, ciclo ou execução nenhuma — para o sistema é uma aposta só; as "três duplas" existem só fisicamente. (1) Com uma aposta completa (Missão, Sintoma, Problema, Mudanças mensuráveis, Hipótese, Ideia, Experimento planejado e ao menos uma Evidência planejada), abrir o Mapa e clicar no botão: abre um modal "Análise com IA" com um diagrama simples (UMA APOSTA → as três lentes) e três cards, cada um com a descrição da lente e os botões "Copiar prompt"/"Copiar mensagem para Teams / e-mail". (2) Em CADA um dos três prompts, conferir que aparecem os oito blocos — MISSÃO, SINTOMA, PROBLEMA, MUDANÇAS MENSURÁVEIS, HIPÓTESE, IDEIA DE SOLUÇÃO, EXPERIMENTO PLANEJADO e "[EVIDÊNCIAS PLANEJADAS — AINDA NÃO OBSERVADAS]" — com o texto exatamente como a dupla escreveu (nada reescrito, resumido ou completado); a Missão em especial tem de vir do cadastro REAL da aposta, nunca de um texto fixo de exemplo. (3) Ponto crítico: a Evidência aparece SEMPRE como "planejada" (indicador, situação inicial, meta, fonte planejada, como será medido) — NUNCA como resultado observado, classificação da hipótese ("Sustentada"/...) ou decisão, mesmo que o experimento já tenha sido executado de verdade; os três prompts trazem a frase "O experimento ainda não foi executado." e a instrução de nunca tratar essas evidências como fato observado, resultado, comprovação ou validação da hipótese. (4) A aposta-base (tudo entre o contexto comum e a lente) é IDÊNTICA nos três prompts — só a seção "[SUA LENTE — ...]" muda, cada uma com sua própria pergunta (Defende: valor potencial, conexão lógica, premissas críticas, sinais futuros, qualidade das evidências; Desafia: premissas frágeis, pontos cegos, riscos, consequências indesejadas, o experimento contraria a hipótese?, evidências revelariam erro?; Investiga: incertezas, informações faltantes, a hipótese é investigável?, avalia o experimento existente antes de propor outro, o que as evidências vão ensinar, menor ajuste necessário). (5) Os três terminam pedindo exatamente "TRÊS AFIRMAÇÕES CENTRAIS", mencionam explicitamente ACEITAR/QUESTIONAR/REJEITAR como as opções que o Conselho Jedi vai escolher depois, e fecham com "DECIDIR CONTINUA SENDO RESPONSABILIDADE DO CONSELHO JEDI." — a IA nunca decide pelo grupo. (6) "Copiar prompt" copia só o prompt; "Copiar mensagem para Teams / e-mail" copia um texto maior identificando a lente e instruindo a dupla a não alterar a aposta-base antes de enviar, com o prompt completo dentro; os dois mostram um retorno curto ("Prompt copiado."/"Mensagem copiada.") sem alert bloqueante — se o navegador recusar a permissão de área de transferência, cai no mesmo modal de "selecione e copie" que "Copiar em texto" já usa. (7) Com a aposta-base INCOMPLETA (faltando qualquer um dos itens do passo 1), o botão mostra "Complete a aposta-base antes de gerar os prompts para IA." com a lista objetiva do que falta — nunca inventa conteúdo nem gera um prompt aparentemente completo; fechar esse aviso não deixa nenhum modal preso, e o resto do site continua navegável normalmente. (8) Com a aposta em mais de um Ciclo, a exportação usa o CICLO ATUAL — Problema, Mudanças mensuráveis, Hipótese, Ideia, Experimento e Evidência planejada do ciclo em construção, nunca misturados com os do(s) ciclo(s) anterior(es) (a Missão continua igual, contexto estável fora dos ciclos). (9) O texto exportado nunca traz e-mail, nome individual, ID de grupo/execução nem timestamp técnico — só o conteúdo da aposta em si. (10) Nada nesta funcionalidade chama uma API de IA, envia dado nenhum para fora do navegador ou grava resposta de IA automaticamente no sistema — é só gerar, mostrar e copiar; enviar o prompt para a ferramenta de IA usada pela dupla é decisão e ação humana, fora do site.' },
    { section: 'Admin',
      title: 'Turmas — público restrito: matricular a lista inteira de uma vez',
      motivo: 'Continuação do teste acima. Na turma fechada, abrir "👥 Público restrito desta turma" com gente na lista que ainda NÃO está na turma. (1) Aparece o botão "＋ Adicionar à turma as N que ainda não estão", com N igual ao número de linhas marcadas "—" na coluna "Na turma" — quem já está na turma não é contada. (2) Clicar: a confirmação diz quantas entram, avisa que entram como INSCRITAS, e lista NOMINALMENTE quem fica de fora por não estar no público restrito do EVENTO (estar só na lista da turma não basta) e quem já é inscrita em outra turma, cuja inscrição anterior será removida. (3) Confirmar: as pessoas passam a aparecer como "Inscrita" na coluna "Na turma", o cabeçalho da turma passa a contá-las em "confirmados", e elas ganham acesso a Conteúdos/Treinamento — se alguma ficar "Confirmação incompleta", o registro nasceu pela metade e é bug. (4) Quem o evento barrou NÃO pode ter sido gravada. (5) Com a lista inteira já na turma, o botão some (não há ninguém a adicionar). (6) Cancelar na confirmação não grava nada.' },
    { section: 'Admin',
      title: 'Turmas — público restrito: as portas do painel recusam quem está fora da lista',
      motivo: 'Continuação do teste acima, com a turma fechada e só a pessoa A na lista. (1) "＋ Participante": conferir o aviso lilás no topo do modal dizendo que a turma tem público restrito, e que a busca SÓ encontra A — digitar parte do nome de B não traz ninguém, e a resposta tem que ser "Ninguém com esse nome na lista de quem pode participar", NÃO "Nenhum cadastro encontrado": B tem cadastro, e culpar o cadastro manda procurar o problema no lugar errado. (2) Ainda nesse modal, clicar no botão "👥 Abrir a lista desta turma" dentro do próprio aviso — ele fecha o "＋ Participante" e abre a lista da turma direto, sem precisar caçar o botão no menu ⋯. Incluir B ali, fechar, voltar ao "＋ Participante" e conferir que B já aparece na busca na hora, sem esperar a aba recarregar; adicionar B como "Interessada". (2b) Conferir o outro caminho até a mesma lista: clicar no selo "👥 PÚBLICO RESTRITO DESTA TURMA · N" no cabeçalho da turma — ele é um botão, abre a lista e NÃO recolhe o card da turma embaixo do modal. (3) Remover B da lista (o modal avisa que ela está NA TURMA e que continuará participando) e conferir que o card passa a mostrar o aviso com o NOME de B: está na turma sem estar na lista. (4) Com B nessa situação, clicar em "Confirmar" na linha dela: tem que recusar com a mensagem que manda incluir no público restrito primeiro, e NADA pode ser gravado (B continua "Interessado"). (5) Pôr B de volta na lista e confirmar de novo: agora funciona. (6) Na Lista de Espera do evento, escolher essa turma no select e clicar "Mover para turma" para alguém que não está na lista: tem que recusar com a mesma mensagem, sem gravar. (7) Marcar "Público restrito" numa turma que JÁ tem participantes e conferir que ninguém foi removido — todos continuam na tabela, e o aviso lista os nomes de quem ficou fora da lista.' },
    { section: 'Admin',
      title: 'Eventos — qualquer admin de verdade consegue criar/editar evento, não só os dois super-admins',
      motivo: 'A escrita de eventos/$eventoKey chegou a ficar restrita só aos dois super-admins nas regras do banco, por engano — "+ Novo evento"/"Editar evento" continuavam abertos para qualquer admin no painel, que via "Erro ao salvar. Tente novamente." (não silencioso, mas enganoso: sugere falha passageira, quando era permanente). Não automatizado porque grava dados reais em eventos. Logado como um admin comum (que não é um dos dois super-admins), abrir "+ Novo evento", preencher nome e carga horária e salvar — tem que criar de verdade (aparece na lista, reabrir "✎ Editar evento" mostra os dados); repetir editando um campo de um evento já existente. Ver skill editar-e-salvar.' },
    { section: 'Admin',
      title: 'Eventos — público restrito do evento: a lista vale para todas as turmas dele',
      motivo: 'Mesma ideia do público restrito da turma, um nível acima — requer duas contas de teste além da sua (A e B, as duas cadastradas) e um evento publicado com DUAS turmas com interesse aberto. (1) Em "✎ Editar evento", marcar "Visível só para um público predefinido (lista de e-mails)" e salvar — o cabeçalho do evento ganha o selo "👥 PÚBLICO RESTRITO · 0" e o botão "👥 Público restrito"; conferir que nome, carga horária, frequência mínima e o conteúdo da Missão continuam iguais. (2) Abrir "👥 Público restrito": a lista está vazia e o texto avisa que, vazia, o evento não aparece para ninguém. Incluir A. (3) Logar como A: o evento aparece na página Turmas com as DUAS turmas, o card de lista de espera e o bloco "A Missão" dele. (4) Logar como B: o evento INTEIRO some — título, as duas turmas, o card de espera e a Missão —, e conferir também que nenhuma das turmas aparece no resumo "Turmas abertas no momento" da Minha Área de B. (5) Como admin, conferir que VOCÊ continua vendo o evento mesmo sem estar na lista. (6) Outro evento, sem a caixa marcada, continua aparecendo normalmente para B — o controle é por evento. (7) Desmarcar a caixa: o evento volta a aparecer para B, e remarcar reencontra a mesma lista — desmarcar não apaga ninguém. (8) Conferir que é independente do público restrito da TURMA: com os dois marcados, quem está só na lista do evento continua sem ver a turma restrita, e quem está só na lista da turma não vê o evento.' },
    { section: 'Admin',
      title: 'Eventos — público restrito do evento: as portas do painel e do site recusam quem está fora',
      motivo: 'Continuação do teste acima, com o evento restrito e só A na lista. (1) "＋ Participante" numa turma desse evento: conferir o aviso lilás dizendo que o EVENTO tem público restrito, e que a busca só encontra A. (2) Incluir B na lista do evento, adicionar B como "Interessada" numa turma, e depois remover B da lista do evento: o card da turma passa a mostrar o aviso com o NOME de B dizendo que ela está na turma sem estar no público restrito do evento, e mandando incluir no cabeçalho do evento. (3) Com B nessa situação, "Confirmar" na linha dela tem que recusar com a mensagem que cita o EVENTO (não a turma) e não gravar nada. (4) Mesma recusa em "Mover para turma" a partir da Lista de Espera. (5) Deixar a página Turmas de A aberta, remover A da lista pelo painel e, sem recarregar, clicar "♡ Tenho interesse" na tela de A: tem que recusar dizendo que o evento é de público restrito, sem gravar — a checagem é refeita no clique, não confiando no que a página leu ao abrir. (6) Mesmo teste com o botão "Entrar na lista de espera" do evento: mesma recusa. (7) Marcar a caixa num evento que JÁ tem gente inscrita e conferir que ninguém perdeu acesso — Conteúdos e Treinamento continuam abertos para quem já estava inscrita, e a Minha Área dela continua mostrando a turma e o evento normalmente.' },
    { section: 'Admin',
    ...
    { section: 'Admin',
      title: 'Turmas — cabeçalho, filtro, sorteio e linha concordam sobre quem está confirmada',
      motivo: 'Requer uma turma com pelo menos 1 pessoa confirmada. Conferir que os quatro lugares do painel dão a MESMA resposta: (1) o "N confirmados" do cabeçalho do card; (2) a contagem do botão de filtro "Confirmados (N)"; (3) o número que o modal de sorteio diz estar participando; (4) quantas linhas da tabela aparecem com status Inscrito. Para provar de verdade, apagar só o confirmedByAdmin de uma pessoa no Firebase e recarregar: ela tem que sair dos quatro ao mesmo tempo e aparecer com o selo "Confirmação incompleta". Este teste existe porque os quatro já divergiram — o cabeçalho contava só pelo status e dizia "2 confirmados" onde só uma linha aparecia como Inscrito, e o sorteio incluía quem a tabela mostrava como pendente.' },
    { section: 'Admin',
      title: 'Documentação viva confere com o que está implementado',
      motivo: 'AUTOMATIZADO NO CI, não aqui: .github/scripts/teste-consistencia-docs.js roda em toda PR e compara o que Manual e Mapa AFIRMAM com o que o código faz — contagem e nomes das 15 abas, rotas do router contra as seções do index.html e contra a lista do CLAUDE.md, todo nó do banco usado pelo código presente na descrição do Mapa, nó órfão nas regras obrigado a estar marcado como legado, contagem de estruturas do banco batendo com a lista, todo .js de forca-agil/ documentado (e nenhum arquivo fantasma citado), e nenhum resquício de funcionalidade removida. Nasceu de uma auditoria em 10/09/2026 que achou 8 divergências reais, entre elas: o painel com 15 abas e a documentação dizendo 14 em três lugares (e 15 num quarto, contradizendo a si mesma), a aba "Tipos de atividade" e o nó roteiro-tipos-atividade documentados em lugar nenhum, e o Mapa afirmando 14 seções no index.html quando são 11. Ao REMOVER uma funcionalidade, acrescente o nome dela na lista REMOVIDAS do script — o CI passa a garantir que nenhum texto continua prometendo o que não existe mais.' },
    { section: 'Admin',
      title: 'Modais que leem o banco abrem no tamanho certo, sem piscar',
      motivo: 'Vale para os três modais que carregam dados antes de ter conteúdo: Roteiro de Facilitação do evento (menu ⋯ do card do evento), Roteiro da turma e Equipe de facilitação. Em cada um, abrir e observar a caixa: ela deve aparecer já perto do tamanho final, com \'Carregando…\' centralizado, e NÃO dar um salto de tamanho quando os dados chegam. Para ver o defeito antigo com clareza, estrangular a rede (DevTools → Network → Slow 3G) antes de abrir — sem a correção, a caixa abria com menos de um terço da altura final (170px contra 606px no computador) e, por ser centralizada, crescia para os dois lados, parecendo que um modal menor tinha piscado por baixo. Conferir nos DOIS formatos de tela: no celular o efeito era igualmente visível (274px contra 654px).' },
    { section: 'Admin',
      title: 'Turmas — os quatro caminhos para "Inscrita" gravam o mesmo registro',
      motivo: 'Requer 4 pessoas de teste. Fazer cada uma virar Inscrita por um caminho diferente: (a) ela clica "Tenho interesse" e o admin clica "Confirmar"; (b) o admin usa "＋ Participante" e escolhe "Inscrita"; (c) ela está na lista de espera do evento e o admin usa "Mover para turma"; (d) numa turma de público restrito, ela está na lista e o admin usa "＋ Adicionar à turma as N que ainda não estão", dentro de "👥 Público restrito desta turma". Verificar que nos QUATRO casos o registro em turmas-interesse fica com status "inscrito" E confirmedByAdmin preenchido, que nenhuma mostra o selo "Confirmação incompleta" no painel, e que as quatro acessam Conteúdos/Treinamento e veem a Minha Área como confirmada. O caminho por onde a pessoa entrou não pode fazer diferença nenhuma para ela.' },
    { section: 'Admin',
    ...
    { section: 'Admin',
      title: 'Turmas — lista de presença para impressão só traz inscritos',
      motivo: 'Numa turma com pessoas Interessadas e Inscritas, clicar em "🖨 Lista de presença" no card da turma (menu "⋯" no mobile). Verificar: abre uma nova aba com uma tabela; só aparecem as pessoas com status Inscrito — quem só demonstrou interesse não entra, porque nunca pôde ter check-in. Testar também numa turma sem nenhum Inscrito: o botão deve avisar que não há o que gerar, sem abrir aba nenhuma.' },
    { section: 'Admin',
    ...
    { section: 'Admin',
      ...
      motivo: 'Abrir a aba Dashboard. Escolher manualmente 2-3 avaliações conhecidas (via aba Eventos ou Firebase Console) e conferir: (1) "Participantes" bate com a soma real de inscritos confirmados; (2) "Avaliações recebidas" bate com a contagem real em avaliacoes/; (3) "Média geral" bate com a média manual dos campos notaGeral; (4) o gráfico "Média por evento e turma" mostra uma barra por turma, agrupada sob o evento dela, com o nome INTEIRO legível e na altura correta; (5) a "Distribuição das notas" soma 100% entre as 5 faixas (0-2, 3-4, 5-6, 7-8, 9-10), todas de duas notas exceto a primeira, e a contagem embaixo é a de quem respondeu a pergunta da nota geral — que pode ser menor que o total de avaliações; (6) "Temas mais solicitados" reflete o que foi marcado no checkbox da seção 8 do formulário de Avaliação. Se não houver nenhuma avaliação ainda, verificar que os cards e gráficos continuam visíveis com valores em 0 (não somem atrás de uma tela vazia) e que aparece o aviso amarelo "Nenhuma avaliação recebida ainda..." no topo; o gráfico deve mostrar TODAS as turmas do escopo com um travessão (—) e "sem avaliação" — nunca nota 0, que é a pior nota possível e não é o que "ninguém respondeu" significa.' },
    { section: 'Admin',
      title: 'Dashboard — escopo por evento e por turma',
      motivo: 'Precisa de pelo menos dois eventos com turmas e avaliações. Enquanto existia uma oficina só, somar tudo dava no mesmo; a partir da segunda, uma média agregada errada é pior que número nenhum, porque parece certa. Conferir no CELULAR e no computador: (1) os filtros do topo são dois controles ("Evento" e "Turma") que abrem uma lista ao clicar, e a lista de turmas traz só as turmas dos eventos escolhidos — com muitos eventos e turmas o filtro NÃO pode crescer: ele tem tamanho fixo, com busca acima de oito opções e rolagem própria; (2) escolher UM evento e conferir que Participantes, Avaliações, Média, NPS, distribuição, destaques, temas, feedbacks e as Respostas individuais passam TODOS a ser só dele — inclusive o "% de participação", que é dividido pelos inscritos daquele escopo; (3) escolher uma turma dentro dele e repetir; (4) marcar dois eventos, e depois duas turmas, ao mesmo tempo — a seleção é múltipla de propósito, é assim que se comparam duas turmas; (5) marcar um evento e uma turma de OUTRO evento: a turma tem que deixar de valer sozinha, sem virar um filtro que não devolve nada; (6) "limpar filtro" volta a somar tudo. O automático equivalente é .github/scripts/teste-dashboard-escopo.js, que roda desktop e iPhone no CI com dois eventos e quatro turmas, com notas escolhidas para que cada escopo tenha uma média diferente — se o filtro parar de filtrar, a conta não bate.' },
    { section: 'Admin',
    ...
    /* ── Minha Área ───────────────────────────────────────────── */
{ section: 'Minha Área',
      title: 'Admin: "Ver esta tela como" mostra a tela da pessoa certa (escolhendo cada situação)',
    ...
    { section: 'Minha Área',
      ...
      motivo: 'Cada estado precisa ser conferido com uma conta na situação correspondente. (1) NUNCA INTERAGIU — conta sem interesse em nenhuma turma e fora da lista de espera: deve ver a tela de boas-vindas com a lista das turmas abertas e o botão para Turmas; se não houver turma aberta, a mensagem dizendo isso. (2) INTERESSE EM ANÁLISE — manifestar interesse numa turma e NÃO confirmar pelo admin: deve aparecer o bloco "Inscrições em análise" com a data do interesse. Confirmar pelo admin e recarregar: o cartão sai de "em análise" e vira turma em "Minhas turmas". (3) LISTA DE ESPERA — mover a pessoa para a lista de espera pelo admin: deve aparecer o bloco "Lista de espera" dizendo desde quando, de qual turma veio E o nome do EVENTO daquela fila (ver teste dedicado "Lista de espera diz de qual evento, um cartão por fila"). (4) COEXISTÊNCIA — deixar a mesma conta confirmada numa turma, em análise em outra e na espera: os três blocos devem aparecer juntos, e a tela de boas-vindas NÃO deve aparecer. Em nenhum dos casos a área pode ficar em branco ou dar erro no console.' },
    { section: 'Minha Área',
      title: 'Contagem por fase e lista de espera dizem de qual evento — um cartão por fila',
      motivo: 'AUTOMATIZADO NO CI: .github/scripts/teste-minha-area-fila.js roda em toda PR, desktop e iPhone. Dois defeitos relatados juntos: o selo ao lado de "Em andamento"/"Programadas"/"Concluídas" mostrava só o número, sem dizer o que contava; e "Lista de espera" mostrava um único cartão genérico, sem nome de evento — pior, fa-espera guarda uma entrada por ORIGEM (uma por evento em que a pessoa clicou "Entrar na lista de espera"), e a tela só lia a mais antiga, então quem esperava por dois eventos só via um cartão, sem saber que o segundo existia. Corrigido: o selo agora escreve "N turma"/"N turmas" por extenso, e a lista de espera mostra um cartão por evento ativo, cada um com o nome dele. Pra conferir com os olhos: colocar a mesma conta na lista de espera de DOIS eventos diferentes (clicar "Entrar na lista de espera" nos cards de dois eventos distintos na página Turmas) e abrir Minha Área — devem aparecer DOIS cartões em "Lista de espera", cada um com o nome do evento certo, não um cartão genérico só.' },
    { section: 'Minha Área',
    ...
    { section: 'Admin',
      title: 'Equipe de facilitação — único responsável, com confirmação ao substituir',
      motivo: 'Não automatizado porque grava dados reais em turmas-equipe (a suíte automática nunca escreve na base de produção). Numa turma de teste: (1) adicionar Facilitador A como Responsável — ⭐ aparece só nele; (2) adicionar Facilitador B como Responsável — o sistema deve perguntar "Esta turma já possui A como facilitador responsável. Deseja substituir por B?" antes de trocar; ao confirmar, A vira facilitador de apoio automaticamente (nunca some da equipe, nunca fica sem responsável); (3) tentar adicionar A de novo à mesma turma — o seletor não deve oferecê-lo, já que está na equipe; (4) remover B da equipe — a turma fica sem responsável (⭐ nenhum) até alguém ser definido de novo; o cadastro de B na aba Facilitadores continua intacto. (5) Inativar um facilitador na aba Facilitadores e confirmar que ele some do seletor de "+ Adicionar facilitador" de qualquer turma nova, mas continua aparecendo nas equipes onde já estava.' },
    { section: 'Admin',
      title: 'Roteiro-base do evento — dias, atividades e reordenação persistem',
      motivo: 'Não automatizado porque grava dados reais em roteiros-evento. Num evento de teste: (1) criar 2 dias e, em cada um, 2+ atividades com horários; (2) preencher só início + duração e conferir que o fim é calculado sozinho (e vice-versa nas outras combinações); (3) reordenar com ▲▼ e recarregar a página — a ordem deve persistir; (4) duplicar uma atividade — a cópia nasce com "(cópia)" no título, mesmo dia, ordem ao final; (5) excluir um dia com atividades — confirmar o aviso, e depois verificar que nenhuma turma deste evento ainda mostra as atividades excluídas (nem nas Personalizações antigas que apontavam pra elas).' },
    { section: 'Admin',
      title: 'Roteiro-base — qualquer admin de verdade consegue gravar, e uma escrita negada mostra erro em vez de sumir em silêncio',
      motivo: 'roteiros-evento/$eventoKey é gravável por qualquer admin de verdade (os dois super-admins ou quem está em fa-admins) — chegou a ficar restrito só aos dois super-admins por um erro de cópia nas regras do banco, o que deixava outro admin abrir e editar o formulário normalmente e ter a gravação recusada em silêncio. (1) Logado como um admin comum (não super-admin), abrir o Roteiro-base de um evento, editar um campo qualquer de uma atividade e salvar — tem que gravar de verdade (reabrir e conferir o valor novo), não só fechar o modal. Não automatizado porque grava dados reais em roteiros-evento. (2) Separadamente, com o Firebase falso configurado pra negar a escrita (window.__CFG.fail — cobre qualquer outra causa de falha, tipo rede), apagar o conteúdo de "Objetivo" e salvar: deve aparecer um alerta "Não foi possível salvar…", nunca o modal fechando em silêncio como se tivesse dado certo, e reabrir a atividade deve mostrar o texto ORIGINAL, nunca vazio nem pela metade; o mesmo vale para criar/excluir/mover/duplicar atividade ou dia, personalizar/restaurar/remover por turma e salvar condução.' },
    { section: 'Admin',
      title: 'Roteiro da turma — personalização por campo, exclusiva e restauração',
      motivo: 'Não automatizado porque grava dados reais em turmas-roteiro. Numa turma com roteiro-base já herdado: (1) editar só a duração de uma atividade "apenas nesta turma" — selo "Alterado nesta turma" aparece, e a mesma atividade em OUTRA turma do mesmo evento continua com a duração original; (2) no roteiro-base, mudar o objetivo dessa mesma atividade — a turma personalizada deve mostrar a duração personalizada JUNTO com o objetivo novo (herda o que não alterou); (3) "Restaurar padrão do evento" com confirmação — a atividade volta a ser idêntica ao roteiro-base, sem selo; (4) "+ Adicionar atividade somente nesta turma" — selo "Exclusivo desta turma" azul, e a atividade não aparece em nenhuma outra turma nem no roteiro-base; (5) "Remover desta turma" numa atividade padrão — ela sai da lista normal e aparece em "Atividades removidas desta turma", de onde "Restaurar" a traz de volta; (6) mudar o horário de uma atividade da turma e conferir que a lista se reordena sozinha pelo novo horário; (7) em "Condução principal"/"Apoio", só as pessoas da equipe DAQUELA turma aparecem como opção, e marcar a mesma pessoa nos dois campos deve ser bloqueado com aviso.' },
    { section: 'Facilitador',
      title: '"Minhas Facilitações" — só as turmas da própria pessoa, papel correto e roteiro editável só para a responsável',
      motivo: 'Não automatizado porque depende de duas contas reais (uma responsável, uma facilitadora de apoio) e de escrever equipe/roteiro. Login com a conta de teste de facilitador (equipe de duas turmas, responsável numa e apoio na outra): (1) a página #facilitador mostra as duas turmas, agrupadas pela fase certa (Em andamento/Programadas/Concluídas), e nenhuma outra turma do sistema; (2) o card da turma onde é responsável mostra "⭐ Você é o responsável por esta turma"; o outro mostra "Você participa como facilitador"; (3) "Abrir roteiro" na turma onde é responsável mostra botões de editar/restaurar/remover/adicionar; na outra, mostra os mesmos dados mas SEM nenhum botão de edição; (4) logar como admin sem estar em nenhuma equipe — a página aparece vazia, sem listar turmas de terceiros.' },
    { section: 'Admin',
      title: 'Roteiro — resumo do dia bate com a matemática (janela = programado + lacunas)',
      motivo: 'Não automatizado porque depende de um roteiro real com horários específicos para conferir a conta na calculadora. Montar um dia com: 3 atividades normais, 1 intervalo de 30 min, e um buraco proposital de horário entre duas atividades (ex: uma termina 11:30, a próxima só começa 15:00). Conferir: (1) "Janela do dia" = do início da primeira à conclusão da última; (2) "Lacunas reais" aparece com o valor do buraco (3h30 no exemplo) e uma faixa "⚠ Lacuna no planejamento" aparece na lista, entre as duas atividades, sem numeração e sem entrar na contagem de atividades; (3) "Pausas" = só a duração do intervalo; (4) "Tempo de facilitação" = "Tempo programado" menos "Pausas"; (5) na calculadora: Janela = Programado + Lacunas reais, e Programado = Facilitação + Pausas — os dois devem fechar exatamente.' },
    { section: 'Admin',
      title: 'Roteiro — seção com sub-etapas: formulário completo, duração somada, numeração e recolher/expandir',
      motivo: 'Não automatizado porque grava dados reais. Numa atividade de nível principal, clicar "+ Etapa" 3 vezes (durações 1, 3 e 2 min) e conferir que cada etapa abre o MESMO formulário completo de qualquer atividade (descrição, objetivo, passo a passo, materiais etc. — não só título/duração). Conferir: (1) a atividade-mãe passa a mostrar uma linha "Tipo · 6min · 3 etapas" abaixo do título, com a duração exibida na lista sendo 6 (a soma), mesmo com o campo Duração dela própria ainda guardando outro valor internamente; (2) um botão "▲ Recolher" aparece na linha da mãe; clicar recolhe as etapas, clicar de novo ("▼ Ver etapas") mostra de volta; (3) as etapas aparecem visualmente mais discretas (recuo maior, linha vertical fina à esquerda, fundo levemente diferente, botões de ação menores), numeradas 4.1, 4.2, 4.3 (usando a posição da seção-mãe — se a seção virar 5, elas viram 5.1, 5.2, 5.3 sozinhas), com sua própria coluna de horário quando têm horário definido, e SEM NENHUM "—" quando não têm (a coluna simplesmente some) — e sem entrar na ordenação cronológica do dia nem no aviso de ordem divergente; (4) editar a duração de uma etapa e conferir que, se a seção-mãe tiver horário de início, o horário de FIM dela é recalculado sozinho (soma nova) e, havendo atividades seguintes com horário na mesma sessão, aparece a mesma pergunta de recalcular/manter horários já usada pra atividades de nível principal; (5) reordenar etapas com ▲▼ nunca move uma etapa pra fora da seção onde ela está; (6) no roteiro da turma, abrir a atividade-mãe mostra as mesmas etapas dentro do corpo expandido, e "+ Etapa (só nesta turma)" cria uma etapa exclusiva da turma mesmo quando a mãe é do roteiro-base; (7) excluir a atividade-mãe (roteiro-base ou exclusiva) apaga as etapas junto, com confirmação avisando disso; duplicar a mãe duplica as etapas junto.' },
    { section: 'Admin',
      title: 'Roteiro-base — recalcular horários seguintes ao mudar duração, e aviso de ordem divergente',
      motivo: 'Não automatizado porque grava dados reais e altera múltiplas atividades em cadeia. (1) Com um dia de 3+ atividades em sequência com horário, editar a duração da primeira (ex: de 15 para 25 min) — deve aparecer a pergunta "Deseja recalcular automaticamente o horário das N atividade(s) seguinte(s)?"; escolher "Recalcular" e conferir que TODAS as atividades seguintes daquele dia tiveram início e fim deslocados pela mesma diferença (10 min no exemplo); repetir escolhendo "Manter horários" e conferir que nada mudou além da atividade editada. (2) Trocar manualmente o horário de uma atividade para um valor fora de ordem (ex: colocar a 3ª atividade às 09:05, antes da 1ª) e conferir que aparece o aviso "Existem atividades fora da ordem cronológica" com o botão "Ordenar por horário", que reorganiza a lista pelos horários ao ser clicado.' },
    { section: 'Admin',
      title: 'Roteiro — "Roteiro completo" mostra só os campos preenchidos, nunca uma mensagem de "vazio"',
      motivo: 'Não automatizado porque abre uma janela nova e depende de conteúdo preenchido nas atividades. Num dia com pelo menos: uma atividade com vários campos preenchidos (ex: objetivo e materiais, mas sem dicas para o facilitador), uma atividade sem nenhum campo de facilitação preenchido (só título e duração), e uma seção com sub-etapas. Clicar "🖨 Roteiro completo" (roteiro-base e roteiro da turma) e conferir: (1) abre uma janela nova com um bloco por atividade, numerado igual à tela (1, 2, 3... e sub-etapas como 4.1, 4.2); (2) na atividade com campos parciais, aparecem só os rótulos dos campos preenchidos (ex: "Objetivo", "Materiais necessários") — o rótulo de um campo vazio (ex: "Dicas para o facilitador") NÃO aparece; (3) a atividade sem nenhum campo preenchido imprime só o título e a duração, sem NENHUMA mensagem tipo "Nenhum campo preenchido" no lugar — isso nunca deve aparecer, em hipótese alguma; (4) uma sub-etapa sem horário próprio não imprime "—" na linha de horário, só a duração; (5) cada sub-etapa impressa traz "[título da seção-mãe] — continuação" antes do próprio título, útil se a seção quebrar entre páginas; (6) o resumo do dia (janela, programado, facilitação, pausas, lacunas, atividades) aparece no topo igual ao "🖨 Agenda resumida"; (7) o botão "Imprimir / salvar como PDF" dentro da janela aciona o diálogo nativo do navegador.' },
    { section: 'Admin',
      title: 'Roteiro — "Passo a passo" mostra só esse campo, sem os demais campos internos',
      motivo: 'Não automatizado porque abre uma janela nova e depende de conteúdo preenchido nas atividades. Numa atividade com Objetivo, Passo a passo, Dicas para o facilitador e Materiais todos preenchidos, e uma seção com sub-etapas (algumas com "Passo a passo" preenchido, outras sem nada), clicar "🖨 Passo a passo" (roteiro-base e roteiro da turma) e conferir: (1) abre uma janela nova com o mesmo formato de bloco por atividade do "🖨 Roteiro completo" (numeração, resumo do dia no topo, contexto "— continuação" nas sub-etapas); (2) em CADA bloco só aparece o rótulo "Passo a passo" (quando preenchido) — Objetivo, Dicas para o facilitador, Conexão com a agilidade, Materiais necessários, Preparação prévia e Observações NÃO aparecem em bloco nenhum, mesmo quando têm conteúdo; (3) uma atividade/etapa sem "Passo a passo" preenchido (mesmo tendo outros campos preenchidos) imprime só título e duração, sem nenhum campo abaixo; (4) o texto do rodapé da janela diz "Roteiro — passo a passo (sem os demais campos internos)"; (5) comparar lado a lado com "🖨 Roteiro completo" da mesma atividade — mesmo título/horário/duração, mas só o "Roteiro completo" mostra os outros campos.' },
    { section: 'Admin',
      title: 'Roteiro — formatação básica (negrito, itálico, sublinhado, listas, alinhar) nas caixas de texto da atividade',
      motivo: 'Não automatizado porque depende de seleção de texto e comandos de formatação do navegador (document.execCommand), que só funcionam com interação real. No formulário de atividade (Objetivo, Passo a passo, Dicas, Conexão com a agilidade, Materiais, Preparação prévia, Observações — todas menos Título), cada caixa tem uma barrinha com os botões N (negrito), I (itálico), S (sublinhado), • Lista, 1. Lista, Centralizar, Justificar, ↵ Espaço, "A" (cor do texto) e o ícone de caneta (cor de fundo/destaque). Selecionar um trecho de texto e clicar em cada botão; conferir que o efeito é aplicado (visualmente em negrito/itálico/sublinhado, ou virando marcador/numeração/alinhamento) e que o botão pode ser clicado de novo para desfazer. Nos botões "A" e caneta, clicar abre o seletor de cor nativo do navegador; escolher uma cor e conferir que ela é aplicada só ao trecho selecionado, sem perder a seleção nem fechar o formulário. Salvar a atividade e reabrir o formulário de edição — a formatação (inclusive as cores) deve continuar lá exatamente como foi deixada. Depois, sem entrar em edição, expandir a atividade na tela de leitura (roteiro da turma) e clicar em "🖨 Roteiro completo": a mesma formatação (negrito, listas, alinhamento, cores) deve aparecer igual nos dois lugares. Testar também "Materiais" (que vira lista com marcadores automaticamente, um item por linha) — adicionar um item em negrito e conferir que ele aparece em negrito na lista, tanto na tela quanto na impressão. Por fim, abrir uma atividade cadastrada ANTES desta funcionalidade (texto puro, sem formatação) e conferir que o texto antigo aparece normalmente na caixa, editável, sem símbolos estranhos.' },
    { section: 'Admin',
      title: 'Roteiro — campos "Resultado esperado" e "Prompt para IA"',
      motivo: 'Não automatizado porque grava dados reais e abre uma janela nova. Abrir "+ Adicionar atividade" (ou editar uma existente) e conferir: (1) o bloco "Propósito" mostra "Resultado esperado" logo abaixo de "Objetivo", com a mesma barra de formatação dos outros campos; (2) existe um bloco "IA" entre "Como conduzir" e "Recursos", com um único campo "Prompt para IA"; (3) preencher só "Resultado esperado" (deixando "Prompt para IA" vazio), salvar, reabrir o formulário — o texto continua lá; (4) expandir a atividade na tela de leitura (roteiro da turma) — aparece "Resultado esperado" com o conteúdo, mas nenhum rótulo "Prompt para IA" (campo vazio não aparece); (5) clicar "🖨 Roteiro completo" — o bloco da atividade mostra "Resultado esperado" na ordem Objetivo → Resultado esperado → Passo a passo → Dicas para o facilitador → (Prompt para IA, se preenchido) → Conexão com a agilidade → Materiais → Preparação prévia → Observações, e nenhum rótulo pra "Prompt para IA" já que está vazio; (6) editar de novo e preencher também "Prompt para IA" com um texto longo (várias linhas) — salvar e clicar "🖨 Roteiro completo" de novo: agora "PROMPT PARA IA" aparece logo depois de "Dicas para o facilitador", com o conteúdo completo; (7) clicar "🖨 Agenda resumida" e "🖨 Passo a passo" na mesma atividade — nenhum dos dois novos campos aparece em nenhum dos dois (Agenda resumida é só tabela; Passo a passo só mostra o campo Passo a passo). Por fim, conferir que uma atividade com "Resultado esperado" e "Prompt para IA" vazios (o caso comum, atividades antigas) não mostra nenhum resquício desses dois campos em lugar nenhum — nem título vazio, nem placeholder.' },
    { section: 'Admin',
      title: 'Roteiro — botões de recuo e seletor de tamanho de fonte nas caixas de texto',
      motivo: 'Não automatizado porque depende de seleção de texto e comandos de formatação do navegador. Numa caixa de texto do formulário de atividade (ex: Passo a passo), digitar um parágrafo e clicar "→ Recuo": o parágrafo inteiro (mesmo sem estar numa lista) desloca pra direita; clicar de novo aumenta mais o recuo; "← Recuo" desfaz um nível por vez até voltar ao normal. Testar dentro de uma lista (criada pelos botões "• Lista"/"1. Lista") também — o comportamento de recuo/desrecuo de item de lista já existia (via Tab/Shift+Tab) e continua igual. Selecionar um trecho de texto e escolher cada opção do seletor "Tamanho" (Pequena, Normal, Grande, Enorme): o trecho selecionado muda de tamanho visivelmente, só ele, sem afetar o resto do texto; o seletor volta a mostrar "Tamanho" (placeholder) depois de aplicar, já que o tamanho de cada trecho pode ser diferente. Salvar a atividade e reabrir o formulário de edição — o recuo e os tamanhos de fonte aplicados continuam exatamente como foram deixados (não desaparecem nem viram texto plano). Sem entrar em edição, expandir a atividade na tela de leitura (roteiro da turma) e clicar em "🖨 Roteiro completo": o mesmo recuo e os mesmos tamanhos de fonte devem aparecer no texto exibido e no PDF exportado, iguais ao formulário.' },
    { section: 'Admin',
      title: 'Roteiro — continuar numeração/marcador digitado à mão e botão "↵ Espaço"',
      motivo: 'Não automatizado porque depende de digitação e da tecla Enter em tempo real. (1) Numa atividade NOVA (caixa vazia), digitar "1. primeiro passo" e apertar Enter: a linha seguinte já deve começar sozinha com "2. " (sem precisar digitar o número); repetir mais uma vez e conferir "3. ". (2) TESTE MAIS IMPORTANTE — abrir uma atividade JÁ EXISTENTE (cadastrada antes desta funcionalidade, com texto simples de várias linhas em "Passo a passo" ou similar) e continuar digitando numeração dentro dela: clicar no fim do texto já existente, apertar Enter, digitar "1. próximo passo", Enter, digitar mais uma linha sem número — ela deve virar "2. " sozinha. Esse é o caso que ficou quebrado numa versão anterior (o texto antigo, sem <div> por linha, fazia a conta de "linha atual" enxergar todas as linhas anteriores coladas, e a numeração nunca continuava). Digitar o "1." e o espaço normalmente pelo teclado em ambos os casos (não colar) — é esse espaço que o navegador às vezes troca por um espaço não separável, e precisa continuar reconhecido. (3) Numa linha nova, apertar Enter sem digitar nada além do número (ex: deixar só "4. " e apertar Enter): o marcador deve sumir e a numeração parar de continuar (a linha seguinte fica em branco, sem "5. "). Repetir com um marcador de lista digitado à mão (ex: "- item") — deve continuar com "- " na linha seguinte. Conferir que isso NÃO acontece dentro de uma lista de verdade criada pelos botões "• Lista"/"1. Lista" (ali quem numera é o próprio navegador). (4) Clicar no botão "↵ Espaço" no meio de um texto: deve inserir uma linha em branco visível, sem apagar o que já estava escrito antes ou depois do cursor. (5) Testar o Tab: dentro de uma lista de verdade, posicionar o cursor num item e apertar Tab — o item deve indentar como sub-item da lista; Shift+Tab desfaz o recuo; fora de uma lista, Tab deve inserir um recuo BEM visível no texto (um espaço bem maior que o espaço normal entre palavras) no lugar do cursor; em NENHUM caso o foco deve sair da caixa e pular para o próximo campo do formulário.' },
    { section: 'Admin',
      title: 'Roteiro — colar (Ctrl+V) numa caixa de texto rica sempre vira texto puro',
      motivo: 'Não automatizado porque depende do clipboard real do sistema operacional. Copiar de uma planilha (Excel ou Google Sheets) uma tabela de pelo menos 2 colunas e 3 linhas (ex: colunas "O que aconteceu" / "Mentalidade envolvida") e colar (Ctrl+V) dentro de uma caixa de texto do formulário de atividade (ex: Passo a passo). Conferir: (1) cada linha da planilha vira uma linha própria na caixa (não gruda tudo numa linha só emaranhada — esse era o bug: colar a tabela de verdade do clipboard e o sanitizador jogar fora as tags de tabela sem preservar quebra nenhuma entre as células); (2) as colunas de cada linha ficam separadas por um espaço bem largo (não uma tabela de verdade com grade, já que essas caixas não suportam tabela — só texto formatado); (3) nenhuma formatação da planilha de origem (cor de fundo das células, fonte, negrito automático de cabeçalho) é trazida — só o texto puro. Repetir colando de um Word/Google Docs com texto em negrito/cor — o negrito e a cor da origem NÃO devem aparecer (só o texto simples); é o comportamento esperado, evita que estilos estranhos da origem "vazem" para o roteiro. Por fim, colar um texto simples de várias linhas (sem tabela, ex: copiado de um bloco de notas) e conferir que cada linha continua numa linha própria, sem juntar tudo.' },
    { section: 'Admin',
      title: 'Roteiro — abrir e salvar uma atividade sem editar não corrompe texto com "&"',
      motivo: 'AUTOMATIZADO NO CI: .github/scripts/teste-roteiro-texto-rico.js roda em toda PR, desktop e iPhone. As caixas de texto rico decidem se um valor já é HTML novo ou texto puro antigo olhando só se tem alguma tag reconhecida — nunca se já tem uma entidade (&amp; &nbsp;). Um "&" sem tag nenhuma (um "&nbsp;" digitado à mão, ou um "Riscos & Oportunidades" de verdade) caía sempre no ramo "texto puro" e era escapado de novo — TODA vez que a atividade era aberta e salva, mesmo sem editar aquele campo. Depois de duas rodadas, um "&nbsp;" virava "&amp;nbsp;" literal na tela, sem erro nenhum em lugar nenhum. Corrigido tornando o escape idempotente. Pra conferir com os olhos: editar uma atividade cadastrada ANTES desta correção que já mostre "&amp;" ou "&nbsp;" como texto visível no meio de um campo — abrir o formulário, sem tocar naquele campo trocar qualquer outra coisa e salvar, reabrir de novo — o texto (por pior que já esteja) tem que ficar EXATAMENTE igual entre a 1ª e a 2ª vez, nunca ganhar mais um "&amp;" na frente. Esse campo específico continua com o texto errado até alguém apagar manualmente o "&amp;nbsp;"/"&nbsp;" e digitar um espaço de verdade — a correção impede que piore, não desfaz sozinha o que já tinha acontecido.' },
    { section: 'Admin',
      title: 'Roteiro — Início/Fim/Duração não têm autocomplete do navegador',
      motivo: 'Relatado: "às vezes escrevo um número na duração e depois ele muda o número" — digitava "10" e, ao sair do campo, via outro número (ex: "3"), sem ter mexido em mais nada. A conta de completar o terceiro campo (roteiro.js) nunca escreve na própria Duração quando é ELA que muda — só em Início ou Fim; então a troca só pode ter vindo do navegador sugerindo/substituindo um valor numérico salvo de outro contexto (telefone, código) ao perder o foco, comportamento mais comum em celular e intermitente por natureza. Os outros campos de texto deste formulário (Título, Sessão etc.) já tinham autocomplete="off"; só faltava em Início, Fim e Duração — corrigido. Não automatizado porque depende do histórico de autopreenchimento do navegador/SO da pessoa, que o Firebase falso não reproduz. Pra conferir: no celular, com o teclado configurado pra sugerir números (alguns tecladillos mostram sugestões numéricas acima da tela), abrir "Editar atividade", digitar um número em Duração e sair do campo repetidas vezes — não deve mais aparecer nenhuma barra/chip de sugestão nem trocar o valor sozinho.' },
    { section: 'Admin',
      title: 'Roteiro — pausa de 30 min dentro de uma seção conta em "Pausas" (não fica 0)',
      motivo: 'Não automatizado porque grava dados reais. Criar uma atividade de nível principal com "+ Sub-etapa" e, dentro dela, uma sub-etapa com tipo "Intervalo" e 30 min de duração (a atividade-mãe pode ter outro tipo, ou nenhum). Conferir no resumo do dia (ou da sessão, se estiver usando sessões): "Pausas" mostra 30min (não 0), "Tempo de facilitação" já vem descontando esses 30min, e "Tempo programado" inclui a soma normalmente. Repetir com o Intervalo como atividade de nível principal (sem seção) pra confirmar que o caso simples continua funcionando igual.' },
    { section: 'Admin',
      title: 'Roteiro — duração de uma seção (com sub-etapas) trava no formulário e é calculada sozinha',
      motivo: 'Não automatizado porque grava dados reais. Criar uma atividade com 2-3 sub-etapas de durações diferentes (ex: 5, 10, 15 min) e reabrir o formulário de EDIÇÃO da atividade-mãe. Conferir: (1) o campo "Duração (min)" mostra a soma (30 no exemplo) e está desabilitado/travado (não dá pra digitar outro valor); (2) preencher só o Início — o campo Fim se completa sozinho usando essa duração travada (Início + 30min); preencher só o Fim faz o mesmo pra trás; (3) salvar sem mexer em nada e conferir que a duração continua certa em todo lugar (lista, resumo do dia, impressão). Por fim, remover todas as sub-etapas e reabrir o formulário da mãe: o campo Duração deve voltar a ficar editável normalmente.' },
    { section: 'Admin',
      title: 'Roteiro — aviso de sobreposição de horário',
      motivo: 'Não automatizado porque grava dados reais. Cadastrar duas atividades da MESMA sessão com horários que se cruzam (ex: atividade A das 09:00 às 09:30, atividade B começando às 09:20). Conferir que aparece um banner vermelho "⚠ [A] se sobrepõe a [B]" com o tempo de sobreposição (10min no exemplo), tanto no roteiro-base quanto no roteiro da turma, e que ele também some quando o Ctrl+V permitir consertar o conflito. Conferir também que a barra de resumo do dia/sessão passa a mostrar um item "Sobreposições" com o total de minutos em conflito e a quantidade de conflitos (ex: "10min · 1 conflito"), e que esse item some sozinho quando não há mais nenhuma sobreposição. Colocar as duas atividades em sessões DIFERENTES (uma na Manhã, outra na Tarde) mesmo com horários sobrepostos e conferir que o aviso e o item "Sobreposições" não aparecem nesse caso (sessões diferentes não são comparadas entre si).' },
    { section: 'Admin',
      title: 'Roteiro — rótulo "Lacunas reais" igual na barra de cada sessão e no resumo agregado',
      motivo: 'Não automatizado porque depende de um dia real com duas sessões. Num dia com Manhã e Tarde (ver cenário de detecção automática de sessões), abrir o roteiro-base e conferir: (1) a barra "Sessões" agregada no topo mostra o rótulo "LACUNAS REAIS"; (2) a barra "Janela da sessão" da Manhã mostra o MESMO rótulo "LACUNAS REAIS" (não "Lacunas" sozinho); (3) a barra da Tarde também mostra "LACUNAS REAIS"; (4) repetir clicando "🖨 Agenda resumida" e conferir que o PDF exportado usa "Lacunas reais" nos três lugares (resumo agregado e as duas barras de sessão) — nunca "Lacunas" sem o "reais" em lugar nenhum.' },
    { section: 'Admin',
      title: 'Roteiro — Sessões/janelas: o intervalo entre Manhã e Tarde não vira "lacuna"',
      motivo: 'Não automatizado porque grava dados reais. Num dia, cadastrar 2-3 atividades com "Sessão / janela" = "Manhã" (ex: 09:00–12:00) e 2-3 com "Sessão / janela" = "Tarde" (ex: 13:30–17:00) — deixando as demais atividades do site sem esse campo preenchido, se houver. Conferir: (1) aparecem DOIS cabeçalhos de sessão ("Manhã" e "Tarde") cada um com sua própria barra de resumo; (2) a "Janela" de cada sessão cobre só o intervalo dela (09:00→12:00 e 13:30→17:00), nunca o dia inteiro; (3) o vão entre 12:00 e 13:30 (o almoço) NÃO aparece como "⚠ Lacuna no planejamento" em lugar nenhum; (4) o botão "Ordenar por horário" (se aparecer, por causa de ordem divergente) só reordena dentro da sessão onde foi clicado, sem mexer na outra. Depois, testar um dia SEM nenhuma sessão preenchida e confirmar que o comportamento continua idêntico a antes desta funcionalidade (uma barra de resumo só, sem cabeçalho de sessão nenhum).' },
    { section: 'Admin',
      title: 'Roteiro — detecção automática de sessões (sem preencher "Sessão / janela") e resumo agregado',
      motivo: 'Não automatizado porque grava dados reais. Cenário do pedido original: cadastrar um dia com 11 atividades entre 09:00 e 10:30 (sem lacuna entre elas) e outras 11 entre 15:00 e 16:23 (também sem lacuna entre elas), e NENHUMA das 22 com o campo "Sessão / janela" preenchido. Conferir: (1) o sistema detecta sozinho DUAS sessões, nomeadas "Manhã" (09:00–10:30) e "Tarde" (15:00–16:23) — não é preciso preencher nada; (2) o vão de 10:30 a 15:00 NÃO aparece em lugar nenhum como "⚠ Lacuna no planejamento" nem em nenhum alerta vermelho; (3) uma dica discreta abaixo do resumo avisa que essas sessões foram detectadas automaticamente; (4) aparece uma barra "Sessões" no topo do dia (antes das barras de cada sessão) mostrando: Sessões = 2; Manhã 09:00–10:30 (1h30) e Tarde 15:00–16:23 (1h23) lado a lado; Tempo disponível = 2h53 (nunca "7h23" — a soma aditiva das duas sessões, não o período entre a primeira e a última atividade do dia inteiro); Tempo programado = 2h53; Tempo de facilitação = 2h53; Pausas = 0 min; Lacunas reais = 0 min; Atividades principais = 22; (5) uma linha pequena embaixo mostra "Período entre a primeira e a última atividade do dia: 09:00–16:23", deixando claro que esse número não é tempo disponível para facilitação. Depois, preencher "Sessão / janela" manualmente em qualquer atividade desse dia e confirmar que a detecção automática para de valer — o agrupamento passa a seguir só o texto digitado. Testar também um vão MENOR que 90 minutos entre duas atividades sem sessão preenchida (ex: uma termina 10:00, a próxima só começa 11:00) e confirmar que esse vão de 1h continua aparecendo como lacuna de verdade — só vãos de 90 minutos ou mais viram corte de sessão automático.' },
    { section: 'Admin',
      title: 'Roteiro-base — "+ Etapa" atrás do menu "⋯" numa atividade simples',
      motivo: 'Não automatizado porque depende de clique real no menu. Numa atividade de nível principal que ainda não tem nenhuma etapa, conferir que a linha mostra um botão "⋯" no lugar de "+ Etapa" solto; clicar nele abre um menuzinho com "+ Etapa" dentro. Clicar em "+ Etapa" dentro do menu abre o mesmo formulário de sempre e cria a etapa normalmente; o menu fecha sozinho depois do clique, e também fecha se a pessoa clicar em qualquer lugar fora dele. Depois de criada a primeira etapa, reabrir a lista e conferir que a atividade agora é uma seção e mostra "+ Etapa" direto na linha, sem precisar mais do menu "⋯".' },
    { section: 'Admin',
      title: 'Roteiro — impressão não trunca conteúdo longo e evita about:blank',
      motivo: 'Não automatizado porque depende do diálogo de impressão do navegador. Numa atividade, escrever um "Passo a passo" bem longo (várias dezenas de linhas, o suficiente pra passar de uma página impressa) e clicar "🖨 Roteiro completo". Usar "Imprimir / salvar como PDF" (ou o preview de impressão do navegador) e conferir: (1) o conteúdo continua na página seguinte em vez de simplesmente desaparecer no fim da primeira página (esse era o bug: um bloco de atividade travado pra nunca quebrar de página, mas maior que uma página inteira, sumia sem aviso no Chrome); (2) o título da atividade e o rótulo de cada campo nunca ficam sozinhos no fim de uma página, sempre grudados com o começo do conteúdo deles. Conferir também que a aba/janela de impressão não mostra "about:blank" (deve mostrar um nome de arquivo, tipo "roteiro-impressao.html", contanto que o navegador permita) e que existe uma dica visível na tela (não impressa) sugerindo desmarcar "Cabeçalhos e rodapés" nas opções de impressão do navegador.' },
    { section: 'Admin',
      title: 'Roteiro — paginação não deixa grandes áreas vazias no PDF exportado (sub-etapas em sequência)',
      motivo: 'Não automatizado porque depende do diálogo de impressão do navegador e de conferência visual em várias páginas. Criar (ou usar) um dia com uma seção/atividade que tenha várias sub-etapas curtas em sequência (uma "+ Etapa" atrás da outra, cada uma só com título/horário/objetivo/um ou dois passos — o suficiente para várias sub-etapas curtas caberem por página) — quanto mais sub-etapas curtas seguidas, melhor pra reproduzir o cenário do bug original. Clicar "🖨 Roteiro completo", usar "Imprimir / salvar como PDF" (ou o preview de impressão) e folhear TODAS as páginas geradas conferindo: (1) nenhuma página termina com uma área vazia grande (mais ou menos um quarto de página ou mais) enquanto ainda existe conteúdo nas páginas seguintes — o conteúdo deve continuar preenchendo cada página até quase o rodapé antes de pular pra próxima; (2) o título de uma sub-etapa nunca aparece sozinho no fim de uma página com o horário/conteúdo dela começando só na página seguinte — os dois sempre saem juntos; (3) é aceitável (não é bug) a linha discreta "[título da seção] — continuação" aparecer sozinha no fim de uma página, com o título da sub-etapa propriamente dito começando na página seguinte — esse é o único elemento que pode ficar separado do card; (4) uma atividade com texto realmente longo (mais que uma página) continua normalmente na página seguinte, sem sumir nem truncar. Repetir em "🖨 Passo a passo".' },
    { section: 'Admin',
      title: 'Roteiro — exportar "Agenda + Objetivos" (tabela + Objetivo/Resultado esperado)',
      motivo: 'Não automatizado porque depende do diálogo de impressão do navegador e de conferência visual. Num dia com pelo menos: uma atividade com Objetivo E Resultado esperado preenchidos, uma atividade SEM nenhum dos dois, e uma seção com sub-etapa (uma delas com Objetivo/Resultado esperado preenchido), clicar "🖨 Agenda + Objetivos" e conferir: (1) a tabela tem exatamente as mesmas colunas e dados da "🖨 Agenda resumida" (#, Horário, Atividade, Tipo, Duração), incluindo as linhas de sub-etapa (numeração X.Y) e de lacuna, se houver; (2) logo abaixo da linha de uma atividade com Objetivo e/ou Resultado esperado preenchido aparece uma linha extra, ocupando a largura toda da tabela, com os rótulos "OBJETIVO"/"RESULTADO ESPERADO" (dourado maiúsculo, igual ao "Roteiro completo") e o texto de cada um; (3) a atividade sem os dois campos preenchidos NÃO ganha linha extra nenhuma (nem rótulo vazio, nem linha em branco); (4) a sub-etapa com Objetivo/Resultado esperado preenchido também ganha sua própria linha extra, logo abaixo da linha dela; (5) nenhum outro campo (Passo a passo, Dicas para o facilitador, Materiais etc.) aparece em lugar nenhum desta exportação — só Objetivo e Resultado esperado; (6) texto em Markdown puro nesses dois campos (lista numerada, negrito, bullets) sai formatado corretamente, igual ao que já acontece no "Roteiro completo". Repetir clicando em "🖨 Agenda + Objetivos" no roteiro de uma turma (não só no roteiro-base).' },
    { section: 'Admin',
      title: 'Roteiro — hierarquia visual atividade/sub-etapa na "Agenda resumida" e na "Agenda + Objetivos"',
      motivo: 'Não automatizado porque depende de conferência visual. Num dia com pelo menos duas atividades de nível principal, sendo uma delas uma seção com 2+ sub-etapas, clicar "🖨 Agenda resumida" e conferir: (1) a atividade de nível principal seguinte (ex: a linha "2") tem uma linha divisória dourada acima dela, separando do bloco da atividade anterior; (2) as sub-etapas (linhas "1.1", "1.2"...) NÃO têm essa linha divisória entre si nem entre elas e a atividade-mãe "1" — só a próxima atividade de nível principal ("2") tem; (3) o título de cada sub-etapa aparece com um "↳" na frente (ex: "↳ Imersão no Storytelling"), além do recuo e da cor mais clara já existentes; (4) a primeira atividade da tabela (logo abaixo do cabeçalho) não tem a linha divisória dourada sobrando ali (só faz sentido separar de um bloco anterior que existe). Repetir em "🖨 Agenda + Objetivos" e conferir o mesmo comportamento, inclusive com as linhas extras de Objetivo/Resultado esperado compartilhando o bloco da atividade a que pertencem.' },
    { section: 'Admin',
      title: 'Roteiro — "Resultado esperado da turma" aparece na "Agenda resumida" (só do Roteiro da turma)',
      motivo: 'Não automatizado porque depende do diálogo de impressão do navegador e de conferência visual. Preencher "Resultado esperado da turma" (⋯ → Editar Turma) de uma turma com um texto de várias linhas, incluindo negrito (**palavra**). No card da turma, abrir "📋 Roteiro da turma" e clicar "🖨 Agenda resumida" nesse roteiro: conferir que aparece um bloco logo abaixo do título/data de geração e ANTES do resumo do dia (Janela/Programado/Facilitação...), com o rótulo "RESULTADO ESPERADO DA TURMA" em dourado maiúsculo e o texto exatamente como foi digitado, com as quebras de linha preservadas e o negrito renderizado de verdade (sem asteriscos). Deixar o campo vazio numa turma sem "Resultado esperado" preenchido e conferir que o bloco não aparece (nem rótulo vazio, nem espaço sobrando). Por fim, abrir o Roteiro-BASE do evento (não de uma turma) e clicar "🖨 Agenda resumida" — conferir que esse bloco nunca aparece ali, já que Roteiro-base é do evento inteiro e não pertence a nenhuma turma.' },
    { section: 'Admin',
      title: 'Roteiro — Markdown de campo de texto renderizado (não mais à mostra) no PDF exportado',
      motivo: 'Não automatizado porque depende do diálogo de impressão do navegador e de conferência visual. Preencher (fora do editor de texto rico, ou editando o valor bruto no banco só pra este teste) um campo como "Passo a passo" com um texto contendo, literalmente: uma lista numerada escapada ("1\\. Primeiro passo.\\n2\\. Segundo passo."), negrito escapado ("\\*\\*palavra-chave\\*\\*") e um item numerado seguido de sub-perguntas com "*"/"-" (ex: "9. Perguntar ao grupo:\\n* Pergunta A?\\n* Pergunta B?"). Clicar "🖨 Roteiro completo" (repetir em "🖨 Passo a passo") e conferir no PDF: (1) NENHUM caractere de escape (barra invertida) nem asterisco literal aparece em lugar nenhum do texto; (2) "1." e "2." aparecem como itens de verdade de uma lista numerada (recuo e alinhamento de lista, não texto corrido com "1." na frente); (3) "**palavra-chave**" aparece em negrito de verdade, sem os asteriscos; (4) as perguntas com "*"/"-" aparecem como bullets RECUADOS dentro do item "9" (sublista), não como novos itens "10", "11" da lista numerada principal; (5) o rótulo do campo ("PASSO A PASSO") continua em dourado maiúsculo — só o texto do negrito dentro do conteúdo é que fica em negrito normal (mesma cor do resto do texto), nunca virando outro rótulo dourado maiúsculo sozinho numa linha. Repetir com um campo que usa o editor de texto rico normalmente (negrito/lista pelos botões da barra de formatação, sem digitar símbolo de Markdown nenhum) e confirmar que continua saindo exatamente igual a antes (sem regressão). Repetir ainda com um campo MISTO — parte HTML de verdade (ex: abrir o campo no editor rico, digitar uma linha e formatar só ela em negrito pelos botões) seguida de linhas ainda em Markdown puro por baixo (ex: cole depois do trecho formatado algo como "1\\. Explicar as regras.\\n**palavra-chave** importante.\\n* pergunta final") — e confirmar que AMBAS as partes saem certas no PDF (a parte formatada pelo editor do jeito que já saía, e a parte em Markdown puro interpretada igual ao caso não-misto) — esse era o bug original desta regra: um campo com QUALQUER pedacinho de HTML de verdade (mesmo só uma quebra de linha) fazia o restante do campo, ainda em Markdown puro, ser exibido sem nenhuma interpretação. Por fim, abrir a mesma atividade na tela normal do roteiro (editor e "ver detalhes") e confirmar que o campo continua mostrando o texto cru, símbolos de Markdown incluídos, exatamente como estava gravado — a interpretação é só do PDF, o valor no banco e a tela normal não mudam.' },
    { section: 'Admin',
      title: 'Roteiro — rodapé próprio de página (numeração e marca) no PDF exportado',
      motivo: 'Não automatizado porque depende do diálogo de impressão do navegador. Num dia com atividades suficientes para gerar 2+ páginas impressas, clicar "🖨 Agenda resumida" (repetir com "🖨 Roteiro completo" e "🖨 Passo a passo") e usar "Imprimir / salvar como PDF". Conferir no PDF gerado: (1) TODAS as páginas trazem, no rodapé, "FORÇA ÁGIL · PREVI | Roteiro de Facilitação" à esquerda e "Página X de Y" à direita, com X incrementando corretamente e Y batendo com o total real de páginas; (2) esse rodapé aparece mesmo com a opção "Cabeçalhos e rodapés" do navegador DESMARCADA (é ele quem fornece a numeração agora, não o navegador); (3) marcando "Cabeçalhos e rodapés" no diálogo do navegador, o cabeçalho/rodapé NATIVO do navegador (data/hora, título da aba, URL, uma numeração tipo "1/2") aparece ALÉM do rodapé próprio do documento — a dica na tela existe justamente pra pessoa desmarcar essa opção e evitar a duplicação; (4) o texto do rodapé próprio nunca sobrepõe a última linha de conteúdo da página (tabela ou cartão de atividade); (5) marcar "Modo econômico" antes de exportar e conferir que o rodapé continua legível (fundo branco, texto escuro) do mesmo jeito, sem mudar.' },
    { section: 'Admin',
      title: 'Roteiro — janela de impressão não estica/desalinha em tela larga',
      motivo: 'Não automatizado porque depende do tamanho real da janela do navegador. Com a janela do navegador BEM larga (maximizada num monitor grande, ou tela dividida ocupando a metade de um monitor ultrawide), clicar "🖨 Agenda resumida" e conferir: (1) o conteúdo da janela de impressão fica centralizado numa largura de página, com bastante espaço vazio dos dois lados — nunca esticado de ponta a ponta da janela; (2) na tabela, as colunas "#", "Horário", "Tipo" e "Duração" mantêm uma largura curta e previsível, e é a coluna "Atividade" que ocupa o espaço que sobra — nunca um vão grande e vazio entre "Tipo" e "Duração"; (3) o cabeçalho "Duração" não quebra em duas linhas dentro da própria célula. Repetir clicando "🖨 Roteiro completo" e "🖨 Passo a passo" (formato em cartão, não tabela) e conferir que os cartões também ficam dentro dessa mesma largura de página, sem esticar. Por fim, usar "Imprimir / salvar como PDF" (ou o preview de impressão) e conferir que o conteúdo aproveita a largura real do papel/PDF normalmente — esse limite de largura vale só pra visualização na tela, não pra impressão física.' },
    { section: 'Admin',
      title: 'Roteiro — identidade visual (tema escuro do site) no PDF exportado',
      motivo: 'Não automatizado porque depende do diálogo de impressão do navegador. Clicar "🖨 Agenda resumida" e, em outro teste, "🖨 Roteiro completo" (roteiro-base ou roteiro da turma) e conferir em cada um: (1) a janela abre com fundo escuro (não branco), igual ao tema espacial do site — não é mais um documento em branco genérico do navegador; (2) logo abaixo do botão "Imprimir / salvar como PDF" aparece uma barra com o mesmo ícone (círculo + triângulo dourado) do menu superior do site, "FORÇA ÁGIL" e "PREVI · OFICINA DE AGILIDADE ORGANIZACIONAL"; (3) os blocos de resumo e de cada atividade aparecem como cartões com borda e fundo levemente mais claro que o restante da página (mesmo estilo dos painéis do site), títulos em dourado, horários em ciano, rótulos de campo ("Objetivo", "Dicas para o facilitador" etc.) em dourado maiúsculo, título principal e títulos de atividade em Oswald; (4) usar "Imprimir / salvar como PDF" com a opção "Gráficos de fundo" (ou "Imprimir cores e imagens de fundo") MARCADA no diálogo do navegador e conferir que o PDF gerado sai com o fundo escuro e as cores certas, igual à tela; repetir com essa opção DESMARCADA (e "Modo econômico" também desmarcado) e confirmar que o PDF sai com fundo branco e o texto claro fica difícil de ler — esse é o comportamento esperado do navegador (não um bug do site), por isso a dica visível na tela insiste nisso.' },
    { section: 'Admin',
      title: 'Roteiro — "Modo econômico" no PDF exportado troca pra fundo claro sem perder a identidade',
      motivo: 'Não automatizado porque depende do diálogo de impressão do navegador. Abrir "🖨 Agenda resumida" ou "🖨 Roteiro completo" e marcar o checkbox "Modo econômico (fundo claro, menos tinta)" ao lado do botão de imprimir. Conferir: (1) o fundo da página inteira (e de cada cartão de resumo/atividade e da tabela) vira claro, com texto escuro — bem menos área preta/escura do que antes de marcar; (2) os títulos, rótulos de campo e horários continuam usando as cores de acento do site (um dourado mais escuro e um ciano mais escuro, pra manter contraste no fundo claro), não viram cinza genérico; (3) a faixa da logo no topo ("FORÇA ÁGIL"/"Previ") NÃO muda — continua com fundo azul-marinho escuro e o ícone/texto em dourado e ciano vivos, iguais a antes de marcar o modo econômico; (4) desmarcar o checkbox e conferir que a página volta inteira pro tema escuro original. Usar "Imprimir / salvar como PDF" com o checkbox marcado (sem precisar marcar "Gráficos de fundo" no diálogo do navegador, já que o fundo agora é claro) e conferir que o PDF sai legível e com pouquíssima área escura, restrita à faixa da logo.' },
    { section: 'Admin',
      title: 'Roteiro — Tipo de atividade: lista administrável em ordem alfabética',
      motivo: 'Não automatizado porque grava dados reais. Na aba "Tipos de atividade" do Admin, conferir que a lista aparece sempre em ordem alfabética (se o banco nunca teve essa lista, abrir a aba já semeia automaticamente com os tipos que eram fixos antes desta funcionalidade — Abertura, Ambientação, Aplicação à Liderança, Autodiagnóstico, Briefing, Compromisso, Compromisso Individual, Conceituação, Dinâmica, Discussão, Experimentação com IA, Fechamento, Integração, Intervalo, Provocação, Quiz, Reflexão, Sinal/Evidência, Transição). Depois, abrir "+ Adicionar atividade" (ou editar uma existente) e conferir que o campo "Tipo de atividade" oferece exatamente essa mesma lista, na mesma ordem, mais a opção em branco "—" no topo. Escolher "Intervalo" numa etapa e conferir que ela ainda conta em "Pausas" no resumo do dia e ganha o badge laranja na lista, exatamente como antes. Abrir uma atividade antiga cadastrada com um tipo que não existe mais nesta lista (ex: "Debrief", "Outro", "Exercício") e conferir que o campo aparece sem nenhuma opção selecionada (nem a atual nem outra por engano) — o valor antigo continua guardado no banco até a pessoa escolher um tipo novo e salvar.' },
    { section: 'Admin',
      title: 'Aba: Tipos de atividade — adicionar, editar e remover',
      motivo: 'Não automatizado porque grava dados reais e depende de vários admins simultâneos pra testar o cenário de nome duplicado. Na aba "Tipos de atividade": (1) preencher "Adicionar tipo de atividade" com um nome novo (ex: "Quebra-gelo") e clicar "Adicionar" — o tipo aparece na lista (reordenada alfabeticamente) e passa a existir no <select> "Tipo de atividade" do formulário de atividade do Roteiro; (2) tentar adicionar um nome já existente, inclusive com maiúsculas/minúsculas diferentes (ex: "QUEBRA-GELO") — deve aparecer a mensagem "Já existe um tipo com esse nome." e nada é gravado; (3) clicar "Editar" numa linha, mudar o nome e "Salvar" — o novo nome aparece na lista e no formulário de atividade; atividades já cadastradas com o nome ANTIGO continuam mostrando esse texto (na tela de detalhes e no PDF), só deixam de aparecer pré-selecionadas no formulário de edição até alguém escolher um tipo novo; (4) clicar "Cancelar" durante uma edição — volta pro nome original, sem gravar nada; (5) clicar "Remover" numa linha — pede confirmação; confirmando, o tipo some da lista e do formulário (atividades que já usavam esse tipo continuam com o valor gravado); (6) abrir a mesma aba em duas abas do navegador (ou dois logins) e adicionar tipos diferentes em cada uma — as duas listas devem convergir para a mesma lista completa depois de recarregar, sem um sobrescrever o outro (cada tipo é uma entrada própria no banco, não um array único regravado inteiro a cada mudança).' },
    { section: 'Admin',
      title: 'Formulários com rótulo em maiúsculo (ex: Roteiro) não forçam maiúscula no texto digitado',
      motivo: 'Não automatizado porque depende de digitação em tempo real. No formulário de atividade do Roteiro (ou qualquer outro formulário que use o mesmo estilo de rótulo do site, ex: Missão/Tópicos do evento), digitar um título em minúsculas ou mistas (ex: "abertura da oficina") no campo Título. Conferir: (1) o texto aparece dentro da caixa exatamente como foi digitado, sem virar maiúsculo sozinho; (2) o rótulo do campo acima da caixa (ex: "TÍTULO") continua em maiúsculo — isso é estilo do site, não deve mudar; (3) salvar e reabrir a atividade — o título continua exatamente como foi digitado, tanto na caixa de edição quanto na lista. Testar também o campo "Tipo de atividade" (um <select>): a lista de opções deve aparecer com a grafia normal de cada tipo (ex: "Dinâmica", não "DINÂMICA"), igual ao que já acontecia na lista de atividades.' }
  ];
