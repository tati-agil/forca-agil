...
{ section: 'Admin',
    ...
    { section: 'Admin',
      ...
      motivo: 'Com pelo menos dois eventos, cada um com gente esperando: abrir a aba Eventos e conferir que a seção "Lista de Espera" de cada evento aparece DENTRO do card recolhível dele — junto das turmas, some quando o card do evento é recolhido e volta quando expande — nunca flutuando abaixo de todos os eventos. Evento sem ninguém esperando (nem ativo, nem já saído) não ganha seção nenhuma, nem vazia. Registro sem eventoKey aparece numa seção separada "Lista de Espera — sem evento", abaixo de todos os cards de evento, nunca some — mas enquanto só um evento tiver turma essa seção não chega a existir, porque a migração adota essas entradas (ver "Migração: eventoKey da fila é preenchido sozinho").' },
    { section: 'Admin',
      title: 'Migração: eventoKey da fila é preenchido sozinho',
      motivo: 'Requer um registro na fila gravado ANTES desta mudança, sem o campo eventoKey. Ao carregar o painel como admin ele ganha dono sozinho (migrarEsperaEventoKey, roda a cada carga), por duas regras nesta ordem. (1) Veio de uma turma: recebe o eventoKey DA TURMA de origem, mesmo que existam vários eventos — é a informação mais precisa e vale sempre. (2) Entrou direto pelo card do site (origem "lista", sem turma): recebe o evento que é o ÚNICO COM TURMA. O card de espera só existe para evento que já tem turma na vitrine, então evento sem turma nunca teve fila e não entra na conta. Conferir os dois lados: (a) cadastrar um evento NOVO, ainda sem turma nenhuma, e recarregar o painel — as órfãs continuam sendo adotadas pelo evento que tem turma, o evento vazio não desliga nada (contar eventos em vez de eventos com turma seria uma trava boba: bastaria criar um evento vazio para as órfãs voltarem ao limbo); (b) dar uma turma a esse segundo evento — agora SÃO DOIS com turma, a dúvida é real, e uma órfã nova continua órfã (aparece em "Lista de Espera — sem evento") sem nenhum eventoKey chutado. Rodar de novo não duplica nem sobrescreve nada — idempotente, e nunca troca um eventoKey já gravado.' },
    { section: 'Admin',
