...
{ section: 'Minha Área',
    ...
    { section: 'Admin',
      title: 'Roteiro-base — botão "📥 Importar resultados esperados" (só aparece na DIRETORES)',
      motivo: 'Não automatizado porque grava dados reais em produção. (1) Abrir o roteiro-base de um evento QUALQUER que não seja a "FORÇA ÁGIL - DIRETORES" e conferir que o botão "📥 Importar resultados esperados" NÃO aparece ao lado de "🗑 Excluir dia" — só deve aparecer no roteiro que já tem a atividade "EPISÓDIO FINAL — O COMPROMISSO DA LIDERANÇA". (2) No roteiro-base da DIRETORES, clicar o botão — aparece um diálogo de confirmação antes de qualquer gravação; cancelar e conferir que nada mudou. (3) Clicar de novo e confirmar — ao final aparece um relatório com "Atualizados: X de 31"; se X for menor que 31, o relatório lista exatamente quais títulos não bateram com nenhuma atividade (não encontrados) ou bateram com mais de uma (duplicados) — nesses dois casos a atividade correspondente NÃO deve ter o campo alterado. (4) Abrir 2-3 atividades ao acaso (incluindo uma sub-etapa, ex: 4.3) e conferir que o campo "Resultado esperado" foi preenchido com o texto certo, e que nenhum outro campo (horário, duração, tipo, objetivo etc.) mudou. (5) Clicar o botão de novo, confirmar de novo — deve rodar sem erro e sem duplicar nada (mesmo resultado no relatório).' },
    { section: 'Admin',
      title: 'Roteiro-base — botões "🔎 Migração Antes x Depois" e "↩ Desfazer última migração" (dry run, status por linha e rollback)',
      motivo: 'Não automatizado porque grava dados reais em produção. (1) Confirmar que os dois botões só aparecem no roteiro-base da DIRETORES, igual ao botão de resultados esperados. (2) Clicar "🔎 Migração Antes x Depois" — abre uma janela com uma TABELA (atividade, campo, status), sem gravar nada ainda ("DRY RUN" no texto). Preparar de antemão um "Passo a passo" com MAIS conteúdo do que o trecho "DE" da planilha (ex: um parágrafo antes e outro depois do trecho que a planilha lista) e conferir que essa linha aparece como "OK PARA ALTERAR" — NUNCA como divergente só porque o campo tem texto a mais em volta do trecho (esse era um bug: comparar o campo inteiro, em vez de procurar o trecho dentro dele, gerava falsa divergência toda vez que o campo real tinha mais linhas do que a planilha citou). Testar também a tolerância a diferença INVISÍVEL de formatação: colar o mesmo trecho "DE" no campo mas trocando as aspas retas por tipográficas (ou vice-versa), inserindo um espaço duplicado, uma linha em branco a mais, ou um &nbsp; no meio de duas palavras — em todos esses casos a linha deve continuar aparecendo como "OK PARA ALTERAR" (visualmente o texto é o mesmo, só muda algo que não se vê) — NUNCA "VALOR ATUAL DIVERGENTE" só por causa disso. Conferir os demais status: "JÁ ATUALIZADO" (o trecho novo já está lá), "VALOR ATUAL DIVERGENTE" (nem o trecho antigo nem o novo aparecem no campo, nem tolerando esse tipo de diferença — testar editando manualmente pra um texto realmente diferente), "TRECHO AMBÍGUO" (colar o mesmo trecho "DE" duas vezes dentro do campo — a linha deve avisar quantas vezes o trecho apareceu), "ATIVIDADE NÃO ENCONTRADA" (renomear temporariamente uma atividade-alvo) e "REGISTRO AMBÍGUO" (duplicar uma atividade-alvo, mesmo título). (3) Clicar "Fechar" sem aplicar nada e conferir que nenhum campo mudou. (4) Reabrir e clicar "Aplicar N alteração(ões)" — pede confirmação; confirmar e conferir que só as linhas "OK PARA ALTERAR" foram gravadas, e que a gravação trocou SÓ o trecho "DE" pelo "PARA" dentro do campo, preservando literalmente o resto do texto que estava antes e depois dele (reabrir a migração depois: essas linhas agora aparecem como "JÁ ATUALIZADO"). (5) Abrir uma das atividades que teve "Dicas para o facilitador" ou "Observações" com ação de acrescentar (não substituir) e conferir que o texto ANTIGO continua lá, com o texto novo acrescentado depois (nunca apagando o que já existia). (6) Conferir a mudança de Tipo em "A VIRADA PARA A LIDERANÇA" (deve virar "Aplicação à Liderança") e "FECHAMENTO INDIVIDUAL" (deve virar "Compromisso Individual") — os dois tipos novos devem aparecer certinho no `<select>` do formulário de edição, não como campo vazio. (7) Clicar "↩ Desfazer última migração" — pede confirmação com a data/hora da aplicação; confirmar e conferir que TODOS os campos tocados no passo 4 voltam exatamente ao valor de antes (reabrir a migração de novo: as linhas voltam a aparecer como "OK PARA ALTERAR"). (8) Clicar "↩ Desfazer última migração" de novo, sem ter aplicado nada no meio tempo — deve avisar "Não há nenhuma migração aplicada pra desfazer", sem erro.' },
    { section: 'Admin',
      title: 'Roteiro-base — revisão interativa de "TRECHO AMBÍGUO" e divergência de Tipo dentro da Migração Antes x Depois',
      motivo: 'Não automatizado porque grava dados reais em produção. Preparar antes de abrir a migração: o "Tipo" de "A VIRADA PARA A LIDERANÇA" pra um tipo que não seja "Conceituação" nem "Aplicação à Liderança" (gera divergência de Tipo) e o "Passo a passo" de outra atividade-alvo colando o trecho "DE" dela DUAS vezes dentro do campo (gera "TRECHO AMBÍGUO", com aviso avisando quantas vezes o trecho apareceu). (1) Abrir "🔎 Migração Antes x Depois" — a janela já abre com o painel da PRIMEIRA pendência expandido sozinho, mostrando "Atual no sistema", "DE do Excel" e "PARA do Excel". (2) Na linha de Tipo, conferir que só aparecem 2 botões ("Manter atual" e "Substituir pelo PARA" — sem "Mesclar manualmente", já que não faz sentido mesclar um tipo). Na linha de "TRECHO AMBÍGUO", conferir os 3 botões ("Manter atual", "Substituir pelo PARA" e "Mesclar manualmente"). (3) Clicar "Manter atual" numa — o painel fecha sozinho e abre automaticamente o da PRÓXIMA pendência ainda sem decisão (nunca fica parado, nunca pula nenhuma); a linha decidida passa a mostrar "DIVERGÊNCIA RESOLVIDA → MANTER ATUAL" com um botão "Revisar de novo". (4) Na de Tipo, clicar "Substituir pelo PARA" — status vira "→ USAR PARA", e o Tipo grava exatamente o valor do "PARA" ao aplicar. (5) Na de "TRECHO AMBÍGUO", clicar "Mesclar manualmente" — abre uma caixa de texto já preenchida com o "PARA" (editável), com um botão "Confirmar mesclagem"; editar o texto livremente, clicar "Confirmar" — só ENTÃO o painel avança. (6) Conferir "Revisadas X de Y divergências" subindo a cada decisão, e o resumo com contadores batendo. (7) Aplicar e conferir os valores gravados; "Revisar de novo" numa linha já decidida deve permitir trocar a decisão antes de aplicar.' },
    { section: 'Admin',
      title: 'Roteiro-base — proposta automática de mesclagem para "VALOR ATUAL DIVERGENTE" (merge de 3 vias)',
      motivo: 'Não automatizado porque grava dados reais em produção. Preparar um "Passo a passo" que seja uma variação real do "DE" da planilha — ex: pegar o texto do "DE" de uma atividade, colar no campo e ACRESCENTAR uma frase extra no meio (algo que a pessoa teria escrito por conta própria) — isso faz o trecho não bater mais (mesmo tolerando formatação) e gera "VALOR ATUAL DIVERGENTE". Preparar também um segundo caso com um texto TOTALMENTE sem relação com o "DE"/"PARA" daquela atividade. (1) Abrir "🔎 Migração Antes x Depois" e clicar "REVISAR" na primeira divergência de texto (nunca numa de Tipo — Tipo não tem mesclagem). Conferir que a tela mostra QUATRO seções: "Atual no sistema", "DE do Excel", "PARA do Excel" e "Proposta de mesclagem (gerada automaticamente)" — a proposta já aparece pronta, sem precisar clicar em nada antes. (2) No caso da frase extra acrescentada no meio: conferir que a proposta preserva essa frase extra (em algum lugar do texto final) E incorpora a mudança do "PARA", sem repetir nenhum trecho duas vezes. Os botões devem ser exatamente "Aprovar mesclagem", "Editar mescla" e "Manter atual" — sem "Substituir pelo PARA" solto. (3) Clicar "Aprovar mesclagem" — grava direto a proposta mostrada na tela, sem abrir nenhuma caixa de texto; o status da linha vira "DIVERGÊNCIA RESOLVIDA → MESCLA APROVADA". (4) No caso do texto totalmente sem relação: conferir que a proposta aparece marcada com "⚠️ CONFLITO" destacando os dois trechos concorrentes, com uma explicação de que o atual e a planilha acrescentam coisas diferentes no mesmo ponto, e que o botão "Aprovar mesclagem" fica DESABILITADO nesse caso (não é possível clicar). (5) Nesse caso de conflito, clicar "Editar mescla" — abre a MESMA proposta (com a marcação de conflito) numa caixa de texto editável; apagar a marcação e escrever a versão final desejada à mão, clicar "Confirmar mesclagem" — o texto gravado deve ser exatamente o que foi digitado, sem sobra da marcação de conflito. (6) Alternativamente, no caso de conflito, clicar "Manter atual" — nada é gravado nesse campo. (7) Aplicar as alterações confirmadas e abrir cada atividade editada: conferir que o texto final é legível, sem marcação de conflito residual e sem repetir informação, e que o restante do campo (fora do que mudou) permanece como estava.' },
    { section: 'Admin',
      title: 'Roteiro-base — botão "🧹 Limpeza final de consistência" (3ª leva: campo inteiro, duração com recálculo e escritas em cadeia no mesmo campo)',
      motivo: 'Não automatizado porque grava dados reais em produção. (1) Confirmar que "🧹 Limpeza final de consistência" só aparece no roteiro-base da DIRETORES, ao lado de "🔎 Migração Antes x Depois". Clicar — abre o mesmo tipo de janela (DRY RUN, tabela, nada gravado ainda), com um título de janela diferente ("Limpeza final de consistência"). (2) Em "PRIMEIRO USO DA IA" (Passo a passo e Preparação prévia): preparar esses dois campos com um texto QUALQUER diferente do texto novo — a linha deve aparecer como "VALOR ATUAL DIVERGENTE" mesmo que o texto atual não tenha nenhuma relação com o "PARA" (esse item substitui o campo inteiro, não busca um trecho — por isso nunca vira "OK PARA ALTERAR" sozinho). Clicar "REVISAR": a seção "DE do Excel" explica "este item substitui o CAMPO INTEIRO, não é busca de trecho", em vez de mostrar um trecho. Clicar "Substituir pelo PARA" e conferir, depois de aplicar, que o campo virou exatamente o texto novo (perdendo o texto antigo por inteiro, como esperado). Agora preencher os dois campos exatamente com o texto novo e reabrir a migração — devem aparecer como "JÁ ATUALIZADO". (3) Nas Dicas do facilitador de "Primeiro uso da IA", garantir que o campo tem a frase antiga ("Isso torna a atividade muito melhor do que...") cercada de outras frases antes/depois — a linha deve vir "OK PARA ALTERAR" (é busca de trecho, não campo inteiro) e, ao aplicar, só aquela frase deve ser trocada, preservando o resto do campo. (4) No Passo a passo de "Agora dê nome ao que eles fizeram", garantir a frase antiga "Saíram das iniciativas para o problema × Outcome × Output" cercada de outro texto — mesma checagem de trecho preservando o resto. (5) Na linha de duração de "Decisão sem IA": preparar a atividade com 5 minutos, horário de início/fim definidos, dentro de um dia com pelo menos duas atividades seguintes de nível principal com horário próprio (uma delas com duração maior que 3 min, ex: 10-15 min). A linha da tabela já deve mostrar, sem clicar em nada, um resumo compacto ("Duração: 5 min → 8 min", novo horário de fim, lista de atividades seguintes com o novo início) — mas o botão "Aplicar" NÃO deve ficar disponível enquanto essa linha não tiver uma decisão (mesmo o status sendo "OK PARA ALTERAR" — duração NUNCA aplica sozinha). Clicar "REVISAR": o painel mostra Duração atual/esperada(DE)/nova(PARA) e DOIS cenários. CENÁRIO A mostra uma tabela "antes/depois" com toda atividade afetada (a própria + as seguintes do dia) e, embaixo, "Horário final da sessão" e "Duração da sessão" (antes → depois) e o mesmo para "Horário final do dia"/"Duração total do dia" — os números "depois" devem bater com a mesma conta que a tela normal do roteiro mostra em "Programado" pra aquele dia/sessão, só deslocada pela diferença de minutos. Clicar "Aceitar cenário A" grava direto (sem abrir mais nada) — conferir depois de aplicar que a atividade principal ficou com 8 min e horário de fim +3 min, TODAS as seguintes do dia deslizaram o início (e fim, se tiver) em +3 min, e NENHUMA outra atividade teve a duração alterada, só o horário. (5b) Reabrir a migração (ou desfazer e refazer o dry run) e desta vez testar o CENÁRIO B: no mesmo painel, escolher no menu uma das atividades seguintes elegíveis (só aparecem as que têm duração suficiente pra não zerar) — ao escolher, uma prévia aparece embaixo do menu com a MESMA estrutura de tabela, mas mostrando que só a atividade principal, as que ficam ENTRE ela e a escolhida, e a própria escolhida mudam; a escolhida aparece com a duração reduzida em 3 min mas TERMINANDO NO MESMO HORÁRIO de antes; e "Horário final da sessão"/"Horário final do dia" e as respectivas durações devem aparecer IDÊNTICOS entre antes e depois (a mudança foi absorvida, não empurrada pro resto do dia). Clicar "Confirmar cenário B" e aplicar — conferir que a atividade escolhida ficou com duração menor (original − 3) mas com o MESMO horário de fim de antes, e que TODAS as atividades depois dela no dia continuam com horário e duração idênticos aos originais (nem sequer aparecem na lista de mudanças). (5c) Mudar a duração de "Decisão sem IA" pra um valor diferente de 5 e 8 (ex: 6 min) e reabrir a migração — vira "VALOR ATUAL DIVERGENTE", mas os DOIS cenários continuam disponíveis normalmente, só com um aviso a mais dizendo que a duração atual não bate nem com o "de" nem com o "para" esperado. (5d) Sem escolher nada no Cenário B, testar o botão "Manter atual" desse painel — nenhuma duração nem horário deve mudar. (6) Nas Dicas de "Decisão sem IA", conferir que a distribuição sugerida do tempo (3+4+1 min) é ACRESCENTADA ao final do que já existia, nunca substituindo. (7) No Passo a passo de "Faça a comparação": deixar o campo com QUALQUER texto diferente do novo (ex: a versão de 5 perguntas de uma leva anterior, OU um texto qualquer sem relação) — deve vir "VALOR ATUAL DIVERGENTE" (é "substituir campo inteiro", não busca de trecho — por isso nunca "OK PARA ALTERAR" sozinho, mesmo vindo de uma leva anterior); "REVISAR" e "Substituir pelo PARA"; aplicar e conferir que virou a versão com 3 perguntas, Modelo A/Modelo B e o fechamento sobre grau de incerteza. Preencher o campo EXATAMENTE com esse texto novo e reabrir — deve vir "JÁ ATUALIZADO". (8) Em Observações de "Primeiro uso da IA": preparar o campo com QUALQUER conteúdo (ex: um bloco de prompt antigo com contexto/hipótese/instruções para a IA) — deve vir "VALOR ATUAL DIVERGENTE" (mesma lógica de campo inteiro); "REVISAR": a seção "PARA do Excel" mostra "(campo deve ficar vazio)" em vez de uma caixa em branco sem explicação; clicar "Substituir pelo PARA" e, depois de aplicar, conferir que o campo ficou realmente vazio (não um texto qualquer, vazio de verdade). Deixar o campo já vazio e reabrir — deve vir "JÁ ATUALIZADO". (9) Nas Observações de "EPISÓDIO FINAL — O COMPROMISSO DA LIDERANÇA": preparar uma lista com os oito itens preservados (Priorização, Foco, Incerteza, Adaptação, Fail small, IA, Pensamento crítico, Liderança ágil) MAIS as quatro formulações antigas exatas — "Feedback: Novas informações entram no sistema.", "Empirismo: Evidências são mais fortes que opiniões.", "MVP / experimento: Podemos aprender sem construir tudo." e "Outcome x Output: Resolver problema é diferente de entregar projeto." — cada uma deve aparecer como uma linha própria "OK PARA ALTERAR" na tabela (quatro linhas com o MESMO título e o MESMO campo "Observações"). Isso é o caso crítico: clicar "Aplicar alterações confirmadas" de uma vez (as quatro linhas OK, mais o que mais estiver pendente) e depois abrir a atividade — as QUATRO formulações devem ter sido trocadas pelas versões novas, e os oito itens preservados devem continuar exatamente como estavam, sem nenhuma sobra da formulação antiga em nenhuma das quatro (bug já corrigido: antes da correção, aplicar de uma vez linhas diferentes que mexem no MESMO campo da MESMA atividade fazia só a ÚLTIMA escrita sobreviver, apagando silenciosamente as outras três, mesmo com "Aplicado: X de X" indicando sucesso em todas — a garantia agora é que múltiplas linhas no mesmo campo sempre se aplicam em cadeia, uma sobre o resultado da anterior, nunca em paralelo). (10) "↩ Desfazer última migração" depois desse "Aplicar" deve restaurar as Observações do "EPISÓDIO FINAL" para o valor de ANTES das quatro trocas (com as quatro formulações antigas de volta, não um estado intermediário) — e restaurar também os horários/duração deslocados pelo Cenário A ou B da mudança de duração, se tiver sido aplicada na mesma leva. (11) No Passo a passo de "DESAFIO 1: ESCOLHAM", garantir que o campo tem a palavra "Exemplos:" seguida da lista dos 24 cartões — a linha deve vir "OK PARA ALTERAR" (é busca de trecho curto, não campo inteiro); aplicar e conferir que só o rótulo virou "Os 24 cartões são:", com a lista dos 24 cartões e o resto do Passo a passo (as instruções de tempo/regras antes do rótulo) intactos, sem nenhuma palavra da lista alterada.' },
    { section: 'Admin',
