...
{ section: 'Minha Área',
    ...
    { section: 'Admin',
      ...
      motivo: 'Não automatizado porque abre uma janela nova e depende de conteúdo preenchido nas atividades. Num dia com pelo menos: uma atividade com vários campos preenchidos (ex: objetivo, descrição e materiais, mas sem perguntas de debrief), uma atividade sem nenhum campo de facilitação preenchido, e uma seção com sub-etapas. Clicar "🖨 Imprimir completo" (roteiro-base e roteiro da turma) e conferir: (1) abre uma janela nova com um bloco por atividade, numerado igual à tela (1, 2, 3... e sub-etapas como 4.1, 4.2); (2) na atividade com campos parciais, aparecem só os rótulos dos campos preenchidos (ex: "Objetivo", "Descrição", "Materiais necessários") — o rótulo de um campo vazio (ex: "Perguntas para o debrief") NÃO aparece; (3) a atividade sem nenhum campo preenchido mostra a mensagem "Nenhum campo preenchido nesta atividade." no lugar dos campos; (4) o resumo do dia (janela, programado, facilitação, pausas, lacunas, atividades) aparece no topo igual ao "🖨 Imprimir" simples; (5) o botão "Imprimir / salvar como PDF" dentro da janela aciona o diálogo nativo do navegador.' },
    { section: 'Admin',
      title: 'Roteiro — formatação básica (negrito, itálico, sublinhado, listas, alinhar) nas caixas de texto da atividade',
      motivo: 'Não automatizado porque depende de seleção de texto e comandos de formatação do navegador (document.execCommand), que só funcionam com interação real. No formulário de atividade (Descrição, Objetivo, Passo a passo, Dicas, Conexão com a agilidade, Perguntas para o debrief, Materiais, Preparação prévia, Observações — todas menos Título), cada caixa tem uma barrinha com os botões N (negrito), I (itálico), S (sublinhado), • Lista, 1. Lista, Centralizar, Justificar e ↵ Espaço. Selecionar um trecho de texto e clicar em cada botão; conferir que o efeito é aplicado (visualmente em negrito/itálico/sublinhado, ou virando marcador/numeração/alinhamento) e que o botão pode ser clicado de novo para desfazer. Salvar a atividade e reabrir o formulário de edição — a formatação deve continuar lá exatamente como foi deixada. Depois, sem entrar em edição, expandir a atividade na tela de leitura (roteiro da turma) e clicar em "🖨 Imprimir completo": a mesma formatação (negrito, listas, alinhamento) deve aparecer igual nos dois lugares. Testar também "Perguntas para o debrief" e "Materiais" (que viram lista com marcadores automaticamente, um item por linha) — adicionar um item em negrito e conferir que ele aparece em negrito na lista, tanto na tela quanto na impressão. Por fim, abrir uma atividade cadastrada ANTES desta funcionalidade (texto puro, sem formatação) e conferir que o texto antigo aparece normalmente na caixa, editável, sem símbolos estranhos.' },
    { section: 'Admin',
