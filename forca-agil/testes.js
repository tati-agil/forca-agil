...
(function () {
  ...
  const TECNICOS = [
    ...
    },
    {
      ...
      tests: [
        ...
        { id: 'adm-tabs',   label: 'Abas Admin presentes (9: Eventos/Certificados/Repositório/Cadastrados/Administradores/Manual/Mapa/Testes/Pedidos)', run: function () { return document.querySelectorAll('.admin-tab-btn').length === 9; } },
  ...
  const COMPORTAMENTO_MANUAL = [
    ...
    { section: 'Admin',
      ...
      motivo: 'Na aba Pedidos, escolher um item com e-mail preenchido. (1) Clicar "✉ Responder por e-mail" — verificar que abre o programa de e-mail padrão do sistema operacional/navegador com o destinatário correto no campo Para, assunto "Força Ágil — resposta ao seu pedido (tipo)", e corpo com saudação (primeiro nome capitalizado, não em CAIXA ALTA), citação da descrição enviada, e assinatura "Um abraço, Equipe Força Ágil" no final. (2) Clicar "✓ Marcar como respondido" — verificar que aparece um seletor "— quem respondeu? —" com a lista de admins; sem escolher ninguém e clicando "Confirmar", nada deve ser gravado (foco volta pro seletor). (3) Escolher um admin na lista e clicar "Confirmar" — verificar que o item ganha badge "✓ Respondido", fica com opacidade reduzida, mostra "Respondido por NOME em DATA — N dias úteis depois" em verde, e os botões viram só "✕ Desmarcar". (4) Clicar "✕ Desmarcar" — verificar que volta ao estado original (sem badge, opacidade normal, mostra "Em aberto há N dias úteis", botão volta a dizer "Marcar como respondido"). (5) Recarregar a página — verificar que o estado respondido/não-respondido e quem respondeu persistem (gravado em pedidos/<key>/respondido, respondidoEm e respondidoPor no Firebase, não é só estado local).' },
    { section: 'Admin',
    ...
    { section: 'Admin',
      title: 'Admin — visibilidade das 9 abas no mobile',
      motivo: 'Acessar o painel Admin em tela estreita (celular). Verificar: todas as 9 abas (Eventos, Certificados, Repositório, Cadastrados, Administradores, Manual, Mapa, Testes, Pedidos) estão visíveis (quebram em 2 linhas); nenhuma aba fica oculta ou cortada.' },
    { section: 'Deploy',
