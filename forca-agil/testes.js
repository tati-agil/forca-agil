...
{ section: 'Minha Área',
    ...
    { section: 'Admin',
      ...
      motivo: 'Não automatizado porque grava dados reais. Numa atividade de nível principal, clicar "+ Sub-etapa" 3 vezes (durações 1, 3 e 2 min) e conferir que cada sub-etapa abre o MESMO formulário completo de qualquer atividade (descrição, objetivo, passo a passo, materiais etc. — não só título/duração). Conferir: (1) a atividade-mãe passa a mostrar "· 3 etapas" no título e a duração exibida na lista é 6 (a soma), mesmo com o campo Duração dela própria ainda guardando outro valor internamente; (2) um botão "▲ Recolher" aparece na linha da mãe; clicar recolhe as sub-etapas, clicar de novo ("▼ Ver etapas") mostra de volta; (3) as sub-etapas aparecem recuadas, numeradas 4.1, 4.2, 4.3 (usando a posição da seção-mãe), com sua própria coluna de horário (vazia se não preenchida) e SEM entrar na ordenação cronológica do dia nem no aviso de ordem divergente; (4) editar a duração de uma sub-etapa NÃO oferece a pergunta de recalcular horários seguintes (só a mãe participa dessa cadeia); (5) no roteiro da turma, abrir a atividade-mãe mostra as mesmas sub-etapas dentro do corpo expandido, e "+ Sub-etapa (só nesta turma)" cria uma sub-etapa exclusiva da turma mesmo quando a mãe é do roteiro-base; (6) excluir a atividade-mãe (roteiro-base ou exclusiva) apaga as sub-etapas junto, com confirmação avisando disso; duplicar a mãe duplica as sub-etapas junto.' },
    { section: 'Admin',
    ...
    { section: 'Admin',
      title: 'Roteiro — "Imprimir completo" mostra só os campos preenchidos',
      motivo: 'Não automatizado porque abre uma janela nova e depende de conteúdo preenchido nas atividades. Num dia com pelo menos: uma atividade com vários campos preenchidos (ex: objetivo e materiais, mas sem dicas para o facilitador), uma atividade sem nenhum campo de facilitação preenchido, e uma seção com sub-etapas. Clicar "🖨 Imprimir completo" (roteiro-base e roteiro da turma) e conferir: (1) abre uma janela nova com um bloco por atividade, numerado igual à tela (1, 2, 3... e sub-etapas como 4.1, 4.2); (2) na atividade com campos parciais, aparecem só os rótulos dos campos preenchidos (ex: "Objetivo", "Materiais necessários") — o rótulo de um campo vazio (ex: "Dicas para o facilitador") NÃO aparece; (3) a atividade sem nenhum campo preenchido mostra a mensagem "Nenhum campo preenchido nesta atividade." no lugar dos campos; (4) o resumo do dia (janela, programado, facilitação, pausas, lacunas, atividades) aparece no topo igual ao "🖨 Imprimir" simples; (5) o botão "Imprimir / salvar como PDF" dentro da janela aciona o diálogo nativo do navegador.' },
    { section: 'Admin',
    ...
    { section: 'Admin',
      ...
      motivo: 'Não automatizado porque grava dados reais. Cadastrar duas atividades da MESMA sessão com horários que se cruzam (ex: atividade A das 09:00 às 09:30, atividade B começando às 09:20). Conferir que aparece um banner vermelho "⚠ [A] se sobrepõe a [B]" com o tempo de sobreposição (10min no exemplo), tanto no roteiro-base quanto no roteiro da turma, e que ele também some quando o Ctrl+V permitir consertar o conflito. Colocar as duas atividades em sessões DIFERENTES (uma na Manhã, outra na Tarde) mesmo com horários sobrepostos e conferir que o aviso não aparece nesse caso (sessões diferentes não são comparadas entre si).' },
    { section: 'Admin',
