...
{ section: 'Turmas',
    ...
    { section: 'Turmas',
      ...
      motivo: 'Na tabela de participantes de uma turma (interessada ou inscrita), clicar "→ Espera". Verificar primeiro que o modal EXIGE um motivo: tentar confirmar sem escolher nada deve mostrar "Escolha um motivo." e não gravar; escolher "Outro" deve abrir um campo de texto que também é obrigatório. As opções devem ser: Turma sem vagas, Turma já encerrada, Remanejada para próxima turma, A pedido da pessoa, Evento encerrado e Outro. Depois de escolher e confirmar, a pessoa sai da turma (removed:true) e entra na lista de espera. Verificar: (1) a data exibida na lista de espera é a data ORIGINAL de interesse na turma, não o momento da migração; (2) a coluna "Origem" mostra o nome da turma de onde ela veio, com uma seta ↩, e logo abaixo a data em que a migração aconteceu; (3) uma pessoa que entrou direto pelo card "Lista de Espera" do site aparece nessa coluna como "Entrou pela lista", sem nome de turma.' },
{ section: 'Admin',
      title: 'Coluna Destino de quem saiu da turma',
      motivo: 'Remover três pessoas da mesma turma por caminhos diferentes: uma marcando "colocar na lista de espera", outra com o motivo "vai fazer em outra turma" (escolhendo a turma), e a terceira sem nenhum dos dois. Depois conferir: (1) cada uma aparece no filtro certo — Foram para a espera, Foram para outra turma e Removidos, respectivamente; (2) na coluna Destino os selos dizem, na ordem, "Lista de espera", o NOME da turma escolhida, e "Saiu"; (3) o mesmo vale na turma já com presença registrada, onde a tabela tem as colunas de dias — as duas tabelas de quem saiu precisam mostrar a mesma coluna Destino, porque antes divergiam; (4) o motivo continua aparecendo na coluna dele, sem o destino grudado no texto.' },
    { section: 'Admin',
      title: 'Saída da turma: motivo, destino e a fila com a data original',
      motivo: 'Numa turma de teste, remover uma pessoa e conferir cada caminho. (1) O modal EXIGE motivo — confirmar sem escolher não pode passar. (2) Motivo "Vai fazer em outra turma": tem que aparecer a lista de turmas e exigir uma escolha; depois, na lista de quem saiu, o registro deve mostrar o motivo E a turma de destino. (3) Motivo "Outro": exige a descrição. (4) Caixa "Colocar na lista de espera": marcar e confirmar — a pessoa deve aparecer na lista de espera da aba Cadastrados com a DATA E HORA DO INTERESSE ORIGINAL, não a data de hoje; conferir esse horário contra o registro anterior dela, porque é o que define a ordem da fila. (5) Sem marcar a caixa: a pessoa sai da turma e NÃO aparece na fila. (6) Conferir que a caixa vem marcada sozinha em "turma sem vagas" e "a data não serviu", e desmarcada nos demais — e que dá para mudar nos dois sentidos.' },
    { section: 'Admin',
      title: 'Substituída registra quem entrou no lugar',
      motivo: 'Na linha de alguém não confirmada, escolher a justificativa "Substituída". Verificar: (1) abre a pergunta de quem entrou no lugar, listando as pessoas da própria turma; (2) confirmar sem escolher ninguém não pode passar; (3) a opção de digitar um nome funciona para quem não está na lista; (4) o selo passa a mostrar o nome de quem entrou; (5) removendo a pessoa da turma depois, o selo continua aparecendo na lista de quem saiu — a marca não pode se perder; (6) voltando a justificativa para vazio, o nome de quem substituiu também some, sem deixar registro solto.' },
    { section: 'Admin',
      title: 'Filtros por destino: a soma tem que fechar',
      motivo: 'Abrir uma turma com gente em várias situações e somar à mão: Confirmados + Aguardando decisão + Não confirmados + Foram para a espera + Removidos tem que dar exatamente o número de "Todos os interessados". Se não fechar, alguém está em dois grupos ou em nenhum. Conferir também: (1) "Aguardando decisão" só traz quem não foi confirmada E não tem motivo registrado; assim que você registra um motivo, a pessoa sai desse grupo e entra em "Não confirmados"; (2) quem foi para a fila aparece em "Foram para a espera" e NÃO em "Removidos"; (3) os dois grupos de quem saiu mostram motivo e destino, sem botões de ação — não faz sentido agir sobre quem já saiu; (4) numa turma em que só existe um grupo com gente, a barra de filtros não aparece.' },
    { section: 'Admin',
      title: 'Lista de espera — data e hora corretas',
      motivo: 'Abrir a aba Cadastrados e conferir a Lista de Espera: a coluna Data deve mostrar dia e hora no formato brasileiro (10/08/2026 12:26), não a data crua do banco. Conferir uma linha contra o horário real em que a pessoa entrou na fila. Para quem foi migrada de uma turma, a coluna Origem também deve trazer a hora da migração. ATENÇÃO AO FUSO: se algum dia aparecer um registro cuja data foi guardada sem hora, ele deve aparecer só com a data — nunca com uma hora inventada, e nunca com o dia anterior ao correto, que é o erro clássico ao converter data sem hora. Vale repetir esta conferência depois de qualquer mexida em exibição de datas no painel.' },
    { section: 'Admin',
    ...
    { section: 'Admin',
      ...
      motivo: 'Requer turma com pelo menos 1 confirmado E 1 não confirmado. Expandir o card da turma e verificar acima da tabela: (1) aparecem 3 botões — Todos (N), Confirmados (N), Não confirmados (N) — com "Todos" ativo por padrão; (2) as contagens batem com o rótulo do cabeçalho do card ("X não confirmados · Y confirmados") e Todos = soma dos outros dois; (3) clicar em "Confirmados" mostra só quem tem status Inscrito; (4) clicar em "Não confirmados" mostra só quem tem status Interessado; (5) clicar em "Todos" restaura a lista completa. Em turma que só tem um dos grupos (ex: todos confirmados), o filtro NÃO deve aparecer.' },
    { section: 'Admin',
    ...
    { section: 'Admin',
      title: 'Turmas — justificar por que um interessado não foi confirmado',
      motivo: 'Requer turma com pelo menos 1 pessoa com status Interessado. Verificar: (1) seletor "Justificar…" aparece antes do botão "Confirmar" em cada linha de Interessado; (2) "Sem vagas" → badge laranja, salva "sem_vagas"; (3) "Substituída" → badge lilás, salva "substituida"; (4) "Já participou" → badge verde-claro, salva "ja_participou" (uso: pessoa inscrita em mais de uma turma já confirmada em outra); (5) "Justificar…" de volta limpa motivo e badge; (6) funciona com turma aberta ou encerrada.' },
    { section: 'Admin',
