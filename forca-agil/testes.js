...
{ section: 'Minha Área',
    ...
    { section: 'Admin',
      title: 'Roteiro-base — revisão interativa de "VALOR ATUAL DIVERGENTE" e "TRECHO AMBÍGUO" dentro da Migração Antes x Depois',
      motivo: 'Não automatizado porque grava dados reais em produção. Preparar pelo menos 3 divergências antes de abrir a migração: editar manualmente o "Passo a passo" de uma atividade-alvo pra um texto qualquer (gera "substituir" divergente), o "Tipo" de "A VIRADA PARA A LIDERANÇA" pra um tipo que não seja "Conceituação" nem "Aplicação à Liderança" (gera "tipo" divergente) a "Conexão com a mentalidade ágil" de "AGORA DÊ NOME AO QUE ELES FIZERAM" pra um texto que não contenha o trecho antigo (gera "VALOR ATUAL DIVERGENTE", com aviso de trecho não encontrado) e o "Passo a passo" de outra atividade-alvo colando o trecho "DE" dela DUAS vezes dentro do campo (gera "TRECHO AMBÍGUO", com aviso avisando quantas vezes o trecho apareceu). (1) Abrir "🔎 Migração Antes x Depois" — a janela já abre com o painel da PRIMEIRA divergência expandido sozinho, mostrando "Atual no sistema", "DE do Excel" e "PARA do Excel" um embaixo do outro, sem precisar clicar em nada. (2) Na linha de Tipo, conferir que só aparecem 2 botões ("Manter atual" e "Substituir pelo PARA" — sem "Mesclar manualmente", já que não faz sentido mesclar um tipo). Nas linhas de texto, conferir os 3 botões. (3) Na divergência preparada com o trecho antigo ausente, conferir que aparece um aviso alertando que "Substituir pelo PARA" não vai achar onde trocar. (4) Clicar "Manter atual" numa divergência — o painel fecha sozinho e abre automaticamente o da PRÓXIMA divergência ainda sem decisão (nunca fica parado, nunca pula nenhuma); a linha decidida passa a mostrar "DIVERGÊNCIA RESOLVIDA → MANTER ATUAL" com um botão "Revisar de novo". (5) Clicar "Substituir pelo PARA" numa segunda divergência — mesmo comportamento, status vira "→ USAR PARA". (6) Na terceira, clicar "Mesclar manualmente" — abre uma caixa de texto já preenchida com o "PARA" (editável), com um botão "Confirmar mesclagem"; editar o texto livremente, clicar "Confirmar" — só ENTÃO o painel avança (clicar nos outros dois botões antes de confirmar a mescla não deve fazer nada acontecer sem essa confirmação). (7) Conferir o texto "Revisadas X de Y divergências" subindo a cada decisão, e o resumo com contadores (OK PARA ALTERAR, JÁ ATUALIZADO, MANTER ATUAL, USAR PARA, MESCLA APROVADA, PENDÊNCIAS) batendo com o que foi decidido. (8) Antes de decidir a última divergência, conferir que o botão "Aplicar alterações confirmadas" NÃO aparece; decidir a última e conferir que ele aparece imediatamente, com a contagem certa (inclui os "OK PARA ALTERAR" originais + as divergências resolvidas como "usar para"/"mesclar", mas NUNCA as resolvidas como "manter atual"). (9) Aplicar e conferir: a atividade "manter atual" continua com o texto editado manualmente (não voltou pro antigo nem foi pro novo); a de "usar para" tem exatamente o texto/tipo do "PARA"; a de "mesclar" tem exatamente o texto que foi digitado na caixa de confirmação. (10) Clicar "Revisar de novo" numa linha já decidida e trocar a decisão antes de aplicar — a nova escolha deve valer na hora de aplicar, não a antiga.' },
    { section: 'Admin',
