...
(function () {
  ...
  const COMPORTAMENTO_MANUAL = [
    ...
    { section: 'Turmas',
      title: 'Confirmar quem tem registro ativo em outra turma — aviso e remoção automática',
      motivo: 'Requer manifestar interesse real numa mesma pessoa em duas turmas (ou confirmá-la como inscrita numa e depois manifestar interesse na outra) e então confirmá-la como inscrita numa delas pelo painel — exige dado real no Firebase, não pode ser simulado no teste automatizado. Verificar: o modal de confirmação avisa que ela também tem registro ativo na outra turma (interessada ou já inscrita); ao confirmar, o registro dela na outra turma vira "removido" com motivo registrado e aparece na seção Removidos daquela turma; ao cancelar, nada muda em nenhuma das duas turmas.' },
    { section: 'Check-in',
    ...
    { section: 'Admin',
      title: 'Turmas — adicionar participante: formulário modal (não prompt), com escolha de status',
      motivo: 'Funciona com a turma aberta ou com interesse encerrado. Clicar em "＋ Participante". Verificar: (1) abre modal visual (não prompt nativo do browser) com campos Nome, E-mail, Área e Status (Interessada/Inscrita); (2) lista de áreas tem as 20 gerências; (3) e-mail inválido (sem @previ.com.br) ou status não selecionado exibem erro no modal; (4) ao confirmar com "Inscrita", pessoa aparece na tabela com status Inscrito e (se a turma tiver interesse encerrado) ganha colunas de presença habilitadas; (5) ao confirmar com "Interessada", pessoa aparece na tabela com status Interessado e ganha o botão "Confirmar" na linha, igual a quem se interessou pelo site; (6) em ambos os casos, Firebase tem addedByAdmin:true e addedByAdminName com nome do admin em turmas-interesse; (7) testar com a turma ainda aberta e também já encerrada.' },
    { section: 'Admin',
    ...
    { section: 'Admin',
      title: 'Turmas — "＋ Participante" como Inscrita não verifica outras turmas (gap conhecido)',
      motivo: 'Diferente do botão "Confirmar" (que sempre avisa e remove sobreposição com outras turmas), adicionar alguém direto como "Inscrita" pelo "＋ Participante" NÃO faz essa checagem. Verificar: confirmar uma pessoa como inscrita na Turma A e depois adicioná-la como "Inscrita" na Turma B pelo "＋ Participante" — ela fica inscrita nas duas ao mesmo tempo, sem aviso nem remoção automática. Reportar como gap se ainda não corrigido.' },
    { section: 'Admin',
